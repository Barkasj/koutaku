// schemasync validates that Koutaku Go config types stay in sync with
// transports/config.schema.json.
//
// Starting from a configured entry-point type (default: ConfigData in
// transports/koutaku-http/lib), it recursively walks every nested struct
...
var entrypoints = []entrypoint{
	{
		pkg:        "github.com/koutaku/koutaku/transports/koutaku-http/lib",
		typeName:   "ConfigData",
...
var opaqueLeafTypes = map[string]string{
	"github.com/koutaku/koutaku/core/schemas.EnvVar": "env-aware string; custom JSON",
}
...
// injection (existingSecret / secretRef / env.KOUTAKU_*) for that path.
...
func main() {
	...
	helmValuesFlag := flag.String("helm-values", "helm-charts/koutaku/values.schema.json", "path to helm values.schema.json (for EnvVar secret-support check)")
	helmHelpersFlag := flag.String("helm-helpers", "helm-charts/koutaku/templates/_helpers.tpl", "path to helm _helpers.tpl (for env.KOUTAKU_* emission detection)")
	flag.Parse()
	...
	// go.work — that's the only configuration koutaku is tested against.
	...
	// schemas.EnvVar, the helm chart must either (a) emit an env.KOUTAKU_*
...
//
//  1. An `env.KOUTAKU_*` string literal appears in _helpers.tpl (indicating
...
func (c *checker) checkEnvVarHelmSupport(valuesPath, helpersPath string) {
	...
	// Extract every env.KOUTAKU_* token mentioned in _helpers.tpl.
	envKoutakuMentions := map[string]bool{}
	for _, line := range strings.Split(helpers, "\n") {
		// crude extraction: look for "env.KOUTAKU_X" substrings
		idx := 0
		for idx < len(line) {
			k := strings.Index(line[idx:], "env.KOUTAKU_")
			if k < 0 {
				break
			}
			start := idx + k
			...
			envKoutakuMentions[line[start:end]] = true
			idx = end
		}
	}
	...
	for _, loc := range c.envVarFields {
		// Heuristic 1: any env.KOUTAKU_* is present in helpers — broad acceptance.
		...
		// has AT LEAST ONE envKoutaku mention that maps to this field's path.
		...
		if !matched && len(envKoutakuMentions) > 0 {
			// Fall back to "some envKoutaku wiring exists somewhere" — we flag it
			...
			for mention := range envKoutakuMentions {
...
// "properties.koutaku.properties.governance.properties.authConfig.properties.adminUsername".
func schemaPathToCamelCase(p string) string {
	parts := strings.Split(strings.TrimPrefix(p, "/"), "/")
	out := []string{"properties", "koutaku"}
...
func (c *checker) walkType(t types.Type, schemaPath, goPath string) {
	...
	if named != nil {
		...
		if _, isOpaque := opaqueLeafTypes[key]; isOpaque {
			if key == "github.com/koutaku/koutaku/core/schemas.EnvVar" {
