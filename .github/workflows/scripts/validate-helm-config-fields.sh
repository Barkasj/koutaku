...
# Helm config.json field validation script for Koutaku
...
render_config() {
  local values_file=$1
  helm template koutaku "$CHART_DIR" \
...
assert_field_value 'schema field' '.$schema' '"https://www.getkoutaku.ai/schema"'
  ...
  plugins:
    ...
    semanticCache:
      ...
      config:
        ...
        vector_store_namespace: "koutaku-cache"
    otel:
      enabled: true
      config:
        service_name: "koutaku-test"
...
assert_field_value 'plugins: semantic_cache vector_store_namespace' '.plugins.[4].config.vector_store_namespace' '"koutaku-cache"'

# OTEL plugin
assert_field_value 'plugins: otel name' '.plugins.[5].name' '"otel"'
assert_field_value 'plugins: otel service_name' '.plugins.[5].config.service_name' '"koutaku-test"'
...
assert_field_value 'plugins: datadog service_name' '.plugins.[6].config.service_name' '"koutaku-dd"'
...
koutaku:
  cluster:
    ...
    region: "us-east-1"
    peers:
      - "koutaku-0.koutaku-headless:7946"
      - "koutaku-1.koutaku-headless:7946"
    gossip:
      port: 7946
      config:
        timeoutSeconds: 10
    ...
    discovery:
      ...
      allowedAddressSpace:
        - "10.0.0.0/8"
      k8sNamespace: "koutaku"
      k8sLabelSelector: "app=koutaku"
VALS
...
assert_field_value 'cluster_config.discovery.k8s_namespace' '.cluster_config.discovery.k8s_namespace' '"koutaku"'
assert_field_value 'cluster_config.discovery.k8s_label_selector' '.cluster_config.discovery.k8s_label_selector' '"app=koutaku"'
...
assert_field_value 'weaviate config.class_name' '.vector_store.config.class_name' '"KoutakuCache"'
...
storage:
  ...
  logsStore:
    ...
    objectStorage:
      ...
      roleArn: "arn:aws:iam::123:role/koutaku"
      forcePathStyle: true
VALS
...
assert_field_value 'logs_store.object_storage.bucket' '.logs_store.object_storage.bucket' '"koutaku-logs"'
...
assert_field_value 'logs_store.object_storage.role_arn' '.logs_store.object_storage.role_arn' '"arn:aws:iam::123:role/koutaku"'
assert_field_value 'logs_store.object_storage.force_path_style' '.logs_store.object_storage.force_path_style' 'true'

# S3 with existingSecret — exercises env.KOUTAKU_OBJECT_STORAGE_* substitution path
...
storage:
  ...
  logsStore:
    ...
    objectStorage:
      ...
      type: s3
      bucket: "koutaku-logs"
      existingSecret: "koutaku-os-creds"
...
assert_field_value 'logs_store.object_storage.access_key_id (env)' '.logs_store.object_storage.access_key_id' '"env.KOUTAKU_OBJECT_STORAGE_ACCESS_KEY_ID"'
assert_field_value 'logs_store.object_storage.secret_access_key (env)' '.logs_store.object_storage.secret_access_key' '"env.KOUTAKU_OBJECT_STORAGE_SECRET_ACCESS_KEY"'
assert_field_value 'logs_store.object_storage.session_token (env)' '.logs_store.object_storage.session_token' '"env.KOUTAKU_OBJECT_STORAGE_SESSION_TOKEN"'
assert_field_value 'logs_store.object_storage.role_arn (env)' '.logs_store.object_storage.role_arn' '"env.KOUTAKU_OBJECT_STORAGE_ROLE_ARN"'
...
assert_field_value 'logs_store.object_storage.bucket (gcs)' '.logs_store.object_storage.bucket' '"koutaku-gcs-bucket"'
