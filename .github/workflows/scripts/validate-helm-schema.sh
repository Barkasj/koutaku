...
HELM_SCHEMA="$REPO_ROOT/helm-charts/koutaku/values.schema.json"
...
HELM_BUDGET_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.budgets.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_RATELIMIT_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.rateLimits.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_CUSTOMER_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.customers.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_TEAM_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.teams.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_VK_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.virtualKeys.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_ROUTING_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.routingRules.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_GUARDRAIL_RULE_REQUIRED=$(jq -r '.properties.koutaku.properties.guardrails.properties.rules.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_GUARDRAIL_PROV_REQUIRED=$(jq -r '.properties.koutaku.properties.guardrails.properties.providers.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_GOSSIP_TOP_REQUIRED=$(jq -r '.properties.koutaku.properties.cluster.properties.gossip.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_GOSSIP_REQUIRED=$(jq -r '.properties.koutaku.properties.cluster.properties.gossip.properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_VK_MCP_REQUIRED=$(jq -r '.properties.koutaku.properties.governance.properties.virtualKeys.items.properties.mcp_configs.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_OKTA_REQUIRED=$(jq -r '.properties.koutaku.properties.scim.allOf[0].then.properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_ENTRA_REQUIRED=$(jq -r '.properties.koutaku.properties.scim.allOf[1].then.properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_SEMCACHE_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.semanticCache.then.properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_OTEL_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.otel.then.properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_PUSHGW_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.telemetry.properties.config.properties.push_gateway.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_PUSHGW_AUTH_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.telemetry.properties.config.properties.push_gateway.properties.basic_auth.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_PLUGIN_ITEMS_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.custom.items.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_CUSTOM_PLUGIN_PROPS=$(jq -r '.properties.koutaku.properties.plugins.properties.custom.items.properties | keys | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_PLACEMENT_ENUM=$(jq -r '.properties.koutaku.properties.plugins.properties.custom.items.properties.placement.enum // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
HELM_MAXIM_REQUIRED=$(jq -r '.properties.koutaku.properties.plugins.properties.maxim.then.anyOf[0].properties.config.required // [] | sort | join(",")' "$HELM_SCHEMA" 2>/dev/null || echo "")
...
for prop in asyncJobResultTTL requiredHeaders loggingHeaders allowedHeaders mcpAgentDepth mcpToolExecutionTimeout mcpCodeModeBindingLevel mcpToolSyncInterval hideDeletedVirtualKeysInFilters; do
  check_property_exists "client.$prop" ".properties.koutaku.properties.client.properties.${prop}" "$HELM_SCHEMA"
done
...
for prop in headers tls_ca_cert insecure; do
  check_property_exists "otel.config.$prop" ".properties.koutaku.properties.plugins.properties.otel.properties.config.properties.${prop}" "$HELM_SCHEMA"
done
...
for prop in required_headers is_enterprise; do
  check_property_exists "governance.plugin.config.$prop" ".properties.koutaku.properties.plugins.properties.governance.properties.config.properties.${prop}" "$HELM_SCHEMA"
done
...
for prop in modelConfigs providers; do
  check_property_exists "governance.$prop" ".properties.koutaku.properties.governance.properties.${prop}" "$HELM_SCHEMA"
done

# Gap 6: MCP properties
echo ""
echo "  Checking MCP properties (Gap 6)..."
check_property_exists "mcp.toolSyncInterval" ".properties.koutaku.properties.mcp.properties.toolSyncInterval" "$HELM_SCHEMA"
check_property_exists "mcp.toolManagerConfig.codeModeBindingLevel" '.properties.koutaku.properties.mcp.properties.toolManagerConfig.properties.codeModeBindingLevel' "$HELM_SCHEMA"
...
check_property_exists "cluster.region" ".properties.koutaku.properties.cluster.properties.region" "$HELM_SCHEMA"

# Gap 8: Miscellaneous properties
echo ""
echo "  Checking miscellaneous properties (Gap 8)..."
check_property_exists "telemetry.custom_labels" ".properties.koutaku.properties.plugins.properties.telemetry.properties.config.properties.custom_labels" "$HELM_SCHEMA"
check_property_exists "semanticCache.default_cache_key" ".properties.koutaku.properties.plugins.properties.semanticCache.properties.config.properties.default_cache_key" "$HELM_SCHEMA"
...
if [ $ERRORS -gt 0 ]; then
  ...
  echo ""
  echo "To fix these errors, update helm-charts/koutaku/values.schema.json to match"
