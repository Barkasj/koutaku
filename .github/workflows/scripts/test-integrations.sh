...
# Test integration tests by building koutaku-http from source, starting it,
...
KOUTAKU_PID=""
TEST_FAILED=0
LOG_FILE="$(mktemp /tmp/koutaku-integrations.XXXXXX.log)"
...
# Step 1: Build koutaku-http from source
echo ""
echo "🔨 Building koutaku-http from source..."
cd "$REPO_ROOT"
...
if [ ! -f "$REPO_ROOT/tmp/koutaku-http" ]; then
  echo "❌ Error: koutaku-http binary not found at $REPO_ROOT/tmp/koutaku-http"
  exit 1
fi

echo "✅ Build complete: $REPO_ROOT/tmp/koutaku-http"

# Step 2: Start Koutaku server with Python integration test config
echo ""
...
KOUTAKU_PID=$!
echo "   Started with PID: $KOUTAKU_PID"


# Wait for server to be ready
echo "⏳ Waiting for Koutaku to be ready..."
MAX_WAIT=30
ELAPSED=0
SERVER_READY=false
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  if curl --connect-timeout 10 --max-time 20 -sf "http://$TEST_HOST:$TEST_PORT/health" > /dev/null 2>&1; then
    SERVER_READY=true
    echo "✅ Koutaku is ready (took ${ELAPSED}s)"
    break
  fi
...
export KOUTAKU_BASE_URL="http://$TEST_HOST:$TEST_PORT"
