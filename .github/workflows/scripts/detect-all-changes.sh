...
KOUTAKU_HTTP_NEEDS_RELEASE="false"
...
DOCKER_CHECK_RESPONSE=$(curl -s "https://registry.hub.docker.com/v2/repositories/koutaku/koutaku/tags/v${TRANSPORT_VERSION}/" 2>/dev/null || echo "")
...
if [ "$GIT_TAG_EXISTS" = "true" ] && [ "$DOCKER_TAG_EXISTS" = "true" ]; then
  ...
  if [ -z "$LATEST_TRANSPORT_TAG" ]; then
    ...
    if [ "$GIT_TAG_EXISTS" = "false" ]; then
      ...
      KOUTAKU_HTTP_NEEDS_RELEASE="true"
    fi
  else
...
echo "   Koutaku HTTP: $KOUTAKU_HTTP_NEEDS_RELEASE (v$TRANSPORT_VERSION)"
...
if [ -n "${GITHUB_OUTPUT:-}" ]; then
  {
    ...
    echo "koutaku-http-needs-release=$KOUTAKU_HTTP_NEEDS_RELEASE"
