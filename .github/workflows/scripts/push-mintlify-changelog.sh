...
<Tabs>
  <Tab title=\"NPX\">
    \`\`\`bash
    npx -y @koutaku/koutaku --transport-version $VERSION
    \`\`\`
  </Tab>
  <Tab title=\"Docker\">
    \`\`\`bash
    docker pull koutaku/koutaku:$VERSION
    docker run -p 8080:8080 koutaku/koutaku:$VERSION
    \`\`\`
  </Tab>
</Tabs>
"
...
# HTTP changelog
append_section "Koutaku(HTTP)" transports/changelog.md
