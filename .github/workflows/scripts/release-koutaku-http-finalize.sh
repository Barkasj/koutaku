...
# Finalize koutaku-http release: changelog, tagging, GitHub release, R2 latest copy
# Usage: ./release-koutaku-http-finalize.sh <version>
...
echo "🏷️ Finalizing koutaku-http v$VERSION release..."
...
done < <(grep "github.com/koutaku/koutaku/plugins/" transports/go.mod)
...
BODY="## Koutaku HTTP Transport Release v$VERSION
...
#### Docker
\`\`\`bash
docker run -p 8080:8080 koutaku/koutaku:v$VERSION
\`\`\`

#### Binary Download
\`\`\`bash
npx @koutaku/koutaku --transport-version v$VERSION
\`\`\`

### Docker Images
- **\`koutaku/koutaku:v$VERSION\`** - This specific version
- **\`koutaku/koutaku:latest\`** - Latest version (updated with this release)
...
echo "✅ Koutaku HTTP released successfully"
...
if [[ "$VERSION" != *-* ]]; then
  if [ -n "${R2_ENDPOINT:-}" ] && [ -n "${R2_BUCKET:-}" ]; then
    ...
    aws s3 sync "s3://$R2_BUCKET/koutaku/v$VERSION/" "s3://$R2_BUCKET/koutaku/latest/" \
