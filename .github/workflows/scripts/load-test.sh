...
# Load Test Script for Koutaku
# Runs a load test against koutaku-http with a mocker provider
# Usage: ./load-test.sh
#
# This script:
# 1. Builds koutaku-http and mocker locally
# 2. Creates a config.json with mocker provider (OpenAI-style)
# 3. Starts mocker with 0ms latency and koutaku-http
# 4. Runs a calibration (Vegeta -> Mocker direct) to measure Vegeta+network baseline
# 5. Runs the overhead test (Vegeta -> Koutaku -> Mocker) to measure total
# 6. Subtracts calibration from test to isolate Koutaku proxy overhead
...
KOUTAKU_HTTP_DIR="${REPO_ROOT}/transports/koutaku-http"
TRANSPORTS_DIR="${REPO_ROOT}/transports"
WORK_DIR="${SCRIPT_DIR}"
MOCKER_DIR="${REPO_ROOT}/../koutaku-benchmarking/mocker"
...
STATS_PID=""
STATS_FILE="${WORK_DIR}/koutaku-stats.csv"

# Overhead-phase process stats (saved before koutaku restart)
...
cleanup() {
  ...
  if [ -n "$KOUTAKU_PID" ] && kill -0 "$KOUTAKU_PID" 2>/dev/null; then
  ...
  rm -f "${WORK_DIR}/config.json" "${WORK_DIR}/logs.db" "${WORK_DIR}/attack.bin" "${WORK_DIR}/calibration.bin" "${WORK_DIR}/stress.bin" "${WORK_DIR}/koutaku.log" "${WORK_DIR}/vegeta-target.json" "${WORK_DIR}/vegeta-target-calibration.json" "${WORK_DIR}/vegeta-target-stress.json" "${WORK_DIR}/vegeta-report.json" "${WORK_DIR}/koutaku-stats.csv" 2>/dev/null || true
...
# Build koutaku-http if binary doesn't exist
build_koutaku_http() {
  if [ -f "${REPO_ROOT}/tmp/koutaku-http" ]; then
    log_success "koutaku-http binary already exists at ${REPO_ROOT}/tmp/koutaku-http"
    return 0
  fi

  log_info "Building koutaku-http..."
  cd "${TRANSPORTS_DIR}"

  if go build -o ${REPO_ROOT}/tmp/koutaku-http .; then
    log_success "koutaku-http built successfully"
  else
    log_error "Failed to build koutaku-http"
    exit 1
  fi
...
# Clone and setup mocker from koutaku-benchmarking
setup_mocker() {
  if [ -d "${REPO_ROOT}/../koutaku-benchmarking" ]; then
    log_info "Updating koutaku-benchmarking repository..."
    cd "${REPO_ROOT}/../koutaku-benchmarking"
    git pull --quiet || true
    cd "${WORK_DIR}"
  else
    log_info "Cloning koutaku-benchmarking repository..."
    cd "${WORK_DIR}"
    git clone --depth 1 https://github.com/koutaku/koutaku-benchmarking.git
  fi
...
# Create config.json for koutaku with mocker provider
create_config() {
  log_info "Creating config.json..."

  cat > "${WORK_DIR}/config.json" << 'EOF'
{
  "$schema": "https://www.getkoutaku.ai/schema",
  "client": {
...
# Stop koutaku-http server
stop_koutaku() {
  if [ -n "$KOUTAKU_PID" ] && kill -0 "$KOUTAKU_PID" 2>/dev/null; then
    log_info "Stopping koutaku (PID: ${KOUTAKU_PID})..."
    kill "$KOUTAKU_PID" 2>/dev/null || true
    wait "$KOUTAKU_PID" 2>/dev/null || true
    KOUTAKU_PID=""
    sleep 1
...
# Start background process stats collection for koutaku
# Samples CPU% and RSS every second, writes to CSV
start_stats_monitor() {
  if [ -z "$KOUTAKU_PID" ] || ! kill -0 "$KOUTAKU_PID" 2>/dev/null; then
    log_warn "Cannot start stats monitor: koutaku not running"
    return
  fi
  ...
  (
    while kill -0 "$KOUTAKU_PID" 2>/dev/null; do
      # ps -o %cpu= -o rss= works on both macOS and Linux
      stats=$(ps -p "$KOUTAKU_PID" -o %cpu=,rss= 2>/dev/null)
      if [ -n "$stats" ]; then
...
stop_stats_monitor() {
  ...
  if command -v awk &> /dev/null; then
    ...
    echo ""
    log_success "Koutaku process stats (single instance, ${samples} samples):"
...
start_koutaku() {
  log_info "Starting koutaku-http on port ${KOUTAKU_PORT}..."

  cd "${WORK_DIR}"
  local koutaku_log="${WORK_DIR}/koutaku.log"
  "${REPO_ROOT}/tmp/koutaku-http" -app-dir "${WORK_DIR}" -port "${KOUTAKU_PORT}" -host "0.0.0.0" -log-level "info" > "${koutaku_log}" 2>&1 &
  KOUTAKU_PID=$!

  # Wait for koutaku to be fully ready (look for "successfully started koutaku" message)
  local max_attempts=60
  local attempt=0
  while ! grep -q "successfully started koutaku" "${koutaku_log}" 2>/dev/null; do
    sleep 1
    attempt=$((attempt + 1))
    if [ $attempt -ge $max_attempts ]; then
      log_error "Koutaku failed to start within ${max_attempts} seconds"
      log_error "Koutaku log output:"
      cat "${koutaku_log}" 2>/dev/null || true
      exit 1
    fi
    # Check if process is still running
    if ! kill -0 "$KOUTAKU_PID" 2>/dev/null; then
      log_error "Koutaku process died unexpectedly"
      log_error "Koutaku log output:"
      cat "${koutaku_log}" 2>/dev/null || true
      exit 1
    fi
  ...
  log_success "Koutaku-http started (PID: ${KOUTAKU_PID})"
}
...
# Overhead test: Vegeta -> Koutaku -> Mocker (with latency)
# Same duration/rate as calibration so percentile distributions are comparable
run_overhead_test() {
  echo ""
  echo "╔═══════════════════════════════════════════════════════════╗"
  echo "║  Overhead Test: Vegeta -> Koutaku -> Mocker (${OVERHEAD_MOCKER_LATENCY_MS}ms)     ║"
  echo "╚═══════════════════════════════════════════════════════════╝"
  echo ""
  log_info "Measuring Koutaku overhead (single instance, mocker at ${OVERHEAD_MOCKER_LATENCY_MS}ms latency)"
  log_info "Duration: ${OVERHEAD_DURATION}s at ${RATE} RPS, ~$(( RATE * OVERHEAD_MOCKER_LATENCY_MS / 1000 )) concurrent requests through Koutaku"
  ...
  cat > "${target_file}" << EOF
{"method": "POST", "url": "http://localhost:${KOUTAKU_PORT}/v1/chat/completions", "header": {"Content-Type": ["application/json"]}, "body": "$(echo -n "${payload}" | base64)"}
EOF
  ...
  # Subtract calibration per bucket: overhead = through_koutaku - direct_to_mocker
  ...
  echo ""
  log_success "Koutaku overhead (per bucket):"
...
# Koutaku Load Test Results (single instance, ${actual_rps} RPS)

## Koutaku Processing Overhead
...
EOF
  ...
  if [ "$failed" -eq 1 ]; then
    echo ""
    log_error "FAILED: Koutaku overhead exceeded tiered thresholds"
...
run_stress_test() {
  ...
  echo "╚═══════════════════════════════════════════════════════════╝"
  echo ""
  log_info "Testing single Koutaku instance under sustained concurrency"
...
## Koutaku Process Stats (single instance)
...
- **Single instance**: All tests run against one koutaku-http process at ${RATE} RPS
- **Overhead measurement**: Mocker at ${OVERHEAD_MOCKER_LATENCY_MS}ms latency, calibration (Vegeta->Mocker) subtracted from test (Vegeta->Koutaku->Mocker)
...
---
*Generated by Koutaku Load Test Script*
EOF
...
main() {
  echo ""
  echo "╔═══════════════════════════════════════════════════════════╗"
  echo "║       Koutaku Load Test (single instance, ${RATE} RPS)        ║"
  echo "╚═══════════════════════════════════════════════════════════╝"
  echo ""

  log_info "Configuration: single koutaku-http instance, ${RATE} RPS"
  ...
  # Restart both mocker and koutaku to ensure a clean fasthttp connection pool.
  # Without restarting koutaku, stale TCP connections from the overhead phase
