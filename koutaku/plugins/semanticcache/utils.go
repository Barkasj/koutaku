...
func (plugin *Plugin) generateEmbedding(ctx *schemas.KoutakuContext, text string) ([]float32, int, error) {
	// Create embedding request
	embeddingReq := &schemas.KoutakuEmbeddingRequest{
	...
	// Generate embedding using koutaku client
...
//
// Parameters:
//   - req: The Koutaku request to hash for semantic cache key generation
//
// Returns:
//   - string: Hexadecimal representation of the xxhash
//   - error: Any error that occurred during request normalization or hashing
func (plugin *Plugin) generateRequestHash(req *schemas.KoutakuRequest) (string, error) {
	...
	hashInput := struct {
		...
		Stream: koutaku.IsStreamRequestType(req.RequestType),
	}
...
func (plugin *Plugin) buildRequestMetadataForCaching(req *schemas.KoutakuRequest) (map[string]interface{}, error) {
...
func (plugin *Plugin) computeRequestParamsHash(req *schemas.KoutakuRequest) (string, error) {
...
func describeRequestShape(req *schemas.KoutakuRequest) string {
	if req == nil {
		return "request=nil"
	}
...
func (plugin *Plugin) extractTextForEmbedding(req *schemas.KoutakuRequest) (string, string, error) {
	metadata, err := plugin.buildRequestMetadataForCaching(req)
	if err != nil {
		return "", "", err
	}
...
func (plugin *Plugin) buildUnifiedMetadata(provider schemas.ModelProvider, model string, paramsHash string, requestHash string, cacheKey string, ttl time.Duration) map[string]interface{} {
	...
	unifiedMetadata["from_koutaku_semantic_cache_plugin"] = true
...
func (plugin *Plugin) addSingleResponse(ctx context.Context, responseID string, res *schemas.KoutakuResponse, embedding []float32, metadata map[string]interface{}, ttl time.Duration) error {
...
func (plugin *Plugin) addStreamingResponse(ctx context.Context, requestID string, storageID string, res *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError, embedding []float32, metadata map[string]interface{}, ttl time.Duration, isFinalChunk bool) error {
	...
	if koutakuErr != nil {
		...
		chunk.FinishReason = koutaku.Ptr("error")
...
func (plugin *Plugin) getInputForCaching(req *schemas.KoutakuRequest) interface{} {
	switch req.RequestType {
	case schemas.TextCompletionRequest, schemas.TextCompletionStreamRequest:
...
func (plugin *Plugin) getNormalizedInputForCaching(req *schemas.KoutakuRequest) interface{} {
	switch req.RequestType {
	case schemas.TextCompletionRequest, schemas.TextCompletionStreamRequest:
...
func (plugin *Plugin) isConversationHistoryThresholdExceeded(req *schemas.KoutakuRequest) bool {
	switch {
	case req.ChatRequest != nil:
