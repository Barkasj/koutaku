...
func TestResponsesAPIBasicFunctionality(t *testing.T) {
	...
	testRequest := CreateBasicResponsesRequest(
		"What is Koutaku? Answer in one short sentence.",
		0.7,
		500,
	)
	...
	// Verify cache hit
	AssertCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response2}, string(CacheTypeDirect))
...
func TestResponsesAPIDifferentParameters(t *testing.T) {
	...
	tests := []struct {
		name        string
		request1    *schemas.KoutakuResponsesRequest
		request2    *schemas.KoutakuResponsesRequest
		shouldCache bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldCache {
				AssertCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response2}, "direct")
				t.Log("✓ Parameters match: cache hit as expected")
			} else {
				AssertNoCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response2})
...
func TestResponsesAPISemanticMatching(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response1})
	WaitForCache(setup.Plugin)
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response2}, "semantic")
...
func TestResponsesAPICacheExpiration(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response3})
...
func TestResponsesAPIWithoutCacheKey(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response})
...
func TestResponsesAPINoStoreFlag(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ResponsesResponse: response2}) // Should not be cached
...
func TestResponsesAPIStreaming(t *testing.T) {
	...
	var streamResponses []schemas.KoutakuResponsesStreamResponse
	for streamMsg := range stream {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in Responses stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuResponsesStreamResponse != nil {
			streamResponses = append(streamResponses, *streamMsg.KoutakuResponsesStreamResponse)
		}
	}
