...
func TestCacheTypeDirectOnly(t *testing.T) {
	...
	testRequest := CreateBasicChatRequest("What is Koutaku?", 0.7, 50)
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response1})
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}, "direct")
...
func TestCacheTypeSemanticOnly(t *testing.T) {
	...
	if response2.ExtraFields.CacheDebug != nil && response2.ExtraFields.CacheDebug.CacheHit {
		AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}, "semantic")
...
func TestCacheTypeDirectWithSemanticFallback(t *testing.T) {
	...
	if response3.ExtraFields.CacheDebug != nil && response3.ExtraFields.CacheDebug.CacheHit {
		AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response3}, "semantic")
		t.Log("✅ Default behavior correctly found semantic match")
	} else {
		t.Log("ℹ️  No semantic match found (normal for different wording)")
		AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response3})
	}
...
func TestCacheTypeWithEmbeddingRequests(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{EmbeddingResponse: response1})
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{EmbeddingResponse: response2}, "direct")
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{EmbeddingResponse: response3})
...
func TestCacheTypePerformanceCharacteristics(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response3}, "direct")
...
func newCrossProviderChatRequest(provider schemas.ModelProvider, model string, requestType schemas.RequestType, prompt string) *schemas.KoutakuRequest {
...
func TestDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	config.CacheByProvider = koutaku.Ptr(false)
	config.CacheByModel = koutaku.Ptr(false)
	...
	seedResponse := &schemas.KoutakuResponse{
			...
			Choices: []schemas.KoutakuResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
						Message: &schemas.ChatMessage{
							...
							Content: &schemas.ChatMessageContent{
								ContentStr: koutaku.Ptr("Go schedules lightweight goroutines in user space onto a smaller pool of OS threads."),
							},
						},
					},
				},
...
func TestStreamingDirectCacheHitPreservesCachedProviderMetadataAcrossProviders(t *testing.T) {
	...
	chunks := []struct {
		...
		{content: " onto a smaller pool of OS threads.", chunkIndex: 1, finishReason: koutaku.Ptr("stop"), streamEnd: true},
	}

	for _, chunk := range chunks {
		seedCtx.SetValue(schemas.KoutakuContextKeyStreamEndIndicator, chunk.streamEnd)
		chunkResponse := &schemas.KoutakuResponse{
			ChatResponse: &schemas.KoutakuChatResponse{
				...
				Choices: []schemas.KoutakuResponseChoice{
					{
						...
						ChatStreamResponseChoice: &schemas.ChatStreamResponseChoice{
							Delta: &schemas.ChatStreamResponseChoiceDelta{
								Content: koutaku.Ptr(chunk.content),
							},
						},
					},
				},
	...
	for chunk := range shortCircuit.Stream {
		...
		extraFields := chunk.KoutakuChatResponse.ExtraFields
		if extraFields.Provider != schemas.OpenAI {
...
func TestCacheTypeDirectUsesChunkLookup(t *testing.T) {
	...
	req := &schemas.KoutakuRequest{
		RequestType: schemas.ChatCompletionRequest,
		ChatRequest: CreateBasicChatRequest("What is Koutaku?", 0.7, 50),
	}
	...
	cachedResponse := &schemas.KoutakuResponse{
		ChatResponse: &schemas.KoutakuChatResponse{
			Choices: []schemas.KoutakuResponseChoice{
				{
					ChatNonStreamResponseChoice: &schemas.ChatNonStreamResponseChoice{
