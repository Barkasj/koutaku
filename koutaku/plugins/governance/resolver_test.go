...
func TestBudgetResolver_ContextPopulation(t *testing.T) {
	...
	vkID, _ := ctx.Value(schemas.KoutakuContextKeyGovernanceVirtualKeyID).(string)
	teamID, _ := ctx.Value(schemas.KoutakuContextKeyGovernanceTeamID).(string)
	customerID, _ := ctx.Value(schemas.KoutakuContextKeyGovernanceCustomerID).(string)
