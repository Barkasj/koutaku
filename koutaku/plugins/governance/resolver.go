...
func (r *BudgetResolver) EvaluateModelAndProviderRequest(ctx *schemas.KoutakuContext, provider schemas.ModelProvider, model string) *EvaluationResult {
...
func (r *BudgetResolver) EvaluateCustomerRequest(ctx *schemas.KoutakuContext, customerID string, request *EvaluationRequest) *EvaluationResult {
...
func (r *BudgetResolver) EvaluateTeamRequest(ctx *schemas.KoutakuContext, teamID string, request *EvaluationRequest) *EvaluationResult {
	// Skip if no teamID
	if teamID == "" {
		return &EvaluationResult{
			Decision: DecisionAllow,
...
func (r *BudgetResolver) EvaluateUserRequest(ctx *schemas.KoutakuContext, userID string, request *EvaluationRequest) *EvaluationResult {
...
func (r *BudgetResolver) EvaluateVirtualKeyRequest(ctx *schemas.KoutakuContext, virtualKeyValue string, provider schemas.ModelProvider, model string, requestType schemas.RequestType, skipRateLimitsAndBudgets bool) *EvaluationResult {
	...
	ctx.SetValue(schemas.KoutakuContextKeyGovernanceVirtualKeyID, vk.ID)
	ctx.SetValue(schemas.KoutakuContextKeyGovernanceVirtualKeyName, vk.Name)
	if vk.Team != nil {
		ctx.SetValue(schemas.KoutakuContextKeyGovernanceTeamID, vk.Team.ID)
		ctx.SetValue(schemas.KoutakuContextKeyGovernanceTeamName, vk.Team.Name)
		if vk.Team.Customer != nil {
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerID, vk.Team.Customer.ID)
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerName, vk.Team.Customer.Name)
		}
	}
	if vk.Customer != nil {
		ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerID, vk.Customer.ID)
		ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerName, vk.Customer.Name)
	}
	if !vk.IsActive {
		return &EvaluationResult{
	...
	for _, pc := range vk.ProviderConfigs {
		if schemas.ModelProvider(pc.Provider) == provider {
			if !pc.AllowAllKeys {
				...
				ctx.SetValue(schemas.KoutakuContextKeyGovernanceIncludeOnlyKeys, includeOnlyKeys)
			}
			break
		}
	}
