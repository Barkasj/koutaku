...
// for the Koutaku HTTP service. It includes middleware for HTTP request tracking
...
const (
	startTimeKey schemas.KoutakuContextKey = "bf-prom-start-time"
)
...
type PushGatewayConfig struct {
	...
	// JobName is the job label for pushed metrics (default: "koutaku")
...
func Init(config *Config, pricingManager *modelcatalog.ModelCatalog, logger schemas.Logger) (*PrometheusPlugin, error) {
	...
	if len(config.CustomLabels) > 0 {
		for _, label := range config.CustomLabels {
			if !containsLabel(defaultKoutakuLabels, label) && !containsLabel(defaultHTTPLabels, label) {
	...
	// Koutaku Upstream Metrics
	koutakuUpstreamRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_upstream_requests_total",
			Help: "Total number of requests forwarded to upstream providers by Koutaku.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuUpstreamLatencySeconds := factory.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "koutaku_upstream_latency_seconds",
			Help:    "Latency of requests forwarded to upstream providers by Koutaku.",
			Buckets: upstreamLatencyBuckets, // Extended range for AI model inference times
		},
		append(append(defaultKoutakuLabels, "is_success"), filteredCustomLabels...),
	)

	koutakuSuccessRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_success_requests_total",
			Help: "Total number of successful requests forwarded to upstream providers by Koutaku.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuErrorRequestsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_error_requests_total",
			Help: "Total number of error requests forwarded to upstream providers by Koutaku.",
		},
		append(append(defaultKoutakuLabels, "status_code"), filteredCustomLabels...),
	)

	koutakuInputTokensTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_input_tokens_total",
			Help: "Total number of input tokens forwarded to upstream providers by Koutaku.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuOutputTokensTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_output_tokens_total",
			Help: "Total number of output tokens forwarded to upstream providers by Koutaku.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuCacheHitsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_cache_hits_total",
			Help: "Total number of cache hits forwarded to upstream providers by Koutaku, separated by cache type (direct/semantic).",
		},
		append(append(defaultKoutakuLabels, "cache_type"), filteredCustomLabels...),
	)

	koutakuCostTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_cost_total",
			Help: "Total cost in USD for requests to upstream providers.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuStreamInterTokenLatencySeconds := factory.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "koutaku_stream_inter_token_latency_seconds",
			Help: "Latency of the intermediate tokens of a stream response.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	koutakuStreamFirstTokenLatencySeconds := factory.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "koutaku_stream_first_token_latency_seconds",
			Help: "Latency of the first token of a stream response.",
		},
		append(defaultKoutakuLabels, filteredCustomLabels...),
	)

	// koutakuKeyRotationEventsTotal counts individual retry/rotation events from the attempt trail.
	...
	koutakuKeyRotationEventsTotal := factory.NewCounterVec(
		prometheus.CounterOpts{
			Name: "koutaku_key_rotation_events_total",
	...
	plugin := &PrometheusPlugin{
		...
		UpstreamRequestsTotal:          koutakuUpstreamRequestsTotal,
		UpstreamLatencySeconds:         koutakuUpstreamLatencySeconds,
		SuccessRequestsTotal:           koutakuSuccessRequestsTotal,
		ErrorRequestsTotal:             koutakuErrorRequestsTotal,
		InputTokensTotal:               koutakuInputTokensTotal,
		OutputTokensTotal:              koutakuOutputTokensTotal,
		CacheHitsTotal:                 koutakuCacheHitsTotal,
		CostTotal:                      koutakuCostTotal,
		StreamInterTokenLatencySeconds: koutakuStreamInterTokenLatencySeconds,
		StreamFirstTokenLatencySeconds: koutakuStreamFirstTokenLatencySeconds,
		KeyRotationEventsTotal:         koutakuKeyRotationEventsTotal,
		customLabels:                   filteredCustomLabels,
		defaultHTTPLabels:              defaultHTTPLabels,
		defaultKoutakuLabels:           defaultKoutakuLabels,
	}
...
func (p *PrometheusPlugin) HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	return nil, nil
}

// HTTPTransportPostHook is not used for this plugin
func (p *PrometheusPlugin) HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	return nil
}

// HTTPTransportStreamChunkHook passes through streaming chunks unchanged
func (p *PrometheusPlugin) HTTPTransportStreamChunkHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, chunk *schemas.KoutakuStreamChunk) (*schemas.KoutakuStreamChunk, error) {
	return chunk, nil
}
...
func (p *PrometheusPlugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
...
//   - Request latency
//   - Total request count
func (p *PrometheusPlugin) PostLLMHook(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	requestType, provider, originalModel, resolvedModel := koutaku.GetResponseFields(result, koutakuErr)
	...
	for _, key := range p.customLabels {
		if value := ctx.Value(schemas.KoutakuContextKey(key)); value != nil {
	...
	promLabelValues := getPrometheusLabelValues(append(p.defaultKoutakuLabels, p.customLabels...), labelValues)

	// Extract stream end indicator BEFORE the goroutine
	streamEndIndicatorValue := ctx.Value(schemas.KoutakuContextKeyStreamEndIndicator)
	...
	go func() {
		...
		latencyLabelValues = append(latencyLabelValues, promLabelValues[:len(p.defaultKoutakuLabels)]...) // all default labels
		latencyLabelValues = append(latencyLabelValues, strconv.FormatBool(koutakuErr == nil))            // is_success
		latencyLabelValues = append(latencyLabelValues, promLabelValues[len(p.defaultKoutakuLabels):]...) // then custom labels
		...
		if koutakuErr != nil {
			...
			if koutakuErr.StatusCode != nil {
				statusCode = strconv.Itoa(*koutakuErr.StatusCode)
			}
			errorPromLabelValues := make([]string, 0, len(promLabelValues)+1)
			errorPromLabelValues = append(errorPromLabelValues, promLabelValues[:len(p.defaultKoutakuLabels)]...) // all default labels
			errorPromLabelValues = append(errorPromLabelValues, statusCode)                                       // status_code
			errorPromLabelValues = append(errorPromLabelValues, promLabelValues[len(p.defaultKoutakuLabels):]...) // then custom labels
		...
		if result != nil {
			...
			if extraFields.CacheDebug != nil && extraFields.CacheDebug.CacheHit {
				...
				cacheHitLabelValues = append(cacheHitLabelValues, promLabelValues[:len(p.defaultKoutakuLabels)]...) // all default labels
				cacheHitLabelValues = append(cacheHitLabelValues, cacheType)                                        // cache_type
				cacheHitLabelValues = append(cacheHitLabelValues, promLabelValues[len(p.defaultKoutakuLabels):]...) // then custom labels
...
//   - HTTP status codes
//   - Koutaku upstream requests and errors
