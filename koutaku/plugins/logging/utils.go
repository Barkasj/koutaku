...
func (p *LoggerPlugin) extractInputHistory(request *schemas.KoutakuRequest) ([]schemas.ChatMessage, []schemas.ResponsesMessage) {
...
func mergeRealtimeMetadata(metadata map[string]interface{}, ctx *schemas.KoutakuContext) map[string]interface{} {
	if ctx == nil {
		return metadata
	}
	set := func(key string, ctxKey schemas.KoutakuContextKey) {
		if value := koutaku.GetStringFromContext(ctx, ctxKey); value != "" {
			if metadata == nil {
	...
	set("realtime_session_id", schemas.KoutakuContextKeyRealtimeSessionID)
	set("provider_session_id", schemas.KoutakuContextKeyRealtimeProviderSessionID)
	set("realtime_source", schemas.KoutakuContextKeyRealtimeSource)
	set("realtime_event_type", schemas.KoutakuContextKeyRealtimeEventType)
	if koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyRealtimeSessionID) != "" {
		if metadata == nil {
			metadata = make(map[string]interface{})
		}
