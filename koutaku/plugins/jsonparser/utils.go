...
func (p *JsonParserPlugin) getRequestID(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse) string {
	...
	if ctx != nil {
		if requestID, ok := ctx.Value(schemas.KoutakuContextKeyRequestID).(string); ok && requestID != "" {
			return requestID
		}
	}
...
func (p *JsonParserPlugin) shouldRun(ctx *schemas.KoutakuContext, requestType schemas.RequestType) bool {
...
// deepCopyKoutakuResponse creates a deep copy of KoutakuResponse to avoid modifying the original
func (p *JsonParserPlugin) deepCopyKoutakuResponse(original *schemas.KoutakuResponse) *schemas.KoutakuResponse {
	if original == nil {
		return nil
	}

	// Create a new KoutakuResponse
	result := &schemas.KoutakuResponse{}

	// Copy ChatResponse if it exists (this is what we're interested in for the JSON parser)
	if original.ChatResponse != nil {
		result.ChatResponse = p.deepCopyKoutakuChatResponse(original.ChatResponse)
	}
...
// deepCopyKoutakuChatResponse creates a deep copy of KoutakuChatResponse
func (p *JsonParserPlugin) deepCopyKoutakuChatResponse(original *schemas.KoutakuChatResponse) *schemas.KoutakuChatResponse {
	if original == nil {
		return nil
	}

	result := &schemas.KoutakuChatResponse{
		ID:                original.ID,
	...
	if original.Choices != nil {
		result.Choices = make([]schemas.KoutakuResponseChoice, len(original.Choices))
		for i, choice := range original.Choices {
			result.Choices[i] = p.deepCopyKoutakuResponseChoice(choice)
		}
	}
...
// deepCopyKoutakuResponseChoice creates a deep copy of KoutakuResponseChoice
func (p *JsonParserPlugin) deepCopyKoutakuResponseChoice(original schemas.KoutakuResponseChoice) schemas.KoutakuResponseChoice {
	result := schemas.KoutakuResponseChoice{
