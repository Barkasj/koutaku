// Package prompts implements the Koutaku LLM plugin that resolves stored prompt templates
...
const (
	...
	// PromptIDHeader and PromptVersionHeader are request headers copied into KoutakuContext
	...
	PromptIDKey      schemas.KoutakuContextKey = PromptIDHeader
	PromptVersionKey schemas.KoutakuContextKey = PromptVersionHeader
)
...
type PromptResolver interface {
	Resolve(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (promptID string, versionNumber int, err error)
}
...
func (r *headerResolver) Resolve(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (string, int, error) {
	promptID := koutaku.GetStringFromContext(ctx, PromptIDKey)
	if promptID == "" {
		return "", 0, nil
	}
...
//   - logger: Koutaku logger for non-fatal merge/param warnings
...
// into KoutakuContext so the default header resolver and PreLLMHook can read them.
func (p *Plugin) HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	if req == nil {
		return nil, nil
	}
...
func (p *Plugin) HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	return nil
}
...
func (p *Plugin) HTTPTransportStreamChunkHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, chunk *schemas.KoutakuStreamChunk) (*schemas.KoutakuStreamChunk, error) {
	return chunk, nil
}
...
//
// Parameters:
//   - ctx: may set KoutakuContextKeySelectedPromptName, KoutakuContextKeySelectedPromptID and KoutakuContextKeySelectedPromptVersion when a prompt is applied
//   - req: chat or Responses request to mutate in place
//
// Returns:
//   - *schemas.KoutakuRequest: possibly modified request
...
func (p *Plugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	if req == nil {
		return req, nil, nil
	}
	...
	if prompt != nil && prompt.Name != "" {
		ctx.SetValue(schemas.KoutakuContextKeySelectedPromptID, prompt.ID)
		ctx.SetValue(schemas.KoutakuContextKeySelectedPromptName, prompt.Name)
	}
	ctx.SetValue(schemas.KoutakuContextKeySelectedPromptVersion, strconv.Itoa(version.VersionNumber))
...
func (p *Plugin) PostLLMHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
...
func applyVersionParamsToChatRequest(version *configstoreTables.TablePromptVersion, req *schemas.KoutakuChatRequest, logger schemas.Logger) {
...
func applyVersionParamsToResponsesRequest(version *configstoreTables.TablePromptVersion, req *schemas.KoutakuResponsesRequest, logger schemas.Logger) {
	if len(version.ModelParams) == 0 {
		return
	}
...
func parseNumberFromContext(ctx *schemas.KoutakuContext, key schemas.KoutakuContextKey) (num int, err error) {
