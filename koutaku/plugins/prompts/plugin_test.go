...
func TestChatMessageFromStoredJSON(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// payload is a KoutakuChatResponse; message lives at choices[0].message.
...
// KoutakuChatRequest. The model chosen by the caller must always win.
...
// in the Koutaku UI's completion_result format (payload.choices[0].message) are
