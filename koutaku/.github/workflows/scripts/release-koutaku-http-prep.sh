...
# Prepare koutaku-http release: update dependencies, build UI, validate, commit/push
# Usage: ./release-koutaku-http-prep.sh <version>
...
echo "🚀 Preparing koutaku-http v$VERSION release..."
...
done < <(grep "github.com/koutaku/koutaku/plugins/" go.mod)
cd ..
...
for plugin_name in "${!PLUGIN_VERSIONS[@]}"; do
  ...
  if grep -q "github.com/koutaku/koutaku/plugins/$plugin_name" go.mod; then
    echo "  📦 Using $plugin_name plugin $plugin_version"
    # Textual require bump — skips loading the currently-declared version's go.mod
    go mod edit -require="github.com/koutaku/koutaku/plugins/$plugin_name@$plugin_version"
  fi
done
...
go mod edit -require="github.com/koutaku/koutaku/core@$CORE_VERSION"

echo "  📦 Updating framework to $FRAMEWORK_VERSION"
go mod edit -require="github.com/koutaku/koutaku/framework@$FRAMEWORK_VERSION"
...
echo "✅ Prep complete for koutaku-http v$VERSION"
