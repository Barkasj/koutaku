...
# Helm template validation script for Koutaku
...
test_template() {
  ...
  if helm template koutaku ./helm-charts/koutaku \
...
test_template "semanticCache: direct mode (dimension: 1)" \
  --set koutaku.plugins.semanticCache.enabled=true \
  --set koutaku.plugins.semanticCache.config.dimension=1 \
  --set koutaku.plugins.semanticCache.config.ttl=30m \
  --set vectorStore.enabled=true \
  --set vectorStore.type=redis \
  --set vectorStore.redis.enabled=true
...
test_template "semanticCache: semantic mode (dimension: 1536)" \
  --set koutaku.plugins.semanticCache.enabled=true \
  --set koutaku.plugins.semanticCache.config.dimension=1536 \
  --set koutaku.plugins.semanticCache.config.provider=openai \
  --set 'koutaku.plugins.semanticCache.config.keys[0]=sk-test' \
  --set vectorStore.enabled=true \
  --set vectorStore.type=redis \
  --set vectorStore.redis.enabled=true
...
test_template "ingress enabled" \
  ...
  --set 'ingress.hosts[0].host=koutaku.example.com' \
...
test_template "client: new properties (Gap 1+2)" \
  --set koutaku.client.asyncJobResultTTL=300 \
  --set 'koutaku.client.requiredHeaders[0]=X-Request-ID' \
  --set 'koutaku.client.loggingHeaders[0]=X-Trace-ID' \
  --set 'koutaku.client.allowedHeaders[0]=Authorization' \
  --set koutaku.client.mcpAgentDepth=5 \
  --set koutaku.client.mcpToolExecutionTimeout=30 \
  --set koutaku.client.mcpCodeModeBindingLevel=server \
  --set koutaku.client.mcpToolSyncInterval=60 \
  --set koutaku.client.hideDeletedVirtualKeysInFilters=true

# Gap 3: OTel plugin with new fields
test_template "otel: headers + tls_ca_cert + insecure (Gap 3)" \
  --set koutaku.plugins.otel.enabled=true \
  --set koutaku.plugins.otel.config.collector_url=otel:4317 \
  --set koutaku.plugins.otel.config.trace_type=genai_extension \
  --set koutaku.plugins.otel.config.protocol=grpc \
  --set 'koutaku.plugins.otel.config.headers.Authorization=Bearer token' \
  --set koutaku.plugins.otel.config.tls_ca_cert=/certs/ca.pem \
  --set koutaku.plugins.otel.config.insecure=true

# Gap 4: Governance plugin with new fields
test_template "governance: required_headers + is_enterprise (Gap 4)" \
  --set koutaku.plugins.governance.enabled=true \
  --set 'koutaku.plugins.governance.config.required_headers[0]=X-Team-ID' \
  --set koutaku.plugins.governance.config.is_enterprise=true

# Gap 5: Governance modelConfigs + providers
test_template "governance: modelConfigs + providers (Gap 5)" \
  --set 'koutaku.governance.modelConfigs[0].id=mc-1' \
  --set 'koutaku.governance.modelConfigs[0].model_name=gpt-4o' \
  --set 'koutaku.governance.providers[0].name=openai'

# Gap 6: MCP new fields
test_template "mcp: toolSyncInterval + codeModeBindingLevel (Gap 6)" \
  --set koutaku.mcp.enabled=true \
  --set koutaku.mcp.toolSyncInterval=10m \
  --set koutaku.mcp.toolManagerConfig.codeModeBindingLevel=server \
  --set 'koutaku.mcp.clientConfigs[0].name=test' \
  --set 'koutaku.mcp.clientConfigs[0].connectionType=http' \
  --set 'koutaku.mcp.clientConfigs[0].httpConfig.url=http://localhost:3000' \
  --set 'koutaku.mcp.clientConfigs[0].clientId=client-1' \
  --set 'koutaku.mcp.clientConfigs[0].isCodeModeClient=true' \
  --set 'koutaku.mcp.clientConfigs[0].toolSyncInterval=5m'

# Gap 7: Cluster with region
test_template "cluster: region (Gap 7)" \
  --set koutaku.cluster.enabled=true \
  --set 'koutaku.cluster.peers[0]=peer-0:7946' \
  --set koutaku.cluster.gossip.port=7946 \
  --set koutaku.cluster.gossip.config.timeoutSeconds=10 \
  --set koutaku.cluster.gossip.config.successThreshold=3 \
  --set koutaku.cluster.gossip.config.failureThreshold=3 \
  --set koutaku.cluster.region=us-east-1
...
test_template "combined: all new Gap 1-8 fields" \
  ...
  --set koutaku.cluster.gossip.config.successThreshold=3 \
  --set koutaku.cluster.gossip.config.failureThreshold=3 \
  --set koutaku.cluster.region=us-west-2
...
test_template "custom plugin with placement and order" \
  --set 'koutaku.plugins.custom[0].name=my-plugin' \
  --set 'koutaku.plugins.custom[0].enabled=true' \
  --set 'koutaku.plugins.custom[0].path=/plugins/my-plugin.so' \
  --set 'koutaku.plugins.custom[0].placement=pre_builtin' \
  --set 'koutaku.plugins.custom[0].order=2'
