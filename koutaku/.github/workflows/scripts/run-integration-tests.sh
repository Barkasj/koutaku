...
# Run integration tests with Koutaku binary and PostgreSQL
# Usage: ./run-integration-tests.sh <koutaku-binary-path> [port]
...
# Parse arguments
if [ "${1:-}" = "" ]; then
  echo "Usage: $0 <koutaku-binary-path> [port]" >&2
  echo "" >&2
  echo "Arguments:" >&2
  echo "  koutaku-binary-path  Path to the koutaku-http binary" >&2
  echo "  port                 Port to run Koutaku on (default: 8080)" >&2
  exit 1
fi
...
POSTGRES_DB="${POSTGRES_DB:-koutaku}"
POSTGRES_SSLMODE="${POSTGRES_SSLMODE:-disable}"

# Validate binary exists and is executable
if [ ! -f "$KOUTAKU_BINARY" ]; then
  echo "❌ Error: Koutaku binary not found: $KOUTAKU_BINARY" >&2
  exit 1
fi

if [ ! -x "$KOUTAKU_BINARY" ]; then
  echo "❌ Error: Koutaku binary is not executable: $KOUTAKU_BINARY" >&2
  exit 1
fi

echo "🧪 Running Koutaku Integration Tests"
echo "   Binary: $KOUTAKU_BINARY"
echo "   Port: $PORT"
...
cleanup() {
  ...
  # Kill Koutaku server if running
  if [ -n "${KOUTAKU_PID:-}" ]; then
    echo "   Stopping Koutaku server (PID: $KOUTAKU_PID)..."
...
postgres_config = {
    ...
    "user": os.environ.get("POSTGRES_USER", "koutaku"),
    "password": os.environ.get("POSTGRES_PASSWORD", "koutaku_password"),
    "db_name": os.environ.get("POSTGRES_DB", "koutaku"),
...
# Start Koutaku server
echo "🚀 Starting Koutaku server..."
SERVER_LOG="$TEMP_DIR/server.log"

"$KOUTAKU_BINARY" --app-dir "$TEMP_DIR" --port "$PORT" --log-level debug > "$SERVER_LOG" 2>&1 &
KOUTAKU_PID=$!

echo "   Started Koutaku with PID: $KOUTAKU_PID"

# Wait for server to be ready
echo "⏳ Waiting for Koutaku to start..."
MAX_WAIT=60
ELAPSED=0
SERVER_READY=false

while [ $ELAPSED -lt $MAX_WAIT ]; do
  if grep -q "successfully started koutaku" "$SERVER_LOG" 2>/dev/null; then
    SERVER_READY=true
    echo "   ✅ Koutaku started successfully"
    break
  fi
  
  # Check if server process is still running
  if ! kill -0 "$KOUTAKU_PID" 2>/dev/null; then
    echo "   ❌ Koutaku process died unexpectedly"
    echo "   Server log:"
    cat "$SERVER_LOG"
    exit 1
  fi
...
if [ "$SERVER_READY" = false ]; then
  echo "   ❌ Koutaku failed to start within ${MAX_WAIT}s"
  echo "   Server log:"
  cat "$SERVER_LOG"
  exit 1
fi
...
export KOUTAKU_BASE_URL="http://localhost:$PORT"
echo "   KOUTAKU_BASE_URL=$KOUTAKU_BASE_URL"
