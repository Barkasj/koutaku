...
IMAGE_TAG="koutaku-test:ci-${GITHUB_SHA:-local}-${ARCH}"
CONTAINER_NAME="koutaku-test-${ARCH}"
TEST_PORT=8080
...
cleanup() {
  ...
  # Stop and remove Koutaku container
  echo "Stopping Koutaku container..."
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  if docker compose -f "$DOCKER_COMPOSE_FILE" exec -T postgres pg_isready -U koutaku -d koutaku > /dev/null 2>&1; then
...
NETWORK_NAME=$(docker compose -f "$DOCKER_COMPOSE_FILE" ps --format json | head -1 | jq -r '.Networks' 2>/dev/null || echo "tests_koutaku_network")
if [ -z "$NETWORK_NAME" ] || [ "$NETWORK_NAME" = "null" ]; then
  NETWORK_NAME="tests_koutaku_network"
fi
...
# Run the Koutaku container connected to the docker-compose network
echo ""
echo "=== Starting Koutaku container ==="
docker run -d \
  --name "${CONTAINER_NAME}" \
  --platform "${PLATFORM}" \
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  if curl -sf "http://localhost:${TEST_PORT}/health" > /dev/null 2>&1; then
    echo "Koutaku health check passed (attempt $((ELAPSED/2 + 1)))"
    HEALTH_OK=1
    break
  fi
  sleep 2
...
if [ $HEALTH_OK -eq 0 ]; then
  echo "ERROR: Koutaku health check failed!"
  echo "Container logs:"
...
# export KOUTAKU_BASE_URL="http://localhost:${TEST_PORT}"
# export CI=1
