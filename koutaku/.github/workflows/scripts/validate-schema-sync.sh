...
# Validate that Go config types in transports/koutaku-http/lib/config.go
...
# it to resolve koutaku's local modules against each other. On fresh CI
# runners go.work is not checked in, so we provision it here inline.
# Sibling scripts (test-koutaku-http.sh etc.) call setup-go-workspace.sh
...
if [ ! -f "$REPO_ROOT/go.work" ]; then
  ...
  (
    ...
    for mod in ./core ./framework \
               ...
               ./plugins/logging  ./plugins/mocker \
...
# GOWORK=<repo-root>/go.work when loading koutaku packages.
(cd "$TOOL_DIR" && GOWORK=off go build -o /tmp/schemasync .)
/tmp/schemasync \
  --schema "$REPO_ROOT/transports/config.schema.json" \
  --pkg-root "$REPO_ROOT" \
  --helm-values "$REPO_ROOT/helm-charts/koutaku/values.schema.json" \
  --helm-helpers "$REPO_ROOT/helm-charts/koutaku/templates/_helpers.tpl"
