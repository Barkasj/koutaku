...
# Usage:
#   ./test-e2e-api.sh                          # Koutaku already running at KOUTAKU_BASE_URL
#   ./test-e2e-api.sh <koutaku-binary> [port]   # Start Koutaku with config, then run tests
# Config: tests/integrations/python/config.json (merged with Postgres when starting server)
# Requires: Newman installed; provider API keys in environment when starting Koutaku
...
export KOUTAKU_BASE_URL="${KOUTAKU_BASE_URL:-http://localhost:8080}"

# ----- Optional: start Koutaku if binary path given -----
if [ -n "${1:-}" ]; then
  KOUTAKU_BINARY="$1"
  PORT="${2:-8080}"
  ...
  if [ ! -f "$KOUTAKU_BINARY" ] || [ ! -x "$KOUTAKU_BINARY" ]; then
    echo "❌ Koutaku binary not found or not executable: $KOUTAKU_BINARY" >&2
    exit 1
  fi
...
pg = {"host": os.environ.get("POSTGRES_HOST", "localhost"), "port": os.environ.get("POSTGRES_PORT", "5432"), "user": os.environ.get("POSTGRES_USER", "koutaku"), "password": os.environ.get("POSTGRES_PASSWORD", "koutaku_password"), "db_name": os.environ.get("POSTGRES_DB", "koutaku"), "ssl_mode": os.environ.get("POSTGRES_SSLMODE", "disable")}
...
PYEOF
  ...
  echo "🚀 Starting Koutaku on port $PORT..."
  ...
  while [ $ELAPSED -lt $MAX_WAIT ]; do
    if grep -q "successfully started koutaku" "$SERVER_LOG" 2>/dev/null; then
      echo "   ✅ Koutaku started"
      break
    fi
    if ! kill -0 "$KOUTAKU_PID" 2>/dev/null; then
      echo "   ❌ Koutaku process exited"
      cat "$SERVER_LOG"
      exit 1
    fi
    sleep 1
  ...
  if [ $ELAPSED -ge $MAX_WAIT ]; then
    echo "   ❌ Koutaku did not start within ${MAX_WAIT}s"
    cat "$SERVER_LOG"
    exit 1
  fi
