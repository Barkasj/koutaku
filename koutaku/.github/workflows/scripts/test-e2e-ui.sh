...
# Build koutaku-http binary
echo "🔨 Building koutaku-http binary..."
mkdir -p tmp
cd transports/koutaku-http
go build -o ../../tmp/koutaku-http .
cd ../..
...
docker exec -e PGPASSWORD=koutaku_password "$(docker compose -f "$CONFIGS_DIR/docker-compose.yml" ps -q postgres)" \
  psql -U koutaku -d postgres \
  -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'koutaku' AND pid <> pg_backend_pid();" \
  -c "DROP DATABASE IF EXISTS koutaku;" \
  -c "CREATE DATABASE koutaku;"

# Start koutaku-http server with default config
SERVER_LOG=$(mktemp)
echo "🚀 Starting koutaku-http server..."
./tmp/koutaku-http --app-dir "$CONFIGS_DIR/default" --port 18080 --log-level debug 2>&1 | tee "$SERVER_LOG" &
SERVER_PID=$!
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  if grep -q "successfully started koutaku, serving UI on http://localhost:18080" "$SERVER_LOG" 2>/dev/null; then
    SERVER_READY=true
...
# Run Playwright tests (BASE_URL = browser; KOUTAKU_BASE_URL = global-setup API calls).
# Forward MCP_SSE_HEADERS so the mcp-registry SSE test can use it (set in workflow env).
echo "🎭 Running Playwright E2E tests..."
CI=true SKIP_WEB_SERVER=1 BASE_URL=http://localhost:18080 KOUTAKU_BASE_URL=http://localhost:18080 \
