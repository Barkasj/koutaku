...
<Update label=\"Koutaku CLI\" description=\"v$VERSION\">
$CHANGELOG_CONTENT
...
# Update docs.json to include this new changelog route in the Koutaku CLI menu
...
if ! grep -q "\"$route\"" docs/docs.json; then
  node -e "
    ...
    // Find the Koutaku CLI menu item
    const cliMenuItem = changelogsTab.menu.find(item => item.item === 'Koutaku CLI');
    if (!cliMenuItem) {
      console.error('Koutaku CLI menu item not found');
      process.exit(1);
    }
