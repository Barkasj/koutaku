...
The repository is always `koutaku/koutaku`.

Fetch full issue details:
```bash
# Get issue with all metadata
gh issue view <ISSUE_ID> --repo koutaku/koutaku --json number,title,body,labels,assignees,state,comments,author,createdAt,updatedAt

# Get issue comments for additional context
gh issue view <ISSUE_ID> --repo koutaku/koutaku --json comments --jq '.comments[].body'
```

If the issue does not exist or `gh` fails:
- Check authentication: `gh auth status`
- Verify the issue number is valid: `gh issue list --repo koutaku/koutaku --limit 5 --json number,title`
...
|---|---|---|
| Core (Go) | `core/`, `core/schemas/`, `core/providers/`, `core/mcp/` | `core/koutaku.go`, `core/utils.go` |
| Framework | `framework/`, `framework/configstore/`, `framework/logstore/` | `framework/config.go`, `framework/list.go` |
| Transports (HTTP) | `transports/koutaku-http/` | `transports/koutaku-http/` |
...
# Check the main koutaku.go for relevant handlers
grep -n "functionName\|handlerName" core/koutaku.go
```
...
grep -rn '"github.com/koutaku/koutaku/core/schemas"' --include='*.go' .
```
...
- Signature: `func Run{Scenario}Test(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig)`
...
### Issue Not Found
```
Issue #<ID> was not found in koutaku/koutaku.
- Verify the issue number is correct
- Run: gh issue list --repo koutaku/koutaku --limit 10 --json number,title
```
...
Quick reference for navigating the Koutaku codebase:

```
koutaku/
├── core/                          # Go core library
│   ├── koutaku.go                 # Main Koutaku implementation (~195K)
...
├── transports/
│   └── koutaku-http/              # HTTP transport + Docker
├── ui/                            # React + Vite UI
