...
This is a sample MCP server implementation that runs over HTTP but **does not support the optional `ping` method**. This demonstrates how to configure Koutaku to use the `listTools` health check method instead of ping.
...
2. **How to configure Koutaku** to work with such servers using `is_ping_available: false`
3. **Why this matters**: When `is_ping_available` is `false`, Koutaku will use `listTools` for health checks instead of the lightweight `ping` method
...
Note: This server does NOT support ping. Use is_ping_available=false in Koutaku config.
```

## Connecting via Koutaku
...
When you add this server to Koutaku with `is_ping_available: false`:

1. Koutaku will **NOT** send `ping` requests (since the server doesn't support them)
2. Instead, Koutaku will use `listTools` every 10 seconds to check server health
...
- ✅ Demonstrate the `is_ping_available=false` behavior in Koutaku
...
- [Koutaku MCP Documentation](../../docs/mcp/connecting-to-servers.mdx)
