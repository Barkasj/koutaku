# Koutaku WASM Plugin (TypeScript/AssemblyScript)

A comprehensive example of a Koutaku plugin written in TypeScript and compiled to WebAssembly using AssemblyScript. This plugin demonstrates proper structure definitions, JSON parsing, context handling, and request/response modification patterns.
...
class LLMPluginShortCircuit {
  response: KoutakuResponse | null = null  // Success short-circuit
  error: KoutakuError | null = null        // Error short-circuit
}
```
...
export function pre_hook(inputPtr: u32, inputLen: u32): u64 {
  ...
  if (model === 'mock-model') {
    ...
    // Build mock response
    const mockResponse = new KoutakuResponse()
    mockResponse.chat_response = new KoutakuChatResponse()
