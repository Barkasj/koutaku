...
- **HTTPTransportPreHook**: Intercepts HTTP requests before they enter Koutaku core
...
- **HTTPTransportPostHook**: Intercepts HTTP responses after Koutaku core processing
...
| `rate_limit` | integer | `10` | Koutakuum requests per window (0 = unlimited) |
| `rate_window` | integer | `60` | Rate limit window in seconds |
| `max_body_size` | integer | `1048576` | Koutakuum request body size in bytes (0 = unlimited) |
...
- Works only when using koutaku-http, not when using Koutaku as a Go SDK
