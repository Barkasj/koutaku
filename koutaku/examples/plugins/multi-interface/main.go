...
func HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	ctx.SetValue(schemas.KoutakuContextKey("multi-http-request-time"), time.Now())
	ctx.SetValue(schemas.KoutakuContextKey("multi-http-path"), req.Path)
...
func HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	...
	if startTime, ok := ctx.Value(schemas.KoutakuContextKey("multi-http-request-time")).(time.Time); ok {
...
func PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	...
	if pluginConfig.EnableLogging {
		...
		httpPath := ctx.Value(schemas.KoutakuContextKey("multi-http-path"))
		fmt.Printf("[Multi-Interface Plugin] Processing LLM request from path: %v\n", httpPath)
	}

	// Store LLM metadata
	ctx.SetValue(schemas.KoutakuContextKey("multi-llm-start-time"), time.Now())
...
func PostLLMHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	...
	if startTime, ok := ctx.Value(schemas.KoutakuContextKey("multi-llm-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		ctx.SetValue(schemas.KoutakuContextKey("multi-llm-duration"), duration)
	}
...
func PreMCPHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	if !pluginConfig.EnableMCPHooks {
		return req, nil, nil
	}
	...
	// Store MCP metadata
	ctx.SetValue(schemas.KoutakuContextKey("multi-mcp-start-time"), time.Now())
	ctx.SetValue(schemas.KoutakuContextKey("multi-mcp-type"), req.RequestType)
...
func PostMCPHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError, error) {
	if !pluginConfig.EnableMCPHooks {
		return resp, koutakuErr, nil
	}
	...
	if startTime, ok := ctx.Value(schemas.KoutakuContextKey("multi-mcp-start-time")).(time.Time); ok {
		duration := time.Since(startTime)
		if pluginConfig.EnableLogging {
		...
		// Store for observability
		ctx.SetValue(schemas.KoutakuContextKey("multi-mcp-duration"), duration)
	}
