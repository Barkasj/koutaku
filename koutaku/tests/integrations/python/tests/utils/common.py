...
# GenAI-specific invalid role content that passes SDK validation but fails at Koutaku
GENAI_INVALID_ROLE_CONTENT = [
    {
        "role": "tester",  # Invalid role that should be caught by Koutaku
...
def assert_error_propagation(error_response: Any, integration: str):
    """
    Assert that error is properly propagated through Koutaku to the integration.
...
def get_bedrock_s3_config() -> Dict[str, Optional[str]]:
    ...
    return {
        ...
        "output_s3_prefix": os.environ.get("AWS_OUTPUT_S3_PREFIX", "koutaku-batch-output/"),
...
def create_bedrock_batch_s3_uri(
    bucket: str, prefix: str = "koutaku-batch-input/", filename: str | None = None
) -> str:
    """
...
def get_ws_base_url():
    ...
    config = get_config()
    base_url = config._config["koutaku"]["base_url"]
