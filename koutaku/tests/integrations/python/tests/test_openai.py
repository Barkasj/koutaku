...
This test suite uses the OpenAI SDK to test against multiple AI providers through Koutaku.
...
class TestOpenAIIntegration:
    ...
    def test_50_batch_e2e_file_api(self, test_config, provider, model, vk_enabled):
        ...
        try:
            ...
            batch = client.batches.create(
                ...
                metadata={
                    "test": "e2e_file",
                    "source": "koutaku-integration-tests",
                },
                extra_body={
                    "provider": provider,
                    "model": model,
    ...
    def test_64_realtime_client_secret_routes(self, test_config, provider, _model, vk_enabled):
        ...
        config = get_config()
        base_url = config._config["koutaku"]["base_url"].rstrip("/")
        api_key = get_api_key(provider)
    ...
    def test_65_realtime_client_secret_openai_base_url_compatibility(
        ...
        config = get_config()
        api_key = get_api_key(provider)
        root_base_url = config._config["koutaku"]["base_url"].rstrip("/")
