...
# that Koutaku's LiteLLM integration doesn't support
...
def setup_litellm(monkeypatch):
    """Setup LiteLLM with Koutaku configuration and dummy credentials"""
    import os
    ...
    os.environ["MISTRAL_API_KEY"] = "dummy-mistral-key-koutaku-handles-auth"
    ...
    # LiteLLM will load these dummy credentials but all actual requests go through Koutaku
    ...
    # Since Koutaku handles auth, we don't need LiteLLM to authenticate
    def mock_refresh(self, request):
        """Mock refresh that sets a dummy token - Koutaku handles real auth"""
        import datetime

        self.token = "dummy-access-token-koutaku-handles-auth"
