...
TypeScript/JavaScript integration test suite for testing AI providers through Koutaku proxy. This test suite uses Vitest and provides comprehensive coverage across multiple AI SDKs.

## Quick Start

```bash
# 1. Install dependencies
cd koutaku/tests/integrations/typescript
npm install
...
The TypeScript integration tests use the same centralized configuration as the Python tests, routing all AI requests through Koutaku:

```text
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Test Client   │───▶│  Koutaku Gateway │───▶│  AI Provider    │
...
The OpenAI test file supports cross-provider testing through Koutaku's model name routing. By formatting the model name as `provider/model`, Koutaku routes the request to the appropriate provider:
...
This allows testing any provider using the OpenAI SDK format while Koutaku handles the routing based on the model name prefix.
...
```
Solution: Ensure Koutaku is running on the expected port.
...
```
Solution: Check network connectivity and Koutaku logs.
