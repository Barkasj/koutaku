...
# Koutaku Bedrock Integration API Newman Test Runner
...
# Configuration
COLLECTION="collections/koutaku-bedrock-integration.postman_collection.json"
ENVIRONMENT="koutaku-v1.postman_environment.json"
...
if [ "$ci_normalized" = "1" ] || [ "$ci_normalized" = "true" ]; then
    echo -e "${GREEN}Koutaku Bedrock Integration API Test Runner with retries: 10${NC}"
else
    echo -e "${GREEN}Koutaku Bedrock Integration API Test Runner${NC}"
fi
echo -e "${GREEN}========================================${NC}"
echo ""
...
if [ -z "$PROVIDER_ENV_FILE" ] && [ -d "$PROVIDER_CONFIG_DIR" ]; then
    ...
    while IFS= read -r -d '' f; do
        # basename: koutaku-v1-openai.postman_environment.json -> openai
        name="${f##*/}"
        name="${name#koutaku-v1-}"
    ...
    done < <(find "$PROVIDER_CONFIG_DIR" -maxdepth 1 -name "koutaku-v1-*.postman_environment.json" -print0 2>/dev/null | sort -z)
fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --env <path>        Postman env .json path or provider name (e.g. provider_config/koutaku-v1-openai.postman_environment.json or openai)"
            echo "  --help              Show this help message"
            echo ""
            echo "Environment Variables:"
            echo "  KOUTAKU_BASE_URL    Override base URL (default: http://localhost:8080)"
            echo "  KOUTAKU_PROVIDER    Override provider (default: openai)"
            echo "  KOUTAKU_MODEL       Override model name (default: gpt-4o)"
            echo "  KOUTAKU_MODEL_INVOKE Override model for Bedrock invoke/invoke-stream (default: amazon.titan-text-express-v1)"
            echo "  KOUTAKU_EMBEDDING_MODEL    Override embedding model (default: text-embedding-3-small)"
            echo "  KOUTAKU_SPEECH_MODEL       Override speech model (default: tts-1)"
            echo "  KOUTAKU_TRANSCRIPTION_MODEL  Override transcription model (default: whisper-1)"
            echo "  KOUTAKU_IMAGE_MODEL        Override image model (default: dall-e-3)"
            echo "  KOUTAKU_BEDROCK_API_KEY    Bedrock API key (when --env bedrock)"
            echo "  KOUTAKU_BEDROCK_ACCESS_KEY Bedrock AWS access key (when --env bedrock)"
            echo "  KOUTAKU_BEDROCK_SECRET_KEY Bedrock AWS secret key (when --env bedrock)"
            echo "  KOUTAKU_BEDROCK_REGION     Bedrock region (default: us-east-1)"
            echo ""
            echo "Examples:"
            echo "  $0                                    # Run collection for all providers (each provider_config/koutaku-v1-*.postman_environment.json)"
            ...
            echo "  KOUTAKU_BASE_URL=http://api:8080 $0  # Custom base URL"
            exit 0
            ;;
        *)
...
# Optional second arg: path to Postman env .json file (e.g. provider_config/koutaku-v1-openai.postman_environment.json).
# When given, uses only that env file; otherwise uses default env and KOUTAKU_* overrides.
run_newman() {
    ...
    if [ -n "${2:-}" ] && [ -f "${2}" ]; then
        ...
        if [[ "${1:-}" == "bedrock" ]]; then
            [ -n "${KOUTAKU_BEDROCK_API_KEY:-}" ] && cmd+=(--env-var "bedrock_api_key=$KOUTAKU_BEDROCK_API_KEY")
            [ -n "${KOUTAKU_BEDROCK_ACCESS_KEY:-}" ] && cmd+=(--env-var "bedrock_access_key=$KOUTAKU_BEDROCK_ACCESS_KEY")
            [ -n "${KOUTAKU_BEDROCK_SECRET_KEY:-}" ] && cmd+=(--env-var "bedrock_secret_key=$KOUTAKU_BEDROCK_SECRET_KEY")
            [ -n "${KOUTAKU_BEDROCK_REGION:-}" ] && cmd+=(--env-var "bedrock_region=$KOUTAKU_BEDROCK_REGION")
            [ -n "${KOUTAKU_BEDROCK_SESSION_TOKEN:-}" ] && cmd+=(--env-var "bedrock_session_token=$KOUTAKU_BEDROCK_SESSION_TOKEN")
        fi
    else
        local base_url="${KOUTAKU_BASE_URL:-http://localhost:8080}"
        local provider="${KOUTAKU_PROVIDER:-openai}"
        local model="${KOUTAKU_MODEL:-gpt-4o}"
        local model_invoke="${KOUTAKU_MODEL_INVOKE:-amazon.titan-text-express-v1}"
        local embedding_model="${KOUTAKU_EMBEDDING_MODEL:-text-embedding-3-small}"
        local speech_model="${KOUTAKU_SPEECH_MODEL:-tts-1}"
        local transcription_model="${KOUTAKU_TRANSCRIPTION_MODEL:-whisper-1}"
        local image_model="${KOUTAKU_IMAGE_MODEL:-dall-e-3}"
...
if [ -n "$PROVIDER_ENV_FILE" ]; then
    ...
    elif [ -f "$PROVIDER_CONFIG_DIR/koutaku-v1-${PROVIDER_ENV_FILE}.postman_environment.json" ]; then
        SINGLE_JSON_ENV="$PROVIDER_CONFIG_DIR/koutaku-v1-${PROVIDER_ENV_FILE}.postman_environment.json"
    fi
    if [ -z "$SINGLE_JSON_ENV" ]; then
        echo -e "${RED}Error: Env file not found: $PROVIDER_ENV_FILE${NC}"
        echo "Use a path to a .json env (e.g. provider_config/koutaku-v1-openai.postman_environment.json) or provider name (e.g. openai)"
        exit 1
    fi
    SINGLE_PROVIDER_NAME="${SINGLE_JSON_ENV##*/}"
    SINGLE_PROVIDER_NAME="${SINGLE_PROVIDER_NAME#koutaku-v1-}"
...
if [ -z "${PROVIDER_JSON_FILES+x}" ] || [ ${#PROVIDER_JSON_FILES[@]} -eq 0 ]; then
    ...
    echo -e "  Base URL: ${YELLOW}${KOUTAKU_BASE_URL:-http://localhost:8080}${NC}"
    echo -e "  Provider: ${YELLOW}${KOUTAKU_PROVIDER:-openai}${NC}"
    echo -e "  Reports:  ${YELLOW}$REPORT_DIR${NC}"
    echo ""
    echo -e "${GREEN}Running tests...${NC}"
    echo ""
