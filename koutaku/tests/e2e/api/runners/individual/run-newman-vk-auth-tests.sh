...
# Koutaku V1 Virtual Key Auth Newman Test Runner
...
# Configuration
COLLECTION="collections/koutaku-v1-vk-auth.postman_collection.json"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help)
            ...
            echo ""
            echo "Options:"
            echo "  --env <provider>    Postman env path or provider name (e.g. openai or provider_config/koutaku-v1-openai.postman_environment.json)"
...
# Print banner
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}Koutaku V1 Virtual Key Auth Test Runner${NC}"
echo -e "${GREEN}==============================================${NC}"
echo ""
...
if [ -n "$PROVIDER_ENV_FILE" ]; then
    if [ -f "$PROVIDER_ENV_FILE" ]; then
        ...
        echo "  - $PROVIDER_CONFIG_DIR/koutaku-v1-${PROVIDER_ENV_FILE}.postman_environment.json"
        exit 1
    fi
fi
...
if [ -z "$SINGLE_JSON_ENV" ]; then
    if [ -f "$PROVIDER_CONFIG_DIR/koutaku-v1-openai.postman_environment.json" ]; then
        SINGLE_JSON_ENV="$PROVIDER_CONFIG_DIR/koutaku-v1-openai.postman_environment.json"
