...
# Koutaku API Management & Health Tests
...
# Configuration
COLLECTION="$API_DIR/collections/koutaku-api-management.postman_collection.json"
...
# Print banner
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Koutaku API Management & Health Tests${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
BAIL=""
DB_VERIFY=""
DB_URL="${KOUTAKU_DB_URL:-}"
LOGS_DB_URL="${KOUTAKU_LOGS_DB_URL:-}"
DB_CONFIG_PATH=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --logs-db-url <dsn> Explicit logs DB url (also reads KOUTAKU_LOGS_DB_URL; auto-detected)"
            echo "                      PostgreSQL: postgresql://user:pass@host:port/db"
            echo "                      SQLite:     sqlite:///path/to/file.db"
            echo "  --config-path <p>   Path to Koutaku config.json for auto DB detection"
            echo "                      (default: ./config.json; also reads KOUTAKU_CONFIG_PATH env)"
...
KOUTAKU_ROOT="$(cd "$API_DIR/../../.." && pwd)"
PLUGIN_DIR="$KOUTAKU_ROOT/examples/plugins/hello-world"
...
HTTP_SERVER_DIR="$KOUTAKU_ROOT/examples/mcps/http-no-ping-server"
