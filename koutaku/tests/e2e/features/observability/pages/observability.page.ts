...
export interface ObservabilityState {
  ...
  koutakuEnabled: boolean
  datadogEnabled: boolean
  bigqueryEnabled: boolean
  newRelicEnabled: boolean
}

export type ObservabilityConnector = 'otel' | 'prometheus' | 'koutaku' | 'datadog' | 'bigquery' | 'newrelic'
...
export class ObservabilityPage extends BasePage {
  ...
  /**
   * Configure Koutaku API key
   */
  async configureKoutakuApiKey(apiKey: string): Promise<void> {
    await this.selectConnector('koutaku')
  ...
  async getCurrentState(): Promise<ObservabilityState> {
    const state: ObservabilityState = {
      ...
      koutakuEnabled: false,
      datadogEnabled: false,
      bigqueryEnabled: false,
      newRelicEnabled: false,
    }

    const connectors: ObservabilityConnector[] = ['otel', 'prometheus', 'koutaku', 'datadog', 'bigquery', 'newrelic']
    for (const connector of connectors) {
      if (await this.isConnectorAvailable(connector)) {
        ...
        else if (connector === 'koutaku') state.koutakuEnabled = enabled
