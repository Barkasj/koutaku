...
test.describe('Observability', () => {
  ...
  test.describe('Navigation', () => {
    ...
    test('should display Koutaku connector tab', async ({ observabilityPage }) => {
      await expect(observabilityPage.getConnectorTab('koutaku')).toBeVisible()
    })
  ...
  test.describe('Koutaku Connector', () => {
    test('should select Koutaku connector', async ({ observabilityPage }) => {
      await observabilityPage.selectConnector('koutaku')

      // Verify Koutaku is selected by checking aria-current
      const selected = await observabilityPage.getSelectedConnector()
      expect(selected).toContain('Koutaku')
    })

    test('should toggle Koutaku connector', async ({ observabilityPage }) => {
      await observabilityPage.selectConnector('koutaku')

      // Check if toggle is enabled
      const isToggleEnabled = await observabilityPage.isToggleEnabled('koutaku')

      if (!isToggleEnabled) {
        test.skip(true, 'Koutaku toggle is disabled (requires configuration)')
        return
      }

      const initialState = await observabilityPage.isConnectorEnabled('koutaku')

      const toggled = await observabilityPage.toggleConnector('koutaku')
      expect(toggled).toBe(true)

      const newState = await observabilityPage.isConnectorEnabled('koutaku')
      expect(newState).toBe(!initialState)
    })

    test('should display Koutaku configuration form', async ({ observabilityPage }) => {
      await observabilityPage.selectConnector('koutaku')
  ...
  test.describe('Multiple Connectors', () => {
    ...
    test('should persist connector selection via URL', async ({ observabilityPage }) => {
      // Select Koutaku (URL update via nuqs is async)
      await observabilityPage.selectConnector('koutaku')
      // Wait for URL to reflect selection before asserting
      await expect(observabilityPage.page).toHaveURL(/plugin=koutaku/, { timeout: 5000 })
    })
  })
})
