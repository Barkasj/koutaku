...
async function waitForKoutakuAPI(baseUrl: string, maxAttempts = 30): Promise<void> {
  for (let i = 0; i < maxAttempts; i++) {
    try {
  ...
  throw new Error(`Koutaku API at ${baseUrl} did not become ready after ${maxAttempts} attempts`)
}
...
async function ensureTestClient001AndSendResponses(baseUrl: string): Promise<void> {
  ...
  if (clientAfter.state !== 'connected') {
    ...
    if (reconnectRes.statusCode < 200 || reconnectRes.statusCode >= 300) {
      throw new Error(
        `POST /api/mcp/client/.../reconnect failed: ${reconnectRes.statusCode} ${reconnectRes.body}. Ensure MCP server is running and reload Koutaku if needed.`
      )
    }
  }
...
/**
 * Seed LLM logs by sending a few chat completion requests through Koutaku.
...
async function globalSetup(): Promise<() => Promise<void>> {
  ...
  try {
    await runKoutakuMCPAndResponsesSetup()
  } catch (error: unknown) {
    const err = error as Error
