...
var cfg = struct {
	BaseURL      string
	VK           string // KOUTAKU_VK — primary virtual key
	AltVK        string // KOUTAKU_ALT_VK — a second, different virtual key for auth tests
	PollTimeout  time.Duration
	PollInterval time.Duration
}{
	BaseURL:      envOr("KOUTAKU_BASE_URL", defaultBaseURL),
	VK:           os.Getenv("KOUTAKU_VK"),
	AltVK:        os.Getenv("KOUTAKU_ALT_VK"),
	PollTimeout:  parseDuration(os.Getenv("KOUTAKU_POLL_TIMEOUT"), defaultPollTimeout),
	PollInterval: parseDuration(os.Getenv("KOUTAKU_POLL_INTERVAL"), defaultPollInterval),
}
...
// When KOUTAKU_VK is unset, only the global (no-VK) round runs.
...
// TestMain checks that the Koutaku gateway is reachable before running any tests.
// Set KOUTAKU_BASE_URL to override the default http://localhost:8080.
func TestMain(m *testing.M) {
	resp, err := httpClient.Get(cfg.BaseURL + "/health")
	if err != nil || resp.StatusCode >= 500 {
		fmt.Printf("SKIP: Koutaku gateway not reachable at %s (err=%v)\n", cfg.BaseURL, err)
		os.Exit(0)
	}
	resp.Body.Close()
	os.Exit(m.Run())
