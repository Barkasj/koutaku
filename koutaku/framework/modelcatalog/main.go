		...
		go func() {
			...
			if hasPricingData {
				...
				if err := mc.withDistributedLock(ctx, "model_catalog_pricing_startup_sync", 10, func() error {
					...
					mc.logger.Warn("initial pricing sync failed (will retry in background): %v", err)
					// Don't fail init — server can start without pricing data
				}
			}
		}()
		go func() {
			...
			if n > 0 {
				...
				if err := mc.withDistributedLock(ctx, "model_catalog_params_startup_sync", 10, func() error {
					...
					mc.logger.Warn("initial model parameters sync failed (will retry in background): %v", err)
				}
			}
		}()
		wg.Wait()
		if pricingErr != nil {
			mc.logger.Warn("pricing init warning: %v", pricingErr)
		}
		if paramsErr != nil {
			mc.logger.Warn("params init warning: %v", paramsErr)
		}
	} else {
