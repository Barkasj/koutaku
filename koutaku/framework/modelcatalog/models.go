...
func (mc *ModelCatalog) UpsertModelDataForProvider(provider schemas.ModelProvider, modelData *schemas.KoutakuListModelsResponse, allowedModels []schemas.Model) {
	if modelData == nil {
		return
	}
	mc.mu.Lock()
...
func (mc *ModelCatalog) UpsertUnfilteredModelDataForProvider(provider schemas.ModelProvider, modelData *schemas.KoutakuListModelsResponse) {
	if modelData == nil {
		return
	}
	mc.mu.Lock()
...
// It only considers catalog entries whose leading segment is a known Koutaku provider,
