...
// PricingLookupScopesFromContext builds a PricingLookupScopes from a KoutakuContext.
...
func PricingLookupScopesFromContext(ctx *schemas.KoutakuContext, provider string) *PricingLookupScopes {
	if ctx == nil {
		return nil
	}
	virtualKeyID, _ := ctx.Value(schemas.KoutakuContextKeyGovernanceVirtualKeyID).(string)
	selectedKeyID, _ := ctx.Value(schemas.KoutakuContextKeySelectedKeyID).(string)
