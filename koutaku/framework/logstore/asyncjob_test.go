...
func TestSubmitJob_PropagatesContextValues(t *testing.T) {
	...
	capturedCtx := schemas.NewKoutakuContext(context.Background(), <-time.After(1*time.Minute))
	capturedCtx.SetValue(schemas.KoutakuContextKeyVirtualKey, "sk-bf-test")
	capturedCtx.SetValue(schemas.KoutakuContextKey("x-bf-eh-custom"), "custom-value")
	capturedCtx.SetValue(schemas.KoutakuContextKey("x-bf-prom-env"), "production")
	...
	assert.Equal(t, "sk-bf-test", capturedCtx.Value(schemas.KoutakuContextKeyVirtualKey))
	assert.Equal(t, "production", capturedCtx.Value(schemas.KoutakuContextKey("x-bf-prom-env")))
	assert.Equal(t, "custom-value", capturedCtx.Value(schemas.KoutakuContextKey("x-bf-eh-custom")))
	assert.Equal(t, true, capturedCtx.Value(schemas.KoutakuIsAsyncRequest))
}

func TestSubmitJob_NilContextValues(t *testing.T) {
	executor := newTestAsyncExecutor(t)

	var capturedCtx *schemas.KoutakuContext
	var done atomic.Bool
...
func TestSubmitJob_AsyncFlagOverridesContextValues(t *testing.T) {
	...
	inputCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	inputCtx.SetValue(schemas.KoutakuIsAsyncRequest, false)
	...
	// KoutakuIsAsyncRequest must be true — set AFTER restoring context values
	assert.Equal(t, true, capturedCtx.Value(schemas.KoutakuIsAsyncRequest))
}
...
func TestSubmitJob_OperationFailure_PreservesContext(t *testing.T) {
	...
	inputCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	inputCtx.SetValue(schemas.KoutakuContextKeyVirtualKey, "sk-bf-test")
