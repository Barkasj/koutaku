// Package tracing provides distributed tracing infrastructure for Koutaku
package tracing
...
// 1. KoutakuContextKeySpanID - existing span in this service (for child spans)
// 2. KoutakuContextKeyParentSpanID - incoming parent from W3C traceparent (for root spans)
...
func (t *Tracer) StartSpan(ctx context.Context, name string, kind schemas.SpanKind) (context.Context, schemas.SpanHandle) {
	...
	parentSpanID, _ := ctx.Value(schemas.KoutakuContextKeySpanID).(string)
	...
	if parentSpanID == "" {
		parentSpanID, _ = ctx.Value(schemas.KoutakuContextKeyParentSpanID).(string)
	}
	...
	newCtx := context.WithValue(ctx, schemas.KoutakuContextKeySpanID, span.SpanID)
...
func (t *Tracer) PopulateLLMRequestAttributes(handle schemas.SpanHandle, req *schemas.KoutakuRequest) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil || req == nil {
		return
	}
...
func (t *Tracer) PopulateLLMResponseAttributes(ctx *schemas.KoutakuContext, handle schemas.SpanHandle, resp *schemas.KoutakuResponse, err *schemas.KoutakuError) {
	h, ok := handle.(*spanHandle)
	if !ok || h == nil {
		return
	}
...
func (t *Tracer) AddStreamingChunk(traceID string, response *schemas.KoutakuResponse) {
...
func (t *Tracer) GetAccumulatedChunks(traceID string) (*schemas.KoutakuResponse, int64, int) {
...
func (t *Tracer) ProcessStreamingChunk(traceID string, isFinalChunk bool, result *schemas.KoutakuResponse, err *schemas.KoutakuError) *schemas.StreamAccumulatorResult {
	if traceID == "" || t.accumulator == nil {
		return nil
	}

	// Create a new context for accumulator that sets the traceID as the accumulator lookup ID.
	accumCtx := schemas.NewKoutakuContext(context.Background(), time.Time{})
	accumCtx.SetValue(schemas.KoutakuContextKeyAccumulatorID, traceID)
	accumCtx.SetValue(schemas.KoutakuContextKeyStreamEndIndicator, isFinalChunk)
	...
	// On final chunk, store the accumulated KoutakuResponse on the deferred span
	// so that completeDeferredSpan can populate span attributes (e.g., gen_ai.output.messages)
	if isFinalChunk {
		if koutakuResp := processedResp.ToKoutakuResponse(); koutakuResp != nil &&
			(koutakuResp.ChatResponse != nil ||
				koutakuResp.TextCompletionResponse != nil ||
				koutakuResp.SpeechResponse != nil ||
				koutakuResp.TranscriptionResponse != nil ||
				koutakuResp.ImageGenerationResponse != nil ||
				koutakuResp.ResponsesResponse != nil) {
			t.store.SetAccumulatedResponse(traceID, koutakuResp)
		}
	}
