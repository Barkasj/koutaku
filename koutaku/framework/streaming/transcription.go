...
func (a *Accumulator) buildCompleteMessageFromTranscriptionStreamChunks(chunks []*TranscriptionStreamChunk) *schemas.KoutakuTranscriptionResponse {
	completeMessage := &schemas.KoutakuTranscriptionResponse{}
	finalContent := ""
...
func (a *Accumulator) processAccumulatedTranscriptionStreamingChunks(requestID string, koutakuErr *schemas.KoutakuError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processTranscriptionStreamingResponse(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*ProcessedStreamResponse, error) {
	...
	if koutakuErr != nil {
		...
		newDelta := &schemas.KoutakuTranscriptionStreamResponse{
		...
		if result.TranscriptionStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = koutaku.Ptr(fmt.Sprintf("%v", result.TranscriptionStreamResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedTranscriptionStreamingChunks(requestID, koutakuErr, isFinalChunk)
		if processErr != nil {
