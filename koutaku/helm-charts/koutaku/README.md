...
[![Artifact Hub](https://img.shields.io/endpoint?url=https://artifacthub.io/badge/repository/koutaku)](https://artifacthub.io/packages/helm/koutaku/koutaku)

Official Helm charts for deploying [Koutaku](https://github.com/koutaku/koutaku) - a high-performance AI gateway with unified interface for multiple providers.
...
- For `koutaku.cluster.discovery.type` set to `consul`, `etcd`, or `udp`, set `koutaku.cluster.discovery.serviceName` explicitly during upgrade.

### v2.1.2

- Removed `encryption_key` requirement — field is now optional; Koutaku will operate without encryption when omitted

### v2.1.1

- Made `koutaku.governance.virtualKeys[].value` optional — template no longer fails when the field is omitted, allowing the backend to auto-generate the virtual key value
...
- When using `storage.mode: sqlite` with `storage.persistence.enabled: true`, Koutaku now deploys as a **StatefulSet** instead of a Deployment
- Each pod gets its own dedicated PersistentVolumeClaim (e.g., `data-koutaku-0`, `data-koutaku-1`, `data-koutaku-2`)
...
2. Uninstall the existing release: `helm uninstall koutaku`
3. Delete the old PVC: `kubectl delete pvc koutaku-data`
4. Install with the new chart version: `helm install koutaku koutaku/koutaku --set image.tag=<latest-image>`
...
```bash
# Add the Koutaku Helm repository
helm repo add koutaku https://koutakuhq.github.io/koutaku/helm-charts

# Update your local Helm chart repository cache
helm repo update

# Install Koutaku with default configuration (SQLite storage)
helm install koutaku koutaku/koutaku --set image.tag=v1.4.3
```
...
helm install koutaku koutaku/koutaku -f my-values.yaml
```
...
git clone https://github.com/koutaku/koutaku.git
cd koutaku/helm-charts

# Install from local chart
helm install koutaku ./koutaku --set image.tag=v1.5.2
```
...
```bash
cd koutaku/helm-charts/koutaku
./scripts/install.sh
```
...
|-----------|-------------|---------|
| `image.repository` | Container image repository | `docker.io/koutaku/koutaku` |
| `image.tag` | Container image tag (required) | `""` |
| `image.pullPolicy` | Image pull policy | `IfNotPresent` |

> **Important:** You must specify the `image.tag`. See available tags at [Docker Hub](https://hub.docker.com/r/koutaku/koutaku/tags).
...
image:
  repository: us-west1-docker.pkg.dev/koutaku-enterprise/your-org/koutaku
  tag: v1.5.0

# AWS ECR
image:
  repository: 123456789.dkr.ecr.us-east-1.amazonaws.com/koutaku
  tag: v1.5.0

# Azure Container Registry
image:
  repository: yourregistry.azurecr.io/koutaku
  tag: v1.5.0

# Self-hosted registry
image:
  repository: registry.yourcompany.com/ai/koutaku
  tag: v1.5.0
```
...
Koutaku supports two storage backends (SQLite and PostgreSQL) that can be configured independently for config and logs stores.
...
| `postgresql.auth.username` | Database username | `koutaku` |
| `postgresql.auth.password` | Database password | `koutaku_password` |
| `postgresql.auth.database` | Database name | `koutaku` |
...
Koutaku supports multiple vector stores for semantic caching:
...
### Koutaku Application Configuration

| Parameter | Description | Default |
|-----------|-------------|---------|
| `koutaku.port` | Application port | `8080` |
| `koutaku.host` | Bind address | `0.0.0.0` |
| `koutaku.logLevel` | Log level | `info` |
| `koutaku.logStyle` | Log format: `json` or `text` | `json` |
| `koutaku.encryptionKey` | Encryption key for sensitive data | `""` |
...
|--------|-----------|-------------|
| Telemetry | `koutaku.plugins.telemetry.enabled` | Enable metrics collection |
| Logging | `koutaku.plugins.logging.enabled` | Enable request logging |
| Governance | `koutaku.plugins.governance.enabled` | Enable budget management |
| Semantic Cache | `koutaku.plugins.semanticCache.enabled` | Enable semantic caching |
| OTEL | `koutaku.plugins.otel.enabled` | Enable OpenTelemetry integration |
| Koutaku | `koutaku.plugins.koutaku.enabled` | Enable Koutaku observability |
| Datadog | `koutaku.plugins.datadog.enabled` | Enable Datadog APM integration |
| Custom | `koutaku.plugins.custom` | Array of custom/dynamic plugins |

#### Custom Plugins

You can add custom/dynamic plugins using the `koutaku.plugins.custom` array:
...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `koutaku.client.disableDbPingsInHealth` | Disable DB pings in health check | `false` |
| `koutaku.client.headerFilterConfig.allowlist` | Headers allowed to forward to LLM providers | `[]` |
| `koutaku.client.headerFilterConfig.denylist` | Headers blocked from forwarding | `[]` |

### MCP Configuration

| Parameter | Description | Default |
|-----------|-------------|---------|
| `koutaku.mcp.enabled` | Enable MCP (Model Context Protocol) | `false` |
| `koutaku.mcp.clientConfigs` | Array of MCP client configurations | `[]` |
| `koutaku.mcp.toolManagerConfig.toolExecutionTimeout` | Tool execution timeout in seconds | `30` |
| `koutaku.mcp.toolManagerConfig.maxAgentDepth` | Koutakuum agent depth | `10` |
...
ingress:
  ...
  hosts:
    - host: koutaku.example.com
      paths:
        - path: /
          pathType: Prefix
  tls:
    - secretName: koutaku-tls
      hosts:
        - koutaku.example.com
```
...
`koutaku.mcp.clientConfigs[].headers` is a free-form `map<string, string>`
...
For `koutaku.mcp.clientConfigs[].connectionString` itself, prefer the
chart-native `secretRef` (`name` + `connectionStringKey`) instead — the
chart will inject `KOUTAKU_MCP_<NAME>_CONNECTION_STRING` and rewrite the
...
helm install koutaku koutaku/koutaku \
  -f https://raw.githubusercontent.com/koutaku/koutaku/main/helm-charts/koutaku/values-examples/postgres-only.yaml \
  --set image.tag=v1.5.2

# From local source
helm install koutaku ./koutaku -f ./koutaku/values-examples/postgres-only.yaml
```
...
# Upgrade release
helm upgrade koutaku koutaku/koutaku --set image.tag=v1.5.2

# Or with custom values
helm upgrade koutaku koutaku/koutaku -f my-values.yaml
```
...
```bash
# Uninstall release
helm uninstall koutaku

# If you want to delete persistent volumes
kubectl delete pvc -l app.kubernetes.io/name=koutaku
```

## Accessing Koutaku

After installation, access Koutaku using one of these methods:

### Port Forwarding (Development)

```bash
kubectl port-forward svc/koutaku 8080:8080
...
Koutaku exposes Prometheus metrics at `/metrics`:
...
```bash
kubectl get pods -l app.kubernetes.io/name=koutaku
...
```bash
kubectl logs -l app.kubernetes.io/name=koutaku -f
```
...
kubectl get configmap koutaku -o yaml

# View generated secrets
kubectl get secret koutaku -o yaml
```
