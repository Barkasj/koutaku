...
const BASE_URL = "https://downloads.getkoutaku.ai";
...
async function checkVersionExists(version, platformDir, archDir, binaryName) {
	const url = `${BASE_URL}/koutaku-cli/${version}/${platformDir}/${archDir}/${binaryName}`;
...
function getShellConfig() {
	...
	if (shell.endsWith("/fish") || shell.endsWith("/fish.exe")) {
		return {
			...
			exportLine: "fish_add_path $HOME/.koutaku/bin",
			shellName: "fish",
		};
	}
	...
	if (shell.endsWith("/zsh")) {
		return {
			...
			exportLine: 'export PATH="$HOME/.koutaku/bin:$PATH"',
			shellName: "zsh",
		};
	}
...
function hasPathLine(filePath) {
	...
	try {
		...
		return content.includes(".koutaku/bin");
	} catch {
		return false;
	}
}
...
function printStartMessage(message) {
	console.log(`\n${message}`);
	console.log(`Enter 'koutaku' when you're ready to start the CLI.`);
}
...
async function main() {
	...
	if (VERSION === "latest") {
		...
		if (!versionExists) {
			...
			console.error(`Please verify the version exists at: ${BASE_URL}/koutaku-cli/`);
			process.exit(1);
		}
		namedVersion = VERSION;
	}

	const downloadUrl = `${BASE_URL}/koutaku-cli/${namedVersion}/${platformDir}/${archDir}/${binaryName}`;

	// Install to ~/.koutaku/bin/
	const installDir = join(homedir(), ".koutaku", "bin");
	...
	try {
		...
		const checksumUrl = `${BASE_URL}/koutaku-cli/${namedVersion}/${platformDir}/${archDir}/${binaryName}.sha256`;
	...
	console.log(`✅ Installed koutaku to ${binaryPath}`);
	...
	if (!shellConfig) {
		...
		printStartMessage("The installer won't start Koutaku automatically.");
		return;
	}
	...
	if (missingRcFiles.length === 0) {
		console.log(`\n✅ PATH already configured for koutaku.`);
		printStartMessage("The installer won't start Koutaku automatically.");
		return;
	}
...
main().catch((error) => {
	console.error(`❌ Failed to install Koutaku CLI: ${error.message}`);
	process.exit(1);
});
