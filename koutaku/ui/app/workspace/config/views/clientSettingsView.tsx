...
export default function ClientSettingsView() {
	...
	const { data: koutakuConfig, isLoading: isCoreConfigLoading } = useGetCoreConfigQuery({ fromDB: true });
	...
	const handleSave = useCallback(async () => {
		...
		if (hasCoreConfigChanges) {
			...
			try {
				await updateCoreConfig({ ...koutakuConfig!, client_config: cleanedConfig }).unwrap();
				...
				<div className="flex items-center justify-between space-x-2">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">
							If enabled, Koutaku will drop requests that exceed pool capacity.{" "}
