...
export default function ModelSettingsView() {
	...
	const frameworkConfig = koutakuConfig?.framework_config;
	const clientConfig = koutakuConfig?.client_config;
	...
	useEffect(() => {
		if (!koutakuConfig || isDirty) return;
		reset({
	...
	const hasChanges = useMemo(() => {
		if (!koutakuConfig || !isDirty) return false;
	...
	}, [koutakuConfig, frameworkConfig, clientConfig, formValues, isDirty]);
	...
	const onSubmit = async (data: ModelSettingsFormData) => {
		try {
			await updateCoreConfig({
				...
				framework_config: {
					...frameworkConfig,
					id: koutakuConfig?.framework_config.id || 0,
					...
					<div className="flex items-center justify-between rounded-sm border p-4">
						<div className="space-y-0.5">
							...
							<p className="text-muted-foreground text-sm">
								Koutakuum number of chained routing rule evaluations per request. Prevents infinite loops from circular rule definitions.
							</p>
						</div>
						<Input
