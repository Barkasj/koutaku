...
const PRODUCTION_SETUP_DISMISSED_COOKIE = "koutaku_production_setup_dismissed";
...
export default function AppSidebar() {
	...
	const items = useMemo(
		() => [
			...
			},
			{
				...
				icon: Network,
				description: "Manage Koutaku cluster",
	...
	const logoSrc = mounted && resolvedTheme === "dark" ? "/koutaku-logo-dark.webp" : "/koutaku-logo.webp";
	const iconSrc = mounted && resolvedTheme === "dark" ? "/koutaku-icon-dark.webp" : "/koutaku-icon.webp";
			...
			<SidebarHeader className="mt-1 ml-2 flex justify-between px-0 group-data-[collapsible=icon]:ml-0 group-data-[collapsible=icon]:h-auto">
				...
				<div className="flex h-10 w-full items-center justify-between px-1.5 group-data-[collapsible=icon]:hidden">
					<Link to="/workspace/logs" className="group flex items-center gap-2 pl-2">
						<img className="h-[22px] w-auto" src={logoSrc} alt="Koutaku" width={70} height={70} />
					</Link>
					<button
				...
				<div
					...
					onClick={toggleSidebar}
				>
					<img className="h-[22px] w-auto" src={iconSrc} alt="Koutaku" width={22} height={22} />
				</div>
			</SidebarHeader>
