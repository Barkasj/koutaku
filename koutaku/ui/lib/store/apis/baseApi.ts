...
import { KoutakuErrorResponse } from "@/lib/types/config";
...
export const clearAuthStorage = () => {
	...
	try {
		...
		localStorage.removeItem("koutaku-auth-token");
...
const baseQueryWithErrorHandling: typeof baseQueryWithRefresh = async (args: any, api: any, extraOptions: any) => {
	...
	if (result.error) {
		...
		// Handle other errors with proper KoutakuErrorResponse format
		if (error?.data) {
			const errorData = error.data as KoutakuErrorResponse;
