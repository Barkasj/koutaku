// Types for the logs interface based on KoutakuResponse schema
...
export interface KoutakuSpeech {
  usage?: AudioLLMUsage;
  audio: string; // base64 encoded audio data
}

export interface KoutakuTranscribe {
  text: string;
...
export interface KoutakuEmbedding {
  index: number;
  object: string;
...
export interface KoutakuImageGenerationData {
  url?: string;
  b64_json?: string;
...
export interface KoutakuOCRResponse {
  model: string;
  pages: OCRPage[];
...
export interface KoutakuImageGenerationOutput {
  id?: string;
  created?: number;
  model?: string;
  data: KoutakuImageGenerationData[];
  background?: string;
  output_format?: string;
  quality?: string;
  size?: string;
...
export interface KoutakuVideoGenerationOutput {
  videos: VideoOutput[];
  id?: string;
  completed_at?: number;
  created_at?: number;
...
export interface KoutakuVideoDownloadOutput {
  video_id: string;
  content_type?: string;
}

export interface KoutakuVideoDeleteOutput {
  id: string;
  deleted: boolean;
  object?: string;
}

export interface KoutakuVideoListOutput {
  object: string;
  data: VideoObject[];
  first_id?: string;
  has_more?: boolean;
...
export interface KoutakuError {
  event_id?: string;
  type?: string;
  is_koutaku_error: boolean;
  status_code?: number;
  error: ErrorField;
}
...
export interface LogEntry {
  ...
  embedding_output?: KoutakuEmbedding[];
  rerank_output?: RerankResult[];
  ocr_input?: OCRDocument;
  ocr_output?: KoutakuOCRResponse;
  image_generation_output?: KoutakuImageGenerationOutput;
  video_generation_output?: KoutakuVideoGenerationOutput;
  video_retrieve_output?: KoutakuVideoGenerationOutput;
  video_download_output?: KoutakuVideoDownloadOutput;
  video_list_output?: KoutakuVideoListOutput;
  video_delete_output?: KoutakuVideoDeleteOutput;
  ...
  speech_output?: KoutakuSpeech;
  transcription_output?: KoutakuTranscribe;
