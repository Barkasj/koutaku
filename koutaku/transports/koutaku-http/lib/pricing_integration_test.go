...
//
// Run: go test ./transports/koutaku-http/lib/... -run TestPricingE2E -v
// =============================================================================
...
import (
	...
	configstore "github.com/koutaku/koutaku/framework/configstore"
...
func TestPricingE2E_Step5A_ValidEnvVar(t *testing.T) {
	t.Setenv("KOUTAKU_E2E_PRICING_URL", "https://env.example.com/pricing.json")
	log := newCapturingLogger()

	fileConfig := &framework.FrameworkConfig{
		Pricing: &modelcatalog.Config{PricingURL: ptStr("env.KOUTAKU_E2E_PRICING_URL")},
	}
...
func TestPricingE2E_Step5B_MissingEnvVar_PreservesLiteral(t *testing.T) {
	...
	rawURL := "env.KOUTAKU_E2E_PRICING_URL_DEFINITELY_NOT_SET_XYZ9999"
...
func TestPricingE2E_Step5C_EmbeddedEnvNotExpanded(t *testing.T) {
	t.Setenv("KOUTAKU_E2E_HOST", "host.example.com")
	log := newCapturingLogger()

	// URL that has env.VAR embedded mid-string — NOT a full-string "env." prefix.
	embeddedURL := "https://env.KOUTAKU_E2E_HOST/pricing.json"
...
func TestPricingE2E_Step9B_MissingEnvURL_NotReplacedWithDefault(t *testing.T) {
	...
	rawURL := "env.KOUTAKU_E2E_PRICING_URL_NONEXISTENT_PRODUCTION_XYZ"
