...
var localSchemaCandidates = []string{
	...
	"../config.schema.json",      // running from transports/koutaku-http/
...
func ValidateConfigSchema(data []byte, schemaOverride ...[]byte) error {
	...
	if len(schemaOverride) > 0 && len(schemaOverride[0]) > 0 {
		...
		// Pulling config.schema from https://www.getkoutaku.ai/schema
		configSchemaJSON, err := http.Get("https://www.getkoutaku.ai/schema")
		if err != nil {
