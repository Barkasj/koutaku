...
func StreamLargeResponseBody(ctx *fasthttp.RequestCtx, koutakuCtx *schemas.KoutakuContext) bool {
	if koutakuCtx == nil {
		return false
	}
	reader, ok := koutakuCtx.Value(schemas.KoutakuContextKeyLargeResponseReader).(io.ReadCloser)
	if !ok || reader == nil {
		return false
	}

	contentLength, _ := koutakuCtx.Value(schemas.KoutakuContextKeyLargeResponseContentLength).(int)
	contentType, _ := koutakuCtx.Value(schemas.KoutakuContextKeyLargeResponseContentType).(string)
	contentDisposition, _ := koutakuCtx.Value(schemas.KoutakuContextKeyLargeResponseContentDisposition).(string)

	// Mirror large-response-mode to fasthttp UserValue so post-hook middleware
	// (which only sees ctx.UserValue, not koutakuCtx) can skip body materialization.
