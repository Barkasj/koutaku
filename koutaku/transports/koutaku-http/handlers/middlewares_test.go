...
func TestChainMiddlewares_SingleMiddleware(t *testing.T) {
	...
	middleware := schemas.KoutakuHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
...
func TestChainMiddlewares_MultipleMiddlewares(t *testing.T) {
	...
	middleware1 := schemas.KoutakuHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware2 := schemas.KoutakuHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
	...
	middleware3 := schemas.KoutakuHTTPMiddleware(func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
...
func TestFasthttpToHTTPRequest_PathParams(t *testing.T) {
	...
	ctx.SetUserValue("KoutakuContextKeyRequestID", "req-123")
	...
	systemKeys := []string{"KoutakuContextKeyRequestID", "trace_id", "span_id"}
...
func TestRequestDecompressionMiddleware_InvalidCompressedBody(t *testing.T) {
	...
	if err := json.Unmarshal(ctx.Response.Body(), &koutakuErr); err != nil {
		t.Fatalf("failed to decode error response: %v", err)
	}
	if koutakuErr.Error == nil || !strings.Contains(koutakuErr.Error.Message, "invalid compressed request body") {
		t.Fatalf("unexpected error message: %#v", koutakuErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_UnsupportedEncoding(t *testing.T) {
	...
	if koutakuErr.Error == nil || !strings.Contains(koutakuErr.Error.Message, "unsupported Content-Encoding") {
		t.Fatalf("unexpected error message: %#v", koutakuErr.Error)
	}
}
...
func TestRequestDecompressionMiddleware_DecompressedSizeLimit(t *testing.T) {
	...
	if koutakuErr.Error == nil || !strings.Contains(koutakuErr.Error.Message, "decompressed request body exceeds max allowed size") {
		t.Fatalf("unexpected error message: %#v", koutakuErr.Error)
	}
}
