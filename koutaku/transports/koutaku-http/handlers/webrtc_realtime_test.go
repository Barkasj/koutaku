...
func TestNewRealtimeRelayContextCopiesValuesWithoutRequestCancellation(t *testing.T) {
	requestCtx, requestCancel := schemas.NewKoutakuContextWithCancel(context.Background())
	requestCtx.SetValue(schemas.KoutakuContextKeyHTTPRequestType, schemas.RealtimeRequest)
	requestCtx.SetValue(schemas.KoutakuContextKeyIntegrationType, "openai")
	requestCtx.SetValue(schemas.KoutakuContextKeyGovernanceVirtualKeyID, "vk_test")
	...
	if got := relayCtx.Value(schemas.KoutakuContextKeyHTTPRequestType); got != schemas.RealtimeRequest {
		t.Fatalf("request type = %v, want %v", got, schemas.RealtimeRequest)
	}
	if got := relayCtx.Value(schemas.KoutakuContextKeyIntegrationType); got != "openai" {
		t.Fatalf("integration type = %v, want %q", got, "openai")
	}
	if got := relayCtx.Value(schemas.KoutakuContextKeyGovernanceVirtualKeyID); got != "vk_test" {
...
func TestResolveRealtimeWebRTCKeys_UnmappedEphemeralTokenStaysAnonymous(t *testing.T) {
	...
	koutakuCtx.SetValue(schemas.KoutakuContextKeyDirectKey, schemas.Key{ID: "header-provided"})
	koutakuCtx.SetValue(schemas.KoutakuContextKeySelectedKeyID, "selected")
	koutakuCtx.SetValue(schemas.KoutakuContextKeySelectedKeyName, "selected-name")
	koutakuCtx.SetValue(schemas.KoutakuContextKeyAPIKeyID, "mapped-id")
	koutakuCtx.SetValue(schemas.KoutakuContextKeyAPIKeyName, "mapped-name")

	authKey, selectedKey, err := handler.resolveRealtimeWebRTCKeys(&ctx, koutakuCtx, schemas.OpenAI, "gpt-realtime")
	if err != nil {
	...
	if got := koutakuCtx.Value(schemas.KoutakuContextKeyDirectKey); got != nil {
		t.Fatalf("direct key context = %#v, want nil", got)
	}
	if got := koutakuCtx.Value(schemas.KoutakuContextKeySelectedKeyID); got != nil {
		t.Fatalf("selected key id context = %#v, want nil", got)
	}
	if got := koutakuCtx.Value(schemas.KoutakuContextKeySelectedKeyName); got != nil {
		t.Fatalf("selected key name context = %#v, want nil", got)
	}
	if got := koutakuCtx.Value(schemas.KoutakuContextKeyAPIKeyID); got != nil {
		t.Fatalf("api key id context = %#v, want nil", got)
	}
	if got := koutakuCtx.Value(schemas.KoutakuContextKeyAPIKeyName); got != nil {
...
func TestApplyRealtimeEphemeralKeyMapping_RestoresVirtualKeyAndKeyID(t *testing.T) {
	...
	koutakuCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	applyRealtimeEphemeralKeyMapping(koutakuCtx, realtimeEphemeralKeyMapping{
		KeyID:      "key_123",
		VirtualKey: "sk-bf-test",
	})

	if got := koutakuCtx.Value(schemas.KoutakuContextKeyVirtualKey); got != "sk-bf-test" {
		t.Fatalf("virtual key context = %#v, want %q", got, "sk-bf-test")
	}
	if got := koutakuCtx.Value(schemas.KoutakuContextKeyAPIKeyID); got != "key_123" {
