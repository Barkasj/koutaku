...
// SendError sends a KoutakuError response
...
// SendKoutakuError sends a KoutakuError response
func SendKoutakuError(ctx *fasthttp.RequestCtx, koutakuErr *schemas.KoutakuError) {
	if koutakuErr.StatusCode != nil {
		ctx.SetStatusCode(*koutakuErr.StatusCode)
	} else if !koutakuErr.IsKoutakuError {
	...
	ctx.SetContentType("application/json")
	if encodeErr := json.NewEncoder(ctx).Encode(koutakuErr); encodeErr != nil {
...
func streamLargeResponseIfActive(ctx *fasthttp.RequestCtx, koutakuCtx *schemas.KoutakuContext) bool {
...
func SendSSEError(ctx *fasthttp.RequestCtx, koutakuErr *schemas.KoutakuError) {
