...
func TestBudgetRemovalRequestDetection(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req:  &UpdateBudgetRequest{MaxLimit: koutakuFloat(10)},
			want: false,
		},
		{
			name: "reset duration only is not removal",
			req:  &UpdateBudgetRequest{ResetDuration: koutakuString("1h")},
			want: false,
		},
		{
			name: "calendar aligned only is treated as removal",
			req:  &UpdateBudgetRequest{CalendarAligned: koutakuBool(true)},
			want: true,
		},
	}
...
func TestRateLimitRemovalRequestDetection(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			req:  &UpdateRateLimitRequest{TokenMaxLimit: koutakuInt64(100)},
			want: false,
		},
		{
			name: "request limit present is not removal",
			req:  &UpdateRateLimitRequest{RequestMaxLimit: koutakuInt64(10)},
			want: false,
		},
		{
			name: "durations only is not removal",
			req: &UpdateRateLimitRequest{
				TokenResetDuration:   koutakuString("1h"),
				RequestResetDuration: koutakuString("1h"),
			},
			want: false,
		},
	}
...
func koutakuFloat(v float64) *float64 {
	return &v
}

func koutakuInt64(v int64) *int64 {
	return &v
}

func koutakuString(v string) *string {
	return &v
}

func koutakuBool(v bool) *bool {
	return &v
}
