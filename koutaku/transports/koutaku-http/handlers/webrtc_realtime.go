...
func NewWebRTCRealtimeHandler(client *koutaku.Koutaku, config *lib.Config) *WebRTCRealtimeHandler {
...
func (h *WebRTCRealtimeHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
	handler := lib.ChainMiddlewares(h.handleRequest, middlewares...)

	// Base koutaku route — GA /calls format (multipart sdp + session)
...
func (h *WebRTCRealtimeHandler) handleCallsRequest(ctx *fasthttp.RequestCtx) {
	sdpOffer, providerKey, model, normalizedSession, koutakuErr := parseCallsWebRTCRequest(ctx)
	if koutakuErr != nil {
		SendKoutakuError(ctx, koutakuErr)
		return
	}

	rtProvider, koutakuErr := h.resolveWebRTCProvider(providerKey)
	if koutakuErr != nil {
		SendKoutakuError(ctx, koutakuErr)
		return
	}

	exchangeSDP := func(rCtx *schemas.KoutakuContext, key schemas.Key, upstreamOffer string) (string, *schemas.KoutakuError) {
...
func parseCallsWebRTCRequest(ctx *fasthttp.RequestCtx) (string, schemas.ModelProvider, string, []byte, *schemas.KoutakuError) {
	...
	if strings.HasPrefix(contentType, "multipart/form-data") {
		...
		providerKey, model, normalizedSession, koutakuErr := resolveRealtimeSDPTarget(path, []byte(sessionField))
		if koutakuErr != nil {
...
func (h *WebRTCRealtimeHandler) handleLegacyRequest(ctx *fasthttp.RequestCtx, defaultProvider schemas.ModelProvider) {
	sdpOffer, rawModel, sessionJSON, koutakuErr := parseLegacyWebRTCRequest(ctx, defaultProvider)
	if koutakuErr != nil {
		SendKoutakuError(ctx, koutakuErr)
		return
	}
	...
	if providerKey == "" || model == "" {
		SendKoutakuError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", "invalid model: "+rawModel, nil))
		return
	}
	...
	if !ok {
		SendKoutakuError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", "provider does not support legacy realtime WebRTC: "+string(providerKey), nil))
		return
	}
...
func parseLegacyWebRTCRequest(ctx *fasthttp.RequestCtx, defaultProvider schemas.ModelProvider) (sdpOffer, rawModel string, sessionJSON json.RawMessage, err *schemas.KoutakuError) {
...
// runWebRTCRelay is the shared relay setup: creates koutaku context, selects key, establishes relay.
func (h *WebRTCRealtimeHandler) runWebRTCRelay(
	...
	model string,
	sdpOffer string,
	exchangeSDP func(ctx *schemas.KoutakuContext, key schemas.Key, upstreamOffer string) (string, *schemas.KoutakuError),
) {
	...
	authKey, selectedKey, err := h.resolveRealtimeWebRTCKeys(ctx, koutakuCtx, providerKey, model)
	if err != nil {
		SendKoutakuError(ctx, newRealtimeWebRTCError(fasthttp.StatusBadRequest, "invalid_request_error", err.Error(), nil))
		return
	}
	...
	boundExchange := func(rCtx *schemas.KoutakuContext, upstreamOffer string) (string, *schemas.KoutakuError) {
		return exchangeSDP(rCtx, authKey, upstreamOffer)
	}

	relayCtx, relayCancel := newRealtimeRelayContext(koutakuCtx)
...
func (h *WebRTCRealtimeHandler) resolveRealtimeWebRTCKeys(
	...
	if mapped {
		applyRealtimeEphemeralKeyMapping(koutakuCtx, mapping)
	}
	if isRealtimeEphemeralToken(inboundToken) && !mapped {
		koutakuCtx.ClearValue(schemas.KoutakuContextKeyDirectKey)
		koutakuCtx.ClearValue(schemas.KoutakuContextKeyAPIKeyID)
		koutakuCtx.ClearValue(schemas.KoutakuContextKeyAPIKeyName)
		koutakuCtx.ClearValue(schemas.KoutakuContextKeySelectedKeyID)
		koutakuCtx.ClearValue(schemas.KoutakuContextKeySelectedKeyName)
		authKey := schemas.Key{Value: *schemas.NewEnvVar(inboundToken)}
		return authKey, nil, nil
	}

	selectedKey, err := h.client.SelectKeyForProviderRequestType(koutakuCtx, schemas.RealtimeRequest, providerKey, model)
...
func applyRealtimeEphemeralKeyMapping(koutakuCtx *schemas.KoutakuContext, mapping realtimeEphemeralKeyMapping) {
	if koutakuCtx == nil {
		return
	}
	if mapping.VirtualKey != "" {
		koutakuCtx.SetValue(schemas.KoutakuContextKeyVirtualKey, mapping.VirtualKey)
	}
	if mapping.KeyID != "" {
		koutakuCtx.SetValue(schemas.KoutakuContextKeyAPIKeyID, mapping.KeyID)
	}
}
...
func (h *WebRTCRealtimeHandler) resolveWebRTCProvider(providerKey schemas.ModelProvider) (schemas.RealtimeProvider, *schemas.KoutakuError) {
...
func (h *WebRTCRealtimeHandler) establishRelay(
	relayCtx *schemas.KoutakuContext,
	...
	key *schemas.Key,
	browserOffer string,
	exchangeSDP func(ctx *schemas.KoutakuContext, upstreamOffer string) (string, *schemas.KoutakuError),
	...
	providerToBrowserTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "koutaku-provider-audio")
	if err != nil {
		relay.close()
	...
	browserToProviderTrack, err := webrtc.NewTrackLocalStaticRTP(defaultAudioCodec, "audio", "koutaku-browser-audio")
	if err != nil {
		relay.close()
...
func (r *webrtcRealtimeRelay) handleDownstreamMessage(msg webrtc.DataChannelMessage) {
	...
	if startsTurn {
		if r.session.PeekRealtimeTurnHooks() != nil {
			r.sendDownstream(newRealtimeTurnErrorEventPayload(newRealtimeWireKoutakuError(400, "invalid_request_error", "Conversation already has an active response in progress.")), true)
			return
		}
		if koutakuErr := startRealtimeTurnHooks(r.client, r.koutakuCtx, r.session, r.provider, r.providerKey, r.model, r.key, event.Type); koutakuErr != nil {
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(koutakuErr))
			return
		}
	}
	...
	if err != nil {
		if startsTurn {
			...
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(newRealtimeWireKoutakuError(400, "invalid_request_error", err.Error())))
			return
		}
...
func (r *webrtcRealtimeRelay) handleUpstreamMessage(msg webrtc.DataChannelMessage) {
	event, err := r.provider.ToKoutakuRealtimeEvent(msg.Data)
	if err != nil {
		if finalizeErr := finalizeRealtimeTurnHooksOnTransportError(
			r.client,
			r.koutakuCtx,
		...
		r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(newRealtimeWireKoutakuError(502, "server_error", "failed to translate upstream realtime event")))
		return
	}
	if event != nil {
	...
	if event != nil {
		...
		if event.Type == r.provider.RealtimeTurnFinalEvent() {
			...
			if koutakuErr := finalizeRealtimeTurnHooks(r.client, r.koutakuCtx, r.session, r.provider, r.providerKey, r.model, r.key, msg.Data, contentOverride); koutakuErr != nil {
				r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(koutakuErr))
				return
			}
			...
			if finalizeErr := finalizeRealtimeTurnHooksWithError(
				...
				event.Type,
				msg.Data,
				newKoutakuErrorFromRealtimeError(r.providerKey, r.model, msg.Data, event.Error),
			); finalizeErr != nil {
				r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(finalizeErr))
				return
			}
		...
		if err != nil {
			...
			r.closeWithErrorEvent(newRealtimeTurnErrorEventPayload(
				newRealtimeWireKoutakuError(502, "server_error", "failed to encode translated realtime event: "+err.Error()),
			))
			return
		}
	}
...
func newRealtimeRelayContext(requestCtx *schemas.KoutakuContext) (*schemas.KoutakuContext, context.CancelFunc) {
	relayCtx, cancel := schemas.NewKoutakuContextWithCancel(context.Background())
	if requestCtx == nil {
		return relayCtx, cancel
	}

	for _, key := range []any{
		schemas.KoutakuContextKeyRequestID,
		schemas.KoutakuContextKeyHTTPRequestType,
		schemas.KoutakuContextKeyIntegrationType,
		schemas.KoutakuContextKeyParentRequestID,
		schemas.KoutakuContextKeyVirtualKey,
		schemas.KoutakuContextKeyAPIKeyName,
		schemas.KoutakuContextKeyAPIKeyID,
		schemas.KoutakuContextKeyDirectKey,
		schemas.KoutakuContextKeyExtraHeaders,
		schemas.KoutakuContextKeyRequestHeaders,
		schemas.KoutakuContextKeyUserAgent,
		schemas.KoutakuContextKeyGovernanceVirtualKeyID,
		schemas.KoutakuContextKeyGovernanceVirtualKeyName,
		schemas.KoutakuContextKeyGovernanceRoutingRuleID,
		schemas.KoutakuContextKeyGovernanceRoutingRuleName,
		schemas.KoutakuContextKeyGovernanceCustomerID,
		schemas.KoutakuContextKeyGovernanceCustomerName,
		schemas.KoutakuContextKeyGovernanceTeamID,
		schemas.KoutakuContextKeyGovernanceTeamName,
		schemas.KoutakuContextKeyUserID,
		schemas.KoutakuContextKeyUserName,
		schemas.KoutakuContextKeyGovernanceIncludeOnlyKeys,
		schemas.KoutakuContextKeyGovernancePluginName,
		schemas.KoutakuContextKeySelectedKeyID,
		schemas.KoutakuContextKeySelectedKeyName,
		schemas.KoutakuContextKeyIsEnterprise,
	} {
...
func resolveRealtimeSDPTarget(path string, sessionJSON []byte) (schemas.ModelProvider, string, []byte, *schemas.KoutakuError) {
...
func newRealtimeWebRTCError(status int, errorType, message string, err error) *schemas.KoutakuError {
