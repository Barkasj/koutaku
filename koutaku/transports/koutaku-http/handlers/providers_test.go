...
func TestAddProvider_ReturnsErrorWhenRuntimeReloadFails(t *testing.T) {
	...
	if koutakuErr.Error == nil || koutakuErr.Error.Message == "" {
		t.Fatalf("expected error message in response, got %#v", koutakuErr)
	}
	if koutakuErr.Error.Message != "Failed to initialize provider after add: context deadline exceeded" {
		t.Fatalf("unexpected error message: %q", koutakuErr.Error.Message)
	}
	if _, exists := h.inMemoryStore.Providers["mock-openai"]; exists {
