...
func (h *ConsentHandler) RegisterRoutes(r *router.Router) {
	...
	// SameSite=Lax browser-binding cookie (__koutaku_flow_secret) combined with
...
<title>Connect to Koutaku</title>
<style>
  %s
...
</html>`, koutakuPageCSS, errorBanner, safeFlowID, safeFlowID, skipOption))
}
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Connect Your Apps — Koutaku</title>
<style>
  %s
...
</html>`, koutakuPageCSS, mcpRows.String(), safeFlowID, safeFlowID))
}
...
// verified identity should use Virtual Keys or an auth layer in front of Koutaku.
...
//  1. Creates a real Koutaku session (TablePerUserOAuthSession)
...
// koutakuPageCSS is the shared inline CSS for all Go-rendered consent/callback pages.
// It mirrors Koutaku's UI design tokens: teal primary, zinc palette, Geist font stack.
const koutakuPageCSS = `
