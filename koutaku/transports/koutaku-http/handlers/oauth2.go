...
func NewOAuthHandler(oauthProvider *oauth2.OAuth2Provider, client *koutaku.Koutaku, store *lib.Config) *OAuthHandler {
	return &OAuthHandler{
		client:        client,
		store:         store,
...
func (h *OAuthHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
// oauthSuccessPage renders a Koutaku-themed success HTML page.
...
</html>`, html.EscapeString(title), koutakuPageCSS, extraScript, html.EscapeString(title), html.EscapeString(message))
}

// oauthErrorPage renders a Koutaku-themed error HTML page.
...
</html>`, koutakuPageCSS, jsEscapedError, htmlError)
}
