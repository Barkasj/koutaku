...
// ConfigHandler manages runtime configuration updates for Koutaku.
...
// It requires the Koutaku client, a logger, and the config store.
...
func (h *ConfigHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *ConfigHandler) updateConfig(ctx *fasthttp.RequestCtx) {
	...
	if frameworkConfig == nil {
		frameworkConfig = &configstoreTables.TableFrameworkConfig{
			ID:                  0,
			PricingURL:          koutaku.Ptr(modelcatalog.DefaultPricingURL),
			PricingSyncInterval: koutaku.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds())),
		}
	}
	// Handling individual nil cases
	if frameworkConfig.PricingURL == nil {
		frameworkConfig.PricingURL = koutaku.Ptr(modelcatalog.DefaultPricingURL)
	}
	if frameworkConfig.PricingSyncInterval == nil {
		frameworkConfig.PricingSyncInterval = koutaku.Ptr(int64(modelcatalog.DefaultSyncInterval.Seconds()))
	}
	...
	if payload.AuthConfig != nil {
		...
		// Note: AuthMiddleware is updated via ServerCallbacks.UpdateAuthConfig (handled by KoutakuHTTPServer)
	}
