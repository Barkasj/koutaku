...
func (h *SessionHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *SessionHandler) issueWSTicket(ctx *fasthttp.RequestCtx) {
	...
	sessionToken,ok := ctx.UserValue(schemas.KoutakuContextKeySessionToken).(string)
	if !ok {
		SendError(ctx, fasthttp.StatusUnauthorized, "Unauthorized")
		return
	}
