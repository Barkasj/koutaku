...
func InstantiatePlugin(ctx context.Context, name string, path *string, pluginConfig any, koutakuConfig *lib.Config) (schemas.BasePlugin, error) {
	// Custom plugin (has path)
	if path != nil {
		return loadCustomPlugin(ctx, path, pluginConfig, koutakuConfig)
	}

	// Built-in plugin (by name)
	return loadBuiltinPlugin(ctx, name, pluginConfig, koutakuConfig)
}

// loadBuiltinPlugin instantiates a built-in plugin by name
func loadBuiltinPlugin(ctx context.Context, name string, pluginConfig any, koutakuConfig *lib.Config) (schemas.BasePlugin, error) {
	switch name {
	case telemetry.PluginName:
		telConfig := &telemetry.Config{
			CustomLabels: koutakuConfig.ClientConfig.PrometheusLabels,
		}
		...
		return telemetry.Init(telConfig, koutakuConfig.ModelCatalog, logger)

	case prompts.PluginName:
		return prompts.Init(ctx, koutakuConfig.ConfigStore, logger)
	...
	case logging.PluginName:
		...
		return logging.Init(ctx, loggingConfig, logger, koutakuConfig.LogsStore,
			koutakuConfig.ModelCatalog, koutakuConfig.MCPCatalog)
	...
	case governance.PluginName:
		...
		inMemoryStore := &GovernanceInMemoryStore{Config: koutakuConfig}
		return governance.Init(ctx, governanceConfig, logger, koutakuConfig.ConfigStore,
			koutakuConfig.GovernanceConfig, koutakuConfig.ModelCatalog,
			koutakuConfig.MCPCatalog, inMemoryStore)
	...
	case semanticcache.PluginName:
		...
		return semanticcache.Init(ctx, semanticConfig, logger, koutakuConfig.VectorStore)
	...
	case otel.PluginName:
		...
		return otel.Init(ctx, otelConfig, logger, koutakuConfig.ModelCatalog, handlers.GetVersion())
	...
	case compat.PluginName:
		...
		return compat.Init(*compatConfig, logger, koutakuConfig.ModelCatalog)
...
func loadCustomPlugin(ctx context.Context, path *string, pluginConfig any, koutakuConfig *lib.Config) (schemas.BasePlugin, error) {
	logger.Info("loading custom plugin from path %s", *path)

	plugin, err := koutakuConfig.PluginLoader.LoadPlugin(*path, pluginConfig)
	if err != nil {
...
func (s *KoutakuHTTPServer) LoadPlugins(ctx context.Context) error {
...
func (s *KoutakuHTTPServer) getPluginConfig(name string) *schemas.PluginConfig {
...
func (s *KoutakuHTTPServer) loadBuiltinPlugins(ctx context.Context) error {
	...
	if s.Config.ConfigStore != nil && ctx.Value(schemas.KoutakuContextKeyIsEnterprise) == nil {
...
func (s *KoutakuHTTPServer) loadCustomPlugins(ctx context.Context) error {
	for _, cfg := range s.Config.PluginConfigs {
