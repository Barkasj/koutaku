...
// GetDefaultConfigDir returns the OS-specific default configuration directory for Koutaku.
// This follows standard conventions:
// - Linux/macOS: ~/.config/koutaku
// - Windows: %APPDATA%\koutaku
...
func GetDefaultConfigDir(appDir string) string {
	...
	case "windows":
		...
		if appData := os.Getenv("APPDATA"); appData != "" {
			configDir = filepath.Join(appData, "koutaku")
		} else {
			// Fallback to user home directory
			if homeDir, err := os.UserHomeDir(); err == nil {
				configDir = filepath.Join(homeDir, "AppData", "Roaming", "koutaku")
			}
		}
	default:
		// Linux, macOS and other Unix-like systems: ~/.config/koutaku
		if homeDir, err := os.UserHomeDir(); err == nil {
			configDir = filepath.Join(homeDir, ".config", "koutaku")
		}
	}

	// If we couldn't determine the config directory, fall back to current directory
	if configDir == "" {
		configDir = "./koutaku-data"
	}
...
func (s *KoutakuHTTPServer) registerPluginWithStatus(ctx context.Context, name string, path *string, config any, failOnError bool) error {
...
func (s *KoutakuHTTPServer) CollectObservabilityPlugins() []schemas.ObservabilityPlugin {
...
func (s *KoutakuHTTPServer) updateKeyStatus(
	ctx context.Context,
