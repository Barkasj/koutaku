...
func TestRequestWithSettableExtraParams_OpenAIChatRequest(t *testing.T) {
	...
	t.Run("extra params propagate through ToKoutakuChatRequest", func(t *testing.T) {
		req := &openai.OpenAIChatRequest{
		...
		ctx := schemas.NewKoutakuContext(nil, schemas.NoDeadline)
		koutakuReq := req.ToKoutakuChatRequest(ctx)

		require.NotNil(t, koutakuReq)
		require.NotNil(t, koutakuReq.Params)
		assert.Contains(t, koutakuReq.Params.ExtraParams, "guardrailConfig")
	})
}
...
func TestExtraParamsRequiresPassthroughHeader(t *testing.T) {
	...
	t.Run("extra_params NOT extracted without passthrough header", func(t *testing.T) {
		...
		koutakuCtx := schemas.NewKoutakuContext(nil, schemas.NoDeadline)
		// Header not set -- simulate router logic
		if koutakuCtx.Value(schemas.KoutakuContextKeyPassthroughExtraParams) == true {
...
func TestExtraParamsPassthrough_EndToEnd(t *testing.T) {
	...
	koutakuReq := req.ToKoutakuChatRequest(koutakuCtx)

	require.NotNil(t, koutakuReq)
	require.NotNil(t, koutakuReq.Params)
	require.Contains(t, koutakuReq.Params.ExtraParams, "guardrailConfig")

	gc, ok := koutakuReq.Params.ExtraParams["guardrailConfig"].(map[string]interface{})
	require.True(t, ok)
	assert.Equal(t, "my-guardrail", gc["guardrailIdentifier"])
	assert.Equal(t, "1", gc["guardrailVersion"])
	assert.Equal(t, "disabled", gc["trace"])

	assert.NotContains(t, koutakuReq.Params.ExtraParams, "model")
	assert.NotContains(t, koutakuReq.Params.ExtraParams, "messages")
	assert.NotContains(t, koutakuReq.Params.ExtraParams, "stream")
	assert.NotContains(t, koutakuReq.Params.ExtraParams, "temperature")
}
...
func TestExtraParamsSetViaInterfaceMutatesOriginalReq(t *testing.T) {
	...
	koutakuCtx := schemas.NewKoutakuContext(nil, schemas.NoDeadline)
	koutakuReq, err := chatRoute.RequestConverter(koutakuCtx, req)
	require.NoError(t, err)
	require.NotNil(t, koutakuReq)
	require.NotNil(t, koutakuReq.ChatRequest)
	require.NotNil(t, koutakuReq.ChatRequest.Params)
	assert.Contains(t, koutakuReq.ChatRequest.Params.ExtraParams, "guardrailConfig",
		"extra params should propagate through RequestConverter to KoutakuChatRequest")
}
