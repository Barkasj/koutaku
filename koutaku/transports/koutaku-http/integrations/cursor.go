...
// extra_fields that KoutakuChatResponse would include.
...
func cursorChunkID(extras *schemas.KoutakuResponseExtraFields) string {
	return "chatcmpl-koutaku-" + strconv.Itoa(extras.ChunkIndex)
}

// cursorModel returns the best model name available from extra fields.
func cursorModel(extras *schemas.KoutakuResponseExtraFields) string {
...
func convertResponsesStreamToChatChunk(resp *schemas.KoutakuResponsesStreamResponse) (string, interface{}, error) {
	switch resp.Type {
...
func convertResponsesResponseToChatCompletion(resp *schemas.KoutakuResponsesResponse) *cursorChatCompletion {
...
// normalizeKoutakuInputContentBlocks ensures all input messages in a KoutakuResponsesRequest
// have ContentBlocks instead of ContentStr. This is a defense-in-depth normalization that runs
// AFTER ToKoutakuResponsesRequest, which can re-introduce ContentStr when the input is a string.
func normalizeKoutakuInputContentBlocks(req *schemas.KoutakuResponsesRequest) {
	if req == nil {
		return
	}
		...
		routes = append(routes, RouteConfig{
			...
			RequestConverter: func(ctx *schemas.KoutakuContext, req interface{}) (*schemas.KoutakuRequest, error) {
				if openaiReq, ok := req.(*openai.OpenAIResponsesRequest); ok {
					koutakuReq := openaiReq.ToKoutakuResponsesRequest(ctx)
					if koutakuReq == nil {
						return nil, errors.New("koutaku responses request conversion returned nil")
					}
			...
			PreCallback: func(ctx *fasthttp.RequestCtx, koutakuCtx *schemas.KoutakuContext, req interface{}) error {
				...
				koutakuCtx.SetValue(schemas.KoutakuContextKeyUserAgent, schemas.Cursor.String())
				return nil
			},
		})
	}
...
// NewCursorRouter creates a new CursorRouter with the given koutaku client.
func NewCursorRouter(client *koutaku.Koutaku, handlerStore lib.HandlerStore, logger schemas.Logger) *CursorRouter {
	routes := []RouteConfig{}
