...
func Test_createBedrockRerankRouteRequestConverter(t *testing.T) {
	...
	assert.Equal(t, schemas.Bedrock, koutakuReq.RerankRequest.Provider)
	assert.Equal(t, "capital of france", koutakuReq.RerankRequest.Query)
	require.Len(t, koutakuReq.RerankRequest.Documents, 1)
...
func Test_s3PutObjectPostCallback(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			response: &schemas.KoutakuFileUploadResponse{
				ID: "file-123",
			},
			wantStatus: 200,
			wantETag:   "\"file-123\"",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, "koutaku", string(ctx.Response.Header.Peek("x-amz-request-id")))
...
func Test_s3GetObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.KoutakuFileContentResponse{
...
func Test_s3HeadObjectPostCallback(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid response",
			response: &schemas.KoutakuFileRetrieveResponse{
				ID:    "file-789",
				Bytes: 1024,
			},
			wantStatus: 200,
...
func Test_extractBedrockBatchListQueryParams(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuCtx := createTestKoutakuContext()
			err := callback(ctx, koutakuCtx, req)
...
func Test_extractBedrockJobArnFromPath(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuCtx := createTestKoutakuContextWithProvider(tt.provider)
			err := callback(ctx, koutakuCtx, req)
...
func Test_extractS3ListObjectsV2Params(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantBucket, koutakuCtx.Value(s3ContextKeyBucket))
			assert.Equal(t, tt.wantPrefix, koutakuCtx.Value(s3ContextKeyPrefix))
			assert.Equal(t, tt.wantMaxKeys, koutakuCtx.Value(s3ContextKeyMaxKeys))
		})
	}
}
...
func createTestKoutakuContext() *schemas.KoutakuContext {
	koutakuCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	koutakuCtx.SetValue(koutakuContextKeyProvider, schemas.Bedrock)
	return koutakuCtx
}

func createTestKoutakuContextWithProvider(provider schemas.ModelProvider) *schemas.KoutakuContext {
	koutakuCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
