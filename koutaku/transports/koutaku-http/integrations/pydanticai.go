...
// NewPydanticAIRouter creates a new PydanticAIRouter with the given koutaku client.
func NewPydanticAIRouter(client *koutaku.Koutaku, handlerStore lib.HandlerStore, logger schemas.Logger) *PydanticAIRouter {
	routes := []RouteConfig{}
...
func withPydanticResponsesNullNormalization(routes []RouteConfig) []RouteConfig {
	for i := range routes {
		...
		if routes[i].ResponsesResponseConverter != nil {
			routes[i].ResponsesResponseConverter = func(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponsesResponse) (interface{}, error) {
				// For pydantic responses endpoint, prefer normalized koutaku output
		...
		if routes[i].StreamConfig != nil && routes[i].StreamConfig.ResponsesStreamResponseConverter != nil {
			...
			routes[i].StreamConfig.ResponsesStreamResponseConverter = func(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponsesStreamResponse) (string, interface{}, error) {
				if resp == nil {
					return "", nil, nil
				}
...
func ensurePydanticResponsesStreamTextFields(resp *schemas.KoutakuResponsesStreamResponse) {
	if resp == nil {
		return
	}
	...
	case schemas.ResponsesStreamResponseTypeOutputTextDelta:
		if resp.Delta == nil {
			resp.Delta = koutaku.Ptr("")
		}
	case schemas.ResponsesStreamResponseTypeOutputTextDone:
		if resp.Text == nil {
			resp.Text = koutaku.Ptr("")
		}
	}
}
