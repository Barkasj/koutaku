...
// ToKoutakuResponsesRequest converts an OpenAI responses request to Koutaku format
func (resp *OpenAIResponsesRequest) ToKoutakuResponsesRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuResponsesRequest {
	if resp == nil {
		return nil
	}
...
// ToOpenAIResponsesRequest converts a Koutaku responses request to OpenAI format
func ToOpenAIResponsesRequest(koutakuReq *schemas.KoutakuResponsesRequest) *OpenAIResponsesRequest {
	...
	messages = make([]schemas.ResponsesMessage, 0, len(koutakuReq.Input))
	for _, message := range koutakuReq.Input {
		...
		if message.ResponsesReasoning != nil {
			isGptOss := strings.Contains(koutakuReq.Model, "gpt-oss")
			isReasoning := isOpenAIReasoningModel(koutakuReq.Model)
	...
	if params != nil {
		...
		if req.ResponsesParameters.Reasoning != nil {
			...
			if koutakuReq.Provider == schemas.XAI &&
				schemas.IsGrokReasoningModel(koutakuReq.Model) &&
				!strings.Contains(koutakuReq.Model, "grok-3-mini") {
			...
			if koutakuReq.Provider == schemas.OpenAI && !isOpenAIReasoningModel(koutakuReq.Model) {
		...
		if isOpenAIReasoningModel(koutakuReq.Model) {
			stripTopP := true
			_, parsedModel := schemas.ParseModelString(koutakuReq.Model, schemas.OpenAI)
		...
		// We must copy the Tools slice since it shares the backing array with koutakuReq.Params.Tools.
		if len(req.Tools) > 0 {
