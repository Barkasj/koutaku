...
func setStreamingRequestBody(ctx *schemas.KoutakuContext, req *fasthttp.Request, jsonBody []byte, providerName schemas.ModelProvider) {
...
) (*largePayloadResult, *schemas.KoutakuError, bool) {
...
func finalizeOpenAIResponse(
	...
	providerName schemas.ModelProvider,
	logger schemas.Logger,
) ([]byte, *largePayloadResult, *schemas.KoutakuError) {
	body, isLarge, koutakuErr := providerUtils.FinalizeResponseWithLargeDetection(ctx, resp, logger)
	if koutakuErr != nil {
		fasthttp.ReleaseResponse(resp)
...
func extractOpenAIUsageFromBytes(data []byte) *schemas.KoutakuLLMUsage {
