...
func TestParseTranscriptionFormDataBodyFromRequest_OrdersMetadataBeforeFile(t *testing.T) {
	...
	if koutakuErr := ParseTranscriptionFormDataBodyFromRequest(writer, req, schemas.OpenAI); koutakuErr != nil {
		t.Fatalf("unexpected koutaku error: %v", koutakuErr.Error.Message)
	}
