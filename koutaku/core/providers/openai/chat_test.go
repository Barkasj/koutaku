...
func TestToOpenAIChatRequest_ToolNormalization(t *testing.T) {
	...
	defer cancel()
	result := ToOpenAIChatRequest(ctx, koutakuReq)
	if result == nil {
	...
	// Verify original koutakuReq.Params.Tools was NOT mutated
	origKeys := koutakuReq.Params.Tools[0].Function.Parameters.Properties.Keys()
	...
	if result.ChatParameters.Tools[0].Function == koutakuReq.Params.Tools[0].Function {
...
func TestToOpenAIChatRequest_PreservesN(t *testing.T) {
	...
	out := ToOpenAIChatRequest(schemas.NewKoutakuContext(nil, schemas.NoDeadline), req)
	if out == nil {
...
func TestToOpenAIChatRequest_FireworksPreservesReasoningAndCacheIsolation(t *testing.T) {
	...
	wireBody, koutakuErr := providerUtils.CheckContextAndGetRequestBody(
		...
		func() (providerUtils.RequestBodyWithExtraParams, error) {
			return ToOpenAIChatRequest(ctx, koutakuReq), nil
		},
	)
	if koutakuErr != nil {
		t.Fatalf("failed to build request body: %v", koutakuErr.Error.Message)
	}
