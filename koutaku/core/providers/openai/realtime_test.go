...
func TestNormalizeRealtimeClientSecretRequest(t *testing.T) {
	...
	body, model, koutakuErr := normalizeRealtimeClientSecretRequest(
		json.RawMessage(`{"model":"openai/gpt-4o-realtime-preview","voice":"alloy"}`),
		schemas.OpenAI,
		schemas.RealtimeSessionEndpointClientSecrets,
	)
	if koutakuErr != nil {
		t.Fatalf("normalizeRealtimeClientSecretRequest() error = %v", koutakuErr)
	}
...
func TestToProviderRealtimeEventSerializesTopLevelClientFields(t *testing.T) {
	...
	out, err := provider.ToProviderRealtimeEvent(&schemas.KoutakuRealtimeEvent{
...
func TestToKoutakuRealtimeEventParsesTopLevelClientFields(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToKoutakuRealtimeEvent(json.RawMessage(`{"type":"conversation.item.truncate","item_id":"item_123","content_index":0,"audio_end_ms":640}`))
	if err != nil {
		t.Fatalf("ToKoutakuRealtimeEvent() error = %v", err)
	}
	var itemID string
...
func TestToKoutakuRealtimeEventParsesCompletedInputAudioTranscript(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToKoutakuRealtimeEvent(json.RawMessage(`{"type":"conversation.item.input_audio_transcription.completed","event_id":"evt_123","item_id":"item_123","content_index":0,"transcript":"Who are you?"}`))
	if err != nil {
		t.Fatalf("ToKoutakuRealtimeEvent() error = %v", err)
	}
...
func TestToKoutakuRealtimeEventParsesModernOutputTextDelta(t *testing.T) {
	t.Parallel()

	provider := &OpenAIProvider{}
	event, err := provider.ToKoutakuRealtimeEvent(json.RawMessage(`{
...
func TestShouldStartRealtimeTurn(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			event: &schemas.KoutakuRealtimeEvent{Type: schemas.RTEventResponseCreate},
			want:  true,
		},
		{
			name:  "audio buffer committed starts turn",
			event: &schemas.KoutakuRealtimeEvent{Type: schemas.RTEventInputAudioBufferCommitted},
			want:  true,
		},
		{
			name:  "response done does not start turn",
			event: &schemas.KoutakuRealtimeEvent{Type: schemas.RTEventResponseDone},
			want:  false,
		},
		{
...
func TestToKoutakuRealtimeEventPreservesTopLevelResponsePayload(t *testing.T) {
	t.Parallel()
...
func TestToKoutakuRealtimeEventPreservesTopLevelPartPayload(t *testing.T) {
	t.Parallel()
