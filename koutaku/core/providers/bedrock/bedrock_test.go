...
// TestKoutakuToBedrockRequestConversion tests the conversion from Koutaku request to Bedrock request
func TestKoutakuToBedrockRequestConversion(t *testing.T) {
...
// TestBedrockToKoutakuRequestConversion tests the conversion from Bedrock request to Koutaku request
func TestBedrockToKoutakuRequestConversion(t *testing.T) {
	maxTokens := testMaxTokens
	temp := testTemp
	topP := testTopP
	trace := testTrace
	...
	tests := []struct {
		...
		expected *schemas.KoutakuResponsesRequest
		wantErr  bool
	}{
		{
			name: "BasicTextMessage",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var actual *schemas.KoutakuResponsesRequest
			var err error
			if tt.input == nil {
				var bedrockReq *bedrock.BedrockConverseRequest
				actual, err = bedrockReq.ToKoutakuResponsesRequest(ctx)
			} else {
				actual, err = tt.input.ToKoutakuResponsesRequest(ctx)
			}
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, actual)
...
// TestKoutakuToBedrockResponseConversion tests the conversion from Koutaku Responses response to Bedrock response
func TestKoutakuToBedrockResponseConversion(t *testing.T) {
	inputTokens := 10
	outputTokens := 20
	totalTokens := 30
	latency := int64(100)
	...
	tests := []struct {
		name     string
		input    *schemas.KoutakuResponsesResponse
...
// TestBedrockToKoutakuResponseConversion tests the conversion from Bedrock response to Koutaku Responses response
func TestBedrockToKoutakuResponseConversion(t *testing.T) {
	inputTokens := 10
	outputTokens := 20
	totalTokens := 30
	toolUseID := "call-123"
	...
	tests := []struct {
		...
		expected *schemas.KoutakuResponsesResponse
		wantErr  bool
	}{
		{
			name: "BasicTextResponse",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var actual *schemas.KoutakuResponsesResponse
			var err error
			if tt.input == nil {
				var bedrockResp *bedrock.BedrockConverseResponse
				actual, err = bedrockResp.ToKoutakuResponsesResponse(ctx)
			} else {
				actual, err = tt.input.ToKoutakuResponsesResponse(ctx)
			}
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, actual)
...
func TestToolResultJSONParsingResponsesAPI(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			messages, _, err := bedrock.ConvertKoutakuMessagesToBedrockMessages(input)
			require.NoError(t, err)
...
// TestConvertKoutakuResponsesMessageContentBlocksToBedrockContentBlocks_EmptyBlocks tests that
...
func TestConvertKoutakuResponsesMessageContentBlocksToBedrockContentBlocks_EmptyBlocks(t *testing.T) {
	tests := []struct {
		name           string
		input          *schemas.KoutakuResponsesResponse
...
func TestAnthropicReasoningConfigUsesThinkingField(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
			result, err := bedrock.ToBedrockChatCompletionRequest(ctx, koutakuReq)
			require.NoError(t, err)
...
func TestAnthropicOrderedOutputConfigRoundTripsReasoning(t *testing.T) {
	...
	ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	result, err := request.ToKoutakuResponsesRequest(ctx)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestBedrockStopReasonMapping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuResp, err := response.ToKoutakuChatResponse(context.Background(), "test-model")
			require.NoError(t, err)
			require.NotNil(t, koutakuResp)
			require.Len(t, koutakuResp.Choices, 1)
			require.NotNil(t, koutakuResp.Choices[0].FinishReason)
			assert.Equal(t, tt.expectedKoutaku, *koutakuResp.Choices[0].FinishReason,
				"Bedrock stop reason %q should map to %q", tt.bedrockStopReason, tt.expectedKoutaku)
		})
	}
}
...
func TestBedrockToolInputKeyOrderPreservation(t *testing.T) {
	...
	ctx, cancel := schemas.NewKoutakuContextWithCancel(nil)
	defer cancel()
	result, err := bedrock.ToBedrockChatCompletionRequest(ctx, koutakuReq)
	require.NoError(t, err)
...
func TestToBedrockInvokeMessagesStreamResponse_NoDuplicateContentBlockStop(t *testing.T) {
	...
	events := []*schemas.KoutakuResponsesStreamResponse{
		{
			Type:         schemas.ResponsesStreamResponseTypeOutputTextDone,
			ContentIndex: &contentIdx,
			ExtraFields:  schemas.KoutakuResponseExtraFields{OriginalModelRequested: model},
		},
		{
