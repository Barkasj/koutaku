...
// ToBedrockChatCompletionRequest converts a Koutaku request to Bedrock Converse API format
func ToBedrockChatCompletionRequest(ctx *schemas.KoutakuContext, koutakuReq *schemas.KoutakuChatRequest) (*BedrockConverseRequest, error) {
	if koutakuReq == nil {
	...
	messages, systemMessages, err := convertMessages(koutakuReq.Input)
	if err != nil {
	...
	if err := convertChatParameters(ctx, koutakuReq, bedrockReq); err != nil {
		return nil, fmt.Errorf("failed to convert chat parameters: %w", err)
	}

	// Ensure tool config is present when needed
	ensureChatToolConfigForConversation(koutakuReq, bedrockReq)

	return bedrockReq, nil
}

// ToKoutakuChatResponse converts a Bedrock Converse API response to Koutaku format
func (response *BedrockConverseResponse) ToKoutakuChatResponse(ctx context.Context, model string) (*schemas.KoutakuChatResponse, error) {
	if response == nil {
	...
	if response.Output.Message != nil {
		for _, contentBlock := range response.Output.Message.Content {
			...
			if contentBlock.ToolUse != nil {
				...
				if structuredOutputToolName, ok := ctx.Value(schemas.KoutakuContextKeyStructuredOutputToolName).(string); ok && contentBlock.ToolUse.Name == structuredOutputToolName {
	...
	// Create the final Koutaku response
	...
	if response.ServiceTier != nil && response.ServiceTier.Type != "" {
		koutakuResponse.ServiceTier = &response.ServiceTier.Type
	}
...
func (chunk *BedrockStreamEvent) ToKoutakuChatCompletionStream(state *BedrockStreamState) (*schemas.KoutakuChatResponse, *schemas.KoutakuError, bool) {
	if state == nil {
	...
	case chunk.Delta != nil:
		...
		case chunk.Delta.ReasoningContent != nil:
			...
			var streamResponse *schemas.KoutakuChatResponse
