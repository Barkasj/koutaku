...
// so we can reuse ToKoutakuResponsesRequest() for messages-based requests.
...
// ToKoutakuTextCompletionRequest handles ALL prompt-based families
// (Anthropic legacy, Mistral, Llama, Cohere Command, Cohere Command R).
func (r *BedrockInvokeRequest) ToKoutakuTextCompletionRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuTextCompletionRequest {
	...
	// Build a BedrockTextCompletionRequest and delegate to its ToKoutakuTextCompletionRequest
	...
	return textReq.ToKoutakuTextCompletionRequest(ctx)
}

// ToKoutakuEmbeddingRequest converts the invoke request to a KoutakuEmbeddingRequest.
// Handles both Titan (inputText) and Cohere (texts) embedding formats.
func (r *BedrockInvokeRequest) ToKoutakuEmbeddingRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuEmbeddingRequest {
	modelID := r.ModelID
	...
	req := &schemas.KoutakuEmbeddingRequest{
		Provider: provider,
		Model:    model,
	}
...
// ToKoutakuImageGenerationRequest converts the invoke request to a KoutakuImageGenerationRequest.
// Handles Titan/Nova Canvas (taskType=TEXT_IMAGE with textToImageParams) and Stability AI (flat prompt fields).
func (r *BedrockInvokeRequest) ToKoutakuImageGenerationRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuImageGenerationRequest {
	modelID := r.ModelID
	if unescaped, err := url.PathUnescape(r.ModelID); err == nil {
		modelID = unescaped
	}
...
// ToKoutakuImageEditRequest converts the invoke request to a KoutakuImageEditRequest.
// Handles Titan/Nova Canvas (taskType in INPAINTING/OUTPAINTING/BACKGROUND_REMOVAL) and Stability AI (flat image/mask fields).
func (r *BedrockInvokeRequest) ToKoutakuImageEditRequest(ctx *schemas.KoutakuContext) (*schemas.KoutakuImageEditRequest, error) {
	modelID := r.ModelID
	if unescaped, err := url.PathUnescape(r.ModelID); err == nil {
		modelID = unescaped
	}
	provider, model := schemas.ParseModelString(modelID, providerUtils.CheckAndSetDefaultProvider(ctx, schemas.Bedrock))
	req := &schemas.KoutakuImageEditRequest{
		Provider: provider,
		Model:    model,
	}
...
// ToKoutakuImageVariationRequest converts the invoke request to a KoutakuImageVariationRequest.
// Reads from imageVariationParams (Titan/Nova Canvas format).
func (r *BedrockInvokeRequest) ToKoutakuImageVariationRequest(ctx *schemas.KoutakuContext) (*schemas.KoutakuImageVariationRequest, error) {
	...
	provider, model := schemas.ParseModelString(modelID, providerUtils.CheckAndSetDefaultProvider(ctx, schemas.Bedrock))
	req := &schemas.KoutakuImageVariationRequest{
		Provider: provider,
		Model:    model,
...
// ToBedrockInvokeMessagesResponse converts a KoutakuResponsesResponse to the model-family-specific
// InvokeModel response format. Switches on model family to produce the correct JSON structure.
func ToBedrockInvokeMessagesResponse(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponsesResponse) (interface{}, error) {
	if resp == nil {
...
func ToBedrockInvokeImagesResponse(ctx *schemas.KoutakuContext, resp *schemas.KoutakuImageGenerationResponse) (interface{}, error) {
	if resp == nil {
		return nil, fmt.Errorf("koutaku response is nil")
	}
	...
	// Default: Titan Image Generator v1/v2, Nova Canvas — reconstruct from Koutaku data
...
// ToBedrockEmbeddingInvokeResponse converts a KoutakuEmbeddingResponse back to the native
...
func ToBedrockEmbeddingInvokeResponse(resp *schemas.KoutakuEmbeddingResponse) (interface{}, error) {
	if resp == nil {
		return nil, fmt.Errorf("koutaku embedding response is nil")
	}
...
// toBedrockInvokeAnthropicResponse converts KoutakuResponsesResponse to Anthropic Messages API format.
func toBedrockInvokeAnthropicResponse(resp *schemas.KoutakuResponsesResponse, model string) *BedrockInvokeMessagesResponse {
...
// toBedrockInvokeAI21Response converts KoutakuResponsesResponse to AI21 Jamba format.
func toBedrockInvokeAI21Response(resp *schemas.KoutakuResponsesResponse) *BedrockInvokeAI21Response {
...
// ToBedrockInvokeMessagesStreamResponse converts a Koutaku Responses stream event to
// a BedrockStreamEvent with InvokeModelRawChunk for the invoke-with-response-stream endpoint.
func ToBedrockInvokeMessagesStreamResponse(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponsesStreamResponse) (string, interface{}, error) {
	if resp == nil {
	...
	// then wrap in InvokeModelRawChunks. Some Koutaku events map to multiple Anthropic events
...
// toAnthropicInvokeStreamBytes converts a Koutaku stream event into raw bytes representing
// the Anthropic Messages API streaming event JSON, suitable for wrapping in InvokeModelRawChunks.
// Returns a slice of byte slices since some Koutaku events map to multiple Anthropic events
// (e.g., Completed → message_delta + message_stop).
func toAnthropicInvokeStreamBytes(resp *schemas.KoutakuResponsesStreamResponse) ([][]byte, error) {
	var event interface{}
