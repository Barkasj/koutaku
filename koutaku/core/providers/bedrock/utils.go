...
var (
	...
	// bedrockFinishReasonToKoutaku maps Bedrock Converse API stop reasons to Koutaku format.
	// Bedrock has additional stop reasons beyond Anthropic (guardrail_intervened, content_filtered).
	bedrockFinishReasonToKoutaku = map[string]string{
		"end_turn":             "stop",
		"max_tokens":           "length",
		"stop_sequence":        "stop",
		"tool_use":             "tool_calls",
...
// convertBedrockStopReason converts a Bedrock stop reason to Koutaku format.
func convertBedrockStopReason(stopReason string) string {
	if reason, ok := bedrockFinishReasonToKoutaku[stopReason]; ok {
		return reason
	}
	return "stop"
}
...
func convertChatParameters(ctx *schemas.KoutakuContext, koutakuReq *schemas.KoutakuChatRequest, bedrockReq *BedrockConverseRequest) error {
	// Parameters are optional - if not provided, just skip conversion
	if koutakuReq.Params == nil {
		return nil
	}
	// Convert inference config
	if inferenceConfig := convertInferenceConfig(koutakuReq.Params); inferenceConfig != nil {
	...
	responseFormatTool, anthropicOutputFormat := convertResponseFormatToTool(ctx, koutakuReq.Model, koutakuReq.Params)
	...
	filteredTools, _ := anthropic.ValidateChatToolsForProvider(koutakuReq.Params.Tools, schemas.Bedrock)

	// Convert tool config (function/custom tools → Converse toolConfig.tools).
	if toolConfig := convertToolConfigFromFiltered(koutakuReq.Model, koutakuReq.Params, filteredTools); toolConfig != nil {
	...
	if serverTools, betaHeaders := collectBedrockServerToolsFromFiltered(filteredTools); len(serverTools) > 0 {
		...
		if responseFormatTool == nil {
			if choice, ok := buildBedrockServerToolChoice(koutakuReq.Params, filteredTools); ok {
		...
		if koutakuReq.Params.Reasoning.MaxTokens != nil {
			...
			if schemas.IsNovaModel(koutakuReq.Model) {
				...
				if anthropic.SupportsAdaptiveThinking(koutakuReq.Model) {
					...
					budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*koutakuReq.Params.Reasoning.Effort, anthropic.MinimumReasoningMaxTokens, maxTokens)
					if err != nil {
						return err
					}
					bedrockReq.AdditionalModelRequestFields.Set("thinking", map[string]any{
	...
	if len(koutakuReq.Params.ExtraParams) > 0 {
		...
		if guardrailConfig, exists := koutakuReq.Params.ExtraParams["guardrailConfig"]; exists {
		...
		if koutakuReq.Params != nil && koutakuReq.Params.ExtraParams != nil {
			...
			if perfConfig, exists := koutakuReq.Params.ExtraParams["performanceConfig"]; exists {
			...
			if promptVars, exists := koutakuReq.Params.ExtraParams["promptVariables"]; exists {
			...
			if reqMetadata, exists := koutakuReq.Params.ExtraParams["requestMetadata"]; exists {
...
func ensureChatToolConfigForConversation(koutakuReq *schemas.KoutakuChatRequest, bedrockReq *BedrockConverseRequest) {
	if bedrockReq.ToolConfig != nil {
		return // Already has tool config
	}

	hasToolContent, tools := extractToolsFromConversationHistory(koutakuReq.Input)
	if hasToolContent && len(tools) > 0 {
		bedrockReq.ToolConfig = &BedrockToolConfig{Tools: tools}
	}
}

// convertMessages converts Koutaku messages to Bedrock format
// Returns regular messages and system messages separately
func convertMessages(koutakuMessages []schemas.ChatMessage) ([]BedrockMessage, []BedrockSystemMessage, error) {
	var messages []BedrockMessage
	var systemMessages []BedrockSystemMessage

	for i := 0; i < len(koutakuMessages); i++ {
		...
		case schemas.ChatMessageRoleTool:
			...
			for j := i + 1; j < len(koutakuMessages) && koutakuMessages[j].Role == schemas.ChatMessageRoleTool; j++ {
				toolMessages = append(toolMessages, koutakuMessages[j])
				i = j
			}
...
// convertSystemMessages converts a Koutaku system message to Bedrock format
...
// convertMessage converts a Koutaku message to Bedrock format
func convertMessage(msg schemas.ChatMessage) (BedrockMessage, error) {
	...
	if msg.ChatAssistantMessage != nil && len(msg.ChatAssistantMessage.ReasoningDetails) > 0 {
		for _, detail := range msg.ChatAssistantMessage.ReasoningDetails {
			if detail.Type == schemas.KoutakuReasoningDetailsTypeText {
...
// convertToolMessages converts multiple consecutive Koutaku tool messages to a single Bedrock message
...
// convertContent converts Koutaku message content to Bedrock content blocks
...
// convertContentBlock converts a Koutaku content block to Bedrock format
func convertContentBlock(block schemas.ChatContentBlock) ([]BedrockContentBlock, error) {
	...
	case schemas.ChatContentBlockTypeText:
		...
		// Once the world onboards on Koutaku - we should remove these shitty patterns.
...
// convertImageToBedrockSource converts a Koutaku image URL to Bedrock image source
...
func convertResponseFormatToTool(
	...
	// set koutaku context key structured output tool name
	toolName = fmt.Sprintf("bf_so_%s", toolName)
	ctx.SetValue(schemas.KoutakuContextKeyStructuredOutputToolName, toolName)
...
func convertTextFormatToTool(ctx *schemas.KoutakuContext, model string, textConfig *schemas.ResponsesTextConfig) (*BedrockTool, any) {
...
// convertInferenceConfig converts Koutaku parameters to Bedrock inference config
...
// convertToolConfig converts Koutaku tools to Bedrock tool config.
//
...
// convertToolChoice converts Koutaku tool choice to Bedrock format
...
// convertToolCallToContentBlock converts a Koutaku tool call to a Bedrock content block
...
// ToBedrockError converts a KoutakuError to BedrockError
// This is a standalone function similar to ToAnthropicChatCompletionError
func ToBedrockError(koutakuErr *schemas.KoutakuError) *BedrockError {
	...
	// Map error type/code
	if koutakuErr.Error != nil && koutakuErr.Error.Code != nil {
		bedrockErr.Type = *koutakuErr.Error.Code
		bedrockErr.Code = koutakuErr.Error.Code
	} else if koutakuErr.Type != nil {
		bedrockErr.Type = *koutakuErr.Type
	} else {
...
// This handles the conversion from flexible parameter formats to Koutaku's structured format
