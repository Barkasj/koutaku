...
// ToKoutakuResponsesStream converts a Bedrock stream event to a Koutaku Responses Stream response
// Returns a slice of responses to support cases where a single event produces multiple responses
func (chunk *BedrockStreamEvent) ToKoutakuResponsesStream(sequenceNumber int, state *BedrockResponsesStreamState) ([]*schemas.KoutakuResponsesStreamResponse, *schemas.KoutakuError, bool) {
	switch {
	case chunk.Role != nil:
		...
		if chunk.Start.ToolUse != nil {
			...
			for prevContentIndex, prevOutputIndex := range state.ContentIndexToOutputIndex {
				...
				if accumulatedArgs != "" {
					...
					argsDoneResponse := &schemas.KoutakuResponsesStreamResponse{
	...
	case chunk.ContentBlockIndex != nil && chunk.Delta != nil:
		...
		if !exists {
			...
			if chunk.Delta.Text != nil && *chunk.Delta.Text != "" {
				...
				textDeltaResponse := &schemas.KoutakuResponsesStreamResponse{
		...
		case chunk.Delta.ReasoningContent != nil:
			...
			if !state.ReasoningContentIndices[contentBlockIndex] {
				...
				if reasoningDelta.Text != nil && *reasoningDelta.Text != "" {
					deltaResponse := &schemas.KoutakuResponsesStreamResponse{
...
func FinalizeBedrockStream(state *BedrockResponsesStreamState, sequenceNumber int, usage *schemas.ResponsesResponseUsage) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
// ToBedrockConverseStreamResponse converts a Koutaku Responses stream response to Bedrock streaming format
// Returns a BedrockStreamEvent that represents the streaming event in Bedrock's format
func ToBedrockConverseStreamResponse(koutakuResp *schemas.KoutakuResponsesStreamResponse) (*BedrockStreamEvent, error) {
	if koutakuResp == nil {
		return nil, fmt.Errorf("koutaku stream response is nil")
	}
	...
	case schemas.ResponsesStreamResponseTypeOutputItemAdded:
		...
		if koutakuResp.Item != nil && koutakuResp.Item.ResponsesToolMessage != nil {
			// Tool use start
			if koutakuResp.Item.ResponsesToolMessage.Name != nil && koutakuResp.Item.ResponsesToolMessage.CallID != nil {
				contentBlockIndex := 0
				if koutakuResp.ContentIndex != nil {
					contentBlockIndex = *koutakuResp.ContentIndex
				}
				event.ContentBlockIndex = &contentBlockIndex
				event.Start = &BedrockContentBlockStart{
					ToolUse: &BedrockToolUseStart{
						ToolUseID: *koutakuResp.Item.ResponsesToolMessage.CallID,
						Name:      *koutakuResp.Item.ResponsesToolMessage.Name,
					},
				}
			}
			...
			if koutakuResp.Item.Content != nil || (koutakuResp.Item.Type != nil && *koutakuResp.Item.Type == schemas.ResponsesMessageTypeMessage) {
				return nil, nil
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeOutputTextDelta:
		// Text delta
		if koutakuResp.Delta != nil && *koutakuResp.Delta != "" {
			contentBlockIndex := 0
			if koutakuResp.ContentIndex != nil {
				contentBlockIndex = *koutakuResp.ContentIndex
			}
	...
	case schemas.ResponsesStreamResponseTypeReasoningSummaryTextDelta:
		...
		} else if koutakuResp.Delta != nil && *koutakuResp.Delta != "" {
	...
	case schemas.ResponsesStreamResponseTypeCompleted:
		...
		stopReason := "end_turn"
		if koutakuResp.Response != nil && koutakuResp.Response.IncompleteDetails != nil {
			stopReason = koutakuResp.Response.IncompleteDetails.Reason
		}
		event.StopReason = &stopReason

		// Add usage if available
		if koutakuResp.Response != nil && koutakuResp.Response.Usage != nil {
			event.Usage = &BedrockTokenUsage{
				InputTokens:  koutakuResp.Response.Usage.InputTokens,
				OutputTokens: koutakuResp.Response.Usage.OutputTokens,
				TotalTokens:  koutakuResp.Response.Usage.TotalTokens,
			}
		}

	case schemas.ResponsesStreamResponseTypeError:
		// Error - errors are handled separately by the router via KoutakuError in the stream chunk
...
// ToKoutakuResponsesRequest converts a BedrockConverseRequest to Koutaku Responses Request format
func (request *BedrockConverseRequest) ToKoutakuResponsesRequest(ctx *schemas.KoutakuContext) (*schemas.KoutakuResponsesRequest, error) {
	if request == nil {
	...
	convertedMessages := ConvertBedrockMessagesToKoutakuMessages(ctx, request.Messages, request.System, false)
	koutakuReq.Input = convertedMessages

	// Convert inference config to parameters
	if request.InferenceConfig != nil {
		if request.InferenceConfig.MaxTokens != nil {
			koutakuReq.Params.MaxOutputTokens = request.InferenceConfig.MaxTokens
		}
		if request.InferenceConfig.Temperature != nil {
			koutakuReq.Params.Temperature = request.InferenceConfig.Temperature
		}
		if request.InferenceConfig.TopP != nil {
			koutakuReq.Params.TopP = request.InferenceConfig.TopP
		}
		if len(request.InferenceConfig.StopSequences) > 0 {
			...
			koutakuReq.Params.ExtraParams["stop"] = request.InferenceConfig.StopSequences
		}
	}
			...
			if tool.ToolSpec != nil {
				...
				if len(tool.ToolSpec.InputSchema.JSON) > 0 {
					...
					if err := sonic.Unmarshal(tool.ToolSpec.InputSchema.JSON, &params); err == nil {
						koutakuTool.ResponsesToolFunction.Parameters = &params
					} else {
				...
				koutakuReq.Params.Tools = append(koutakuReq.Params.Tools, koutakuTool)
			} else if tool.CachePoint != nil && !schemas.IsNovaModel(koutakuReq.Model) {
				// add cache control to last tool in tools array
				if len(koutakuReq.Params.Tools) > 0 {
					koutakuReq.Params.Tools[len(koutakuReq.Params.Tools)-1].CacheControl = &schemas.CacheControl{
	...
	if request.ToolConfig != nil && request.ToolConfig.ToolChoice != nil {
		...
		if toolChoice.Auto != nil {
			...
			koutakuReq.Params.ToolChoice = &schemas.ResponsesToolChoice{
	...
	if request.GuardrailConfig != nil {
		...
		koutakuReq.Params.ExtraParams["guardrailConfig"] = guardrailMap
	}
				...
				if typeStr, ok := schemas.SafeExtractString(reasoningConfigMap["type"]); ok {
					if typeStr == "enabled" || typeStr == "adaptive" {
						...
						if found {
							...
							koutakuReq.Params.Reasoning = &schemas.ResponsesParametersReasoning{
							...
							minBudgetTokens := 0
							defaultMaxTokens := providerUtils.GetMaxOutputTokensOrDefault(koutakuReq.Model, DefaultCompletionMaxTokens)
							if request.InferenceConfig != nil && request.InferenceConfig.MaxTokens != nil {
								defaultMaxTokens = *request.InferenceConfig.MaxTokens
							}
							if schemas.IsAnthropicModel(koutakuReq.Model) {
	...
	if include, ok := schemas.SafeExtractStringSlice(request.ExtraParams["include"]); ok {
		koutakuReq.Params.Include = include
	}
	...
	if request.PerformanceConfig != nil {
		...
		if len(perfConfigMap) > 0 {
			koutakuReq.Params.ExtraParams["performanceConfig"] = perfConfigMap
		}
	}
	...
	if len(request.PromptVariables) > 0 {
		...
		if len(promptVarsMap) > 0 {
			koutakuReq.Params.ExtraParams["promptVariables"] = promptVarsMap
		}
	}
	...
	if len(request.RequestMetadata) > 0 {
		...
		koutakuReq.Params.ExtraParams["requestMetadata"] = request.RequestMetadata
	}
	...
	if request.AdditionalModelRequestFields.Len() > 0 {
		...
		koutakuReq.Params.ExtraParams["additionalModelRequestFieldPaths"] = request.AdditionalModelRequestFields
	}
	...
	if len(request.AdditionalModelResponseFieldPaths) > 0 {
		...
		koutakuReq.Params.ExtraParams["additionalModelResponseFieldPaths"] = request.AdditionalModelResponseFieldPaths
	}

	return koutakuReq, nil
}

// ToBedrockResponsesRequest converts a KoutakuRequest (Responses structure) back to BedrockConverseRequest
func ToBedrockResponsesRequest(ctx *schemas.KoutakuContext, koutakuReq *schemas.KoutakuResponsesRequest) (*BedrockConverseRequest, error) {
	if koutakuReq == nil {
		return nil, fmt.Errorf("koutaku request is nil")
	}

	// Validate tools are supported by Bedrock
	if koutakuReq.Params != nil && koutakuReq.Params.Tools != nil {
		if toolErr := anthropic.ValidateToolsForProvider(koutakuReq.Params.Tools, schemas.Bedrock); toolErr != nil {
			return nil, toolErr
		}
	}
	...
	// map koutaku messages to bedrock messages using the new conversion method
	if koutakuReq.Input != nil {
		messages, systemMessages, err := ConvertKoutakuMessagesToBedrockMessages(koutakuReq.Input)
		if err != nil {
		...
		if len(systemMessages) > 0 {
			...
			if koutakuReq.Params != nil && koutakuReq.Params.Instructions != nil {
	...
	if koutakuReq.Params != nil {
		...
		if koutakuReq.Params.MaxOutputTokens != nil {
			inferenceConfig.MaxTokens = koutakuReq.Params.MaxOutputTokens
		}
		if koutakuReq.Params.Temperature != nil {
			inferenceConfig.Temperature = koutakuReq.Params.Temperature
		}
		if koutakuReq.Params.TopP != nil {
			inferenceConfig.TopP = koutakuReq.Params.TopP
		}
		if koutakuReq.Params.Reasoning != nil {
			...
			if koutakuReq.Params.Reasoning.MaxTokens != nil {
				tokenBudget := *koutakuReq.Params.Reasoning.MaxTokens
				...
				if schemas.IsAnthropicModel(koutakuReq.Model) && tokenBudget < anthropic.MinimumReasoningMaxTokens {
				...
				} else if schemas.IsNovaModel(koutakuReq.Model) {
					minBudgetTokens := MinimumReasoningMaxTokens
					modelDefaultMaxTokens := providerUtils.GetMaxOutputTokensOrDefault(koutakuReq.Model, DefaultCompletionMaxTokens)
				...
				if koutakuReq.Params.Reasoning.Effort != nil && *koutakuReq.Params.Reasoning.Effort != "none" {
					if schemas.IsNovaModel(koutakuReq.Model) {
						effort := *koutakuReq.Params.Reasoning.Effort
						typeStr := "enabled"
						switch effort {
						case "high":
						...
						bedrockReq.AdditionalModelRequestFields.Set("reasoningConfig", config)
					} else if schemas.IsAnthropicModel(koutakuReq.Model) {
						if anthropic.SupportsAdaptiveThinking(koutakuReq.Model) {
							// Opus 4.6+: adaptive thinking + output_config.effort
							effort := anthropic.MapKoutakuEffortToAnthropic(*koutakuReq.Params.Reasoning.Effort)
							bedrockReq.AdditionalModelRequestFields.Set("thinking", map[string]any{
								"type": "adaptive",
							})
							...
							budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*koutakuReq.Params.Reasoning.Effort, anthropic.MinimumReasoningMaxTokens, defaultMaxTokens)
							if err != nil {
								return nil, err
							}
							bedrockReq.AdditionalModelRequestFields.Set("thinking", map[string]any{
						...
						budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*koutakuReq.Params.Reasoning.Effort, minBudgetTokens, defaultMaxTokens)
						if err != nil {
							return nil, err
						}
		...
		if koutakuReq.Params.Text != nil {
			if koutakuReq.Params.Text.Format != nil {
				responseFormatTool, anthropicOutputFormat := convertTextFormatToTool(ctx, koutakuReq.Model, koutakuReq.Params.Text)
		...
		if koutakuReq.Params.ExtraParams != nil {
			...
			if requestFields, exists := koutakuReq.Params.ExtraParams["additionalModelRequestFieldPaths"]; exists {
			...
			if responseFields, exists := koutakuReq.Params.ExtraParams["additionalModelResponseFieldPaths"]; exists {
		...
		if koutakuReq.Params.ServiceTier != nil {
			bedrockReq.ServiceTier = &BedrockServiceTier{
				Type: *koutakuReq.Params.ServiceTier,
			}
		}
	}
	...
	if koutakuReq.Params != nil && koutakuReq.Params.Tools != nil {
		...
		for _, tool := range koutakuReq.Params.Tools {
			if tool.ResponsesToolFunction != nil {
				...
				if tool.CacheControl != nil && !schemas.IsNovaModel(koutakuReq.Model) {
					bedrockTools = append(bedrockTools, BedrockTool{
	...
	// Convert tool choice
	if koutakuReq.Params != nil && koutakuReq.Params.ToolChoice != nil {
		bedrockToolChoice := convertResponsesToolChoice(*koutakuReq.Params.ToolChoice)
	...
	ensureResponsesToolConfigForConversation(koutakuReq, bedrockReq)

	return bedrockReq, nil
}

// ToKoutakuResponsesResponse converts BedrockConverseResponse to KoutakuResponsesResponse
func (response *BedrockConverseResponse) ToKoutakuResponsesResponse(ctx *schemas.KoutakuContext) (*schemas.KoutakuResponsesResponse, error) {
	if response == nil {
	...
	if response.Output != nil && response.Output.Message != nil {
		outputMessages := ConvertBedrockMessagesToKoutakuMessages(ctx, []BedrockMessage{*response.Output.Message}, []BedrockSystemMessage{}, true)
		koutakuResp.Output = outputMessages
	}

	if response.Usage != nil {
		// Convert usage information
		koutakuResp.Usage = &schemas.ResponsesResponseUsage{
		...
		if response.Usage.CacheReadInputTokens > 0 {
			if koutakuResp.Usage.InputTokensDetails == nil {
				koutakuResp.Usage.InputTokensDetails = &schemas.ResponsesResponseInputTokens{}
			}
			koutakuResp.Usage.InputTokensDetails.CachedReadTokens = response.Usage.CacheReadInputTokens
			koutakuResp.Usage.InputTokens = koutakuResp.Usage.InputTokens + response.Usage.CacheReadInputTokens
		}
		if response.Usage.CacheWriteInputTokens > 0 {
			if koutakuResp.Usage.InputTokensDetails == nil {
				koutakuResp.Usage.InputTokensDetails = &schemas.ResponsesResponseInputTokens{}
			}
			koutakuResp.Usage.InputTokensDetails.CachedWriteTokens = response.Usage.CacheWriteInputTokens
			koutakuResp.Usage.InputTokens = koutakuResp.Usage.InputTokens + response.Usage.CacheWriteInputTokens
		}
	}

	if response.ServiceTier != nil && response.ServiceTier.Type != "" {
		koutakuResp.ServiceTier = &response.ServiceTier.Type
	}

	return koutakuResp, nil
}

// ToBedrockConverseResponse converts Koutaku Responses response to Bedrock Converse response
func ToBedrockConverseResponse(koutakuResp *schemas.KoutakuResponsesResponse) (*BedrockConverseResponse, error) {
	if koutakuResp == nil {
		return nil, fmt.Errorf("koutaku response is nil")
	}
	...
	if len(koutakuResp.Output) > 0 {
		// Convert Koutaku messages back to Bedrock messages using the new conversion method
		bedrockMessages, _, err := ConvertKoutakuMessagesToBedrockMessages(koutakuResp.Output)
		if err != nil {
			return nil, fmt.Errorf("failed to convert koutaku output messages: %w", err)
		}
	...
	stopReason := "end_turn"
	if koutakuResp.IncompleteDetails != nil {
		stopReason = koutakuResp.IncompleteDetails.Reason
	} else if hasToolUse {
		stopReason = "tool_use"
	}
	...
	if koutakuResp.Usage != nil {
		bedrockResp.Usage.InputTokens = koutakuResp.Usage.InputTokens
		bedrockResp.Usage.OutputTokens = koutakuResp.Usage.OutputTokens
		bedrockResp.Usage.TotalTokens = koutakuResp.Usage.TotalTokens

		if koutakuResp.Usage.InputTokensDetails != nil {
			if koutakuResp.Usage.InputTokensDetails.CachedReadTokens > 0 {
				bedrockResp.Usage.CacheReadInputTokens = koutakuResp.Usage.InputTokensDetails.CachedReadTokens
				bedrockResp.Usage.InputTokens = bedrockResp.Usage.InputTokens - koutakuResp.Usage.InputTokensDetails.CachedReadTokens
			}
			if koutakuResp.Usage.InputTokensDetails.CachedWriteTokens > 0 {
				bedrockResp.Usage.CacheWriteInputTokens = koutakuResp.Usage.InputTokensDetails.CachedWriteTokens
				bedrockResp.Usage.InputTokens = bedrockResp.Usage.InputTokens - koutakuResp.Usage.InputTokensDetails.CachedWriteTokens
			}
		}
	}

	// Set metrics
	if koutakuResp.ExtraFields.Latency > 0 {
		bedrockResp.Metrics.LatencyMs = koutakuResp.ExtraFields.Latency
	}
...
func ensureResponsesToolConfigForConversation(koutakuReq *schemas.KoutakuResponsesRequest, bedrockReq *BedrockConverseRequest) {
	if bedrockReq.ToolConfig != nil {
		return // Already has tool config
	}

	hasToolContent, tools := extractToolsFromResponsesConversationHistory(koutakuReq.Input)
...
// ConvertKoutakuMessagesToBedrockMessages converts an array of Koutaku ResponsesMessage to Bedrock message format
// This is the main conversion method from Koutaku to Bedrock - handles all message types and returns messages + system messages
// Uses a state machine to properly track and manage tool call lifecycles
func ConvertKoutakuMessagesToBedrockMessages(koutakuMessages []schemas.ResponsesMessage) ([]BedrockMessage, []BedrockSystemMessage, error) {
	...
	for i, msg := range koutakuMessages {
		...
		case schemas.ResponsesMessageTypeFunctionCallOutput:
			...
			if i+1 < len(koutakuMessages) {
				nextMsg := koutakuMessages[i+1]
		...
		case schemas.ResponsesMessageTypeMessage:
			...
			if role == schemas.ResponsesInputMessageRoleSystem {
				...
				systemMsgs := convertKoutakuMessageToBedrockSystemMessages(&msg)
				systemMessages = append(systemMessages, systemMsgs...)
			} else {
				// Convert user/assistant text message
				bedrockMsg := convertKoutakuMessageToBedrockMessage(&msg)
				if bedrockMsg != nil {
		...
		case schemas.ResponsesMessageTypeReasoning:
			...
			reasoningBlocks := convertKoutakuReasoningToBedrockReasoning(&msg)
...
// ConvertBedrockMessagesToKoutakuMessages converts an array of Bedrock messages to Koutaku ResponsesMessage format
// This is the main conversion method from Bedrock to Koutaku - handles all message types and content blocks
func ConvertBedrockMessagesToKoutakuMessages(ctx *schemas.KoutakuContext, bedrockMessages []BedrockMessage, systemMessages []BedrockSystemMessage, isOutputMessage bool) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage

	// Convert system messages first
	systemKoutakuMsgs := convertBedrockSystemMessageToKoutakuMessages(systemMessages)
	if len(systemKoutakuMsgs) > 0 {
		koutakuMessages = append(koutakuMessages, systemKoutakuMsgs...)
	}

	// Convert regular messages
	for _, msg := range bedrockMessages {
		convertedMessages := convertSingleBedrockMessageToKoutakuMessages(ctx, &msg, isOutputMessage)
...
// convertKoutakuMessageToBedrockSystemMessages converts a Koutaku system message to Bedrock system messages
func convertKoutakuMessageToBedrockSystemMessages(msg *schemas.ResponsesMessage) []BedrockSystemMessage {
	var systemMessages []BedrockSystemMessage
...
// convertKoutakuMessageToBedrockMessage converts a regular Koutaku message to Bedrock message
func convertKoutakuMessageToBedrockMessage(msg *schemas.ResponsesMessage) *BedrockMessage {
	...
	// Convert content
	contentBlocks, err := convertKoutakuResponsesMessageContentBlocksToBedrockContentBlocks(*msg.Content)
	if err != nil {
		return nil
	}
...
// convertBedrockSystemMessageToKoutakuMessages converts a Bedrock system message to Koutaku messages
func convertBedrockSystemMessageToKoutakuMessages(systemMessages []BedrockSystemMessage) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage
	...
	for _, sysMsg := range systemMessages {
		if sysMsg.CachePoint != nil {
			...
			if len(koutakuMessages) > 0 {
				lastMessage := &koutakuMessages[len(koutakuMessages)-1]
...
// Helper to convert Bedrock role to Koutaku role
func convertBedrockRoleToKoutakuRole(bedrockRole BedrockMessageRole) schemas.ResponsesMessageRoleType {
	switch bedrockRole {
...
func createTextMessage(
	...
	if isOutputMessage {
		koutakuMsg.ID = schemas.Ptr("msg_" + fmt.Sprintf("%d", time.Now().UnixNano()))
	}
	return koutakuMsg
}

// convertSingleBedrockMessageToKoutakuMessages converts a single Bedrock message to Koutaku messages
func convertSingleBedrockMessageToKoutakuMessages(ctx *schemas.KoutakuContext, msg *BedrockMessage, isOutputMessage bool) []schemas.ResponsesMessage {
	...
	for _, block := range msg.Content {
		if block.Text != nil {
			// Text content
			role := convertBedrockRoleToKoutakuRole(msg.Role)
			...
			koutakuMsg := createTextMessage(block.Text, role, textBlockType, isOutputMessage)
			if isOutputMessage {
				koutakuMsg.ID = schemas.Ptr("msg_" + fmt.Sprintf("%d", time.Now().UnixNano()))
			}
			outputMessages = append(outputMessages, koutakuMsg)
			...
			if structuredOutputToolName != "" && toolUseName == structuredOutputToolName {
				...
				koutakuMsg := createTextMessage(&contentStr, role, schemas.ResponsesOutputMessageContentTypeText, isOutputMessage)
				if isOutputMessage {
					koutakuMsg.ID = schemas.Ptr("msg_" + fmt.Sprintf("%d", time.Now().UnixNano()))
				}
				outputMessages = append(outputMessages, koutakuMsg)
...
// convertKoutakuReasoningToBedrockReasoning converts a Koutaku reasoning message to Bedrock reasoning blocks
func convertKoutakuReasoningToBedrockReasoning(msg *schemas.ResponsesMessage) []BedrockContentBlock {
...
// convertKoutakuResponsesMessageContentBlocksToBedrockContentBlocks converts Koutaku content to Bedrock content blocks
func convertKoutakuResponsesMessageContentBlocksToBedrockContentBlocks(content schemas.ResponsesMessageContent) ([]BedrockContentBlock, error) {
