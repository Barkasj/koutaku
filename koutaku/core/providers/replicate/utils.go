...
func checkForErrorStatus(prediction *ReplicatePredictionResponse) *schemas.KoutakuError {
...
func listenToReplicateStreamURL(
	...
	streamURL string,
	key schemas.Key,
) (io.Reader, *fasthttp.Response, *schemas.KoutakuError) {
	// Create request
	...
	if err != nil {
		...
		if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
			return nil, nil, providerUtils.NewKoutakuTimeoutError(schemas.ErrProviderRequestTimedOut, err)
		}
		return nil, nil, providerUtils.NewKoutakuOperationError(schemas.ErrProviderDoRequest, err)
	}
...
func buildPredictionURL(ctx *schemas.KoutakuContext, baseURL, model string, customProviderConfig *schemas.CustomProviderConfig, requestType schemas.RequestType, useDeploymentsEndpoint bool) string {
