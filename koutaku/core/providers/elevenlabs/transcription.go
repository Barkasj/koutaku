...
func ToElevenlabsTranscriptionRequest(koutakuReq *schemas.KoutakuTranscriptionRequest) *ElevenlabsTranscriptionRequest {
	if koutakuReq == nil {
		return nil
	}
	...
	if koutakuReq.Input != nil && len(koutakuReq.Input.File) > 0 {
		req.File = koutakuReq.Input.File
		req.Filename = koutakuReq.Input.Filename
	}
...
func ToKoutakuTranscriptionResponse(chunks []ElevenlabsSpeechToTextChunkResponse) *schemas.KoutakuTranscriptionResponse {
	if len(chunks) == 0 {
		return nil
	}
