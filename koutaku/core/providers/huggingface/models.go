...
func (response *HuggingFaceListModelsResponse) ToKoutakuListModelsResponse(providerKey schemas.ModelProvider, inferenceProvider inferenceProvider, allowedModels schemas.WhiteList, blacklistedModels schemas.BlackList, aliases map[string]string, unfiltered bool) *schemas.KoutakuListModelsResponse {
	if response == nil {
		return nil
	}
	...
	for _, model := range response.Models {
		...
		for _, result := range pipeline.FilterModel(model.ModelID) {
			...
			koutakuResponse.Data = append(koutakuResponse.Data, newModel)
	...
	for _, m := range pipeline.BackfillModels(included) {
		...
		koutakuResponse.Data = append(koutakuResponse.Data, m)
	}
