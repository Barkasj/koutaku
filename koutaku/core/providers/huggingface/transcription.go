...
func ToHuggingFaceTranscriptionRequest(request *schemas.KoutakuTranscriptionRequest) (*HuggingFaceTranscriptionRequest, error) {
	if request == nil {
		return nil, nil
	}
...
func (response *HuggingFaceTranscriptionResponse) ToKoutakuTranscriptionResponse(requestedModel string) (*schemas.KoutakuTranscriptionResponse, error) {
	if response == nil {
		return nil, nil
	}
	...
	// Create the base Koutaku response
	koutakuResponse := &schemas.KoutakuTranscriptionResponse{
		Text: response.Text,
	}
	...
	if len(response.Chunks) > 0 {
		...
		koutakuResponse.Segments = segments
	}
