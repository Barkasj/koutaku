...
// ToGeminiVideoGenerationRequest converts a Koutaku video generation request to Gemini REST API format
// This creates the request body for POST /models/{model}:predictLongRunning
func ToGeminiVideoGenerationRequest(koutakuReq *schemas.KoutakuVideoGenerationRequest) (*GeminiVideoGenerationRequest, error) {
	...
	if koutakuReq.Input.InputReference != nil && *koutakuReq.Input.InputReference != "" {
	...
	if koutakuReq.Params != nil && koutakuReq.Params.VideoURI != nil {
		instance.Video = &VideoGenerationVideoInput{
			URI: koutakuReq.Params.VideoURI,
		}
	}
	...
	if koutakuReq.Params != nil {
		...
		if koutakuReq.Params.NegativePrompt != nil {
			params.NegativePrompt = koutakuReq.Params.NegativePrompt
		}
		...
		if koutakuReq.Params.Seed != nil {
			params.Seed = koutakuReq.Params.Seed
		}

		if koutakuReq.Params.Audio != nil {
			params.GenerateAudio = koutakuReq.Params.Audio
		}

		if koutakuReq.Params.ExtraParams != nil {
			req.ExtraParams = koutakuReq.Params.ExtraParams
			if aspectRatio, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["aspectRatio"]); ok {
				params.AspectRatio = aspectRatio
			}
			if resolution, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["resolution"]); ok {
				params.Resolution = resolution
			}

			if sampleCount, ok := schemas.SafeExtractIntPointer(koutakuReq.Params.ExtraParams["sampleCount"]); ok {
				params.SampleCount = sampleCount
			}

			if personGeneration, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["personGeneration"]); ok {
				params.PersonGeneration = personGeneration
			}

			if numberOfVideos, ok := schemas.SafeExtractIntPointer(koutakuReq.Params.ExtraParams["numberOfVideos"]); ok {
				params.NumberOfVideos = numberOfVideos
			}
			if storageURI, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["storageURI"]); ok {
				params.StorageURI = storageURI
			}
			if compressionQuality, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["compressionQuality"]); ok {
			...
			if resizeMode, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["resizeMode"]); ok {
				params.ResizeMode = resizeMode
			}
			if referenceImages, ok := koutakuReq.Params.ExtraParams["referenceImages"]; ok {
			...
			if lastFrame, ok := koutakuReq.Params.ExtraParams["lastFrame"]; ok {
		...
		if params.AspectRatio == nil && koutakuReq.Params.Size != "" {
			aspectRatio := sizeToAspectRatio(koutakuReq.Params.Size)
			if aspectRatio != "" {
				params.AspectRatio = &aspectRatio
			}
		}
...
// ToKoutakuVideoGenerationResponse converts Gemini operation response to Koutaku format
func ToKoutakuVideoGenerationResponse(operation *GenerateVideosOperation, model string) (*schemas.KoutakuVideoGenerationResponse, *schemas.KoutakuError) {
	if operation == nil {
		return nil, providerUtils.NewKoutakuOperationError("operation is nil",  nil)
	}
...
func (request *GeminiVideoGenerationRequest) ToKoutakuVideoGenerationRequest(ctx *schemas.KoutakuContext) (*schemas.KoutakuVideoGenerationRequest, error) {
	...
	if instance.Image != nil && instance.Image.BytesBase64Encoded != nil && *instance.Image.BytesBase64Encoded != "" {
		...
		koutakuReq.Input.InputReference = schemas.Ptr(fmt.Sprintf("data:%s;base64,%s", mimeType, *instance.Image.BytesBase64Encoded))
	}

	// Helper to ensure params are initialized
	ensureParams := func() {
		if koutakuReq.Params == nil {
			koutakuReq.Params = &schemas.VideoGenerationParameters{
	...
	if len(instance.ReferenceImages) > 0 {
		ensureParams()
		koutakuReq.Params.ExtraParams["referenceImages"] = instance.ReferenceImages
	}

	// Handle video URI
	if instance.Video != nil && instance.Video.URI != nil {
		ensureParams()
		koutakuReq.Params.VideoURI = instance.Video.URI
	}

	// Handle last frame
	if instance.LastFrame != nil {
		ensureParams()
		koutakuReq.Params.ExtraParams["lastFrame"] = instance.LastFrame
	}
...
func ToGeminiVideoGenerationResponse(response *schemas.KoutakuVideoGenerationResponse) *GenerateVideosOperation {
	if response == nil {
		return nil
	}
