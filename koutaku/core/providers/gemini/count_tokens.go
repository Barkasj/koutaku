...
// ToKoutakuCountTokensResponse converts a Gemini count tokens response to Koutaku format.
func (resp *GeminiCountTokensResponse) ToKoutakuCountTokensResponse(model string) *schemas.KoutakuCountTokensResponse {
	if resp == nil {
		return nil
	}
...
// ToGeminiCountTokensResponse converts a Koutaku count tokens response to Gemini format.
func ToGeminiCountTokensResponse(koutakuResp *schemas.KoutakuCountTokensResponse) *GeminiCountTokensResponse {
	if koutakuResp == nil {
		return nil
	}

	response := &GeminiCountTokensResponse{
		TotalTokens: int32(koutakuResp.InputTokens),
	}

	// Map cached content token count if available
	if koutakuResp.InputTokensDetails != nil && koutakuResp.InputTokensDetails.CachedReadTokens > 0 {
		response.CachedContentTokenCount = int32(koutakuResp.InputTokensDetails.CachedReadTokens)
	} else {
