...
// ToPerplexityChatCompletionRequest converts a Koutaku request to Perplexity chat completion request
func ToPerplexityChatCompletionRequest(koutakuReq *schemas.KoutakuChatRequest) *PerplexityChatRequest {
	...
	if koutakuReq.Params != nil {
		// Core parameters
		perplexityReq.MaxTokens = koutakuReq.Params.MaxCompletionTokens
		perplexityReq.Temperature = koutakuReq.Params.Temperature
		perplexityReq.TopP = koutakuReq.Params.TopP
		perplexityReq.PresencePenalty = koutakuReq.Params.PresencePenalty
		perplexityReq.FrequencyPenalty = koutakuReq.Params.FrequencyPenalty
		perplexityReq.ResponseFormat = koutakuReq.Params.ResponseFormat

		// Tool calling parameters
		perplexityReq.Tools = koutakuReq.Params.Tools
		perplexityReq.ToolChoice = koutakuReq.Params.ToolChoice
		perplexityReq.ParallelToolCalls = koutakuReq.Params.ParallelToolCalls

		// Standard parameters
		perplexityReq.Stop = koutakuReq.Params.Stop
		perplexityReq.LogProbs = koutakuReq.Params.LogProbs
		perplexityReq.TopLogProbs = koutakuReq.Params.TopLogProbs
		...
		if koutakuReq.Params.ExtraParams != nil {
			perplexityReq.ExtraParams = koutakuReq.Params.ExtraParams
...
// ToKoutakuChatResponse converts a Perplexity chat completion response to Koutaku format
func (response *PerplexityChatResponse) ToKoutakuChatResponse(model string) *schemas.KoutakuChatResponse {
	if response == nil {
		return nil
	}
	...
	if len(response.Choices) > 0 {
		koutakuResponse.Choices = response.Choices
	}
