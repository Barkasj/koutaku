...
func TestListModels_QueriesAllBackends(t *testing.T) {
	...
	resp, koutakuErr := provider.ListModels(ctx, keys, request)
	if koutakuErr != nil {
		t.Fatalf("ListModels returned error: %v", koutakuErr.Error)
	}
	...
	// Model IDs are prefixed with "vllm/" by ToKoutakuListModelsResponse.
...
func TestListModels_SingleBackendFailure(t *testing.T) {
	...
	resp, koutakuErr := provider.ListModels(ctx, keys, request)
	if koutakuErr != nil {
		t.Fatalf("ListModels should not return a top-level error when one backend succeeds, got: %v", koutakuErr.Error)
	}
...
func TestListModels_ErrorsWithoutPerKeyURL(t *testing.T) {
	...
	_, koutakuErr := provider.ListModels(ctx, keys, request)
	if koutakuErr == nil {
