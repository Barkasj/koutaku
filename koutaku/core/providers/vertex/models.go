...
// ToKoutakuListModelsResponse converts a Vertex AI list models response to Koutaku's format.
...
func (response *VertexListModelsResponse) ToKoutakuListModelsResponse(allowedModels schemas.WhiteList, blacklistedModels schemas.BlackList, aliases map[string]string, unfiltered bool) *schemas.KoutakuListModelsResponse {
	if response == nil {
		return nil
	}
	...
	for _, model := range response.Models {
		...
		for _, deployedModel := range model.DeployedModels {
			...
			for _, result := range pipeline.FilterModel(customModelID) {
				...
				koutakuResponse.Data = append(koutakuResponse.Data, modelEntry)
	...
	koutakuResponse.NextPageToken = response.NextPageToken

	return koutakuResponse
}

// ToKoutakuListModelsResponse converts a Vertex AI publisher models response to Koutaku's format.
// This is for foundation models from the Model Garden (publishers.models.list endpoint).
func (response *VertexListPublisherModelsResponse) ToKoutakuListModelsResponse(allowedModels schemas.WhiteList, blacklistedModels schemas.BlackList, aliases map[string]string, unfiltered bool) *schemas.KoutakuListModelsResponse {
	if response == nil {
		return nil
	}
