...
func TestApplyLargePayloadRequestBodyWithModelNormalization(t *testing.T) {
	...
	ctx.SetValue(schemas.KoutakuContextKeyLargePayloadMode, true)
	ctx.SetValue(
		schemas.KoutakuContextKeyLargePayloadReader,
		strings.NewReader(payload),
	)
	ctx.SetValue(schemas.KoutakuContextKeyLargePayloadContentLength, len(payload))
	ctx.SetValue(schemas.KoutakuContextKeyLargePayloadContentType, "application/json")
	ctx.SetValue(schemas.KoutakuContextKeyLargePayloadMetadata, &schemas.LargePayloadMetadata{
		Model: "openai/gpt-5",
	})
...
// always includes the raw response body in KoutakuError.ExtraFields.RawResponse
func TestHandleProviderAPIError_RawResponseIncluded(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuErr := HandleProviderAPIError(resp, &errorResp)

			if koutakuErr == nil {
				t.Fatal("HandleProviderAPIError() returned nil")
			}

			if koutakuErr.ExtraFields.RawResponse == nil {
			...
			if tt.name != "Decode failure" {
				rawResponseBytes, err := sonic.Marshal(koutakuErr.ExtraFields.RawResponse)
				if err != nil {
...
func TestEnrichError_PreservesExistingRawResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, koutakuErr, requestBody, nil, true, true)
...
func TestEnrichError_OverwritesWithProvidedResponse(t *testing.T) {
	...
	enrichedErr := EnrichError(ctx, koutakuErr, requestBody, responseBody, true, true)
...
func TestEnrichError_RespectsFlags(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			enrichedErr := EnrichError(ctx, koutakuErr, requestBody, responseBody, tt.sendBackRawRequest, tt.sendBackRawResponse)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextPath != nil {
				ctx = context.WithValue(ctx, schemas.KoutakuContextKeyURLPath, *tt.contextPath)
			}
...
func TestParseAndSetRawRequest_CompactsJSON(t *testing.T) {
  ...
  "temperature": 0.7
	...
	var extraFields schemas.KoutakuResponseExtraFields
...
func TestParseAndSetRawRequest_SSEStreamingChunks(t *testing.T) {
  ...
  "temperature": 0.7
	...
	// Uses KoutakuChatResponse which is the actual type marshaled in the streaming path.
	chunk := schemas.KoutakuChatResponse{
		ID:     "chatcmpl-test",
		Model:  "gpt-4",
	...
	var parsed schemas.KoutakuChatResponse
...
// BuildClientStreamChunk correctly handles KoutakuImageGenerationStreamResponse:
...
func TestBuildClientStreamChunk_ImageGenerationStripping(t *testing.T) {
	...
	imgResp := &schemas.KoutakuImageGenerationStreamResponse{
	...
	response := &schemas.KoutakuResponse{ImageGenerationStreamResponse: imgResp}

	t.Run("logging-only: raw fields stripped from image gen chunk, original preserved", func(t *testing.T) {
		ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
		ctx.SetValue(schemas.KoutakuContextKeyDropRawRequestFromClient, true)
		ctx.SetValue(schemas.KoutakuContextKeyDropRawResponseFromClient, true)

		chunk := BuildClientStreamChunk(ctx, response, nil)
		if chunk.KoutakuImageGenerationStreamResponse == nil {
			t.Fatal("expected KoutakuImageGenerationStreamResponse in chunk")
		}
		if chunk.KoutakuImageGenerationStreamResponse.ExtraFields.RawRequest != nil {
			t.Error("expected RawRequest stripped from chunk, but it was present")
		}
		if chunk.KoutakuImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			t.Error("expected RawResponse stripped from chunk, but it was present")
		}
		// Original must not be mutated.
		if imgResp.ExtraFields.RawRequest == nil {
			t.Error("original KoutakuImageGenerationStreamResponse.ExtraFields.RawRequest was mutated")
		}
		if imgResp.ExtraFields.RawResponse == nil {
			t.Error("original KoutakuImageGenerationStreamResponse.ExtraFields.RawResponse was mutated")
		}
		if chunk.KoutakuImageGenerationStreamResponse == imgResp {
...
// It also verifies that the original KoutakuResponse is not mutated
...
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromResponseChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			passThrough := func(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
				return resp, err
			}

			responseChan := make(chan *schemas.KoutakuStreamChunk, 1)
			ProcessAndSendResponse(ctx, passThrough, response, responseChan, nil)

			chunk := <-responseChan
			if chunk.KoutakuChatResponse == nil {
				t.Fatal("expected non-nil KoutakuChatResponse in stream chunk")
			}

			hasRawReq := chunk.KoutakuChatResponse.ExtraFields.RawRequest != nil
			hasRawResp := chunk.KoutakuChatResponse.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				if response.ChatResponse.ExtraFields.RawRequest == nil {
					t.Error("original KoutakuResponse.ChatResponse.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if response.ChatResponse.ExtraFields.RawResponse == nil {
					t.Error("original KoutakuResponse.ChatResponse.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must be a copy, not the same pointer as the original.
				if chunk.KoutakuChatResponse == response.ChatResponse {
					t.Error("chunk.KoutakuChatResponse is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
// that when drop-raw context flags are set, raw data is stripped from KoutakuError
// payloads embedded in stream chunks, without mutating the shared KoutakuError object
// (shared object safety for PostLLMHook goroutines).
func TestProcessAndSendResponse_StoreRawLoggingOnly_StripsRawDataFromErrorChunk(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Use a postHookRunner that converts the response to a KoutakuError with raw data
			koutakuErr := &schemas.KoutakuError{
				IsKoutakuError: false,
			...
			errorRunner := func(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
				return nil, koutakuErr
			}

			responseChan := make(chan *schemas.KoutakuStreamChunk, 1)
			ProcessAndSendResponse(ctx, errorRunner, &schemas.KoutakuResponse{
				ChatResponse: &schemas.KoutakuChatResponse{ID: "chatcmpl-001"},
			}, responseChan, nil)

			chunk := <-responseChan
			if chunk.KoutakuError == nil {
				t.Fatal("expected non-nil KoutakuError in stream chunk")
			}

			hasRawReq := chunk.KoutakuError.ExtraFields.RawRequest != nil
			hasRawResp := chunk.KoutakuError.ExtraFields.RawResponse != nil
			...
			if tt.expectStripped {
				...
				// Critical: the original shared KoutakuError must NOT have been mutated.
				if koutakuErr.ExtraFields.RawRequest == nil {
					t.Error("original KoutakuError.ExtraFields.RawRequest was mutated (nil); shared object must be preserved")
				}
				if koutakuErr.ExtraFields.RawResponse == nil {
					t.Error("original KoutakuError.ExtraFields.RawResponse was mutated (nil); shared object must be preserved")
				}
				// The chunk must hold a copy, not the same pointer as the original.
				if chunk.KoutakuError == koutakuErr {
					t.Error("chunk.KoutakuError is the same pointer as the original; it must be a copy to avoid data races")
				}
			} else {
				if !hasRawReq {
...
//   - KoutakuContextKeyCaptureRawRequest=true in context: always returns true
//   - Logging-only mode: requestWorker sets KoutakuContextKeyCaptureRawRequest=true,
...
func TestShouldSendBackRawRequest(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets KoutakuContextKeyCaptureRawRequest=true in logging-only
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.contextSendBack {
				ctx.SetValue(schemas.KoutakuContextKeyCaptureRawRequest, true)
			}
...
func TestShouldSendBackRawResponse(t *testing.T) {
	tests := []struct {
		...
		{
			// requestWorker sets KoutakuContextKeyCaptureRawResponse=true in logging-only
			// mode so a single flag covers both full send-back and logging-only cases.
			name:            "logging-only: context SendBack=true set by requestWorker",
			contextSendBack: true,
			providerDefault: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
			if tt.contextSendBack {
				ctx.SetValue(schemas.KoutakuContextKeyCaptureRawResponse, true)
			}
