...
import (
	...
	"github.com/koutaku/koutaku/core/network"
...
func MakeRequestWithContext(ctx context.Context, client *fasthttp.Client, req *fasthttp.Request, resp *fasthttp.Response) (time.Duration, *schemas.KoutakuError, func()) {
	startTime := time.Now()
	...
	case <-ctx.Done():
		...
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			...
			return latency, &schemas.KoutakuError{
				IsKoutakuError: true,
	...
	case err := <-errChan:
		...
		if err != nil {
			...
			if errors.Is(err, fasthttp.ErrTimeout) || errors.Is(err, context.DeadlineExceeded) {
				return latency, NewKoutakuTimeoutError(schemas.ErrProviderRequestTimedOut, err), noop
			}
...
func SetExtraHeaders(ctx context.Context, req *fasthttp.Request, extraHeaders map[string]string, skipHeaders []string) {
	...
	if extraHeaders, ok := (ctx).Value(schemas.KoutakuContextKeyExtraHeaders).(map[string][]string); ok {
...
func GetPathFromContext(ctx context.Context, defaultPath string) string {
	if pathInContext, ok := ctx.Value(schemas.KoutakuContextKeyURLPath).(string); ok {
...
func CheckAndGetRawRequestBody(ctx context.Context, request RequestBodyGetter) ([]byte, bool) {
	if rawBody, ok := ctx.Value(schemas.KoutakuContextKeyUseRawRequestBody).(bool); ok && rawBody {
...
func IsLargePayloadPassthroughEnabled(ctx context.Context) bool {
	...
	isLargePayload, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadMode).(bool)
	if !ok || !isLargePayload {
		return false
	}
	reader, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadReader).(io.Reader)
...
func ApplyLargePayloadRequestBodyWithModelNormalization(
	...
	bodyReader, _ := ctx.Value(schemas.KoutakuContextKeyLargePayloadReader).(io.Reader)
	bodySize := -1
	if contentLength, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadContentLength).(int); ok {
		bodySize = contentLength
	}

	if contentType, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadContentType).(string); ok && contentType != "" {
		ctLower := strings.ToLower(contentType)
		if metadata, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadMetadata).(*schemas.LargePayloadMetadata); ok && metadata != nil {
...
func CheckContextAndGetRequestBody(ctx context.Context, request RequestBodyGetter, requestConverter RequestBodyConverter) ([]byte, *schemas.KoutakuError) {
	if IsLargePayloadPassthroughEnabled(ctx) {
		return nil, nil
	}
	...
	if !ok {
		...
		if err != nil {
			return nil, NewKoutakuOperationError(schemas.ErrRequestBodyConversion, err)
		}
		if convertedBody == nil {
			return nil, NewKoutakuOperationError("request body is not provided", nil)
		}

		jsonBody, err := MarshalSortedIndent(convertedBody, "", "  ")
		if err != nil {
			return nil, NewKoutakuOperationError(schemas.ErrProviderRequestMarshal, err)
		}
		// Merge ExtraParams into the JSON if passthrough is enabled
		if ctx.Value(schemas.KoutakuContextKeyPassthroughExtraParams) != nil && ctx.Value(schemas.KoutakuContextKeyPassthroughExtraParams) == true {
...
// It attempts to unmarshal the error response and returns a KoutakuError
...
func HandleProviderAPIError(resp *fasthttp.Response, errorResp any) *schemas.KoutakuError {
...
// EnrichError attaches the raw request and response to a KoutakuError.
// Returns the request and response from provider embedded in KoutakuError.ExtraFields.
func EnrichError(
	...
	if ShouldSendBackRawRequest(ctx, sendBackRawRequest) && len(requestBody) > 0 {
		...
		koutakuErr.ExtraFields.RawRequest = compactRawJSON(requestBody)
	} else {
		koutakuErr.ExtraFields.RawRequest = nil
	}

	if ShouldSendBackRawResponse(ctx, sendBackRawResponse) {
		if len(responseBody) > 0 {
			koutakuErr.ExtraFields.RawResponse = compactRawJSON(responseBody)
		}
	} else {
		koutakuErr.ExtraFields.RawResponse = nil
	}
...
// and returns either the parsed response or a KoutakuError if parsing fails.
...
func HandleProviderResponse[T any](responseBody []byte, response *T, requestBody []byte, sendBackRawRequest bool, sendBackRawResponse bool) (rawRequest interface{}, rawResponse interface{}, koutakuErr *schemas.KoutakuError) {
	// Check for empty response
	trimmed := strings.TrimSpace(string(responseBody))
	if len(trimmed) == 0 {
		return nil, nil, &schemas.KoutakuError{
			IsKoutakuError: true,
			Error: &schemas.ErrorField{
...
func ParseAndSetRawRequest(extraFields *schemas.KoutakuResponseExtraFields, jsonBody []byte) {
	if len(jsonBody) > 0 {
...
func ParseAndSetRawRequestIfJSON(fasthttpReq *fasthttp.Request, extraFields *schemas.KoutakuResponseExtraFields) {
...
func NewUnsupportedOperationError(requestType schemas.RequestType, providerName schemas.ModelProvider) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: false,
		Error: &schemas.ErrorField{
...
func CheckOperationAllowed(defaultProvider schemas.ModelProvider, config *schemas.CustomProviderConfig, operation schemas.RequestType) *schemas.KoutakuError {
...
func NewConfigurationError(message string) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: false,
		Error: &schemas.ErrorField{
			Message: message,
...
// NewKoutakuOperationError creates a standardized error for koutaku operation errors.
// This helper reduces code duplication across providers that have koutaku operation errors.
func NewKoutakuOperationError(message string, err error) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: true,
		Error: &schemas.ErrorField{
			Message: message,
...
// NewKoutakuTimeoutError creates a standardized error for provider request timeout errors.
...
func NewKoutakuTimeoutError(message string, err error) *schemas.KoutakuError {
	statusCode := 504
	errorType := schemas.RequestTimedOut
	return &schemas.KoutakuError{
		IsKoutakuError: true,
...
func NewProviderAPIError(message string, err error, statusCode int, errorType *string, eventID *string) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: false,
		StatusCode:     &statusCode,
		Type:           errorType,
...
// koutaku.go always writes KoutakuContextKeyCaptureRawRequest before provider dispatch,
// combining provider config, per-request overrides, and store_raw_request_response.
// The default parameter is a fallback for callers outside the normal koutaku dispatch path.
func ShouldSendBackRawRequest(ctx context.Context, defaultSendBackRawRequest bool) bool {
	if capture, ok := ctx.Value(schemas.KoutakuContextKeyCaptureRawRequest).(bool); ok {
		return capture
	}
...
// koutaku.go always writes KoutakuContextKeyCaptureRawResponse before provider dispatch,
// combining provider config, per-request overrides, and store_raw_request_response.
// The default parameter is a fallback for callers outside the normal koutaku dispatch path.
func ShouldSendBackRawResponse(ctx context.Context, defaultSendBackRawResponse bool) bool {
	if capture, ok := ctx.Value(schemas.KoutakuContextKeyCaptureRawResponse).(bool); ok {
		return capture
	}
...
func SendCreatedEventResponsesChunk(ctx *schemas.KoutakuContext, postHookRunner schemas.PostHookRunner, startTime time.Time, responseChan chan *schemas.KoutakuStreamChunk, postHookSpanFinalizer func(context.Context)) {
	firstChunk := &schemas.KoutakuResponsesStreamResponse{
	...
	// TODO add koutaku response pooling here
	koutakuResponse := &schemas.KoutakuResponse{
		ResponsesStreamResponse: firstChunk,
	}
	ProcessAndSendResponse(ctx, postHookRunner, koutakuResponse, responseChan, postHookSpanFinalizer)
}

// SendInProgressEventResponsesChunk sends a ResponsesStreamResponseTypeInProgress event
func SendInProgressEventResponsesChunk(ctx *schemas.KoutakuContext, postHookRunner schemas.PostHookRunner, startTime time.Time, responseChan chan *schemas.KoutakuStreamChunk, postHookSpanFinalizer func(context.Context)) {
	chunk := &schemas.KoutakuResponsesStreamResponse{
...
// BuildClientStreamChunk constructs a KoutakuStreamChunk from post-hook results.
...
func BuildClientStreamChunk(ctx context.Context, processedResponse *schemas.KoutakuResponse, processedError *schemas.KoutakuError) *schemas.KoutakuStreamChunk {
	...
	streamResponse := &schemas.KoutakuStreamChunk{}
	if processedResponse != nil {
		streamResponse.KoutakuTextCompletionResponse = processedResponse.TextCompletionResponse
		streamResponse.KoutakuChatResponse = processedResponse.ChatResponse
		streamResponse.KoutakuResponsesStreamResponse = processedResponse.ResponsesStreamResponse
		streamResponse.KoutakuSpeechStreamResponse = processedResponse.SpeechStreamResponse
		streamResponse.KoutakuTranscriptionStreamResponse = processedResponse.TranscriptionStreamResponse
		streamResponse.KoutakuImageGenerationStreamResponse = processedResponse.ImageGenerationStreamResponse
		...
		if drop {
			if streamResponse.KoutakuTextCompletionResponse != nil {
				cp := *streamResponse.KoutakuTextCompletionResponse
				if dropReq {
				...
				streamResponse.KoutakuTextCompletionResponse = &cp
			}
			if streamResponse.KoutakuChatResponse != nil {
				cp := *streamResponse.KoutakuChatResponse
				if dropReq {
					cp.ExtraFields.RawRequest = nil
				}
				if dropResp {
				...
				streamResponse.KoutakuChatResponse = &cp
			}
			if streamResponse.KoutakuResponsesStreamResponse != nil {
				cp := *streamResponse.KoutakuResponsesStreamResponse
				if dropReq {
					cp.ExtraFields.RawRequest = nil
				}
				if dropResp {
				...
				streamResponse.KoutakuResponsesStreamResponse = &cp
			}
			if streamResponse.KoutakuSpeechStreamResponse != nil {
				cp := *streamResponse.KoutakuSpeechStreamResponse
				if dropReq {
					cp.ExtraFields.RawRequest = nil
				}
				if dropResp {
				...
				streamResponse.KoutakuSpeechStreamResponse = &cp
			}
			if streamResponse.KoutakuTranscriptionStreamResponse != nil {
				cp := *streamResponse.KoutakuTranscriptionStreamResponse
				if dropReq {
					cp.ExtraFields.RawRequest = nil
				}
				if dropResp {
				...
				streamResponse.KoutakuTranscriptionStreamResponse = &cp
			}
			if streamResponse.KoutakuImageGenerationStreamResponse != nil {
				cp := *streamResponse.KoutakuImageGenerationStreamResponse
				if dropReq {
					cp.ExtraFields.RawRequest = nil
				}
				if dropResp {
				...
				streamResponse.KoutakuImageGenerationStreamResponse = &cp
			}
		}
	}
	if processedError != nil {
		if drop {
			...
			streamResponse.KoutakuError = &errCopy
		} else {
			streamResponse.KoutakuError = processedError
		}
	}
...
func ProcessAndSendResponse(
	...
	if tracer, ok := ctx.Value(schemas.KoutakuContextKeyTracer).(schemas.Tracer); ok && tracer != nil {
	...
	if HandleStreamControlSkip(processedError) {
		...
		if isFinalChunk := ctx.Value(schemas.KoutakuContextKeyStreamEndIndicator); isFinalChunk != nil {
...
// ProcessAndSendKoutakuError handles post-hook processing and sends the koutaku error to the channel.
// This utility reduces code duplication across streaming implementations by encapsulating
// the common pattern of running post hooks, handling errors, and sending responses with
// proper context cancellation handling.
// It also completes the deferred LLM span when the final chunk is sent (StreamEndIndicator is true).
func ProcessAndSendKoutakuError(
	ctx *schemas.KoutakuContext,
	postHookRunner schemas.PostHookRunner,
	koutakuErr *schemas.KoutakuError,
	responseChan chan *schemas.KoutakuStreamChunk,
	...
	processedResponse, processedError := postHookRunner(ctx, nil, koutakuErr)
...
// before koutaku considers the connection stalled and closes it. This protects
...
func SetStreamIdleTimeoutIfEmpty(ctx *schemas.KoutakuContext, configSeconds int) {
	if existing, ok := ctx.Value(schemas.KoutakuContextKeyStreamIdleTimeout).(time.Duration); ok && existing > 0 {
		return // already set from upstream (transport/header), respect it
	}
	if configSeconds > 0 {
		ctx.SetValue(schemas.KoutakuContextKeyStreamIdleTimeout, time.Duration(configSeconds)*time.Second)
	}
}

// GetStreamIdleTimeout reads the per-chunk idle timeout from context,
// falling back to DefaultStreamIdleTimeout if not set.
func GetStreamIdleTimeout(ctx *schemas.KoutakuContext) time.Duration {
	if timeout, ok := ctx.Value(schemas.KoutakuContextKeyStreamIdleTimeout).(time.Duration); ok && timeout > 0 {
		return timeout
	}
...
func HandleStreamCancellation(
	...
	if indicator := ctx.GetAndSetValue(schemas.KoutakuContextKeyStreamEndIndicator, true); indicator != nil {
		if set, ok := indicator.(bool); ok && set {
			return // Already handled
		}
	}
	// Create cancellation error
	cancelErr := &schemas.KoutakuError{
	...
	ProcessAndSendKoutakuError(ctx, postHookRunner, cancelErr, responseChan, logger, postHookSpanFinalizer)
}
...
func HandleStreamTimeout(
	...
	timeoutErr := &schemas.KoutakuError{
	...
	// Send through PostHook chain - this updates the log to "error" status
	ProcessAndSendKoutakuError(ctx, postHookRunner, timeoutErr, responseChan, logger, postHookSpanFinalizer)
}
...
func ProcessAndSendError(
	...
	koutakuError := &schemas.KoutakuError{
		IsKoutakuError: true,
		Error: &schemas.ErrorField{
	...
	processedResponse, processedError := postHookRunner(ctx, nil, koutakuError)
...
// CreateKoutakuTextCompletionChunkResponse creates a koutaku text completion chunk response.
func CreateKoutakuTextCompletionChunkResponse(
	id string,
	usage *schemas.KoutakuLLMUsage,
	finishReason *string,
	currentChunkIndex int,
	requestType schemas.RequestType,
) *schemas.KoutakuTextCompletionResponse {
...
// CreateKoutakuChatCompletionChunkResponse creates a koutaku chat completion chunk response.
func CreateKoutakuChatCompletionChunkResponse(
	id string,
	usage *schemas.KoutakuLLMUsage,
	finishReason *string,
	currentChunkIndex int,
...
func HandleStreamControlSkip(koutakuErr *schemas.KoutakuError) bool {
	if koutakuErr == nil || koutakuErr.StreamControl == nil {
		return false
	}
	if koutakuErr.StreamControl.SkipStream != nil && *koutakuErr.StreamControl.SkipStream {
		if koutakuErr.StreamControl.LogError != nil && *koutakuErr.StreamControl.LogError {
			getLogger().Warn("Error in stream: " + koutakuErr.Error.Message)
		}
		return true
	}
	return false
...
// Note: CustomProviderKey is internally set by Koutaku and should always match the provider name.
...
// GetKoutakuResponseForStreamResponse converts the provided responses to a koutaku response.
func GetKoutakuResponseForStreamResponse(
	textCompletionResponse *schemas.KoutakuTextCompletionResponse,
	chatResponse *schemas.KoutakuChatResponse,
	responsesStreamResponse *schemas.KoutakuResponsesStreamResponse,
	speechStreamResponse *schemas.KoutakuSpeechStreamResponse,
	transcriptionStreamResponse *schemas.KoutakuTranscriptionStreamResponse,
	imageGenerationStreamResponse *schemas.KoutakuImageGenerationStreamResponse,
	...
	case textCompletionResponse != nil:
		koutakuResponse.TextCompletionResponse = textCompletionResponse
		return koutakuResponse
	case chatResponse != nil:
		koutakuResponse.ChatResponse = chatResponse
		return koutakuResponse
	case responsesStreamResponse != nil:
		koutakuResponse.ResponsesStreamResponse = responsesStreamResponse
		return koutakuResponse
	case speechStreamResponse != nil:
		koutakuResponse.SpeechStreamResponse = speechStreamResponse
		return koutakuResponse
	case transcriptionStreamResponse != nil:
		koutakuResponse.TranscriptionStreamResponse = transcriptionStreamResponse
		return koutakuResponse
	case imageGenerationStreamResponse != nil:
		koutakuResponse.ImageGenerationStreamResponse = imageGenerationStreamResponse
		return koutakuResponse
	}
	return nil
}

// aggregateListModelsResponses merges multiple KoutakuListModelsResponse objects into a single response.
...
func aggregateListModelsResponses(responses []*schemas.KoutakuListModelsResponse) *schemas.KoutakuListModelsResponse {
	if len(responses) == 0 {
		return &schemas.KoutakuListModelsResponse{
			Data: []schemas.Model{},
		}
	}
	...
	aggregated := &schemas.KoutakuListModelsResponse{
...
func extractSuccessfulListModelsResponses(results chan schemas.ListModelsByKeyResult, provider schemas.ModelProvider) ([]*schemas.KoutakuListModelsResponse, []schemas.KeyStatus, *schemas.KoutakuError) {
	var successfulResponses []*schemas.KoutakuListModelsResponse
	...
	if len(successfulResponses) == 0 {
		...
		return nil, keyStatuses, &schemas.KoutakuError{
			IsKoutakuError: false,
			Error: &schemas.ErrorField{
...
func HandleKeylessListModelsRequest(
	...
	listFunc func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError),
) (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
	resp, koutakuErr := listFunc()
	...
	if koutakuErr != nil {
		...
		keyStatus.Error = koutakuErr
		koutakuErr.ExtraFields.KeyStatuses = []schemas.KeyStatus{keyStatus}
...
func HandleMultipleListModelsRequests(
	...
	listModelsByKey func(ctx *schemas.KoutakuContext, key schemas.Key, request *schemas.KoutakuListModelsRequest) (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError),
) (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
	startTime := time.Now()
	...
	for _, key := range keys {
		...
		go func(k schemas.Key) {
			...
			resp, koutakuErr := listModelsByKey(ctx, k, request)
			results <- schemas.ListModelsByKeyResult{Response: resp, Err: koutakuErr, KeyID: k.ID}
		}(key)
	}
...
func completeDeferredSpan(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, err *schemas.KoutakuError, postHookSpanFinalizer func(context.Context)) {
	if ctx == nil {
		return
	}

	// Get the trace ID from context (this IS available in the provider's goroutine)
	traceID, ok := ctx.Value(schemas.KoutakuContextKeyTraceID).(string)
	if !ok || traceID == "" {
		return
	}

	// Get the tracer from context
	tracerVal := ctx.Value(schemas.KoutakuContextKeyTracer)
	if tracerVal == nil {
		return
	}
	...
	// This builds a complete KoutakuResponse from all the streaming chunks
	...
	if postHookSpanFinalizer != nil {
		...
		if spanID != "" {
			finalizerCtx := context.WithValue(ctx, schemas.KoutakuContextKeySpanID, spanID)
...
func CheckAndSetDefaultProvider(ctx *schemas.KoutakuContext, defaultProvider schemas.ModelProvider) schemas.ModelProvider {
	if ctx != nil {
		if ctx.Value(schemas.KoutakuContextKeyDirectKey) != nil || ctx.Value(schemas.KoutakuContextKeySkipKeySelection) != nil {
			return defaultProvider
		}
		if ctx.Value(schemas.KoutakuContextKeyAvailableProviders) != nil {
			availableProviders, ok := ctx.Value(schemas.KoutakuContextKeyAvailableProviders).([]schemas.ModelProvider)
