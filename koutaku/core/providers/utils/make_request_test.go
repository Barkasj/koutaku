...
func TestMakeRequestWithContext_SuccessReturnsNoopWait(t *testing.T) {
	...
	latency, koutakuErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()

	if koutakuErr != nil {
		t.Fatalf("expected no error, got: %v", koutakuErr.Error.Message)
	}
	if latency <= 0 {
...
func TestMakeRequestWithContext_DeadlineExceededReturnsTimeoutError(t *testing.T) {
	...
	_, koutakuErr, wait := MakeRequestWithContext(ctx, client, req, resp)

	// Should get a timeout error with 504 status
	if koutakuErr == nil {
		t.Fatal("expected timeout error")
	}
	if koutakuErr.Error.Type == nil || *koutakuErr.Error.Type != schemas.RequestTimedOut {
		t.Fatalf("expected RequestTimedOut error type, got: %v", koutakuErr.Error.Type)
	}
	if koutakuErr.StatusCode == nil || *koutakuErr.StatusCode != 504 {
		t.Fatalf("expected status 504, got: %v", koutakuErr.StatusCode)
	}
...
func TestMakeRequestWithContext_ContextCancelReturnsCancelledError(t *testing.T) {
	...
	if koutakuErr.Error.Type == nil || *koutakuErr.Error.Type != schemas.RequestCancelled {
		t.Fatalf("expected RequestCancelled error type, got: %v", koutakuErr.Error.Type)
	}
	if koutakuErr.StatusCode == nil || *koutakuErr.StatusCode != 499 {
		t.Fatalf("expected status 499, got: %v", koutakuErr.StatusCode)
	}
...
func TestNewKoutakuTimeoutError(t *testing.T) {
	err := NewKoutakuTimeoutError("test timeout", context.DeadlineExceeded)

	if !err.IsKoutakuError {
		t.Fatal("expected IsKoutakuError to be true")
	}
	...
	// Note: ExtraFields.Provider is populated by koutaku.go's dispatcher via
	// PopulateExtraFields, not by NewKoutakuTimeoutError — the constructor has
	// no provider context.
}
...
func TestMakeRequestWithContext_ClientError(t *testing.T) {
	...
	_, koutakuErr, wait := MakeRequestWithContext(context.Background(), client, req, resp)
	defer wait()
