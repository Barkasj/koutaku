...
func TestHandleProviderAPIErrorWithHTML(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if koutakuErr.StatusCode == nil || *koutakuErr.StatusCode != tt.statusCode {
				t.Errorf("HandleProviderAPIError() status code = %v, want %v", koutakuErr.StatusCode, tt.statusCode)
			}
			...
			if tt.expectedInMessage != "" && !strings.Contains(koutakuErr.Error.Message, tt.expectedInMessage) {
				t.Errorf("Expected message to contain %q, got %q", tt.expectedInMessage, koutakuErr.Error.Message)
			}

			t.Logf("Handled %s: status=%d, message=%q", tt.name, *koutakuErr.StatusCode, koutakuErr.Error.Message)
		})
	}
}
