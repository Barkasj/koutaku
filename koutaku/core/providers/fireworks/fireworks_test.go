...
import (
	...
	fireworksprovider "github.com/koutaku/koutaku/core/providers/fireworks"
...
func resolveFireworksModels(t *testing.T, client *koutaku.Koutaku, ctx context.Context) (string, string, string) {
	t.Helper()
	...
	if chatModel == "" || textModel == "" || embeddingModel == "" {
		...
		for page := 0; page < 5; page++ {
			...
			resp, koutakuErr := client.ListModelsRequest(bfCtx, req)
			if koutakuErr != nil {
				if chatModel == "" {
					t.Fatalf("Failed to list Fireworks models for test discovery: %v", llmtests.GetErrorMessage(koutakuErr))
				}
				t.Logf("Fireworks model discovery failed: %v", llmtests.GetErrorMessage(koutakuErr))
				break
			}
...
func fireworksModelSupportsTextCompletions(t *testing.T, client *koutaku.Koutaku, ctx context.Context, model string) bool {
	t.Helper()

	prompt := "Say ok"
	bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
	resp, koutakuErr := client.TextCompletionRequest(bfCtx, &schemas.KoutakuTextCompletionRequest{
		Provider: schemas.Fireworks,
		Model:    model,
	...
	if koutakuErr != nil {
		t.Logf("Fireworks /v1/completions probe failed for %q: %v", model, llmtests.GetErrorMessage(koutakuErr))
		return false
	}
...
func fireworksModelSupportsEmbeddings(t *testing.T, client *koutaku.Koutaku, ctx context.Context, model string) bool {
	t.Helper()

	text := "embedding probe"
	bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
	resp, koutakuErr := client.EmbeddingRequest(bfCtx, &schemas.KoutakuEmbeddingRequest{
		Provider: schemas.Fireworks,
		Model:    model,
	...
	if koutakuErr != nil {
		t.Logf("Fireworks /v1/embeddings probe failed for %q: %v", model, llmtests.GetErrorMessage(koutakuErr))
		return false
	}
...
// normalizeFireworksModelID strips any provider prefix so tests can pass raw Fireworks model IDs to Koutaku requests.
...
func TestFireworksProviderUsesNativeEndpoints(t *testing.T) {
	tests := []struct {
		name         string
		expectedPath string
		run          func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.KoutakuContext, key schemas.Key)
	}{
		{
			name:         "TextCompletion",
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.KoutakuContext, key schemas.Key) {
				...
				resp, err := provider.TextCompletion(ctx, key, &schemas.KoutakuTextCompletionRequest{
					Provider: schemas.Fireworks,
		...
		},
		{
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.KoutakuContext, key schemas.Key) {
				resp, err := provider.Responses(ctx, key, &schemas.KoutakuResponsesRequest{
					Provider: schemas.Fireworks,
					Model:    "accounts/fireworks/models/deepseek-v3p2",
		...
		},
		{
			...
			run: func(t *testing.T, provider *fireworksprovider.FireworksProvider, ctx *schemas.KoutakuContext, key schemas.Key) {
				resp, err := provider.Embedding(ctx, key, &schemas.KoutakuEmbeddingRequest{
					Provider: schemas.Fireworks,
...
func TestFireworksResponsesStreamUsesNativeResponsesEndpoint(t *testing.T) {
	...
	ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	key := schemas.Key{Value: *schemas.NewEnvVar("test-key")}
	postHookRunner := func(_ *schemas.KoutakuContext, result *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
		return result, err
	}

	stream, err := provider.ResponsesStream(ctx, postHookRunner, nil, key, &schemas.KoutakuResponsesRequest{
		Provider: schemas.Fireworks,
		Model:    "accounts/fireworks/models/deepseek-v3p2",
		Input: []schemas.ResponsesMessage{
			llmtests.CreateBasicResponsesMessage("hello"),
	...
	for chunk := range stream {
		if chunk != nil && chunk.KoutakuResponsesStreamResponse != nil &&
			chunk.KoutakuResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCompleted {
			sawCompleted = true
		}
	}
