...
func TestToAnthropicChatRequest_ServerTools(t *testing.T) {
	mk := func(rawTool string) *schemas.KoutakuChatRequest {
		var tool schemas.ChatTool
...
// TestToKoutakuResponsesRequest_MCPToolsetPreservesAnthropicFlags verifies
...
// the flags because convertAnthropicToolToKoutaku skips mcp_toolset entries.
func TestToKoutakuResponsesRequest_MCPToolsetPreservesAnthropicFlags(t *testing.T) {
	...
	got := req.ToKoutakuResponsesRequest(nil)
	if got == nil || got.Params == nil {
		t.Fatalf("ToKoutakuResponsesRequest returned nil params")
	}

	// The mcp_toolset tool should have been dropped by convertAnthropicToolToKoutaku
