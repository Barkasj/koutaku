...
// anthropicToResponsesStreamState holds per-request state for the Koutaku→Anthropic
...
func getOrCreateAnthropicToResponsesStreamState(ctx *schemas.KoutakuContext) *anthropicToResponsesStreamState {
...
// ToKoutakuResponsesStream converts an Anthropic stream event to a Koutaku Responses Stream response
...
func (chunk *AnthropicStreamEvent) ToKoutakuResponsesStream(ctx context.Context, sequenceNumber int, state *AnthropicResponsesStreamState) ([]*schemas.KoutakuResponsesStreamResponse, *schemas.KoutakuError, bool) {
	switch chunk.Type {
	case AnthropicStreamEventTypeMessageStart:
		...
		if chunk.Message != nil {
			...
			var responses []*schemas.KoutakuResponsesStreamResponse
			...
			if !state.HasEmittedCreated {
				...
				if chunk.Message.Usage != nil {
					...
					if chunk.Message.Usage.CacheReadInputTokens > 0 || chunk.Message.Usage.CacheCreationInputTokens > 0 {
						...
						// Koutaku convention: InputTokens includes cached tokens
				...
				responses = append(responses, &schemas.KoutakuResponsesStreamResponse{
		...
		if chunk.ContentBlock != nil && chunk.Index != nil {
			...
			if chunk.ContentBlock.Type == AnthropicContentBlockTypeToolUse &&
				...
				*chunk.ContentBlock.Name == string(AnthropicToolNameComputer) &&
				...
				return []*schemas.KoutakuResponsesStreamResponse{{
			...
			default:
				...
				if ctx.Value(schemas.KoutakuContextKeyIntegrationType) == "anthropic" {
					return []*schemas.KoutakuResponsesStreamResponse{{
						Type:           "",
						SequenceNumber: sequenceNumber,
					}}, nil, false
		...
		if chunk.Index != nil && chunk.Delta != nil {
			...
			case AnthropicStreamDeltaTypeText:
				if chunk.Delta.Text != nil && *chunk.Delta.Text != "" {
					...
					itemID := state.ItemIDs[outputIndex]
					response := &schemas.KoutakuResponsesStreamResponse{
					...
					return []*schemas.KoutakuResponsesStreamResponse{response}, nil, false
				}
		...
		if chunk.Index != nil {
			...
			if chunk.Index != nil {
				if state.TextContentIndices[*chunk.Index] {
					...
					emptyText := ""
					textDoneResponse := &schemas.KoutakuResponsesStreamResponse{
					...
					partDoneResponse := &schemas.KoutakuResponsesStreamResponse{
				...
				if state.ReasoningContentIndices[*chunk.Index] {
					...
					emptyText := ""
					reasoningDoneResponse := &schemas.KoutakuResponsesStreamResponse{
	...
	case AnthropicStreamEventTypeMessageDelta:
		if chunk.Delta.StopReason != nil {
			state.StopReason = schemas.Ptr(ConvertAnthropicFinishReasonToKoutaku(*chunk.Delta.StopReason))
		}
		// Check if integration type in ctx is anthropic
		if ctx.Value(schemas.KoutakuContextKeyIntegrationType) == "anthropic" {
			// Convert usage from Anthropic format to Koutaku
			koutakuUsage := ConvertAnthropicUsageToKoutakuUsage(chunk.Usage)

			// Convert stop reason if present
			var stopReason *string
			if chunk.Delta != nil && chunk.Delta.StopReason != nil {
				converted := ConvertAnthropicFinishReasonToKoutaku(*chunk.Delta.StopReason)
...
// ToAnthropicResponsesStreamResponse converts a Koutaku Responses stream response to Anthropic SSE string format
func ToAnthropicResponsesStreamResponse(ctx *schemas.KoutakuContext, koutakuResp *schemas.KoutakuResponsesStreamResponse) []*AnthropicStreamEvent {
	if koutakuResp == nil {
		return nil
	}
	...
	case schemas.ResponsesStreamResponseTypeCreated:
		...
		if koutakuResp.Response != nil {
			...
			if koutakuResp.Response.Usage != nil {
				messageUsage = ConvertKoutakuUsageToAnthropicUsage(koutakuResp.Response.Usage)
			} else {
			...
			if koutakuResp.Response.ID != nil {
				streamMessage.ID = *koutakuResp.Response.ID
			}
			// Prefer Response.Model, then ResolvedModelUsed, then OriginalModelRequested
			if koutakuResp.Response != nil && koutakuResp.Response.Model != "" {
				streamMessage.Model = koutakuResp.Response.Model
			} else if koutakuResp.ExtraFields.ResolvedModelUsed != "" {
				streamMessage.Model = koutakuResp.ExtraFields.ResolvedModelUsed
			} else if koutakuResp.ExtraFields.OriginalModelRequested != "" {
				streamMessage.Model = koutakuResp.ExtraFields.OriginalModelRequested
			}
	...
	case schemas.ResponsesStreamResponseTypeOutputItemAdded:
		...
		if koutakuResp.Item != nil &&
			koutakuResp.Item.Type != nil &&
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeComputerCall {

			// Computer tool - emit content_block_start
			streamResp.Type = AnthropicStreamEventTypeContentBlockStart

			if koutakuResp.OutputIndex != nil {
				streamResp.Index = koutakuResp.OutputIndex
			} else if koutakuResp.ContentIndex != nil {
				streamResp.Index = koutakuResp.ContentIndex
			}
			...
			contentBlock := &AnthropicContentBlock{
				...
				ID:   koutakuResp.Item.ID,                            // The tool use ID
		...
		} else if koutakuResp.Item != nil &&
			koutakuResp.Item.Type != nil &&
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {

			// Web search call - emit content_block_start with server_tool_use
			streamResp.Type = AnthropicStreamEventTypeContentBlockStart

			if koutakuResp.ContentIndex != nil {
				streamResp.Index = koutakuResp.ContentIndex
			} else if koutakuResp.OutputIndex != nil {
				streamResp.Index = koutakuResp.OutputIndex
			}
			...
			if koutakuResp.Item != nil {
				...
				if isCompactionItem(koutakuResp.Item) {
					contentBlock.Type = AnthropicContentBlockTypeCompaction
					contentBlock.Content = &AnthropicContent{ContentStr: schemas.Ptr("")}
					if koutakuResp.Item.Content.ContentBlocks[0].CacheControl != nil {
						contentBlock.CacheControl = koutakuResp.Item.Content.ContentBlocks[0].CacheControl
					}
				} else if koutakuResp.Item.Type != nil {
					switch *koutakuResp.Item.Type {
					...
					case schemas.ResponsesMessageTypeReasoning:
						...
						if koutakuResp.Item.ResponsesReasoning != nil && koutakuResp.Item.ResponsesReasoning.EncryptedContent != nil && *koutakuResp.Item.ResponsesReasoning.EncryptedContent != "" {
							contentBlock.Data = koutakuResp.Item.ResponsesReasoning.EncryptedContent
					...
					case schemas.ResponsesMessageTypeFunctionCall:
						...
						if koutakuResp.Item.ResponsesReasoning != nil {
							// This is actually reasoning content, not a function call
							contentBlock.Type = AnthropicContentBlockTypeThinking
							contentBlock.Thinking = schemas.Ptr("")
							contentBlock.Signature = schemas.Ptr("")
							// Check if there's encrypted content for redacted_thinking
							if koutakuResp.Item.ResponsesReasoning.EncryptedContent != nil && *koutakuResp.Item.ResponsesReasoning.EncryptedContent != "" {
								contentBlock.Type = AnthropicContentBlockTypeRedactedThinking
								contentBlock.Data = koutakuResp.Item.ResponsesReasoning.EncryptedContent
							}
						} else {
							...
							contentIndex := 0
							if koutakuResp.ContentIndex != nil {
								contentIndex = *koutakuResp.ContentIndex
							}
							...
							if koutakuResp.Response != nil && koutakuResp.Response.Output != nil {
								for _, msg := range koutakuResp.Response.Output {
							...
							if isFirstBlock && hasReasoningInResponse {
								...
								if koutakuResp.Item.ResponsesToolMessage != nil {
									contentBlock.ID = koutakuResp.Item.ResponsesToolMessage.CallID
									contentBlock.Name = koutakuResp.Item.ResponsesToolMessage.Name
									// Always start with empty input for streaming compatibility
									contentBlock.Input = json.RawMessage("{}")
									...
									if koutakuResp.Item.ResponsesToolMessage.Name != nil &&
										*koutakuResp.Item.ResponsesToolMessage.Name == "WebSearch" &&
										koutakuResp.Item.ID != nil {
										...
										streamState.webSearchItemIDs[*koutakuResp.Item.ID] = true
									}
								}
							}
						}
					case schemas.ResponsesMessageTypeMCPCall:
						...
						if koutakuResp.Item.ResponsesToolMessage != nil {
							contentBlock.ID = koutakuResp.Item.ID
							contentBlock.Name = koutakuResp.Item.ResponsesToolMessage.Name
							if koutakuResp.Item.ResponsesToolMessage.ResponsesMCPToolCall != nil {
								contentBlock.ServerName = &koutakuResp.Item.ResponsesToolMessage.ResponsesMCPToolCall.ServerLabel
							}
							// Always start with empty input for streaming compatibility
							contentBlock.Input = json.RawMessage("{}")
						}
		...
		if isCompactionItem(koutakuResp.Item) {
			block := koutakuResp.Item.Content.ContentBlocks[0]
			if block.ResponsesOutputMessageContentCompaction != nil {
				var indexToUse *int
				if koutakuResp.OutputIndex != nil {
					indexToUse = koutakuResp.OutputIndex
				} else if koutakuResp.ContentIndex != nil {
					indexToUse = koutakuResp.ContentIndex
				}
		...
		if koutakuResp.Item != nil && koutakuResp.Item.ResponsesToolMessage != nil {
			var argumentsJSON string
			var shouldGenerateDeltas bool

			switch *koutakuResp.Item.Type {
			case schemas.ResponsesMessageTypeFunctionCall:
				if koutakuResp.Item.ResponsesToolMessage.Arguments != nil && *koutakuResp.Item.ResponsesToolMessage.Arguments != "" {
					argumentsJSON = *koutakuResp.Item.ResponsesToolMessage.Arguments
			...
			case schemas.ResponsesMessageTypeComputerCall:
				if koutakuResp.Item.ResponsesToolMessage.Action != nil && koutakuResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction != nil {
					actionInput := convertResponsesToAnthropicComputerAction(koutakuResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction)
	...
	case schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDelta:
		...
		if koutakuResp.Arguments != nil {
			streamResp.Delta = &AnthropicStreamDelta{
				Type:        AnthropicStreamDeltaTypeInputJSON,
				PartialJSON: koutakuResp.Arguments,
			}
		} else if koutakuResp.Delta != nil {
			// Handle cases where Delta field is used instead of Arguments
			streamResp.Delta = &AnthropicStreamDelta{
				Type:        AnthropicStreamDeltaTypeInputJSON,
				PartialJSON: koutakuResp.Delta,
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeOutputTextAnnotationAdded:
		...
		if koutakuResp.Annotation != nil {
			...
			citation := convertAnnotationToAnthropicCitation(*koutakuResp.Annotation)
	...
	case schemas.ResponsesStreamResponseTypeOutputItemDone:
		...
		if koutakuResp.Item != nil &&
			koutakuResp.Item.Type != nil &&
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall &&
			koutakuResp.Item.ResponsesToolMessage != nil &&
			koutakuResp.Item.ResponsesToolMessage.Name != nil &&
			*koutakuResp.Item.ResponsesToolMessage.Name == "WebSearch" &&
			koutakuResp.Item.ResponsesToolMessage.Arguments != nil {

			argumentsJSON := sanitizeWebSearchArguments(*koutakuResp.Item.ResponsesToolMessage.Arguments)
			koutakuResp.Item.ResponsesToolMessage.Arguments = &argumentsJSON
			...
			if koutakuResp.Item.ID != nil {
				streamState := getOrCreateAnthropicToResponsesStreamState(ctx)
				delete(streamState.webSearchItemIDs, *koutakuResp.Item.ID)
			}
		...
		if koutakuResp.Item != nil &&
			...
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeComputerCall {
			...
			if koutakuResp.Item.ResponsesToolMessage != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction != nil {

				actionInput := convertResponsesToAnthropicComputerAction(
					koutakuResp.Item.ResponsesToolMessage.Action.ResponsesComputerToolCallAction,
				)
			...
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
			...
			if koutakuResp.Item.ResponsesToolMessage != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Query != nil {

				// Create input map with query
				inputMap := map[string]interface{}{
					"query": *koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Query,
				}
			...
			if koutakuResp.ContentIndex != nil {
				stopEvent.Index = koutakuResp.ContentIndex
			} else if koutakuResp.OutputIndex != nil {
				stopEvent.Index = koutakuResp.OutputIndex
			}
			events = append(events, stopEvent)
			...
			if koutakuResp.Item.ResponsesToolMessage != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action != nil &&
				koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction != nil &&
				len(koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Sources) > 0 {

				// Calculate next index for result block
				var resultIndex *int
				if koutakuResp.OutputIndex != nil {
					nextIdx := *koutakuResp.OutputIndex + 1
					resultIndex = &nextIdx
				} else if koutakuResp.ContentIndex != nil {
					nextIdx := *koutakuResp.ContentIndex + 1
					resultIndex = &nextIdx
				}

				// Create content blocks for each source
				var resultContentBlocks []AnthropicContentBlock
				for _, source := range koutakuResp.Item.ResponsesToolMessage.Action.ResponsesWebSearchToolCallAction.Sources {
				...
				resultStartEvent := &AnthropicStreamEvent{
					...
					ContentBlock: &AnthropicContentBlock{
						...
						ToolUseID: koutakuResp.Item.ID, // Link to the server_tool_use block
			...
			koutakuResp.Item.Type != nil &&
			(*koutakuResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall ||
				*koutakuResp.Item.Type == schemas.ResponsesMessageTypeMCPCall) {
	...
	case schemas.ResponsesStreamResponseTypeCompleted:
		...
		if alreadyEmitted, ok := ctx.Value(schemas.KoutakuContextKeyHasEmittedMessageDelta).(bool); ok && alreadyEmitted {
		...
		// Convert usage from Koutaku to Anthropic
		if koutakuResp.Response != nil {
			anthropicContentDeltaEvent.Usage = ConvertKoutakuUsageToAnthropicUsage(koutakuResp.Response.Usage)
			if koutakuResp.Response.StopReason != nil {
				anthropicContentDeltaEvent.Delta = &AnthropicStreamDelta{
					StopReason:   schemas.Ptr(ConvertKoutakuFinishReasonToAnthropic(*koutakuResp.Response.StopReason)),
					StopSequence: nil,
				}
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeMCPCallArgumentsDelta:
		...
		} else if koutakuResp.Arguments != nil {
	...
	case schemas.ResponsesStreamResponseTypeMCPCallFailed:
		...
		if koutakuResp.Message != nil {
			errorMsg = *koutakuResp.Message
		}
	...
	case "message_delta":
		...
		if ctx.Value(schemas.KoutakuContextKeyIntegrationType) == "anthropic" {
			...
			// Convert usage from Koutaku format to Anthropic format using common converter
			if koutakuResp.Response != nil {
				streamResp.Usage = ConvertKoutakuUsageToAnthropicUsage(koutakuResp.Response.Usage)
			}

			// Convert stop reason from Koutaku format to Anthropic format
			if koutakuResp.Response != nil && koutakuResp.Response.StopReason != nil {
				streamResp.Delta = &AnthropicStreamDelta{
					StopReason: schemas.Ptr(ConvertKoutakuFinishReasonToAnthropic(*koutakuResp.Response.StopReason)),
				}
			} else if koutakuResp.Delta != nil {
...
// ToKoutakuResponsesRequest converts an Anthropic message request to Koutaku format
func (req *AnthropicMessageRequest) ToKoutakuResponsesRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuResponsesRequest {
	provider, model := schemas.ParseModelString(req.Model, providerUtils.CheckAndSetDefaultProvider(ctx, schemas.Anthropic))

	koutakuReq := &schemas.KoutakuResponsesRequest{
		Provider:  provider,
		Model:     model,
	...
	if req.Thinking != nil {
		if req.Thinking.Type == "enabled" || req.Thinking.Type == "adaptive" {
			...
			if req.OutputConfig != nil && req.OutputConfig.Effort != nil {
				// Native effort present — map to Koutaku enum (e.g., "max" → "high")
	...
	koutakuReq.Params = params

	// Convert messages directly to ChatMessage format
	var koutakuMessages []schemas.ResponsesMessage

	// Convert regular messages using the new conversion method
	convertedMessages := ConvertAnthropicMessagesToKoutakuMessages(ctx, req.Messages, req.System, false, provider == schemas.Bedrock)
	koutakuMessages = append(koutakuMessages, convertedMessages...)

	// Convert tools if present
	if req.Tools != nil {
		var koutakuTools []schemas.ResponsesTool
		for _, tool := range req.Tools {
			koutakuTool := convertAnthropicToolToKoutaku(&tool)
			if koutakuTool != nil {
				applyAnthropicToolFlagsToResponsesTool(&tool, koutakuTool)
				koutakuTools = append(koutakuTools, *koutakuTool)
			}
		}
		if len(koutakuTools) > 0 {
	...
	if req.MCPServers != nil {
		...
		// because convertAnthropicToolToKoutaku skips them.
		...
		var koutakuMCPTools []schemas.ResponsesTool
		for _, mcpServer := range req.MCPServers {
			koutakuMCPTool := convertAnthropicMCPServerV2ToKoutakuTool(&mcpServer)
			if koutakuMCPTool != nil {
				...
				if toolWithFlags, ok := toolsetByServer[mcpServer.Name]; ok {
					applyMCPToolsetConfigToKoutakuTool(koutakuMCPTool, toolWithFlags.MCPToolset)
					applyAnthropicToolFlagsToResponsesTool(toolWithFlags, koutakuMCPTool)
				}
				koutakuMCPTools = append(koutakuMCPTools, *koutakuMCPTool)
			}
		}
		if len(koutakuMCPTools) > 0 {
			koutakuReq.Params.Tools = append(koutakuReq.Params.Tools, koutakuMCPTools...)
		}
	}

	// Convert tool choice if present
	if req.ToolChoice != nil {
		koutakuToolChoice := convertAnthropicToolChoiceToKoutaku(req.ToolChoice)
		if koutakuToolChoice != nil {
			koutakuReq.Params.ToolChoice = koutakuToolChoice
		}
	}

	// Set the converted messages
	if len(koutakuMessages) > 0 {
...
// ToAnthropicResponsesRequest converts a KoutakuRequest with Responses structure back to AnthropicMessageRequest
func ToAnthropicResponsesRequest(ctx *schemas.KoutakuContext, koutakuReq *schemas.KoutakuResponsesRequest) (*AnthropicMessageRequest, error) {
	if koutakuReq == nil {
	...
	if koutakuReq.Params != nil {
		if koutakuReq.Params.MaxOutputTokens != nil {
			anthropicReq.MaxTokens = *koutakuReq.Params.MaxOutputTokens
		}
		...
		if koutakuReq.Params.User != nil {
			anthropicReq.Metadata = &AnthropicMetaData{
				UserID: koutakuReq.Params.User,
			}
		}
		if koutakuReq.Params.Text != nil {
			// Vertex doesn't support native structured outputs, so convert to tool
			if koutakuReq.Provider == schemas.Vertex {
				if koutakuReq.Params.Text.Format != nil {
					responseFormatTool := convertResponsesTextFormatToTool(ctx, koutakuReq.Params.Text)
				...
				for _, message := range koutakuReq.Input {
				...
				if !hasCitationsEnabled {
					...
					outputFormat := convertResponsesTextConfigToAnthropicOutputFormat(koutakuReq.Params.Text)
					if outputFormat != nil {
		...
		if koutakuReq.Params.Reasoning != nil {
			if koutakuReq.Params.Reasoning.MaxTokens != nil {
				...
				if koutakuReq.Params.Reasoning.Effort != nil {
			...
			if anthropicReq.Thinking != nil && anthropicReq.Thinking.Type != "disabled" {
				if koutakuReq.Params.Reasoning != nil &&
					koutakuReq.Params.Reasoning.Summary != nil && *koutakuReq.Params.Reasoning.Summary == "none" {
		...
		if koutakuReq.Params.ExtraParams != nil {
			anthropicReq.ExtraParams = make(map[string]interface{}, len(koutakuReq.Params.ExtraParams))
			...
			topK, ok := schemas.SafeExtractIntPointer(koutakuReq.Params.ExtraParams["top_k"])
			if ok {
			...
			if speed, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["speed"]); ok {
				delete(anthropicReq.ExtraParams, "speed")
				anthropicReq.Speed = speed
			}
			if stop, ok := schemas.SafeExtractStringSlice(koutakuReq.Params.ExtraParams["stop"]); ok {
				delete(anthropicReq.ExtraParams, "stop")
				anthropicReq.StopSequences = stop
			}
			if inferenceGeo, ok := schemas.SafeExtractStringPointer(koutakuReq.Params.ExtraParams["inference_geo"]); ok {
				delete(anthropicReq.ExtraParams, "inference_geo")
				anthropicReq.InferenceGeo = inferenceGeo
			}
			if cmVal := koutakuReq.Params.ExtraParams["context_management"]; cmVal != nil {
			...
			if tbVal, exists := koutakuReq.Params.ExtraParams["task_budget"]; exists {
		...
		if koutakuReq.Params.Tools != nil {
			anthropicTools, mcpServers := convertKoutakuToolsToAnthropic(koutakuReq.Model, koutakuReq.Params.Tools, koutakuReq.Provider)
		...
		if koutakuReq.Params.ToolChoice != nil {
			anthropicToolChoice := convertResponsesToolChoiceToAnthropic(koutakuReq.Params.ToolChoice)
	...
	if koutakuReq.Input != nil {
		anthropicMessages, systemContent := ConvertKoutakuMessagesToAnthropicMessages(ctx, koutakuReq.Input)

		// Set system message if present
		if systemContent != nil {
			anthropicReq.System = systemContent
		} else if koutakuReq.Params != nil && koutakuReq.Params.Instructions != nil && *koutakuReq.Params.Instructions != "" {
				...
				ContentBlocks: []AnthropicContentBlock{
					{
						...
						Text: koutakuReq.Params.Instructions,
					},
				},
			}
		}
...
// ConvertAnthropicUsageToKoutakuUsage converts Anthropic usage format to Koutaku usage format
// Handles iterations recursively
func ConvertAnthropicUsageToKoutakuUsage(anthropicUsage *AnthropicUsage) *schemas.ResponsesResponseUsage {
	if anthropicUsage == nil {
		return nil
	}

	koutakuUsage := &schemas.ResponsesResponseUsage{
	...
	if anthropicUsage.CacheReadInputTokens > 0 {
		if koutakuUsage.InputTokensDetails == nil {
			koutakuUsage.InputTokensDetails = &schemas.ResponsesResponseInputTokens{}
		}
		koutakuUsage.InputTokensDetails.CachedReadTokens = anthropicUsage.CacheReadInputTokens
		koutakuUsage.InputTokens = koutakuUsage.InputTokens + anthropicUsage.CacheReadInputTokens
		koutakuUsage.TotalTokens = koutakuUsage.TotalTokens + anthropicUsage.CacheReadInputTokens
	}
	...
	if anthropicUsage.CacheCreationInputTokens > 0 {
		...
		koutakuUsage.InputTokensDetails.CachedWriteTokens = anthropicUsage.CacheCreationInputTokens
		koutakuUsage.InputTokens = koutakuUsage.InputTokens + anthropicUsage.CacheCreationInputTokens
		koutakuUsage.TotalTokens = koutakuUsage.TotalTokens + anthropicUsage.CacheCreationInputTokens
	}
	...
	if anthropicUsage.ServerToolUse != nil && anthropicUsage.ServerToolUse.WebSearchRequests > 0 {
		if koutakuUsage.OutputTokensDetails == nil {
			koutakuUsage.OutputTokensDetails = &schemas.ResponsesResponseOutputTokens{}
		}
		koutakuUsage.OutputTokensDetails.NumSearchQueries = schemas.Ptr(anthropicUsage.ServerToolUse.WebSearchRequests)
	}

	// Recursively convert iterations
	if len(anthropicUsage.Iterations) > 0 {
		koutakuUsage.Iterations = make([]schemas.ResponsesResponseUsage, len(anthropicUsage.Iterations))
		for i, iteration := range anthropicUsage.Iterations {
			if converted := ConvertAnthropicUsageToKoutakuUsage(&iteration); converted != nil {
				koutakuUsage.Iterations[i] = *converted
			}
		}
	}
...
// ConvertKoutakuUsageToAnthropicUsage converts Koutaku usage format to Anthropic usage format
// Handles iterations recursively
func ConvertKoutakuUsageToAnthropicUsage(koutakuUsage *schemas.ResponsesResponseUsage) *AnthropicUsage {
	if koutakuUsage == nil {
		return nil
	}

	anthropicUsage := &AnthropicUsage{
		Type:         koutakuUsage.Type,
		InputTokens:  koutakuUsage.InputTokens,
		OutputTokens: koutakuUsage.OutputTokens,
	}

	// Handle cache read tokens
	if koutakuUsage.InputTokensDetails != nil {
		if koutakuUsage.InputTokensDetails.CachedReadTokens > 0 {
			anthropicUsage.CacheReadInputTokens = koutakuUsage.InputTokensDetails.CachedReadTokens
			anthropicUsage.InputTokens = anthropicUsage.InputTokens - koutakuUsage.InputTokensDetails.CachedReadTokens
		}
		if koutakuUsage.InputTokensDetails.CachedWriteTokens > 0 {
			anthropicUsage.CacheCreationInputTokens = koutakuUsage.InputTokensDetails.CachedWriteTokens
			anthropicUsage.InputTokens = anthropicUsage.InputTokens - koutakuUsage.InputTokensDetails.CachedWriteTokens
			// Populate the cache_creation breakdown — default to ephemeral (5m) since
			// the Koutaku internal format doesn't distinguish TTL variants.
			anthropicUsage.CacheCreation = AnthropicUsageCacheCreation{
				Ephemeral5mInputTokens: koutakuUsage.InputTokensDetails.CachedWriteTokens,
			}
		}
	}

	// Handle server tool use statistics (e.g., web search)
	if koutakuUsage.OutputTokensDetails != nil && koutakuUsage.OutputTokensDetails.NumSearchQueries != nil && *koutakuUsage.OutputTokensDetails.NumSearchQueries > 0 {
		anthropicUsage.ServerToolUse = &AnthropicServerToolUseUsage{
			WebSearchRequests: *koutakuUsage.OutputTokensDetails.NumSearchQueries,
		}
	}

	// Recursively convert iterations
	if len(koutakuUsage.Iterations) > 0 {
		anthropicUsage.Iterations = make([]AnthropicUsage, len(koutakuUsage.Iterations))
		for i, iteration := range koutakuUsage.Iterations {
			if converted := ConvertKoutakuUsageToAnthropicUsage(&iteration); converted != nil {
...
// ToKoutakuResponsesResponse converts an Anthropic response to KoutakuResponse with Responses structure
func (response *AnthropicMessageResponse) ToKoutakuResponsesResponse(ctx *schemas.KoutakuContext) *schemas.KoutakuResponsesResponse {
	if response == nil {
		return nil
	}

	// Create the KoutakuResponse with Responses structure
	koutakuResp := &schemas.KoutakuResponsesResponse{
		ID:        schemas.Ptr(response.ID),
		CreatedAt: int(time.Now().Unix()),
	}

	// Convert usage information using common converter (handles iterations recursively)
	koutakuResp.Usage = ConvertAnthropicUsageToKoutakuUsage(response.Usage)
	...
	if len(response.Content) > 0 {
		...
		outputMessages := ConvertAnthropicMessagesToKoutakuMessages(ctx, []AnthropicMessage{tempMsg}, nil, true, false)
		if len(outputMessages) > 0 {
			koutakuResp.Output = outputMessages
		}
	}

	koutakuResp.Model = response.Model

	// Preserve stop reason from Anthropic response
	if response.StopReason != "" {
		koutakuResp.StopReason = schemas.Ptr(string(response.StopReason))
	}

	return koutakuResp
}

// ToAnthropicResponsesResponse converts a KoutakuResponse with Responses structure back to AnthropicMessageResponse
func ToAnthropicResponsesResponse(ctx *schemas.KoutakuContext, koutakuResp *schemas.KoutakuResponsesResponse) *AnthropicMessageResponse {
	anthropicResp := &AnthropicMessageResponse{
		Type: "message",
		Role: "assistant",
	}
	if koutakuResp.ID != nil {
		anthropicResp.ID = *koutakuResp.ID
	}

	// Convert usage information using common converter (handles iterations recursively)
	anthropicResp.Usage = ConvertKoutakuUsageToAnthropicUsage(koutakuResp.Usage)

	// Convert output messages to Anthropic content blocks using the new conversion method
	var contentBlocks []AnthropicContentBlock
	if koutakuResp.Output != nil {
		anthropicMessages, _ := ConvertKoutakuMessagesToAnthropicMessages(ctx, koutakuResp.Output)
	...
	// Map stop reason from Koutaku response if available, otherwise infer from content
	if koutakuResp.StopReason != nil {
		anthropicResp.StopReason = ConvertKoutakuFinishReasonToAnthropic(*koutakuResp.StopReason)
	} else {
	...
	anthropicResp.Model = koutakuResp.Model

	return anthropicResp
}

// ConvertAnthropicMessagesToKoutakuMessages converts an array of Anthropic messages to Koutaku ResponsesMessage format
func ConvertAnthropicMessagesToKoutakuMessages(ctx *schemas.KoutakuContext, anthropicMessages []AnthropicMessage, systemContent *AnthropicContent, isOutputMessage bool, keepToolsGrouped bool) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage
	...
	if systemContent != nil {
		systemMessages := convertAnthropicSystemToKoutakuMessages(systemContent)
		koutakuMessages = append(koutakuMessages, systemMessages...)
	}
	...
	for _, msg := range anthropicMessages {
		...
		if keepToolsGrouped {
			convertedMessages = convertSingleAnthropicMessageToKoutakuMessagesGrouped(&msg, isOutputMessage, structuredOutputToolName)
		} else {
			convertedMessages = convertSingleAnthropicMessageToKoutakuMessages(ctx, &msg, isOutputMessage, structuredOutputToolName)
		}
		koutakuMessages = append(koutakuMessages, convertedMessages...)
	}

	return koutakuMessages
}

// ConvertKoutakuMessagesToAnthropicMessages converts an array of Koutaku ResponsesMessage to Anthropic message format
// This is the main conversion method from Koutaku to Anthropic - handles all message types and returns messages + system content
func ConvertKoutakuMessagesToAnthropicMessages(ctx *schemas.KoutakuContext, koutakuMessages []schemas.ResponsesMessage) ([]AnthropicMessage, *AnthropicContent) {
	...
	for _, msg := range koutakuMessages {
		...
		case schemas.ResponsesMessageTypeMessage:
			...
			if msg.Role != nil && *msg.Role == schemas.ResponsesInputMessageRoleSystem {
				systemContent = convertKoutakuMessageToAnthropicSystemContent(&msg)
				continue
			}
			...
			anthropicMsg := convertKoutakuMessageToAnthropicMessage(&msg, &pendingReasoningContentBlocks)
			if anthropicMsg != nil {
		...
		case schemas.ResponsesMessageTypeReasoning:
			...
			reasoningBlocks := convertKoutakuReasoningToAnthropicThinking(&msg)
		...
		case schemas.ResponsesMessageTypeFunctionCall:
			...
			toolUseBlock := convertKoutakuFunctionCallToAnthropicToolUse(ctx, &msg)
			if toolUseBlock != nil {
		...
		case schemas.ResponsesMessageTypeFunctionCallOutput:
			...
			toolResultBlock := convertKoutakuFunctionCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeItemReference:
			...
			referenceMsg := convertKoutakuItemReferenceToAnthropicMessage(&msg)
			if referenceMsg != nil {
		...
		case schemas.ResponsesMessageTypeComputerCall:
			...
			computerToolUseBlock := convertKoutakuComputerCallToAnthropicToolUse(&msg)
		...
		case schemas.ResponsesMessageTypeMCPCall:
			...
			if msg.ResponsesToolMessage != nil {
				if msg.ResponsesToolMessage.Name != nil {
					...
					mcpToolUseBlock := convertKoutakuMCPCallToAnthropicToolUse(&msg)
					...
					mcpToolResultBlock := convertKoutakuMCPCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeMCPApprovalRequest:
			...
			mcpApprovalBlock := convertKoutakuMCPApprovalToAnthropicToolUse(&msg)
		...
		case schemas.ResponsesMessageTypeWebSearchCall:
			...
			webSearchBlocks := convertKoutakuWebSearchCallToAnthropicBlocks(&msg)
		...
		case schemas.ResponsesMessageTypeFileSearchCall,
			...
			unsupportedToolMsg := convertKoutakuUnsupportedToolCallToAnthropicMessage(&msg, msgType)
		...
		case schemas.ResponsesMessageTypeComputerCallOutput:
			...
			computerResultBlock := convertKoutakuComputerCallOutputToAnthropicToolResultBlock(&msg)
		...
		case schemas.ResponsesMessageTypeLocalShellCallOutput,
			...
			toolOutputMsg := convertKoutakuToolOutputToAnthropicMessage(&msg)
...
// Helper function to convert Anthropic system content to Koutaku messages
func convertAnthropicSystemToKoutakuMessages(systemContent *AnthropicContent) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage

	if systemContent.ContentStr != nil && *systemContent.ContentStr != "" {
		koutakuMessages = append(koutakuMessages, schemas.ResponsesMessage{
...
// Helper function to convert a single Anthropic message to Koutaku messages
func convertSingleAnthropicMessageToKoutakuMessages(ctx *schemas.KoutakuContext, msg *AnthropicMessage, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
...
// Helper function to convert a single Anthropic message to Koutaku messages, grouping text and tool calls
// This keeps assistant messages with mixed text and tool_use blocks together
func convertSingleAnthropicMessageToKoutakuMessagesGrouped(msg *AnthropicMessage, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
	// Determine if this message should use output types based on role
	// Assistant messages in conversation history should use output_text
	isOutput := isOutputMessage || msg.Role == AnthropicMessageRoleAssistant
...
// Helper function to convert Anthropic content blocks to Koutaku ResponsesMessages, grouping text and tool_use blocks
func convertAnthropicContentBlocksToResponsesMessagesGrouped(contentBlocks []AnthropicContentBlock, role *schemas.ResponsesMessageRoleType, isOutputMessage bool) []schemas.ResponsesMessage {
	...
	for _, block := range contentBlocks {
		...
		case AnthropicContentBlockTypeText:
			if block.Text != nil {
				if isOutputMessage {
					...
					koutakuMsg := schemas.ResponsesMessage{
						Type: schemas.Ptr(schemas.ResponsesMessageTypeMessage),
						Role: role,
						Content: &schemas.ResponsesMessageContent{
							ContentBlocks: []schemas.ResponsesMessageContentBlock{
					...
					koutakuMessages = append(koutakuMessages, koutakuMsg)
				}
			}
		...
		case AnthropicContentBlockTypeImage:
			...
			if block.Source != nil && block.Source.SourceObj != nil {
				koutakuMsg := schemas.ResponsesMessage{
					...
					Role: role,
					Content: &schemas.ResponsesMessageContent{
						ContentBlocks: []schemas.ResponsesMessageContentBlock{block.toKoutakuResponsesImageBlock()},
					},
				}
				if isOutputMessage {
					koutakuMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
				}
				koutakuMessages = append(koutakuMessages, koutakuMsg)
			}
		...
		case AnthropicContentBlockTypeDocument:
			...
			if block.Source != nil && block.Source.SourceObj != nil {
				koutakuMsg := schemas.ResponsesMessage{
					...
					Role: role,
					Content: &schemas.ResponsesMessageContent{
						ContentBlocks: []schemas.ResponsesMessageContentBlock{block.toKoutakuResponsesDocumentBlock()},
					},
				}
				if isOutputMessage {
					koutakuMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
		...
		case AnthropicContentBlockTypeToolResult:
			...
			if block.ToolUseID != nil {
				if block.Content != nil {
					...
					koutakuMsg.ResponsesToolMessage.Output = &schemas.ResponsesToolMessageOutputStruct{}

					if block.Content.ContentStr != nil {
						koutakuMsg.ResponsesToolMessage.Output.ResponsesToolCallOutputStr = block.Content.ContentStr
						...
						for _, contentBlock := range block.Content.ContentBlocks {
							...
							case AnthropicContentBlockTypeImage:
								if contentBlock.Source != nil && contentBlock.Source.SourceObj != nil {
									toolMsgContentBlocks = append(toolMsgContentBlocks, contentBlock.toKoutakuResponsesImageBlock())
								}
							}
						}
						koutakuMsg.ResponsesToolMessage.Output.ResponsesFunctionToolCallOutputBlocks = toolMsgContentBlocks
					}

					// Handle is_error from Anthropic
					if block.IsError != nil && *block.IsError {
						koutakuMsg.Status = schemas.Ptr("incomplete")
					}
		...
		case AnthropicContentBlockTypeWebSearchResult:
			...
			if block.ToolUseID != nil {
				attachWebSearchSourcesToCall(koutakuMessages, *block.ToolUseID, block, true)
			}
		}
	}
	...
	if len(accumulatedTextContent) > 0 {
		...
		if isOutputMessage {
			koutakuMsg.ID = schemas.Ptr("msg_" + providerUtils.GetRandomString(50))
			koutakuMsg.Content = &schemas.ResponsesMessageContent{
	...
	if len(pendingToolUseBlocks) > 0 {
		for _, toolBlock := range pendingToolUseBlocks {
			...
			if isOutputMessage {
				koutakuMsg.ID = schemas.Ptr("fc_" + providerUtils.GetRandomString(50))
			}

			// Check for computer tool use
			if toolBlock.Name != nil && *toolBlock.Name == string(AnthropicToolNameComputer) {
				koutakuMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeComputerCall)
				koutakuMsg.ResponsesToolMessage.Name = nil
				var inputMap map[string]interface{}
				if err := sonic.Unmarshal(toolBlock.Input, &inputMap); err == nil {
					koutakuMsg.ResponsesToolMessage.Action = &schemas.ResponsesToolMessageActionStruct{
				...
				koutakuMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeWebSearchCall)
				koutakuMsg.ResponsesToolMessage.Name = nil
				...
				koutakuMsg.Type = schemas.Ptr(schemas.ResponsesMessageTypeWebFetchCall)
				koutakuMsg.ResponsesToolMessage.Name = nil
				...
				if len(toolBlock.Input) > 0 {
					koutakuMsg.ResponsesToolMessage.Arguments = schemas.Ptr(string(toolBlock.Input))
				}
			}
...
// Helper function to convert Anthropic content blocks to Koutaku ResponsesMessages
func convertAnthropicContentBlocksToResponsesMessages(ctx *schemas.KoutakuContext, contentBlocks []AnthropicContentBlock, role *schemas.ResponsesMessageRoleType, isOutputMessage bool, structuredOutputToolName string) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage
	...
	for _, block := range contentBlocks {
		...
		case AnthropicContentBlockTypeText:
			if block.Text != nil {
				var koutakuMsg schemas.ResponsesMessage
				if isOutputMessage {
			...
			if structuredOutputToolName != "" && block.Name != nil && *block.Name == structuredOutputToolName {
				...
				if block.ID != nil && block.Name != nil {
					...
					if block.Name != nil && *block.Name == string(AnthropicToolNameComputer) {
						...
						koutakuMsg.ResponsesToolMessage.Arguments = schemas.Ptr(string(block.Input))
					}
					koutakuMessages = append(koutakuMessages, koutakuMsg)
				}
			}
		...
		case AnthropicContentBlockTypeWebSearchToolResultError:
			...
			if block.ToolUseID != nil {
				for i := len(koutakuMessages) - 1; i >= 0; i-- {
					msg := &koutakuMessages[i]
		...
		case AnthropicContentBlockTypeMCPToolUse:
			...
			if block.ID != nil && block.Name != nil {
				...
				if block.ServerName != nil {
					koutakuMsg.ResponsesToolMessage.ResponsesMCPToolCall = &schemas.ResponsesMCPToolCall{
	...
	if len(reasoningContentBlocks) > 0 {
		...
		koutakuMessages = append([]schemas.ResponsesMessage{reasoningMessage}, koutakuMessages...)
	}

	return koutakuMessages
}

// Helper functions for converting individual Koutaku message types to Anthropic messages
// convertKoutakuMessageToAnthropicSystemContent converts a Koutaku system message to Anthropic system content
func convertKoutakuMessageToAnthropicSystemContent(msg *schemas.ResponsesMessage) *AnthropicContent {
	if msg.Content != nil {
		if msg.Content.ContentStr != nil {
			...
			contentBlocks := convertKoutakuContentBlocksToAnthropic(msg.Content.ContentBlocks)
			if len(contentBlocks) > 0 {
				return &AnthropicContent{
					ContentBlocks: contentBlocks,
				}
...
// convertKoutakuMessageToAnthropicMessage converts a regular Koutaku message to Anthropic message
func convertKoutakuMessageToAnthropicMessage(msg *schemas.ResponsesMessage, pendingReasoningContentBlocks *[]AnthropicContentBlock) *AnthropicMessage {
		...
		*pendingReasoningContentBlocks = nil
		...
		if msg.Content != nil {
			if msg.Content.ContentStr != nil {
				...
				contentBlocks = append(contentBlocks, convertKoutakuContentBlocksToAnthropic(msg.Content.ContentBlocks)...)
			}
		}
...
// convertKoutakuReasoningToAnthropicThinking converts a Koutaku reasoning message to Anthropic thinking blocks
func convertKoutakuReasoningToAnthropicThinking(msg *schemas.ResponsesMessage) []AnthropicContentBlock {
...
// convertKoutakuFunctionCallToAnthropicToolUse converts a Koutaku function call to Anthropic tool use
func convertKoutakuFunctionCallToAnthropicToolUse(ctx *schemas.KoutakuContext, msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
...
// convertKoutakuFunctionCallOutputToAnthropicToolResultBlock converts a Koutaku function call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertKoutakuFunctionCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
...
// convertKoutakuComputerCallOutputToAnthropicToolResultBlock converts a Koutaku computer call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertKoutakuComputerCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
...
// convertKoutakuMCPCallOutputToAnthropicToolResultBlock converts a Koutaku MCP call output to a single tool result block
// This is used to accumulate multiple tool results into a single user message
func convertKoutakuMCPCallOutputToAnthropicToolResultBlock(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.CallID != nil {
		toolResultBlock := AnthropicContentBlock{
...
// convertKoutakuItemReferenceToAnthropicMessage converts a Koutaku item reference to Anthropic message
func convertKoutakuItemReferenceToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
...
// convertKoutakuComputerCallToAnthropicToolUse converts a Koutaku computer call to Anthropic tool use
func convertKoutakuComputerCallToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil {
		toolUseBlock := AnthropicContentBlock{
			Type: AnthropicContentBlockTypeToolUse,
...
// convertKoutakuMCPCallToAnthropicToolUse converts a Koutaku MCP call to Anthropic tool use
func convertKoutakuMCPCallToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
...
// convertKoutakuMCPCallOutputToAnthropicMessage converts a Koutaku MCP call output to Anthropic message
func convertKoutakuMCPCallOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	toolResultBlock := AnthropicContentBlock{
		Type: AnthropicContentBlockTypeMCPToolResult,
...
// convertKoutakuMCPApprovalToAnthropicToolUse converts a Koutaku MCP approval request to Anthropic tool use
func convertKoutakuMCPApprovalToAnthropicToolUse(msg *schemas.ResponsesMessage) *AnthropicContentBlock {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.Name != nil {
		toolUseBlock := AnthropicContentBlock{
			Type: AnthropicContentBlockTypeMCPToolUse,
		}
...
// convertKoutakuWebSearchCallToAnthropicBlocks converts a Koutaku web_search_call to Anthropic server_tool_use and web_search_tool_result blocks
func convertKoutakuWebSearchCallToAnthropicBlocks(msg *schemas.ResponsesMessage) []AnthropicContentBlock {
...
// convertKoutakuUnsupportedToolCallToAnthropicMessage converts unsupported tool calls to text messages
func convertKoutakuUnsupportedToolCallToAnthropicMessage(msg *schemas.ResponsesMessage, msgType schemas.ResponsesMessageType) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
...
// convertKoutakuComputerCallOutputToAnthropicMessage converts a Koutaku computer call output to Anthropic message
func convertKoutakuComputerCallOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
		toolResultBlock := AnthropicContentBlock{
			Type:      AnthropicContentBlockTypeToolResult,
			ToolUseID: msg.ResponsesToolMessage.CallID,
...
// convertKoutakuToolOutputToAnthropicMessage converts tool outputs to user messages
func convertKoutakuToolOutputToAnthropicMessage(msg *schemas.ResponsesMessage) *AnthropicMessage {
	if msg.ResponsesToolMessage != nil {
...
// convertAnthropicToolToKoutaku converts AnthropicTool to schemas.Tool
func convertAnthropicToolToKoutaku(tool *AnthropicTool) *schemas.ResponsesTool {
	if tool == nil {
		return nil
	}

	// Skip mcp_toolset entries — these are merged with mcp_servers in ToKoutakuResponsesRequest
	...
	if tool.Type != nil {
		switch *tool.Type {
		case AnthropicToolTypeComputer20250124, AnthropicToolTypeComputer20251124:
			koutakuTool := &schemas.ResponsesTool{
				Type: schemas.ResponsesToolTypeComputerUsePreview,
			}
			if tool.AnthropicToolComputerUse != nil {
				koutakuTool.ResponsesToolComputerUsePreview = &schemas.ResponsesToolComputerUsePreview{
					Environment: "browser", // Default environment
				}
				if tool.AnthropicToolComputerUse.DisplayWidthPx != nil {
					koutakuTool.ResponsesToolComputerUsePreview.DisplayWidth = *tool.AnthropicToolComputerUse.DisplayWidthPx
				}
				if tool.AnthropicToolComputerUse.DisplayHeightPx != nil {
					koutakuTool.ResponsesToolComputerUsePreview.DisplayHeight = *tool.AnthropicToolComputerUse.DisplayHeightPx
				}
				if tool.AnthropicToolComputerUse.EnableZoom != nil {
					koutakuTool.ResponsesToolComputerUsePreview.EnableZoom = tool.AnthropicToolComputerUse.EnableZoom
				}
			}
			return koutakuTool
		...
		case AnthropicToolTypeWebSearch20250305, AnthropicToolTypeWebSearch20260209:
			...
			if tool.AnthropicToolWebSearch != nil {
				koutakuTool.ResponsesToolWebSearch = &schemas.ResponsesToolWebSearch{
				...
				if tool.AnthropicToolWebSearch.MaxUses != nil {
					koutakuTool.ResponsesToolWebSearch.MaxUses = tool.AnthropicToolWebSearch.MaxUses
				}
				if tool.AnthropicToolWebSearch.UserLocation != nil {
					koutakuTool.ResponsesToolWebSearch.UserLocation = &schemas.ResponsesToolWebSearchUserLocation{
		...
		case AnthropicToolTypeWebFetch20250910, AnthropicToolTypeWebFetch20260209, AnthropicToolTypeWebFetch20260309:
			...
			if tool.AnthropicToolWebFetch != nil {
				koutakuTool.ResponsesToolWebFetch = &schemas.ResponsesToolWebFetch{
				...
				if len(tool.AnthropicToolWebFetch.AllowedDomains) > 0 || len(tool.AnthropicToolWebFetch.BlockedDomains) > 0 {
					koutakuTool.ResponsesToolWebFetch.Filters = &schemas.ResponsesToolWebSearchFilters{
	...
	if tool.InputSchema != nil || tool.Strict != nil {
		koutakuTool.ResponsesToolFunction = &schemas.ResponsesToolFunction{
	...
	if tool.CacheControl != nil {
		koutakuTool.CacheControl = tool.CacheControl
	}

	return koutakuTool
}

// convertAnthropicToolChoiceToKoutaku converts AnthropicToolChoice to schemas.ToolChoice
func convertAnthropicToolChoiceToKoutaku(toolChoice *AnthropicToolChoice) *schemas.ResponsesToolChoice {
	if toolChoice == nil {
		return nil
	}

	koutakuToolChoice := &schemas.ResponsesToolChoice{}

	// Handle string format
	if toolChoice.Type != "" {
		switch toolChoice.Type {
		case "auto":
			koutakuToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAuto))
		case "any":
			koutakuToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAny))
		case "none":
			koutakuToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeNone))
		case "tool":
			// Handle forced tool choice with specific function name
			koutakuToolChoice.ResponsesToolChoiceStruct = &schemas.ResponsesToolChoiceStruct{
				Type: schemas.ResponsesToolChoiceTypeFunction,
				Name: &toolChoice.Name,
			}
			return koutakuToolChoice
		default:
			koutakuToolChoice.ResponsesToolChoiceStr = schemas.Ptr(string(schemas.ResponsesToolChoiceTypeAuto))
		}
	}
...
// convertKoutakuToolsToAnthropic converts all Koutaku tools to Anthropic tools and MCP servers.
...
func convertKoutakuToolsToAnthropic(model string, tools []schemas.ResponsesTool, provider schemas.ModelProvider) ([]AnthropicTool, []AnthropicMCPServerV2) {
	...
	for _, tool := range tools {
		if tool.Type == schemas.ResponsesToolTypeMCP && tool.ResponsesToolMCP != nil {
			server, toolset := convertKoutakuMCPToolToAnthropicNew(&tool)
			if server != nil {
		...
		anthropicTool := convertKoutakuToolToAnthropic(model, &tool, provider, hasWebSearchOrFetch)
...
// convertAnthropicToolToKoutaku benefits.
...
// per converted tool so every branch in convertKoutakuToolToAnthropic
...
func convertKoutakuToolToAnthropic(model string, tool *schemas.ResponsesTool, provider schemas.ModelProvider, hasWebSearchOrFetch bool) *AnthropicTool {
	if tool == nil {
		return nil
	}
...
// Helper to convert Koutaku content blocks slice to Anthropic content blocks
func convertKoutakuContentBlocksToAnthropic(blocks []schemas.ResponsesMessageContentBlock) []AnthropicContentBlock {
	if len(blocks) == 0 {
		return nil
	}
...
func (block AnthropicContentBlock) toKoutakuResponsesImageBlock() schemas.ResponsesMessageContentBlock {
...
func (block AnthropicContentBlock) toKoutakuResponsesDocumentBlock() schemas.ResponsesMessageContentBlock {
...
// convertAnthropicMCPServerV2ToKoutakuTool converts a new-format MCP server to a Koutaku ResponsesTool.
func convertAnthropicMCPServerV2ToKoutakuTool(mcpServer *AnthropicMCPServerV2) *schemas.ResponsesTool {
	if mcpServer == nil {
		return nil
	}
	...
	if mcpServer.URL != "" {
		koutakuTool.ResponsesToolMCP.ServerURL = schemas.Ptr(mcpServer.URL)
	}
	if mcpServer.AuthorizationToken != nil {
		koutakuTool.ResponsesToolMCP.Authorization = mcpServer.AuthorizationToken
	}

	return koutakuTool
}

// applyMCPToolsetConfigToKoutakuTool merges mcp_toolset tool configs (from tools[]) into a Koutaku MCP tool.
// Extracts the allowlist pattern: tools explicitly enabled in configs while default_config has enabled=false.
func applyMCPToolsetConfigToKoutakuTool(koutakuTool *schemas.ResponsesTool, toolset *AnthropicMCPToolsetTool) {
	if koutakuTool == nil || koutakuTool.ResponsesToolMCP == nil || toolset == nil {
		return
	}
	...
	if toolset.Configs != nil {
		...
		if !defaultEnabled {
			...
			koutakuTool.ResponsesToolMCP.AllowedTools = &schemas.ResponsesToolMCPAllowedTools{
	...
	if toolset.CacheControl != nil {
		koutakuTool.CacheControl = toolset.CacheControl
	}
}

// convertAnthropicMCPServerToKoutakuTool converts a deprecated-format Anthropic MCP server to a Koutaku ResponsesTool.
func convertAnthropicMCPServerToKoutakuTool(mcpServer *AnthropicMCPServer) *schemas.ResponsesTool {
	if mcpServer == nil {
		return nil
	}
...
// convertKoutakuMCPToolToAnthropicNew converts a Koutaku MCP tool to the new mcp-client-2025-11-20 format.
// Returns both a simplified server entry (for mcp_servers[]) and a toolset entry (for tools[]).
func convertKoutakuMCPToolToAnthropicNew(tool *schemas.ResponsesTool) (*AnthropicMCPServerV2, *AnthropicMCPToolsetTool) {
...
// convertKoutakuMCPToolToAnthropicServer converts a Koutaku MCP tool to the deprecated mcp-client-2025-04-04 format.
// Kept for backward compatibility.
func convertKoutakuMCPToolToAnthropicServer(tool *schemas.ResponsesTool) *AnthropicMCPServer {
	if tool == nil || tool.Type != schemas.ResponsesToolTypeMCP || tool.ResponsesToolMCP == nil {
		return nil
	}
