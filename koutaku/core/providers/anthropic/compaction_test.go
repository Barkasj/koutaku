...
// --- Streaming: Anthropic → Koutaku (inbound) ---

func TestToKoutakuResponsesStream_CompactionContentBlockStart(t *testing.T) {
	t.Parallel()
	...
	responses, err, isLast := chunk.ToKoutakuResponsesStream(context.Background(), 0, state)
	if err != nil {
...
func TestToKoutakuResponsesStream_CompactionDelta(t *testing.T) {
	t.Parallel()
...
func TestToKoutakuResponsesStream_CompactionContentBlockStop(t *testing.T) {
	t.Parallel()
...
// --- Streaming: Koutaku → Anthropic (outbound, non-passthrough) ---
...
func TestToAnthropicResponsesStreamResponse_CompactionOutputItemAdded(t *testing.T) {
	...
	koutakuResp := &schemas.KoutakuResponsesStreamResponse{
	...
	events := ToAnthropicResponsesStreamResponse(ctx, koutakuResp)
...
func TestToKoutakuResponsesResponse_PreservesStopReason(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuResp := resp.ToKoutakuResponsesResponse(ctx)

			if koutakuResp.StopReason == nil {
				t.Fatal("expected StopReason to be non-nil")
			}
			if *koutakuResp.StopReason != tt.expectedStopReason {
				t.Errorf("StopReason = %q, want %q", *koutakuResp.StopReason, tt.expectedStopReason)
			}
		})
	}
}

func TestToKoutakuResponsesResponse_EmptyStopReason(t *testing.T) {
	t.Parallel()
	...
	if koutakuResp.StopReason != nil {
		t.Errorf("expected nil StopReason for empty stop_reason, got %q", *koutakuResp.StopReason)
	}
}

func TestToAnthropicResponsesResponse_StopReasonFromKoutaku(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:           "compaction stop reason from koutaku",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := ToAnthropicResponsesResponse(ctx, koutakuResp)
...
func TestCompactionContentBlock_NonStreamingRoundTrip(t *testing.T) {
	...
	// Step 1: Anthropic → Koutaku
	koutakuResp := anthropicResp.ToKoutakuResponsesResponse(ctx)

	if koutakuResp.StopReason == nil || *koutakuResp.StopReason != "compaction" {
		t.Fatalf("expected stop_reason='compaction', got %v", koutakuResp.StopReason)
	}
	...
	for _, msg := range koutakuResp.Output {
		if msg.Content != nil {
	...
	if !foundCompaction {
		t.Error("compaction block not found in Koutaku output")
	}

	// Step 2: Koutaku → Anthropic
	result := ToAnthropicResponsesResponse(ctx, koutakuResp)
