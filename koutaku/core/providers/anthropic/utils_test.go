...
func TestAddMissingBetaHeadersToContext_PassthroughWins(t *testing.T) {
	...
	t.Run("passthrough_mcp_header_prevents_auto_inject", func(t *testing.T) {
		...
		ctx.SetValue(schemas.KoutakuContextKeyExtraHeaders, map[string][]string{
		...
		extraHeaders := ctx.Value(schemas.KoutakuContextKeyExtraHeaders).(map[string][]string)
...
func TestGetRequestBodyForResponses_RawBodyStripsFallbacks(t *testing.T) {
	...
	ctx := schemas.NewKoutakuContext(nil, time.Time{})
	ctx.SetValue(schemas.KoutakuContextKeyUseRawRequestBody, true)
	...
	result, koutakuErr := getRequestBodyForResponses(ctx, request, false, nil)
	if koutakuErr != nil {
		t.Fatalf("unexpected error: %v", koutakuErr)
	}
...
func TestApplyMCPToolsetConfigToKoutakuTool(t *testing.T) {
	t.Run("allowlist pattern merges correctly", func(t *testing.T) {
		...
		applyMCPToolsetConfigToKoutakuTool(koutakuTool, toolset)

		if koutakuTool.ResponsesToolMCP.AllowedTools == nil {
			t.Fatal("expected AllowedTools to be set")
		}
		allowedNames := koutakuTool.ResponsesToolMCP.AllowedTools.ToolNames
	...
	t.Run("nil inputs are handled safely", func(t *testing.T) {
		// Should not panic
		applyMCPToolsetConfigToKoutakuTool(nil, nil)
		applyMCPToolsetConfigToKoutakuTool(&schemas.ResponsesTool{}, nil)
	})
}
