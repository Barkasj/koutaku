...
// rather than Koutaku dropping a tool Anthropic has just added).
...
var (
	...
	anthropicFinishReasonToKoutaku = map[AnthropicStopReason]string{
	...
	koutakuToAnthropicFinishReason = map[string]AnthropicStopReason{
...
// MapKoutakuEffortToAnthropic maps a Koutaku effort level to an Anthropic effort level.
// Anthropic supports "low", "medium", "high", "max"; Koutaku also has "minimal" which maps to "low".
func MapKoutakuEffortToAnthropic(effort string) string {
	if effort == "minimal" {
		return "low"
	}
	return effort
...
func getRequestBodyForResponses(ctx *schemas.KoutakuContext, request *schemas.KoutakuResponsesRequest, isStreaming bool, excludeFields []string) ([]byte, *schemas.KoutakuError) {
	...
	if useRawBody, ok := ctx.Value(schemas.KoutakuContextKeyUseRawRequestBody).(bool); ok && useRawBody {
		...
		if convErr != nil {
			return nil, providerUtils.NewKoutakuOperationError(schemas.ErrRequestBodyConversion, convErr)
		}
		if reqBody == nil {
			return nil, providerUtils.NewKoutakuOperationError("request body is not provided", nil)
		}
		...
		if err != nil {
			return nil, providerUtils.NewKoutakuOperationError(schemas.ErrProviderRequestMarshal, fmt.Errorf("failed to marshal request body: %w", err))
		}
...
func AddMissingBetaHeadersToContext(ctx *schemas.KoutakuContext, req *AnthropicMessageRequest, provider schemas.ModelProvider) error {
	...
	if ctx.Value(schemas.KoutakuContextKeyExtraHeaders) == nil {
		extraHeaders = map[string][]string{}
	} else {
		if ctxExtraHeaders, ok := ctx.Value(schemas.KoutakuContextKeyExtraHeaders).(map[string][]string); ok {
	...
	ctx.SetValue(schemas.KoutakuContextKeyExtraHeaders, extraHeaders)
	return nil
}
...
func MergeBetaHeaders(providerExtraHeaders map[string]string, ctx context.Context) []string {
	...
	if ctxHeaders, ok := ctx.Value(schemas.KoutakuContextKeyExtraHeaders).(map[string][]string); ok {
...
func convertChatResponseFormatToTool(ctx *schemas.KoutakuContext, params *schemas.ChatParameters) *AnthropicTool {
...
// This is used when the provider is Vertex, which doesn't support native structured outputs
func convertResponsesTextFormatToTool(ctx *schemas.KoutakuContext, textConfig *schemas.ResponsesTextConfig) *AnthropicTool {
...
// ConvertAnthropicFinishReasonToKoutaku converts provider finish reasons to Koutaku format
func ConvertAnthropicFinishReasonToKoutaku(providerReason AnthropicStopReason) string {
	if koutakuReason, ok := anthropicFinishReasonToKoutaku[providerReason]; ok {
		return koutakuReason
	}
	return string(providerReason)
}

// ConvertKoutakuFinishReasonToAnthropic converts Koutaku finish reasons to provider format
func ConvertKoutakuFinishReasonToAnthropic(koutakuReason string) AnthropicStopReason {
	if providerReason, ok := koutakuToAnthropicFinishReason[koutakuReason]; ok {
		return providerReason
	}
	return AnthropicStopReason(koutakuReason)
}

// ConvertToAnthropicImageBlock converts a Koutaku image block to Anthropic format
...
// ConvertToAnthropicDocumentBlock converts a Koutaku file block to Anthropic document format
...
func (block AnthropicContentBlock) ToKoutakuContentImageBlock() schemas.ChatContentBlock {
...
// It searches backwards through koutakuMessages to find the matching call and updates its action.
func attachWebSearchSourcesToCall(koutakuMessages []schemas.ResponsesMessage, toolUseID string, resultBlock AnthropicContentBlock, includeExtendedFields bool) {
...
func IsClaudeCodeMaxMode(ctx *schemas.KoutakuContext) bool {
	userAgent, _ := ctx.Value(schemas.KoutakuContextKeyUserAgent).(string)
	skipKeySelection, _ := ctx.Value(schemas.KoutakuContextKeySkipKeySelection).(bool)
...
func IsClaudeCodeRequest(ctx *schemas.KoutakuContext) bool {
	if userAgent, ok := ctx.Value(schemas.KoutakuContextKeyUserAgent).(string); ok {
