...
func (a *AgentModeExecutor) ExecuteAgentForChatRequest(
	...
	executeToolFunc func(ctx *schemas.KoutakuContext, request *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error),
	...
	chatResponse, ok := result.(*schemas.KoutakuChatResponse)
	// Should never happen, but just in case
	if !ok {
		return nil, &schemas.KoutakuError{
			IsKoutakuError: false,
			Error: &schemas.ErrorField{
				Message: "Failed to convert result to schemas.KoutakuChatResponse",
			},
		}
	}
...
func (a *AgentModeExecutor) ExecuteAgentForResponsesRequest(
	...
	responsesResponse, ok := result.(*schemas.KoutakuResponsesResponse)
	// Should never happen, but just in case
	if !ok {
		return nil, &schemas.KoutakuError{
			IsKoutakuError: false,
			Error: &schemas.ErrorField{
				Message: "Failed to convert result to schemas.KoutakuResponsesResponse",
			},
		}
	}
...
func (a *AgentModeExecutor) executeAgent(
	...
	executeToolFunc func(ctx *schemas.KoutakuContext, request *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error),
	clientManager ClientManager,
) (interface{}, *schemas.KoutakuError) {
	...
	originalRequestID, ok := ctx.Value(schemas.KoutakuContextKeyRequestID).(string)
	if ok {
		ctx.SetValue(schemas.KoutakuMCPAgentOriginalRequestID, originalRequestID)
	}
	...
	for depth < maxAgentDepth {
		...
		if len(autoExecutableTools) > 0 {
			...
			for _, toolCall := range autoExecutableTools {
				go func(toolCall schemas.ChatAssistantMessageToolCall) {
					...
					toolCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
					toolCtx.SetValue(schemas.KoutakuContextKeyMCPLogID, uuid.New().String())

					// Create MCP request for this tool call
					mcpRequest := &schemas.KoutakuMCPRequest{
		...
		if fetchNewRequestIDFunc != nil {
			...
			if newID != "" {
				ctx.SetValue(schemas.KoutakuContextKeyRequestID, newID)
			}
		}
...
// mergeUsage sums token counts and costs from two KoutakuLLMUsage values.
// Detail sub-fields are summed when both are present; if only one is non-nil it is kept as-is.
func mergeUsage(base, add *schemas.KoutakuLLMUsage) *schemas.KoutakuLLMUsage {
	if add == nil {
		return base
	}
	if base == nil {
	...
	merged := &schemas.KoutakuLLMUsage{
	...
	if base.Cost != nil || add.Cost != nil {
		...
		ac := add.Cost
		if bc == nil {
			bc = &schemas.KoutakuCost{}
		}
		if ac == nil {
			ac = &schemas.KoutakuCost{}
		}
		merged.Cost = &schemas.KoutakuCost{
...
func extractToolCalls(response *schemas.KoutakuChatResponse) []schemas.ChatAssistantMessageToolCall {
...
func buildAllowedAutoExecutionTools(ctx *schemas.KoutakuContext, clientManager ClientManager) ([]string, map[string][]string) {
