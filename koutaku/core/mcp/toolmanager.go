...
import (
	...
	"github.com/koutaku/koutaku/core/mcp/utils"
...
// This allows ToolsManager to run plugin hooks without direct dependency on Koutaku
type PluginPipeline interface {
	RunMCPPreHooks(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, int)
	RunMCPPostHooks(ctx *schemas.KoutakuContext, mcpResp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError, runFrom int) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError)
}
...
type ToolsManager struct {
	...
	// This id is attached to ctx.Value(schemas.KoutakuContextKeyRequestID) in the agent mode.
	// If not provided, same request ID is used for all tool call result messages without any overrides.
	fetchNewRequestIDFunc func(ctx *schemas.KoutakuContext) string
...
// This is used when an externally-created MCPManager is attached to a Koutaku instance
// via SetMCPManager, so the CodeMode can route nested tool calls through Koutaku's plugin hooks.
...
func (m *ToolsManager) GetAvailableTools(ctx *schemas.KoutakuContext) []schemas.ChatTool {
	...
	for clientName, clientTools := range availableToolsPerClient {
		...
		for _, tool := range clientTools {
			if tool.Function != nil && tool.Function.Name != "" && !seenToolNames[tool.Function.Name] {
				...
				schemas.AppendToContextList(ctx, schemas.KoutakuContextKeyMCPAddedTools, tool.Function.Name)
				if !client.ExecutionConfig.IsCodeModeClient {
...
func buildIntegrationDuplicateCheckMap(existingTools []schemas.ChatTool, integrationUserAgent string, _ schemas.Logger) map[string]bool {
	...
	case schemas.ClaudeCLI.Matches(integrationUserAgent):
		...
		// Examples:
		//   mcp__koutaku__executeToolCode -> executeToolCode
		//   mcp__koutaku__listToolFiles -> listToolFiles
		//   mcp__koutaku__readToolFile -> readToolFile
	...
	case schemas.GeminiCLI.Matches(integrationUserAgent):
		...
		// and {tool_name} is Koutaku's full tool name (may contain hyphens and underscores).
		// Extract by stripping "mcp_" then skipping to the first "_" (server name boundary).
		// mcp_koutaku_testing_exa-web_fetch_exa -> testing_exa-web_fetch_exa
		// mcp_koutaku_ctx7-resolve-library-id   -> ctx7-resolve-library-id
		// mcp_koutaku_testing_websets-cancel_enrichment -> testing_websets-cancel_enrichment
		for _, tool := range existingTools {
			if tool.Function != nil && tool.Function.Name != "" {
				existingToolName := tool.Function.Name
	...
	case schemas.QwenCodeCLI.Matches(integrationUserAgent):
		...
		// Hyphens in the original Koutaku tool name are preserved.
		// mcp__koutaku__testing_exa-web_search_exa -> testing_exa-web_search_exa
		// mcp__koutaku__ctx7-resolve-library-id    -> ctx7-resolve-library-id
		for _, tool := range existingTools {
			if tool.Function != nil && tool.Function.Name != "" {
				existingToolName := tool.Function.Name
				if strings.HasPrefix(existingToolName, "mcp__") {
	...
	case schemas.CodexCLI.Matches(integrationUserAgent):
		...
		// but ALL hyphens in the original Koutaku tool name are converted to underscores.
		// Strip "mcp__" then skip past the first "__" to get the all-underscore tool name.
		// mcp__koutaku__testing_exa_web_fetch_exa -> testing_exa_web_fetch_exa
		// mcp__koutaku__ctx7_query_docs           -> ctx7_query_docs
		// Callers must also normalize Koutaku tool names (replace "-" with "_") before lookup.
		for _, tool := range existingTools {
			if tool.Function != nil && tool.Function.Name != "" {
				existingToolName := tool.Function.Name
				if strings.HasPrefix(existingToolName, "mcp__") {
	...
	case schemas.OpenCode.Matches(integrationUserAgent):
		...
		// Strip up to and including the first "_" to extract the Koutaku tool name.
		// koutaku_testing_exa-web_fetch_exa    -> testing_exa-web_fetch_exa
		// koutaku_ctx7-query-docs              -> ctx7-query-docs
		// koutaku_filesystem-create_directory  -> filesystem-create_directory
		for _, tool := range existingTools {
			if tool.Function != nil && tool.Function.Name != "" {
				existingToolName := tool.Function.Name
...
// ParseAndAddToolsToRequest parses the available tools per client and adds them to the Koutaku request.
//
// Parameters:
...
//
// Returns:
//   - *schemas.KoutakuRequest: Koutaku request with MCP tools added
func (m *ToolsManager) ParseAndAddToolsToRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) *schemas.KoutakuRequest {
	...
	integrationUserAgent := ctx.Value(schemas.KoutakuContextKeyUserAgent)
...
//
// Returns:
//   - *schemas.KoutakuMCPResponse: Tool execution result (Chat or Responses format)
//   - error: Any execution error
func (m *ToolsManager) ExecuteTool(ctx *schemas.KoutakuContext, request *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error) {
	...
	extraFields := schemas.KoutakuMCPResponseExtraFields{
	...
	switch request.RequestType {
	case schemas.MCPRequestTypeChatToolCall:
		return &schemas.KoutakuMCPResponse{
...
func (m *ToolsManager) executeToolInternal(ctx *schemas.KoutakuContext, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, string, string, error) {
...
//
// Returns:
//   - *schemas.KoutakuChatResponse: The final response after agent execution
//   - *schemas.KoutakuError: Any error that occurred during agent execution
func (m *ToolsManager) ExecuteAgentForChatRequest(
	ctx *schemas.KoutakuContext,
	req *schemas.KoutakuChatRequest,
	resp *schemas.KoutakuChatResponse,
...
//
// Returns:
//   - *schemas.KoutakuResponsesResponse: The final response after agent execution
//   - *schemas.KoutakuError: Any error that occurred during agent execution
	...
	initRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			ClientInfo: mcp.Implementation{
				Name:    fmt.Sprintf("Koutaku-%s-user", config.Name),
				Version: "1.0.0",
			},
		},
	}
