...
import (
	...
	codemcp "github.com/koutaku/koutaku/core/mcp"
...
func (s *StarlarkCodeMode) handleExecuteToolCode(ctx *schemas.KoutakuContext, toolCall schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, error) {
	toolName := "unknown"
...
func (s *StarlarkCodeMode) executeCode(ctx *schemas.KoutakuContext, code string) ExecutionResult {
	logs := []string{}
...
func (s *StarlarkCodeMode) callMCPTool(ctx *schemas.KoutakuContext, clientName, toolName string, args map[string]interface{}, appendLog func(string)) (interface{}, error) {
	...
	nestedCtx := schemas.NewKoutakuContext(ctx, deadline)
	nestedCtx.SetValue(schemas.KoutakuContextKeyRequestID, newRequestID)
	if originalRequestID != "" {
		nestedCtx.SetValue(schemas.KoutakuContextKeyParentMCPRequestID, originalRequestID)
	}
	...
	if callErr != nil {
		...
		if after, ok := strings.CutPrefix(rawResult, "Error: "); ok {
			...
			mcpResp = &schemas.KoutakuMCPResponse{
	...
	// Run post-hooks
	finalResp, finalErr := pipeline.RunMCPPostHooks(nestedCtx, mcpResp, koutakuErr, preCount)
