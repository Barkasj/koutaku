	...
	initRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			ClientInfo: mcp.Implementation{
				Name:    fmt.Sprintf("Koutaku-%s-verify", config.Name),
				Version: "1.0.0",
			},
		},
	}
...
func (m *MCPManager) RegisterTool(name, description string, toolFunction MCPToolFunction[any], toolSchema schemas.ChatTool) error {
	...
	internalClient, ok := m.clientMap[KoutakuMCPClientKey]
	if !ok {
		return fmt.Errorf("koutaku client not found")
	}

	// Create prefixed tool name for consistency with external tools
	// Format: koutakuInternal-toolName
	prefixedToolName := fmt.Sprintf("%s-%s", KoutakuMCPClientKey, name)
	...
	m.logger.Debug("%s Registering typed tool: %s -> prefixed as %s (client: %s)", MCPLogPrefix, name, prefixedToolName, KoutakuMCPClientKey)
...
func (m *MCPManager) connectToMCPClient(config *schemas.MCPClientConfig) error {
	...
	extInitRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			Capabilities:    mcp.ClientCapabilities{},
			ClientInfo: mcp.Implementation{
				Name:    fmt.Sprintf("Koutaku-%s", config.Name),
				Version: "1.0.0",
			},
		},
	}
	...
	// Start tool syncing for the client (skip for internal koutaku client)
	if config.ID != KoutakuMCPClientKey {
...
func (m *MCPManager) setupLocalHost() error {
	...
	m.server = server
	m.clientMap[KoutakuMCPClientKey] = client
	m.mu.Unlock()
...
func (m *MCPManager) createLocalMCPClient() (*schemas.MCPClientState, error) {
	...
	return &schemas.MCPClientState{
		ExecutionConfig: &schemas.MCPClientConfig{
			ID:             KoutakuMCPClientKey,
			Name:           KoutakuMCPClientKey, // Use same value as ID for consistent prefixing
...
func (m *MCPManager) startLocalMCPServer() error {
	...
	clientEntry, ok := m.clientMap[KoutakuMCPClientKey]
	if !ok {
		return fmt.Errorf("koutaku client not found")
	}
	...
	initRequest := mcp.InitializeRequest{
		Params: mcp.InitializeParams{
			...
			Capabilities:    mcp.ClientCapabilities{},
			ClientInfo: mcp.Implementation{
				Name:    KoutakuMCPClientName,
				Version: KoutakuMCPVersion,
			},
		},
	}
