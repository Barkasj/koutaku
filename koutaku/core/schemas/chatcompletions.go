...
// KoutakuChatRequest is the request struct for chat completion requests
type KoutakuChatRequest struct {
	Provider       ModelProvider   `json:"provider"`
	Model          string          `json:"model"`
	Input          []ChatMessage   `json:"input,omitempty"`
	Params         *ChatParameters `json:"params,omitempty"`
	Fallbacks      []Fallback      `json:"fallbacks,omitempty"`
	RawRequestBody []byte          `json:"-"` // set koutaku-use-raw-request-body to true in ctx to use the raw request body. Koutaku will directly send this to the downstream provider.
}

// GetRawRequestBody returns the raw request body
func (cr *KoutakuChatRequest) GetRawRequestBody() []byte {
	return cr.RawRequestBody
}

func (cr *KoutakuChatRequest) GetExtraParams() map[string]interface{} {
	if cr.Params == nil {
...
// KoutakuChatResponse represents the complete result from a chat completion request.
type KoutakuChatResponse struct {
	ID                string                     `json:"id"`
	Choices           []KoutakuResponseChoice    `json:"choices"`
	Created           int                        `json:"created"` // The Unix timestamp (in seconds).
	Model             string                     `json:"model"`
	Object            string                     `json:"object"` // "chat.completion" or "chat.completion.chunk"
	ServiceTier       *string                    `json:"service_tier,omitempty"`
	SystemFingerprint string                     `json:"system_fingerprint"`
	Usage             *KoutakuLLMUsage           `json:"usage"`
...
func (cr *KoutakuChatResponse) BackfillParams(request *KoutakuChatRequest) {
...
// ToTextCompletionResponse converts a KoutakuChatResponse to a KoutakuTextCompletionResponse
func (cr *KoutakuChatResponse) ToTextCompletionResponse() *KoutakuTextCompletionResponse {
	if cr == nil {
		return nil
	}
...
type ChatStreamOptions struct {
	...
	IncludeUsage       *bool `json:"include_usage,omitempty"` // Koutaku marks this as true by default
}
...
type ChatTool struct {
	...
	Annotations  *MCPToolAnnotations `json:"-"`                       // MCP tool annotations (Koutaku-internal, never forwarded to providers)
...
func (cm *ChatAssistantMessage) UnmarshalJSON(data []byte) error {
	...
	*cm = ChatAssistantMessage(aux.Alias)
	...
	// Map xAI's reasoning_content to Koutaku's Reasoning field
	...
	if cm.Reasoning != nil && len(cm.ReasoningDetails) == 0 {
		...
		cm.ReasoningDetails = []ChatReasoningDetails{
			{
				Index: 0,
				Type:  KoutakuReasoningDetailsTypeText,
				Text:  &text,
			},
		}
	}
...
// KoutakuResponseChoice represents a choice in the completion result.
...
type KoutakuResponseChoice struct {
	Index        int              `json:"index"`
	FinishReason *string          `json:"finish_reason,omitempty"`
	LogProbs     *KoutakuLogProbs `json:"logprobs,omitempty"`
...
// KoutakuFinishReason represents the reason why the model stopped generating.
type KoutakuFinishReason string

// KoutakuFinishReason values
const (
	KoutakuFinishReasonStop      KoutakuFinishReason = "stop"
	KoutakuFinishReasonLength    KoutakuFinishReason = "length"
	KoutakuFinishReasonToolCalls KoutakuFinishReason = "tool_calls"
)

type KoutakuReasoningDetailsType string

const (
	KoutakuReasoningDetailsTypeSummary       KoutakuReasoningDetailsType = "reasoning.summary"
	KoutakuReasoningDetailsTypeEncrypted     KoutakuReasoningDetailsType = "reasoning.encrypted"
	KoutakuReasoningDetailsTypeText          KoutakuReasoningDetailsType = "reasoning.text"
	KoutakuReasoningDetailsTypeContentBlocks KoutakuReasoningDetailsType = "reasoning.content_blocks"
)
...
type ChatReasoningDetails struct {
	ID        *string                     `json:"id,omitempty"`
	Index     int                         `json:"index"`
	Type      KoutakuReasoningDetailsType `json:"type"`
...
// KoutakuLogProbs represents the log probabilities for different aspects of a response.
type KoutakuLogProbs struct {
...
// KoutakuLLMUsage represents token usage information
type KoutakuLLMUsage struct {
	...
	Cost                    *KoutakuCost                 `json:"cost,omitempty"` // Only for the providers which support cost calculation
}
...
// UnmarshalJSON implements custom JSON unmarshalling for KoutakuCost.
func (bc *KoutakuCost) UnmarshalJSON(data []byte) error {
	...
	// Try to unmarshal as a full KoutakuCost struct
	// Use a type alias to avoid infinite recursion
	type Alias KoutakuCost
	var costStruct Alias
	if err := Unmarshal(data, &costStruct); err == nil {
		*bc = KoutakuCost(costStruct)
		return nil
	}
