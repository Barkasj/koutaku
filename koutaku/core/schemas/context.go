...
var reservedKeys = []any{
	KoutakuContextKeyVirtualKey,
	KoutakuContextKeyAPIKeyName,
	KoutakuContextKeyAPIKeyID,
	KoutakuContextKeyRequestID,
	KoutakuContextKeyFallbackRequestID,
	KoutakuContextKeyDirectKey,
	KoutakuContextKeySelectedKeyID,
	KoutakuContextKeySelectedKeyName,
	KoutakuContextKeyNumberOfRetries,
	KoutakuContextKeyFallbackIndex,
	KoutakuContextKeySkipKeySelection,
	KoutakuContextKeyURLPath,
	KoutakuContextKeyDeferTraceCompletion,
	KoutakuContextKeyAttemptTrail,
}

// pluginLogStore holds plugin log entries accumulated during request processing.
// It is shared between the root KoutakuContext and all scoped contexts derived from it.
...
// pluginScopePool pools KoutakuContext instances used as scoped plugin contexts.
var pluginScopePool = sync.Pool{
	New: func() any {
		return &KoutakuContext{}
	},
}

// KoutakuContext is a custom context.Context implementation that tracks user-set values.
// It supports deadlines, can be derived from other contexts, and provides layered
// value inheritance when derived from another KoutakuContext.
type KoutakuContext struct {
	...
	valueDelegate *KoutakuContext                // For scoped contexts: delegate Value/SetValue to this root context
}

// NewKoutakuContext creates a new KoutakuContext with the given parent context and deadline.
...
func NewKoutakuContext(parent context.Context, deadline time.Time) *KoutakuContext {
	if parent == nil {
...
// NewKoutakuContextWithValue creates a new KoutakuContext with the given value set.
func NewKoutakuContextWithValue(parent context.Context, deadline time.Time, key any, value any) *KoutakuContext {
	ctx := NewKoutakuContext(parent, deadline)
	ctx.SetValue(key, value)
	return ctx
}

// NewKoutakuContextWithTimeout creates a new KoutakuContext with a timeout duration.
// This is a convenience wrapper around NewKoutakuContext.
// Returns the context and a cancel function that should be called to release resources.
func NewKoutakuContextWithTimeout(parent context.Context, timeout time.Duration) (*KoutakuContext, context.CancelFunc) {
	ctx := NewKoutakuContext(parent, time.Now().Add(timeout))
	return ctx, func() { ctx.Cancel() }
}

// NewKoutakuContextWithCancel creates a new KoutakuContext with a cancel function.
// This is a convenience wrapper around NewKoutakuContext.
// Returns the context and a cancel function that should be called to release resources.
func NewKoutakuContextWithCancel(parent context.Context) (*KoutakuContext, context.CancelFunc) {
	ctx := NewKoutakuContext(parent, NoDeadline)
	return ctx, func() { ctx.Cancel() }
}

// WithValue returns a new context with the given value set.
func (bc *KoutakuContext) WithValue(key any, value any) *KoutakuContext {
	bc.SetValue(key, value)
	return bc
}

// BlockRestrictedWrites returns true if restricted writes are blocked.
func (bc *KoutakuContext) BlockRestrictedWrites() {
	bc.blockRestrictedWrites.Store(true)
}

// UnblockRestrictedWrites unblocks restricted writes.
func (bc *KoutakuContext) UnblockRestrictedWrites() {
...
func (bc *KoutakuContext) cancel(err error) {
	bc.doneOnce.Do(func() {
		bc.errMu.Lock()
		bc.err = err
		bc.errMu.Unlock()
...
func (bc *KoutakuContext) Deadline() (time.Time, bool) {
...
func (bc *KoutakuContext) Done() <-chan struct{} {
	return bc.done
}
...
func (bc *KoutakuContext) Err() error {
	if bc.valueDelegate != nil {
...
func (bc *KoutakuContext) Value(key any) any {
	if bc.valueDelegate != nil {
...
func (bc *KoutakuContext) SetValue(key, value any) {
	if bc.valueDelegate != nil {
...
// For scoped contexts, delegates to the root context via valueDelegate.
func (bc *KoutakuContext) ClearValue(key any) {
	if bc.valueDelegate != nil {
...
// For scoped contexts, delegates to the root context via valueDelegate.
func (bc *KoutakuContext) GetAndSetValue(key any, value any) any {
	if bc.valueDelegate != nil {
...
func (bc *KoutakuContext) GetUserValues() map[any]any {
	result := make(map[any]any)

	// First, get parent's user values if parent is a PluginContext
	if parentCtx, ok := bc.parent.(*KoutakuContext); ok {
...
func (bc *KoutakuContext) GetParentCtxWithUserValues() context.Context {
	parentCtx := bc.parent
	bc.valuesMu.RLock()
	for k, v := range bc.userValues {
...
func (bc *KoutakuContext) AppendRoutingEngineLog(engineName string, message string) {
	entry := RoutingEngineLogEntry{
		Engine:    engineName,
		Message:   message,
		Timestamp: time.Now().UnixMilli(),
	}
	AppendToContextList(bc, KoutakuContextKeyRoutingEngineLogs, entry)
}
...
func (bc *KoutakuContext) GetRoutingEngineLogs() []RoutingEngineLogEntry {
	if val := bc.Value(KoutakuContextKeyRoutingEngineLogs); val != nil {
...
func AppendToContextList[T any](ctx *KoutakuContext, key KoutakuContextKey, value T) {
	if ctx == nil {
		return
	}
...
// WithPluginScope returns a lightweight scoped KoutakuContext from the pool.
...
func (bc *KoutakuContext) WithPluginScope(name *string) *KoutakuContext {
	...
	scoped := pluginScopePool.Get().(*KoutakuContext)
...
func (bc *KoutakuContext) ReleasePluginScope() {
	if bc.valueDelegate == nil {
...
func (bc *KoutakuContext) Log(level LogLevel, msg string) {
...
func (bc *KoutakuContext) GetPluginLogs() []PluginLogEntry {
	store := bc.pluginLogs.Load()
	if store == nil {
		return nil
	}
...
func (bc *KoutakuContext) DrainPluginLogs() []PluginLogEntry {
	if bc.valueDelegate != nil {
