...
func TestRealtimeCanonicalEventClassifiers(t *testing.T) {
	...
	userEvent := &KoutakuRealtimeEvent{
	...
	toolEvent := &KoutakuRealtimeEvent{
	...
	transcriptEvent := &KoutakuRealtimeEvent{Type: RTEventInputAudioTransCompleted}
	...
	if IsRealtimeInputTranscriptEvent(&KoutakuRealtimeEvent{Type: RTEventInputAudioTransDelta}) {
