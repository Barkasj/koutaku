...
type StreamAccumulatorResult struct {
	...
	TokenUsage            *KoutakuLLMUsage                // Token usage
	Cost                  *float64                        // Cost in dollars
	ErrorDetails          *KoutakuError                   // Error details if any
	AudioOutput           *KoutakuSpeechResponse          // For speech streaming
	TranscriptionOutput   *KoutakuTranscriptionResponse   // For transcription streaming
	ImageGenerationOutput *KoutakuImageGenerationResponse // For image generation streaming
	FinishReason          *string                         // Finish reason
	RawResponse           *string                         // Raw response
	RawRequest            interface{}                     // Raw request
}

// Tracer defines the interface for distributed tracing in Koutaku.
// Implementations can be injected via KoutakuConfig to enable automatic instrumentation.
...
type Tracer interface {
	...
	PopulateLLMRequestAttributes(handle SpanHandle, req *KoutakuRequest)
	...
	PopulateLLMResponseAttributes(ctx *KoutakuContext, handle SpanHandle, resp *KoutakuResponse, err *KoutakuError)
	...
	// Pass the full KoutakuResponse to capture content, tool calls, reasoning, etc.
	// This is called for each streaming chunk to build up the complete response.
	AddStreamingChunk(traceID string, response *KoutakuResponse)
	...
	GetAccumulatedChunks(traceID string) (response *KoutakuResponse, ttftNs int64, chunkCount int)
	...
	ProcessStreamingChunk(traceID string, isFinalChunk bool, result *KoutakuResponse, err *KoutakuError) *StreamAccumulatorResult
...
func (n *NoOpTracer) PopulateLLMRequestAttributes(_ SpanHandle, _ *KoutakuRequest) {}

// PopulateLLMResponseAttributes does nothing.
func (n *NoOpTracer) PopulateLLMResponseAttributes(_ *KoutakuContext, _ SpanHandle, _ *KoutakuResponse, _ *KoutakuError) {
}
...
func (n *NoOpTracer) AddStreamingChunk(_ string, _ *KoutakuResponse) {}

// GetAccumulatedChunks returns nil, 0, 0.
func (n *NoOpTracer) GetAccumulatedChunks(_ string) (*KoutakuResponse, int64, int) { return nil, 0, 0 }
...
func (n *NoOpTracer) ProcessStreamingChunk(_ string, _ bool, _ *KoutakuResponse, _ *KoutakuError) *StreamAccumulatorResult {
	return nil
}
