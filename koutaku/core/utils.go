...
func validateRequest(req *schemas.KoutakuRequest) *schemas.KoutakuError {
	if req == nil {
		return newKoutakuErrorFromMsg("koutaku request cannot be nil")
	}
	provider, model, _ := req.GetRequestFields()
	if provider == "" {
		return newKoutakuErrorFromMsg("provider is required")
	}
	if isModelRequired(req.RequestType) && model == "" {
		return newKoutakuErrorFromMsg("model is required")
	}
	return nil
}
...
func newKoutakuError(err error) *schemas.KoutakuError {
...
// newKoutakuErrorFromMsg creates a KoutakuError with a custom message.
// This helper function is used for static error messages.
func newKoutakuErrorFromMsg(message string) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: false,
		Error: &schemas.ErrorField{
			Message: message,
...
// newKoutakuCtxDoneError creates a KoutakuError from a cancelled/expired context.
// It distinguishes DeadlineExceeded (504 RequestTimedOut) from Canceled (499 RequestCancelled).
func newKoutakuCtxDoneError(ctx *schemas.KoutakuContext, stage string) *schemas.KoutakuError {
	var statusCode int
...
// newKoutakuMessageChan creates a channel that sends a koutaku response.
// It is used to send a koutaku response to the client.
func newKoutakuMessageChan(message *schemas.KoutakuResponse) chan *schemas.KoutakuStreamChunk {
	ch := make(chan *schemas.KoutakuStreamChunk)

	go func() {
		defer close(ch)
		ch <- &schemas.KoutakuStreamChunk{
			KoutakuTextCompletionResponse:      message.TextCompletionResponse,
			KoutakuChatResponse:                message.ChatResponse,
			KoutakuResponsesStreamResponse:     message.ResponsesStreamResponse,
			KoutakuSpeechStreamResponse:        message.SpeechStreamResponse,
			KoutakuTranscriptionStreamResponse: message.TranscriptionStreamResponse,
		}
	}()
...
func clearCtxForFallback(ctx *schemas.KoutakuContext) {
	ctx.ClearValue(schemas.KoutakuContextKeyAPIKeyID)
	ctx.ClearValue(schemas.KoutakuContextKeyAPIKeyName)
	ctx.ClearValue(schemas.KoutakuContextKeyGovernanceIncludeOnlyKeys)
	ctx.ClearValue(schemas.KoutakuContextKeyChangeRequestType)
	ctx.ClearValue(schemas.KoutakuContextKeyAttemptTrail)
	ctx.ClearValue(schemas.KoutakuContextKeyStreamEndIndicator)
}
...
func GetTracerFromContext(ctx *schemas.KoutakuContext) (schemas.Tracer, string, error) {
...
func IsFinalChunk(ctx *schemas.KoutakuContext) bool {
	if ctx == nil {
		return false
	}

	isStreamEndIndicator := ctx.Value(schemas.KoutakuContextKeyStreamEndIndicator)
...
func GetResponseFields(result *schemas.KoutakuResponse, err *schemas.KoutakuError) (requestType schemas.RequestType, provider schemas.ModelProvider, originalModel string, resolvedModel string) {
	if result != nil {
...
func wrapConvertedStreamPostHookRunner(postHookRunner schemas.PostHookRunner, targetType schemas.RequestType) schemas.PostHookRunner {
	return func(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
		if result != nil {
			switch targetType {
			case schemas.ChatCompletionRequest:
				// text→chat: convert chat stream chunk back to text completion
				if result.ChatResponse != nil {
					if converted := result.ChatResponse.ToKoutakuTextCompletionResponse(); converted != nil {
						result = &schemas.KoutakuResponse{TextCompletionResponse: converted}
					}
				}
			case schemas.ResponsesRequest:
				// chat→responses: convert responses stream chunk back to chat
				if result.ResponsesStreamResponse != nil {
					if converted := result.ResponsesStreamResponse.ToKoutakuChatResponse(); converted != nil {
						result = &schemas.KoutakuResponse{ChatResponse: converted}
					}
				}
			}
		}
		return postHookRunner(ctx, result, koutakuErr)
	}
}
