...
import (
	...
	mistralprovider "github.com/koutaku/koutaku/core/providers/mistral"
...
// Helper function to create a KoutakuError
func createKoutakuError(message string, statusCode *int, errorType *string, isKoutakuError bool) *schemas.KoutakuError {
	return &schemas.KoutakuError{
		IsKoutakuError: isKoutakuError,
...
func TestExecuteRequestWithRetries_SuccessScenarios(t *testing.T) {
	...
	ctx.SetValue(schemas.KoutakuContextKeyTracer, &schemas.NoOpTracer{})
	// Test immediate success
	t.Run("ImmediateSuccess", func(t *testing.T) {
		callCount := 0
		handler := func(_ schemas.Key) (string, *schemas.KoutakuError) {
			callCount++
			return "success", nil
		}
	...
	t.Run("SuccessAfterRetries", func(t *testing.T) {
		...
		handler := func(_ schemas.Key) (string, *schemas.KoutakuError) {
			...
			if callCount <= 2 {
				...
				return "", createKoutakuError("rate limit exceeded", Ptr(429), nil, false)
			}
			// Third call succeeds
			return "success", nil
		}
...
func TestExecuteRequestWithRetries_NonRetryableErrors(t *testing.T) {
	...
	testCases := []struct {
		name  string
		error *schemas.KoutakuError
	}{
		{
			name:  "KoutakuError",
			error: createKoutakuError("validation error", nil, nil, true),
		},
		{
			name:  "RequestCancelled",
			error: createKoutakuError("request cancelled", nil, Ptr(schemas.ErrRequestCancelled), false),
		},
		{
			name:  "Non-retryable status code",
			error: createKoutakuError("bad request", Ptr(400), nil, false),
		},
		{
			name:  "Non-retryable error message",
			error: createKoutakuError("invalid model", nil, nil, false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestExecuteRequestWithRetries_RetryableConditions(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:  "StatusCode_500",
			error: createKoutakuError("internal server error", Ptr(500), nil, false),
		},
		{
			name:  "StatusCode_502",
			error: createKoutakuError("bad gateway", Ptr(502), nil, false),
		},
		{
			name:  "StatusCode_503",
			error: createKoutakuError("service unavailable", Ptr(503), nil, false),
		},
		{
			name:  "StatusCode_504",
			error: createKoutakuError("gateway timeout", Ptr(504), nil, false),
		},
		{
			name:  "StatusCode_429",
			error: createKoutakuError("too many requests", Ptr(429), nil, false),
		},
		{
			name:  "ErrProviderDoRequest",
			error: createKoutakuError(schemas.ErrProviderDoRequest, nil, nil, false),
		},
		{
			name:  "RateLimitMessage",
			error: createKoutakuError("rate limit exceeded", nil, nil, false),
		},
		{
			name:  "RateLimitType",
			error: createKoutakuError("some error", nil, Ptr("rate_limit"), false),
		},
	}
	logger := NewDefaultLogger(schemas.LogLevelError)
...
func TestHandleProviderRequest_OCROperationNotAllowed(t *testing.T) {
	...
	request := &ChannelMessage{
		Context: ctx,
		KoutakuRequest: schemas.KoutakuRequest{
			RequestType: schemas.OCRRequest,
			OCRRequest: &schemas.KoutakuOCRRequest{
	...
	response, err := koutaku.handleProviderRequest(provider, providerConfig, request, schemas.Key{}, nil)
	if response != nil {
...
func TestSelectKeyFromProviderForModel_SessionStickiness(t *testing.T) {
	...
	deterministicSelector := func(ctx *schemas.KoutakuContext, keys []schemas.Key, _ schemas.ModelProvider, _ string) (schemas.Key, error) {
		keySelectorCalls++
		return keys[0], nil // always return first key
	}

	ctx := context.Background()
	koutaku, err := Init(ctx, schemas.KoutakuConfig{
		Account:     account,
		Logger:      NewDefaultLogger(schemas.LogLevelError),
		KVStore:     kvStore,
	...
	bfCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	bfCtx.SetValue(schemas.KoutakuContextKeySessionID, "sess-123")

	// First call: cache miss, keySelector runs, key stored; returns single-element pool (canRotate=false)
	keys1, canRotate1, err := koutaku.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
	...
	keys2, canRotate2, err := koutaku.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if err != nil {
...
func TestSelectKeyFromProviderForModel_NoStickinessWithoutSessionID(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		pool, canRotate, err := koutaku.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
		if err != nil {
...
func TestSelectKeyFromProviderForModel_SessionStickinessNoRotation(t *testing.T) {
	...
	bfCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	bfCtx.SetValue(schemas.KoutakuContextKeySessionID, "sess-sticky")
	bfCtx.SetValue(schemas.KoutakuContextKeyTracer, &schemas.NoOpTracer{})

	config := createTestConfig(3, 0, 0)
	logger := NewDefaultLogger(schemas.LogLevelError)

	// Build keyProvider the same way requestWorker does.
	pool, canRotate, poolErr := koutaku.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
	if poolErr != nil {
	...
	callCount := 0
	handler := func(k schemas.Key) (string, *schemas.KoutakuError) {
...
func TestSelectKeyFromProviderForModel_BlacklistedModels(t *testing.T) {
	...
	t.Run("all keys blacklist model", func(t *testing.T) {
		...
		_, _, err := koutaku.selectKeyFromProviderForModelWithPool(bfCtx, schemas.ChatCompletionRequest, schemas.OpenAI, "gpt-4", schemas.OpenAI)
		if err == nil {
...
func TestExecuteRequestWithRetries_KeyRotation(t *testing.T) {
	...
	t.Run("SameKeyOnNetworkError", func(t *testing.T) {
		...
		handler := func(k schemas.Key) (string, *schemas.KoutakuError) {
			...
			callCount++
			if callCount <= 2 {
				return "", createKoutakuError(schemas.ErrProviderDoRequest, nil, nil, false)
			}
			return "success", nil
		}
	...
	t.Run("NilKeyProviderUsesZeroKey", func(t *testing.T) {
		cleanCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
		cleanCtx.SetValue(schemas.KoutakuContextKeyTracer, &schemas.NoOpTracer{})
		...
		if trail, ok := cleanCtx.Value(schemas.KoutakuContextKeyAttemptTrail).([]schemas.KeyAttemptRecord); ok && len(trail) > 0 {
			t.Fatalf("expected no attempt trail for nil keyProvider, got %v", trail)
		}
		if selectedID, _ := cleanCtx.Value(schemas.KoutakuContextKeySelectedKeyID).(string); selectedID != "" {
			t.Fatalf("expected empty selected key id, got %q", selectedID)
		}
		if selectedName, _ := cleanCtx.Value(schemas.KoutakuContextKeySelectedKeyName).(string); selectedName != "" {
...
func TestUpdateProvider(t *testing.T) {
	t.Run("SuccessfulUpdate", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("Failed to initialize Koutaku: %v", err)
		}

		// Verify initial provider exists
		initialProvider := koutaku.getProviderByKey(schemas.OpenAI)
		...
		// Perform update
		err = koutaku.UpdateProvider(schemas.OpenAI)
		if err != nil {
			t.Fatalf("UpdateProvider failed: %v", err)
		}

		// Verify provider was replaced
		updatedProvider := koutaku.getProviderByKey(schemas.OpenAI)
	...
	t.Run("UpdateNonExistentProvider", func(t *testing.T) {
		...
		err = koutaku.UpdateProvider(schemas.Anthropic)
		if err == nil {
	...
	t.Run("UpdateInactiveProvider", func(t *testing.T) {
		// Setup account with provider but don't initialize it in Koutaku
		account := NewMockAccount()
		...
		if koutaku.getProviderByKey(schemas.Ollama) != nil {
			t.Fatal("Provider should not exist initially")
		}

		// Add provider to account after koutaku initialization
	...
	t.Run("MultipleProviderUpdates", func(t *testing.T) {
		...
		initialOpenAI := koutaku.getProviderByKey(schemas.OpenAI)
		initialAnthropic := koutaku.getProviderByKey(schemas.Anthropic)
		initialCohere := koutaku.getProviderByKey(schemas.Cohere)
		...
		for _, provider := range providers {
			err = koutaku.UpdateProvider(provider)
			if err != nil {
		...
		newOpenAI := koutaku.getProviderByKey(schemas.OpenAI)
		newAnthropic := koutaku.getProviderByKey(schemas.Anthropic)
		newCohere := koutaku.getProviderByKey(schemas.Cohere)
	...
	t.Run("ConcurrentProviderUpdates", func(t *testing.T) {
		...
		provider := koutaku.getProviderByKey(schemas.OpenAI)
		if provider == nil {
...
func TestUpdateProvider_ProviderSliceIntegrity(t *testing.T) {
	t.Run("ProviderSliceConsistency", func(t *testing.T) {
		...
		initialProviders := koutaku.providers.Load()
		...
		updatedProviders := koutaku.providers.Load()
...
// Koutaku instance required. They validate the core safety invariants that
...
func newTestChannelMessage(ctx *schemas.KoutakuContext) *ChannelMessage {
	return &ChannelMessage{
		KoutakuRequest: schemas.KoutakuRequest{
			RequestType: schemas.ChatCompletionRequest,
			ChatRequest: &schemas.KoutakuChatRequest{
				Provider: schemas.OpenAI,
				Model:    "gpt-4",
			},
		},
		Context:  ctx,
		Response: make(chan *schemas.KoutakuResponse, 1),
		Err:      make(chan schemas.KoutakuError, 1),
	}
}
...
drainLoop:
	...
	for i, msg := range msgs {
		select {
		case koutakuErr := <-msg.Err:
			if koutakuErr.Error == nil {
				t.Errorf("message %d: received nil Error field", i)
				continue
			}
			if koutakuErr.Error.Message != "provider is shutting down" {
				t.Errorf("message %d: expected 'provider is shutting down', got %q",
					i, koutakuErr.Error.Message)
			}
			if koutakuErr.ExtraFields.Provider != schemas.OpenAI {
				t.Errorf("message %d: expected provider %s, got %s",
					i, schemas.OpenAI, koutakuErr.ExtraFields.Provider)
			}
			if koutakuErr.ExtraFields.RequestType != schemas.ChatCompletionRequest {
				t.Errorf("message %d: expected requestType %v, got %v",
					i, schemas.ChatCompletionRequest, koutakuErr.ExtraFields.RequestType)
			}
		default:
...
//
//	if pq.isClosing() {
//	    if newPq, err := koutaku.getProviderQueue(provider); err == nil && newPq != pq {
//	        pq = newPq   // transparent re-route
//	    }
//	}
//
// This test exercises that exact sequence without a full Koutaku instance.
...
drainLoop:
	...
	for i, msg := range msgs {
		...
		case koutakuErr := <-msg.Err:
			if koutakuErr.Error == nil {
				t.Errorf("message %d: got nil Error field in KoutakuError", i)
				continue
			}
			if koutakuErr.Error.Message != "provider is shutting down" {
				t.Errorf("message %d: expected 'provider is shutting down', got %q",
