...
func RunTranscriptionStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("TranscriptionStream", func(t *testing.T) {
		...
		streamRoundTripCases := []struct {
			...
			},
			{
				...
				voiceType:      "secondary",
				format:         "mp3",
				responseFormat: koutaku.Ptr("json"),
			},
			{
		...
		for _, tc := range streamRoundTripCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				ttsRequest := &schemas.KoutakuSpeechRequest{
				...
				ttsResponse, err := WithSpeechTestRetry(t, ttsSpeechRetryConfig, ttsRetryContext, ttsExpectations, "TranscriptionStream_TTS", func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {
				...
				streamRequest := &schemas.KoutakuTranscriptionRequest{
					...
					Params: &schemas.TranscriptionParameters{
						Language:       koutaku.Ptr("en"),
						Format:         koutaku.Ptr(tc.format),
				...
				lastResponse := &schemas.KoutakuStreamChunk{}
				streamErrors := []string{}
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.KoutakuTranscriptionStreamResponse == nil {
							streamErrors = append(streamErrors, "Stream response missing transcription stream payload")
							continue
						}

						if response.KoutakuTranscriptionStreamResponse != nil {
							lastTokenLatency = response.KoutakuTranscriptionStreamResponse.ExtraFields.Latency
						}

						if response.KoutakuTranscriptionStreamResponse.Text == "" && response.KoutakuTranscriptionStreamResponse.Delta == nil {
							streamErrors = append(streamErrors, "Stream response missing transcription data")
							continue
						}

						chunkIndex := response.KoutakuTranscriptionStreamResponse.ExtraFields.ChunkIndex

						// Log latency for each chunk (can be 0 for inter-chunks)
						t.Logf("📊 Transcription chunk %d latency: %d ms", chunkIndex, response.KoutakuTranscriptionStreamResponse.ExtraFields.Latency)

						// Collect transcription chunks
						transcribeData := response.KoutakuTranscriptionStreamResponse
						if transcribeData.Text != "" {
							t.Logf("✅ Received transcription text chunk %d with latency %d ms: '%s'", chunkIndex, response.KoutakuTranscriptionStreamResponse.ExtraFields.Latency, transcribeData.Text)
						}
						...
						if transcribeData.Delta != nil {
							...
							t.Logf("✅ Received transcription delta chunk %d with latency %d ms: '%s'", chunkIndex, response.KoutakuTranscriptionStreamResponse.ExtraFields.Latency, deltaText)
						}

						// Validate chunk structure
						if response.KoutakuTranscriptionStreamResponse.Type != schemas.TranscriptionStreamResponseTypeDelta {
							t.Logf("⚠️ Unexpected object type in stream: %s", response.KoutakuTranscriptionStreamResponse.Type)
						}
						gotModel := response.KoutakuTranscriptionStreamResponse.ExtraFields.OriginalModelRequested
						if gotModel == "" {
...
func RunTranscriptionStreamAdvancedTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.TranscriptionStream {
		t.Logf("Transcription streaming not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("TranscriptionStreamAdvanced", func(t *testing.T) {
			...
			for _, lang := range languages {
				t.Run("StreamLang_"+lang, func(t *testing.T) {
					...
					for response := range responseChannel {
						...
						if response.KoutakuError != nil {
							streamErrors = append(streamErrors, fmt.Sprintf("Error in stream for language %s: %s", lang, FormatErrorConcise(ParseKoutakuError(response.KoutakuError))))
							continue
						}
		...
		t.Run("WithCustomPrompt_Streaming", func(t *testing.T) {
			...
			request := &schemas.KoutakuTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language: koutaku.Ptr("en"),
					Prompt:   koutaku.Ptr("This audio contains technical terms, proper nouns, and streaming-related vocabulary."),
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
			...
			for response := range responseChannel {
				...
				if response.KoutakuTranscriptionStreamResponse != nil && response.KoutakuTranscriptionStreamResponse.Text != "" {
					chunkCount++
					chunkText := response.KoutakuTranscriptionStreamResponse.Text
