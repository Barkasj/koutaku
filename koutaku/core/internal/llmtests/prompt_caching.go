	...
	return []schemas.ChatTool{
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Retrieve account information for a given account ID"),
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Calculate the cost for cloud resource usage based on service type and quantity"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Check the current status and health of the API service"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Create a new support ticket for a customer issue"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Retrieve documentation for a specific service or feature"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Get a list of available cloud regions for deployment"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("service_type", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Estimate the monthly cost for a cloud deployment configuration"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Validate an API key and return its permissions and status"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Retrieve usage analytics and metrics for an account"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
						schemas.KV("account_id", map[string]interface{}{
		...
		},
		{
			...
			Function: &schemas.ChatToolFunction{
				...
				Description: koutaku.Ptr("Check compliance status and certifications for security and regulatory requirements"),
				Parameters: &schemas.ToolFunctionParameters{
					Type: "object",
					Properties: schemas.NewOrderedMapFromPairs(
...
// content blocks survives the Koutaku round-trip (Anthropic format -> Koutaku ResponsesMessage -> Provider format).
...
func RunPromptCachingToolBlocksTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PromptCachingToolBlocks", func(t *testing.T) {
		...
		systemMsg := schemas.ResponsesMessage{
			Type: koutaku.Ptr(schemas.ResponsesMessageTypeMessage),
			Role: koutaku.Ptr(schemas.ResponsesInputMessageRoleSystem),
			Content: &schemas.ResponsesMessageContent{
				ContentBlocks: []schemas.ResponsesMessageContentBlock{
					{
						Type:         schemas.ResponsesInputMessageContentBlockTypeText,
						Text:         koutaku.Ptr(longSharedPrefix),
		...
		userMsg1 := schemas.ResponsesMessage{
			Type: koutaku.Ptr(schemas.ResponsesMessageTypeMessage),
			Role: koutaku.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: koutaku.Ptr("What's the weather in San Francisco?"),
			},
		}

		// Assistant responds with a tool call (function_call with cache_control)
		toolCallMsg := schemas.ResponsesMessage{
			Type:         koutaku.Ptr(schemas.ResponsesMessageTypeFunctionCall),
			Status:       koutaku.Ptr("completed"),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID:    koutaku.Ptr("call_weather_001"),
				Name:      koutaku.Ptr("weather"),
				Arguments: koutaku.Ptr(`{"location":"San Francisco"}`),
			},
		}

		// Tool result (function_call_output with cache_control)
		toolResultMsg := schemas.ResponsesMessage{
			Type:         koutaku.Ptr(schemas.ResponsesMessageTypeFunctionCallOutput),
			CacheControl: cacheControl,
			ResponsesToolMessage: &schemas.ResponsesToolMessage{
				CallID: koutaku.Ptr("call_weather_001"),
				Output: &schemas.ResponsesToolMessageOutputStruct{
					ResponsesToolCallOutputStr: koutaku.Ptr(`{"temperature": 18, "unit": "celsius", "condition": "partly cloudy", "humidity": 72}`),
				},
			},
		}
		...
		userMsg2 := schemas.ResponsesMessage{
			...
			Role: koutaku.Ptr(schemas.ResponsesInputMessageRoleUser),
			Content: &schemas.ResponsesMessageContent{
				ContentStr: koutaku.Ptr("Summarize the weather information you received."),
			},
		}
		...
		responsesReq := &schemas.KoutakuResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: koutaku.Ptr(200),
			},
		}

		// Enable raw request capture so we can inspect the outgoing provider request
		rawCtx := context.WithValue(ctx, schemas.KoutakuContextKeySendBackRawRequest, true)
		...
		operation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(rawCtx, schemas.NoDeadline)
		...
		require.NotNil(t, rawReq, "Raw request should be present (KoutakuContextKeySendBackRawRequest was set)")
...
// Vertex caching bug where Koutaku's key reordering in tool_use input fields caused
...
func RunPromptCachingMultipleToolCallsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.PromptCaching {
	...
	t.Run("PromptCachingMultipleToolCalls", func(t *testing.T) {
		...
		makeAssistantMsg := func(text string) schemas.ResponsesMessage {
			return schemas.ResponsesMessage{
				Type: koutaku.Ptr(schemas.ResponsesMessageTypeMessage),
				Role: koutaku.Ptr(schemas.ResponsesInputMessageRoleAssistant),
				Content: &schemas.ResponsesMessageContent{
					ContentStr: koutaku.Ptr(text),
				},
			}
		...
		makeToolCall := func(callID, name, args string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				Status: koutaku.Ptr("completed"),
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID:    koutaku.Ptr(callID),
					Name:      koutaku.Ptr(name),
					Arguments: koutaku.Ptr(args),
				},
			}
			if withCacheControl {
		...
		makeToolResult := func(callID, output string, withCacheControl bool) schemas.ResponsesMessage {
			msg := schemas.ResponsesMessage{
				...
				ResponsesToolMessage: &schemas.ResponsesToolMessage{
					CallID: koutaku.Ptr(callID),
					Output: &schemas.ResponsesToolMessageOutputStruct{
						ResponsesToolCallOutputStr: koutaku.Ptr(output),
					},
				},
			}
			if withCacheControl {
		...
		// If Koutaku re-orders these keys, the cache prefix diverges at this point.
...
func RunPromptCachingTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for i, query := range testQueries {
			t.Run(query.name, func(t *testing.T) {
				userMessage := schemas.ChatMessage{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: koutaku.Ptr(query.message),
					},
				}
				...
				chatReq := &schemas.KoutakuChatRequest{
					...
					Params: &schemas.ChatParameters{
						...
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: koutaku.Ptr("auto"),
						},
					},
...
func RunPromptCachingMultiTurnTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SimpleChat {
			...
			t.Run(turnName, func(t *testing.T) {
				...
				chatReq := &schemas.KoutakuChatRequest{
					...
					Params: &schemas.ChatParameters{
						Tools: tools,
						ToolChoice: &schemas.ChatToolChoice{
							ChatToolChoiceStr: koutaku.Ptr("none"),
						},
					},
					Fallbacks: testConfig.Fallbacks,
				}
