...
func RunEmbeddingTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Embedding", func(t *testing.T) {
		...
		request := &schemas.KoutakuEmbeddingRequest{
			...
			Params: &schemas.EmbeddingParameters{
				EncodingFormat: koutaku.Ptr("float"),
			},
		...
		embeddingResponse, koutakuErr := WithEmbeddingTestRetry(t, embeddingRetryConfig, retryContext, expectations, "Embedding", func() (*schemas.KoutakuEmbeddingResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			return client.EmbeddingRequest(bfCtx, request)
		})

		if koutakuErr != nil {
			t.Fatalf("❌ Embedding request failed after retries: %v", GetErrorMessage(koutakuErr))
		}
...
func validateEmbeddingSemantics(t *testing.T, response *schemas.KoutakuEmbeddingResponse, testTexts []string) {
