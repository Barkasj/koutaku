...
func RunChatCompletionStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ChatCompletionStream", func(t *testing.T) {
		...
		for {
			...
			case response, ok := <-responseChannel:
				...
				if response.KoutakuChatResponse != nil {
					if response.KoutakuChatResponse.ExtraFields.Provider != testConfig.Provider {
						t.Logf("⚠️ Warning: Provider mismatch - expected %s, got %s", testConfig.Provider, response.KoutakuChatResponse.ExtraFields.Provider)
					}
					if response.KoutakuChatResponse.ID == "" {
						t.Logf("⚠️ Warning: Response ID is empty")
					}

					// Per-chunk Object validation: koutaku normalizes every streaming chunk
					// to the OpenAI shape with Object="chat.completion.chunk", whether the
					// upstream provider natively emits it (OpenAI family) or koutaku
					...
					if response.KoutakuChatResponse.Object != "chat.completion.chunk" {
						t.Errorf("Chunk %d: Object field must be 'chat.completion.chunk', got %q", responseCount+1, response.KoutakuChatResponse.Object)
					}

					// Log latency for each chunk (can be 0 for inter-chunks)
					t.Logf("📊 Chunk %d latency: %d ms", responseCount+1, response.KoutakuChatResponse.ExtraFields.Latency)

					// Process each choice in the response
					for _, choice := range response.KoutakuChatResponse.Choices {
	...
	streamComplete:
		...
		consolidatedResponse := &schemas.KoutakuChatResponse{
		...
		if lastResponse != nil && lastResponse.KoutakuChatResponse != nil {
			consolidatedResponse.Usage = lastResponse.KoutakuChatResponse.Usage
			consolidatedResponse.Model = lastResponse.KoutakuChatResponse.Model
			consolidatedResponse.ID = lastResponse.KoutakuChatResponse.ID
			consolidatedResponse.Created = lastResponse.KoutakuChatResponse.Created

			// Copy finish reason from last choice if available
			if len(lastResponse.KoutakuChatResponse.Choices) > 0 && lastResponse.KoutakuChatResponse.Choices[0].FinishReason != nil {
				consolidatedResponse.Choices[0].FinishReason = lastResponse.KoutakuChatResponse.Choices[0].FinishReason
			}
			consolidatedResponse.ExtraFields = lastResponse.KoutakuChatResponse.ExtraFields
		}
	...
	if testConfig.Scenarios.ToolCalls {
		t.Run("ChatCompletionStreamWithTools", func(t *testing.T) {
			...
			validationResult := WithChatStreamValidationRetry(
				...
				func(responseChannel chan *schemas.KoutakuStreamChunk) ChatStreamValidationResult {
					...
					for {
						...
						case response, ok := <-responseChannel:
							...
							if response == nil || response.KoutakuChatResponse == nil {
							...
							if response.KoutakuChatResponse.Choices != nil {
								for _, choice := range response.KoutakuChatResponse.Choices {
									if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
										delta := choice.ChatStreamResponseChoice.Delta
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ChatCompletionStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.KoutakuChatRequest{
				...
				Input:    messages,
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(1800),
					Reasoning: &schemas.ChatReasoning{
						Effort:    koutaku.Ptr("high"),
						MaxTokens: koutaku.Ptr(1500),
					},
				},
				Fallbacks: testConfig.Fallbacks,
			}
			...
			for {
				...
				case response, ok := <-responseChannel:
					...
					if response.KoutakuChatResponse != nil {
						chatResp := response.KoutakuChatResponse
								...
								if choice.ChatStreamResponseChoice != nil && choice.ChatStreamResponseChoice.Delta != nil {
									...
									if len(delta.ReasoningDetails) > 0 {
										...
										for _, detail := range delta.ReasoningDetails {
											...
											switch detail.Type {
											case schemas.KoutakuReasoningDetailsTypeText:
											...
											case schemas.KoutakuReasoningDetailsTypeSummary:
												if detail.Summary != nil {
													t.Logf("    Summary length: %d", len(*detail.Summary))
												}
											case schemas.KoutakuReasoningDetailsTypeEncrypted:
												if detail.Data != nil {
		...
		t.Run("ChatCompletionStreamWithReasoningValidated", func(t *testing.T) {
			...
			if testConfig.Provider == schemas.Cerebras {
				problemPrompt = "Hello how are you, can you search hackernews news regarding koutaku for me? use your tools for this"
			}
