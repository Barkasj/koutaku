...
func TestProviderWebSearchVersionSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := anthropic.ToAnthropicResponsesRequest(ctx, koutakuReq)
...
func TestProviderAnthropicRequestPipeline(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 2: Convert koutaku request → anthropic request
			koutakuReq := &schemas.KoutakuResponsesRequest{
				Provider: tt.provider,
				Model:    model,
				Input: []schemas.ResponsesMessage{
...
// koutaku tool → anthropic tool version → correct beta header
func TestComputerUseVersionAndBetaHeaderEndToEnd(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Step 1: Convert koutaku tool → anthropic tool (selects version based on model)
			koutakuReq := &schemas.KoutakuResponsesRequest{
				Provider: tt.provider,
				Model:    tt.model,
				Input: []schemas.ResponsesMessage{
