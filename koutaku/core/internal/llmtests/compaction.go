...
// forwarded through Koutaku via the Responses API.
//
...
//  1. The context_management field survives the Koutaku request round-trip
//  2. The compact-2026-01-12 beta header is properly sent
//  3. The API accepts the request without error (non-streaming + streaming)
func RunCompactionTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
			...
			for {
				...
				case chunk, ok := <-responseChan:
					...
					if chunk.KoutakuResponsesStreamResponse != nil {
						if chunk.KoutakuResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCreated {
							hasCreated = true
						}
						if chunk.KoutakuResponsesStreamResponse.Type == schemas.ResponsesStreamResponseTypeCompleted {
							hasCompleted = true
						}
						if chunk.KoutakuResponsesStreamResponse.Delta != nil {
							fullContent.WriteString(*chunk.KoutakuResponsesStreamResponse.Delta)
						}
					}
				case <-streamCtx.Done():
