...
func RunPassthroughExtraParamsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughExtraParams", func(t *testing.T) {
		...
		// Create a Koutaku context with passthrough extraParams enabled
		bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
		bfCtx.SetValue(schemas.KoutakuContextKeyPassthroughExtraParams, true)
		bfCtx.SetValue(schemas.KoutakuContextKeySendBackRawRequest, true)
		...
		if response.ExtraFields.RawRequest == nil {
			...
			t.Logf("   Check Koutaku logs for the raw request body sent to provider")
