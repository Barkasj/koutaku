...
func RunSpeechSynthesisTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("SpeechSynthesis", func(t *testing.T) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				speechResponse, koutakuErr := WithSpeechTestRetry(t, speechRetryConfig, retryContext, expectations, "SpeechSynthesis_"+tc.name, func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {
					requestCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
					return client.SpeechRequest(requestCtx, request)
				})

				if koutakuErr != nil {
					t.Fatalf("❌ SpeechSynthesis_"+tc.name+" request failed after retries: %v", GetErrorMessage(koutakuErr))
				}
...
func RunSpeechSynthesisAdvancedTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SpeechSynthesis {
		t.Logf("Speech synthesis not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("SpeechSynthesisAdvanced", func(t *testing.T) {
		...
		t.Run("LongText_HDModel", func(t *testing.T) {
			...
			speechResponse, koutakuErr := WithSpeechTestRetry(t, speechRetryConfig, retryContext, expectations, "SpeechSynthesis_HD", func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {
				requestCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
				return client.SpeechRequest(requestCtx, request)
			})
			if koutakuErr != nil {
				t.Fatalf("❌ SpeechSynthesis_HD request failed after retries: %v", GetErrorMessage(koutakuErr))
			}
		...
		t.Run("AllVoiceOptions", func(t *testing.T) {
			...
			for _, voiceType := range voiceTypes {
				t.Run("VoiceType_"+voiceType, func(t *testing.T) {
					...
					speechResponse, koutakuErr := WithSpeechTestRetry(t, voiceSpeechRetryConfig, voiceRetryContext, expectations, "SpeechSynthesis_VoiceType_"+voiceType, func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {
						requestCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
						return client.SpeechRequest(requestCtx, request)
					})

					if koutakuErr != nil {
						t.Fatalf("❌ SpeechSynthesis_Voice_"+voiceType+" request failed after retries: %v", GetErrorMessage(koutakuErr))
					}
...
func validateSpeechSynthesisSpecific(t *testing.T, response *schemas.KoutakuSpeechResponse, expectMinBytes int, expectedModel string) {
	if response == nil {
