...
func RunImageVariationTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageVariation", func(t *testing.T) {
		...
		imageVariationOperation := func() (*schemas.KoutakuImageGenerationResponse, *schemas.KoutakuError) {
			request := &schemas.KoutakuImageVariationRequest{
				...
				Params: &schemas.ImageVariationParameters{
					...
					N:    koutaku.Ptr(2), // Generate 2 variations
				},
				Fallbacks: testConfig.ImageVariationFallbacks,
			}

			response, err := client.ImageVariationRequest(schemas.NewKoutakuContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageVariationStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
