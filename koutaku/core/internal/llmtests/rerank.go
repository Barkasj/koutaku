...
func BasicRerankExpectations(t *testing.T, rerankResponse *schemas.KoutakuRerankResponse, documents []schemas.RerankDocument) {
	t.Helper()
...
func RunRerankTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Rerank", func(t *testing.T) {
		...
		request := &schemas.KoutakuRerankRequest{
			...
			Params: &schemas.RerankParameters{
				ReturnDocuments: koutaku.Ptr(true),
			},
			Fallbacks: testConfig.RerankFallbacks,
		}

		bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
		rerankResponse, koutakuErr := client.RerankRequest(bfCtx, request)

		if koutakuErr != nil {
			t.Fatalf("❌ Rerank request failed: %v", GetErrorMessage(koutakuErr))
		}
