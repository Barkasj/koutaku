...
func RunMultipleToolCallsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("MultipleToolCalls", func(t *testing.T) {
		...
		validateChatMultipleToolCalls := func(response *schemas.KoutakuChatResponse, apiName string) {
		...
		validateResponsesMultipleToolCalls := func(response *schemas.KoutakuResponsesResponse, apiName string) {
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response == nil || response.KoutakuChatResponse == nil {
							...
							if response != nil && response.KoutakuError != nil {
								errMsg += fmt.Sprintf(" - error: %s", GetErrorMessage(response.KoutakuError))
							}
