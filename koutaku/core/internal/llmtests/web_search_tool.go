...
func RunWebSearchToolTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		webSearchTool := &schemas.ResponsesTool{
			...
			ResponsesToolWebSearch: &schemas.ResponsesToolWebSearch{
				UserLocation: &schemas.ResponsesToolWebSearchUserLocation{
					Type:    koutaku.Ptr("approximate"),
					Country: koutaku.Ptr("US"),
					City:    koutaku.Ptr("New York"),
				},
			},
		}
...
func RunWebSearchToolStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolStream", func(t *testing.T) {
		...
		webSearchTool := &schemas.ResponsesTool{
			...
			ResponsesToolWebSearch: &schemas.ResponsesToolWebSearch{
				UserLocation: &schemas.ResponsesToolWebSearchUserLocation{
					Type:     koutaku.Ptr("approximate"),
					Country:  koutaku.Ptr("US"),
					City:     koutaku.Ptr("San Francisco"),
					Region:   koutaku.Ptr("California"),
					Timezone: koutaku.Ptr("America/Los_Angeles"),
				},
			},
		}
		...
		request := &schemas.KoutakuResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				Tools:           []schemas.ResponsesTool{*webSearchTool},
				MaxOutputTokens: koutaku.Ptr(1500),
			},
			Fallbacks: testConfig.Fallbacks,
		}
		...
		validationResult := WithResponsesStreamValidationRetry(t, retryConfig, retryContext,
			...
			func(responseChannel chan *schemas.KoutakuStreamChunk) ResponsesStreamValidationResult {
				...
				for {
					...
					case stream, ok := <-responseChannel:
						...
						if stream.KoutakuResponsesStreamResponse != nil {
							streamType := stream.KoutakuResponsesStreamResponse.Type

							// Check for output_item.added with web_search_call
							if streamType == schemas.ResponsesStreamResponseTypeOutputItemAdded {
								if stream.KoutakuResponsesStreamResponse.Item != nil {
									if stream.KoutakuResponsesStreamResponse.Item.Type != nil &&
										*stream.KoutakuResponsesStreamResponse.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
										hasWebSearchCall = true
										t.Logf("✅ Found web_search_call in streaming event: %s", streamType)

										// Extract query and sources if available
										if stream.KoutakuResponsesStreamResponse.Item.ResponsesToolMessage != nil &&
											stream.KoutakuResponsesStreamResponse.Item.ResponsesToolMessage.Action != nil {
											action := stream.KoutakuResponsesStreamResponse.Item.ResponsesToolMessage.Action
											if action.ResponsesWebSearchToolCallAction != nil {
							...
							if streamType == schemas.ResponsesStreamResponseTypeOutputTextDelta {
								if stream.KoutakuResponsesStreamResponse.Delta != nil && *stream.KoutakuResponsesStreamResponse.Delta != "" {
									hasMessageContent = true
									t.Logf("✅ Found message text delta: %s", *stream.KoutakuResponsesStreamResponse.Delta)
								}
							}
						}
...
func RunWebSearchToolWithDomainsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolWithDomains", func(t *testing.T) {
		...
		responsesOperation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			...
			responsesReq := &schemas.KoutakuResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					Tools:           []schemas.ResponsesTool{*webSearchTool},
					MaxOutputTokens: koutaku.Ptr(1200),
				},
				Fallbacks: testConfig.Fallbacks,
			}
...
func RunWebSearchToolContextSizesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
...
func RunWebSearchToolMultiTurnTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolMultiTurn", func(t *testing.T) {
		...
		firstOperation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.KoutakuResponsesRequest{
				Provider: testConfig.Provider,
				Model:    testConfig.ChatModel,
		...
		secondOperation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.KoutakuResponsesRequest{
				Provider: testConfig.Provider,
				Model:    testConfig.ChatModel,
...
func RunWebSearchToolMaxUsesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.WebSearchTool {
		t.Logf("Web search tool not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("WebSearchToolMaxUses", func(t *testing.T) {
		...
		responsesOperation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			...
			responsesReq := &schemas.KoutakuResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					Tools:           []schemas.ResponsesTool{*webSearchTool},
					MaxOutputTokens: koutaku.Ptr(2000),
				},
				Fallbacks: testConfig.Fallbacks,
			}
