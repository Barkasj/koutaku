...
func TestConcurrent_MultipleToolExecutions(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id int) {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, koutakuErr)
				return
			}
...
func TestConcurrent_DifferentTools(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("tool type %d execution %d failed: %v", tType, id, koutakuErr)
				return
			}
...
func TestConcurrent_AddClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 20; i++ {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", i, koutakuErr)
			}
...
func TestConcurrent_RemoveClientDuringExecution(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if koutakuErr != nil {
				t.Logf("Execution %d failed (expected after removal): %v", i, koutakuErr)
			}
			executions <- true
	...
	go func() {
		...
		for i := 0; i < 10; i++ {
			...
			if len(clients) > 0 && len(clients[0].ToolMap) > 0 {
				...
				_, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
				if koutakuErr != nil {
					t.Logf("Execution %d: %v", i, koutakuErr)
				}
			}
			executions <- true
...
func TestConcurrent_HealthCheckDuringExecution(t *testing.T) {
	...
	go func() {
		...
		if koutakuErr != nil {
			errors <- fmt.Errorf("long-running tool failed: %v", koutakuErr)
		} else {
	...
	for i := 0; i < 5; i++ {
		...
		go func(id int) {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("health check %d failed: %v", id, koutakuErr)
			}
		}(i)
	}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			case 1:
				...
				_, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
				if koutakuErr != nil {
					errors <- fmt.Errorf("calc %d failed: %v", id, koutakuErr)
				}
			case 2:
				toolCall := GetSampleWeatherToolCall(fmt.Sprintf("weather-%d", id), "Tokyo", "celsius")
				_, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
				if koutakuErr != nil {
					errors <- fmt.Errorf("weather %d failed: %v", id, koutakuErr)
				}
			}
			...
			for j := 0; j < 5; j++ {
				...
				default:
					...
					if koutakuErr != nil {
						...
						t.Logf("Execution %d-%d: %v", id, j, koutakuErr)
					}
					time.Sleep(10 * time.Millisecond)
				}
			}
...
func TestConcurrent_PluginHooks(t *testing.T) {
	...
	// Setup Koutaku with plugin
	account := &testAccount{}
	koutakuInstance, err := core.Init(context.Background(), schemas.KoutakuConfig{
		Account:    account,
		MCPPlugins: []schemas.MCPPlugin{loggingPlugin},
		Logger:     core.NewDefaultLogger(schemas.LogLevelInfo),
	})
	require.NoError(t, err)
	koutakuInstance.SetMCPManager(manager)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, tType int) {
			...
			case 0: // Echo
				...
				result, koutakuErr = koutakuInstance.ExecuteChatMCPTool(ctx, &toolCall)
			case 1: // Calculator
				toolCall := GetSampleCalculatorToolCall(fmt.Sprintf("calc-%d", id), "add", float64(id), float64(id+1))
				result, koutakuErr = koutakuInstance.ExecuteChatMCPTool(ctx, &toolCall)
			}
...
func TestConcurrent_MultiplePlugins(t *testing.T) {
	...
	// Setup Koutaku with multiple plugins
	account := &testAccount{}
	koutakuInstance, err := core.Init(context.Background(), schemas.KoutakuConfig{
		Account: account,
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(id, opType int) {
			...
			case 0: // Execute echo tool
				...
				_, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
				if koutakuErr != nil {
					errors <- fmt.Errorf("echo %d failed: %v", id, koutakuErr)
				}
