...
func TestAgent_StateTransition_LargeMixedToolBatch(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("koutakuInternal-auto_tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	for i := 0; i < 5; i++ {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("koutakuInternal-manual_tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	require.Nil(t, koutakuErr, "should handle large mixed batch")
...
func TestAgent_StateTransition_DepthCountingBasic(t *testing.T) {
	...
	responses := []*schemas.KoutakuChatResponse{}
...
func TestAgent_StateTransition_AlternatingAutoNonAuto(t *testing.T) {
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-manual_tool"),
						Arguments: "{}",
					},
				},
			}),
	...
	if finalMessage.ChatAssistantMessage != nil && len(finalMessage.ChatAssistantMessage.ToolCalls) > 0 {
		assert.Equal(t, "koutakuInternal-manual_tool", *finalMessage.ChatAssistantMessage.ToolCalls[0].Function.Name)
	}
	...
	toolCalls := []schemas.ChatAssistantMessageToolCall{
		{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name: schemas.Ptr("koutakuInternal-tool_0"),
				Arguments: "{}",
			},
		},
		{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name: schemas.Ptr("koutakuInternal-tool_1"),
				Arguments: "{}",
			},
		},
	}
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-stateful_tool_0"),
						Arguments: "{}",
					},
				},
			}),
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-stateful_tool_1"),
						Arguments: "{}",
					},
				},
			}),
