...
func TestAgent_ErrorHandling_AllToolsFail(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("koutakuInternal-failing_tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	require.Nil(t, koutakuErr, "agent should not crash when all tools fail")
...
func TestAgent_ErrorHandling_TimeoutInLoop(t *testing.T) {
	...
	toolCalls := []schemas.ChatAssistantMessageToolCall{
		{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name: schemas.Ptr("koutakuInternal-slow_tool"),
				Arguments: "{}",
			},
		},
	}
	...
	if koutakuErr != nil {
		t.Logf("Timeout resulted in error (expected): %v", koutakuErr.Error)
	} else if result != nil {
...
func TestAgent_ErrorHandling_MalformedResponse(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if koutakuErr != nil {
				t.Logf("Malformed response (%s) handled with error: %v", tc.name, koutakuErr.Error)
			} else if result != nil {
...
func TestAgent_ErrorHandling_PartialBatchFailure(t *testing.T) {
	...
	for i := range outcomes {
		toolCalls = append(toolCalls, schemas.ChatAssistantMessageToolCall{
			...
			Type: schemas.Ptr("function"),
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr(fmt.Sprintf("koutakuInternal-tool_%d", i)),
				Arguments: "{}",
			},
		})
	}
	...
	require.Nil(t, koutakuErr, "partial failures should not crash agent")
	require.NotNil(t, result)
...
func TestAgent_ErrorHandling_RecoveryAndContinuation(t *testing.T) {
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-fail_tool"),
						Arguments: "{}",
					},
				},
			}),
	...
	require.Nil(t, koutakuErr, "agent should recover from error")
	require.NotNil(t, result)
...
func TestAgent_ErrorHandling_ErrorInToolArguments(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if koutakuErr != nil {
				t.Logf("Argument error (%s) handled with error: %v", tc.name, koutakuErr.Error)
			} else if result != nil {
...
func TestAgent_ErrorHandling_MultipleErrorsInSequence(t *testing.T) {
	...
	require.Nil(t, koutakuErr, "agent should handle multiple sequential errors")
	require.NotNil(t, result)
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-error_tool"),
						Arguments: "{}",
					},
				},
			}),
