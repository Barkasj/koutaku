...
import (
	...
	core "github.com/koutaku/koutaku/core"
...
func TestPlugin_PreMCPHook(t *testing.T) {
	...
	koutaku, err := core.Init(context.Background(), schemas.KoutakuConfig{
		Account:    &testAccount{},
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &echoCall)
...
func TestPlugin_ShortCircuit(t *testing.T) {
	...
	governancePlugin.BlockTool("koutakuInternal-echo")
	...
	// Tool name might be "echo" or "koutakuInternal-echo" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "echo") || strings.Contains(*result.Content.ContentStr, "koutakuInternal-echo")
...
func TestPlugin_ShortCircuit_CustomMessage(t *testing.T) {
	...
	governancePlugin.BlockTool("koutakuInternal-calculator")
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &calcCall)
	...
	require.NotNil(t, result.Content)
	require.NotNil(t, result.Content.ContentStr)
	// Tool name might be "calculator" or "koutakuInternal-calculator" in the response
	hasToolName := strings.Contains(*result.Content.ContentStr, "calculator") || strings.Contains(*result.Content.ContentStr, "koutakuInternal-calculator")
...
func TestPlugin_MultiplePlugins(t *testing.T) {
	...
	// Setup Koutaku with multiple plugins in pipeline
	koutaku, err := core.Init(context.Background(), schemas.KoutakuConfig{
		Account:    &testAccount{},
...
func TestPlugin_ResponsesFormat(t *testing.T) {
	...
	responsesToolCall := GetSampleResponsesToolCallMessage("test-responses-format", "koutakuInternal-echo", map[string]interface{}{
		"message": "test message",
	})
	result, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, responsesToolCall.ResponsesToolMessage)
...
func TestPlugin_LoggingPlugin(t *testing.T) {
	...
	_, err1 := koutaku.ExecuteChatMCPTool(ctx, &echoCall)
	require.Nil(t, err1)

	calcCall := GetSampleCalculatorToolCall("log-test-2", "add", 1.0, 2.0)
	_, err2 := koutaku.ExecuteChatMCPTool(ctx, &calcCall)
	require.Nil(t, err2)

	weatherCall := GetSampleWeatherToolCall("log-test-3", "Tokyo", "celsius")
	_, err3 := koutaku.ExecuteChatMCPTool(ctx, &weatherCall)
	require.Nil(t, err3)
