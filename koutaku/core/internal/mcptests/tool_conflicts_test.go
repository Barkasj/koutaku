...
func TestToolNameConflict_MultipleClients(t *testing.T) {
	...
	if koutakuErr == nil {
		...
		t.Logf("Tool execution failed: %v", koutakuErr)
	}
}
...
func TestToolNameConflict_Resolution(t *testing.T) {
	...
	// Register echo tool - will be available as koutakuInternal-echo
	...
	for i := 0; i < 10; i++ {
		...
		result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, koutakuErr, "tool should execute")
...
func TestToolNameConflict_WithFiltering(t *testing.T) {
	...
	// Register the echo tool for koutakuInternal
	require.NoError(t, RegisterEchoTool(manager))
	...
	// Execute "echo" - should use koutakuInternal client (it's the only one with the tool registered in-process)
...
func TestMultipleSameNameTools_ThreeClients(t *testing.T) {
	...
	// Register calculator tool - will be available as koutakuInternal-calculator
	...
	for i := 0; i < 15; i++ {
		...
		result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, koutakuErr, "calculator should execute")
		require.NotNil(t, result)
	}

	// Verify only the koutakuInternal client is active (disconnected clients may be
	...
	assert.Equal(t, 1, activeClients, "should have 1 koutakuInternal client")
...
func TestMultipleSameNameTools_DifferentImplementations(t *testing.T) {
	...
	// Use koutakuInternal client only with registered tools
	inProcessClient := GetSampleInProcessClientConfig()
	...
	for i := 0; i < 5; i++ {
		toolCall := schemas.ChatAssistantMessageToolCall{
			...
			Function: schemas.ChatAssistantMessageToolCallFunction{
				Name:      schemas.Ptr("koutakuInternal-process_data"),
				Arguments: `{"data": "test"}`,
			},
		}

		result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
		require.Nil(t, koutakuErr, "process_data should execute")
		require.NotNil(t, result)
	}
...
func TestToolConflict_OneClientDisconnected(t *testing.T) {
	...
	// Register echo tool - will be available as koutakuInternal-echo on client1
	require.NoError(t, RegisterEchoTool(manager))
	...
	// Verify only koutakuInternal is active (disconnected clients may be retained
	...
	require.Equal(t, 1, activeClients, "should only have 1 active client (koutakuInternal)")
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
	require.Nil(t, koutakuErr, "should use connected client")
	require.NotNil(t, result)
...
func TestToolConflict_BothClientsDisconnected(t *testing.T) {
	...
	assert.NotNil(t, koutakuErr, "should fail when all clients are disconnected")
	assert.Nil(t, result)
	if koutakuErr != nil && koutakuErr.Error != nil {
		errorMsg := koutakuErr.Error.Message
...
func TestToolConflict_ChatFormat(t *testing.T) {
	...
	// Use koutakuInternal clients with registered tools
	inProcessClient := GetSampleInProcessClientConfig()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &tc.toolCall)
			if koutakuErr != nil && koutakuErr.Error != nil {
				fmt.Println("koutakuErr", koutakuErr.Error.Message)
			}
			require.Nil(t, koutakuErr, "should resolve conflict and execute")
			require.NotNil(t, result)
...
func TestToolConflict_ResponsesFormat(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, &tc.responsesToolMsg)
			require.Nil(t, koutakuErr, "should resolve conflict and execute")
			require.NotNil(t, result)
...
func TestToolConflict_ComprehensiveScenarios(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			name:          "echo_tool",
			toolName:      "koutakuInternal-echo",
			expectSuccess: true,
		},
		{
			name:          "calculator_tool",
			toolName:      "koutakuInternal-calculator",
			expectSuccess: true,
		},
		{
			name:          "weather_tool",
			toolName:      "koutakuInternal-get_weather",
			expectSuccess: true,
		},
	}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			if scenario.expectSuccess {
				require.Nil(t, koutakuErr, "should execute successfully")
				require.NotNil(t, result)
				t.Logf("Tool %s executed successfully", scenario.toolName)
			} else {
				assert.NotNil(t, koutakuErr, "should fail")
			}
		})
	}
}
