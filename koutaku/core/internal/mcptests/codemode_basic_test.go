...
func TestCodeMode_MultiServer_BasicCalls(t *testing.T) {
	...
	temperatureClient := GetTemperatureMCPClientConfig(koutakuRoot)
	temperatureClient.IsCodeModeClient = true
	temperatureClient.ToolsToExecute = []string{"*"}

	goTestClient := GetGoTestServerConfig(koutakuRoot)
...
echo = koutakuInternal.echo(message="multi-server")
...
`
	...
	require.Nil(t, koutakuErr, "should execute without koutaku error")
...
func TestCodeMode_MultiServer_ParallelExecution(t *testing.T) {
	...
	parallelClient := GetParallelTestServerConfig(koutakuRoot)
	parallelClient.ToolsToExecute = []string{"*"}

	edgeClient := GetEdgeCaseServerConfig(koutakuRoot)
