...
func TestHealthCheckSTDIOServerDropAndRecoverIn20Seconds(t *testing.T) {
	...
	koutakuRoot := "/Users/koutaku/Desktop/koutaku"
	clientConfig := GetTemperatureMCPClientConfig(koutakuRoot)
	clientConfig.ID = "stdio-health-recovery-test"

	// 1. Create STDIO client with koutaku manager
...
func TestHealthCheckDuringToolExecution(t *testing.T) {
	...
	clients := manager.GetClients()
	require.Len(t, clients, 1, "should have koutakuInternal client")
	...
	require.Nil(t, koutakuErr, "tool execution should succeed during health monitoring")
