...
func InitMCPServerPaths(t *testing.T) {
	if mcpServerPaths.KoutakuRoot != "" {
		return // Already initialized
	}

	koutakuRoot := GetKoutakuRoot(t)
	examplesRoot := filepath.Join(koutakuRoot, "..", "examples")

	mcpServerPaths.KoutakuRoot = koutakuRoot
	...
	t.Logf("  - Koutaku Root: %s", mcpServerPaths.KoutakuRoot)
...
func GetSampleCalculatorToolCall(id string, operation string, x, y float64) schemas.ChatAssistantMessageToolCall {
	...
	return schemas.ChatAssistantMessageToolCall{
		...
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-calculator"),
...
func GetSampleEchoToolCall(id string, message string) schemas.ChatAssistantMessageToolCall {
	...
	return schemas.ChatAssistantMessageToolCall{
		...
		Type: schemas.Ptr("function"),
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-echo"),
			Arguments: string(argsJSON),
		},
	}
}
...
func GetSampleWeatherToolCall(id string, location string, units string) schemas.ChatAssistantMessageToolCall {
	...
	return schemas.ChatAssistantMessageToolCall{
		...
		Type: schemas.Ptr("function"),
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-get_weather"),
			Arguments: string(argsJSON),
		},
	}
}
...
func GetSampleDelayToolCall(id string, seconds float64) schemas.ChatAssistantMessageToolCall {
	...
	return schemas.ChatAssistantMessageToolCall{
		...
		Type: schemas.Ptr("function"),
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-delay"),
			Arguments: string(argsJSON),
		},
	}
}
...
// SetInternalClientAutoExecute configures which tools should be auto-executed for the internal Koutaku client
func SetInternalClientAutoExecute(manager *mcp.MCPManager, toolNames []string) error {
	...
	for i := range clients {
		if clients[i].ExecutionConfig.ID == "koutakuInternal" {
	...
	if internalClient == nil {
		return fmt.Errorf("internal koutaku client not found")
	}
...
// SetInternalClientAsCodeMode configures the internal Koutaku client as a CodeMode client
...
// The path is relative to the koutaku root directory.
func GetTemperatureMCPClientConfig(koutakuRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.TemperatureServer
	if serverPath == "" {
		serverPath = koutakuRoot + "/examples/mcps/temperature-server/dist/index.js"
	}
...
func GetGoTestServerConfig(koutakuRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.GoTestServer
	if serverPath == "" {
		serverPath = koutakuRoot + "/../examples/mcps/go-test-server/bin/go-test-server"
	}
...
func GetEdgeCaseServerConfig(koutakuRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.EdgeCaseServer
	if serverPath == "" {
		serverPath = koutakuRoot + "/../examples/mcps/edge-case-server/bin/edge-case-server"
	}
...
func GetErrorTestServerConfig(koutakuRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.ErrorTestServer
	if serverPath == "" {
		serverPath = koutakuRoot + "/../examples/mcps/error-test-server/bin/error-test-server"
	}
...
func GetParallelTestServerConfig(koutakuRoot string) schemas.MCPClientConfig {
	// Use global path if available, otherwise fall back to parameter
	serverPath := mcpServerPaths.ParallelTestServer
	if serverPath == "" {
		serverPath = koutakuRoot + "/../examples/mcps/parallel-test-server/bin/parallel-test-server"
	}
...
// GetKoutakuRoot returns the koutaku root directory by walking up from the current directory
func GetKoutakuRoot(t *testing.T) string {
	// Start from current working directory
	cwd, err := os.Getwd()
	require.NoError(t, err, "should get current working directory")

	// Walk up the directory tree to find the koutaku root (contains go.mod with module github.com/koutaku/koutaku)
	dir := cwd
	for {
		goModPath := filepath.Join(dir, "go.mod")
		if _, err := os.Stat(goModPath); err == nil {
			// Found go.mod, this is likely the koutaku root
			return dir
		}
		...
		if parent == dir {
			...
			t.Fatal("could not find koutaku root (go.mod not found)")
		}
		dir = parent
	}
}
...
func GetFilteringScenarios() []FilteringScenario {
	return []FilteringScenario{
		// Nil config scenarios
		{
			...
			ConfigTools:      nil,
			ContextTools:     nil,
			RequestedTool:    "koutakuInternal-echo",
			ShouldExecute:    false,
		...
		},
		{
			...
			ConfigTools:      nil,
			ContextTools:     []string{"koutakuInternal-echo"},
			RequestedTool:    "koutakuInternal-echo",
			ShouldExecute:    true,
		...
		},
		{
			...
			ConfigTools:      []string{"*"},
			ContextTools:     []string{"koutakuInternal-echo"},
			RequestedTool:    "koutakuInternal-calculator",
			ShouldExecute:    false,
		...
		},
		{
			...
			ContextTools:     []string{"koutakuInternal-calculator"},
			RequestedTool:    "koutakuInternal-calculator",
			ShouldExecute:    true,
...
const (
	...
	// Koutaku API configuration
	EnvKoutakuAPIKey       = "OPENAI_API_KEY"
	EnvKoutakuTestProvider = "KOUTAKU_TEST_PROVIDER"
	EnvKoutakuTestModel    = "KOUTAKU_TEST_MODEL"
...
func (a *testAccount) GetKeysForProvider(ctx context.Context, providerKey schemas.ModelProvider) ([]schemas.Key, error) {
	...
	apiKey := os.Getenv(EnvKoutakuAPIKey)
	if apiKey == "" {
...
// setupKoutaku creates a Koutaku instance for testing
func setupKoutaku(t *testing.T) *koutaku.Koutaku {
	t.Helper()

	account := &testAccount{}

	// Create koutaku instance
	koutakuInstance, err := koutaku.Init(context.Background(), schemas.KoutakuConfig{
		Account: account,
		Logger:  koutaku.NewDefaultLogger(schemas.LogLevelError),
	})
	require.NoError(t, err, "failed to create koutaku instance")

	// Cleanup
	t.Cleanup(func() {
		koutakuInstance.Shutdown()
	})

	return koutakuInstance
}

// noopPluginPipeline is a passthrough pipeline used in tests that don't need plugin hooks.
type noopPluginPipeline struct{}

func (n *noopPluginPipeline) RunMCPPreHooks(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, int) {
	return req, nil, 0
}

func (n *noopPluginPipeline) RunMCPPostHooks(ctx *schemas.KoutakuContext, mcpResp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError, runFrom int) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError) {
...
func setupMCPManager(t *testing.T, clientConfigs ...schemas.MCPClientConfig) *mcp.MCPManager {
	...
	// work correctly even when no Koutaku instance is attached.
...
func loadTestConfig() *TestConfig {
	...
	testConfig := &TestConfig{
		...
		APIKey:        os.Getenv(EnvKoutakuAPIKey),
		Provider:      schemas.ModelProvider(getEnvOrDefault(EnvKoutakuTestProvider, DefaultTestProvider)),
		Model:         getEnvOrDefault(EnvKoutakuTestModel, DefaultTestModel),
		UseRealLLM:    os.Getenv(EnvKoutakuAPIKey) != "",
		MaxRetries:    3,
...
func AssertToolResponse(t *testing.T, resp *schemas.KoutakuMCPResponse, expectedContent string) {
	t.Helper()
...
func AssertToolExecuted(t *testing.T, resp *schemas.KoutakuMCPResponse, err error) {
	t.Helper()
...
func AssertToolResponseContains(t *testing.T, resp *schemas.KoutakuMCPResponse, expectedText string) {
	t.Helper()
	require.NotNil(t, resp, "response should not be nil")
...
// AssertKoutakuErrorContains asserts that koutaku error contains expected message
func AssertKoutakuErrorContains(t *testing.T, koutakuErr *schemas.KoutakuError, expectedMessage string) {
	t.Helper()
	require.NotNil(t, koutakuErr, "koutaku error should not be nil")
	require.NotNil(t, koutakuErr.Error, "koutaku error.Error should not be nil")
	assert.Contains(t, koutakuErr.Error.Message, expectedMessage, "error message should contain expected text")
}
...
func AssertResponseHasToolCalls(t *testing.T, resp *schemas.KoutakuChatResponse, expectedCount int) {
	t.Helper()
	require.NotNil(t, resp, "response should not be nil")
...
func AssertAgentCompletedSuccessfully(t *testing.T, resp *schemas.KoutakuChatResponse, koutakuErr *schemas.KoutakuError) {
	t.Helper()
	if koutakuErr != nil && koutakuErr.Error != nil {
		fmt.Println("koutakuErr", koutakuErr.Error.Message)
	}
	assert.Nil(t, koutakuErr, "agent should complete without error")
...
func CreateTestContextWithMCPFilter(includeClients []string, includeTools []string) *schemas.KoutakuContext {
	...
	return schemas.NewKoutakuContext(baseCtx, schemas.NoDeadline)
}

// CreateTestContextWithTimeout creates a test context with custom timeout
func CreateTestContextWithCustomTimeout(timeout time.Duration) (*schemas.KoutakuContext, context.CancelFunc) {
	baseCtx, cancel := context.WithTimeout(context.Background(), timeout)
	return schemas.NewKoutakuContext(baseCtx, schemas.NoDeadline), cancel
}
...
func GetTestDataPath(t *testing.T, filename string) string {
	t.Helper()
	koutakuRoot := GetKoutakuRoot(t)
	return filepath.Join(koutakuRoot, "core", "internal", "mcptests", "testdata", filename)
}

// CreateTempTestFile creates a temporary test file
func CreateTempTestFile(t *testing.T, content string) string {
	t.Helper()
	tmpFile, err := os.CreateTemp("", "koutaku-test-*")
	require.NoError(t, err)
...
type ChatResponseFunc func(history []schemas.ChatMessage) (*schemas.KoutakuChatResponse, *schemas.KoutakuError)

// ResponsesResponseFunc is a function that generates a Responses response based on message history
type ResponsesResponseFunc func(history []schemas.ResponsesMessage) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError)
...
func (m *DynamicLLMMocker) AddStaticChatResponse(response *schemas.KoutakuChatResponse) {
	m.AddChatResponse(func(history []schemas.ChatMessage) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		return response, nil
	})
}

// AddStaticResponsesResponse adds a static Responses response (backwards compatible)
func (m *DynamicLLMMocker) AddStaticResponsesResponse(response *schemas.KoutakuResponsesResponse) {
	m.AddResponsesResponse(func(history []schemas.ResponsesMessage) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
		return response, nil
	})
}
...
func (m *DynamicLLMMocker) SetDefaultStaticChatResponse(response *schemas.KoutakuChatResponse) {
	m.SetDefaultChatResponse(func(history []schemas.ChatMessage) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		return response, nil
	})
}

// SetDefaultStaticResponsesResponse sets a static default Responses response
func (m *DynamicLLMMocker) SetDefaultStaticResponsesResponse(response *schemas.KoutakuResponsesResponse) {
	m.SetDefaultResponsesResponse(func(history []schemas.ResponsesMessage) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
		return response, nil
	})
}

// MakeChatRequest implements the LLM caller interface for Chat API
func (m *DynamicLLMMocker) MakeChatRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuChatRequest) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
...
func (m *DynamicLLMMocker) MakeResponsesRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuResponsesRequest) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
	// Store the message history
				...
				for _, tc := range msg.ChatAssistantMessage.ToolCalls {
					if tc.Function.Name != nil {
						...
						if fullName == toolName ||
							fullName == "koutakuInternal-"+toolName ||
...
// Supports both prefixed (koutakuInternal-toolName) and unprefixed tool names
	...
	for _, msg := range history {
		if msg.Type != nil && *msg.Type == schemas.ResponsesMessageTypeFunctionCall {
			if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.Name != nil {
				...
				// Check with koutakuInternal- prefix
				if fullName == "koutakuInternal-"+toolName {
					return true
				}
...
func CreateChatResponseWithToolCalls(toolCalls []schemas.ChatAssistantMessageToolCall) *schemas.KoutakuChatResponse {
...
func CreateChatResponseWithText(text string) *schemas.KoutakuChatResponse {
	return &schemas.KoutakuChatResponse{
		Choices: []schemas.KoutakuResponseChoice{
			{
...
func CreateResponsesResponseWithToolCalls(toolCalls []schemas.ResponsesToolMessage) *schemas.KoutakuResponsesResponse {
...
func CreateResponsesResponseWithText(text string) *schemas.KoutakuResponsesResponse {
...
func CreateDynamicChatResponse(fn func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse) ChatResponseFunc {
	return func(history []schemas.ChatMessage) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		return fn(history), nil
	}
}

// CreateDynamicResponsesResponse is a convenience function for creating a dynamic Responses response
func CreateDynamicResponsesResponse(fn func(history []schemas.ResponsesMessage) *schemas.KoutakuResponsesResponse) ResponsesResponseFunc {
	return func(history []schemas.ResponsesMessage) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
		return fn(history), nil
	}
}
...
func CreateValidatingChatResponse(callID string, mustContain []string, successText string, failureText string) ChatResponseFunc {
	return CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse {
...
func CreateConditionalChatResponse(condition func(history []schemas.ChatMessage) bool, trueResponse, falseResponse *schemas.KoutakuChatResponse) ChatResponseFunc {
	return CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse {
...
func CreateSequentialChatResponses(responses []*schemas.KoutakuChatResponse) []ChatResponseFunc {
	funcs := make([]ChatResponseFunc, len(responses))
	for i, resp := range responses {
		r := resp // Capture for closure
		funcs[i] = CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse {
			return r
		})
	}
	return funcs
...
func CreateToolCallSequence(sequences []struct {
	...
	for i, seq := range sequences {
		...
		if isLast {
			...
			funcs = append(funcs, CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse {
				if toolCall.ID != nil {
...
mocker.AddChatResponse(CreateDynamicChatResponse(func(history []schemas.ChatMessage) *schemas.KoutakuChatResponse {
...
// The tool name is automatically prefixed with "koutakuInternal-" to match
...
func CreateToolCallForExecution(callID string, toolName string, args map[string]interface{}) schemas.ChatAssistantMessageToolCall {
	argsJSON, _ := json.Marshal(args)
	prefixedToolName := "koutakuInternal-" + toolName
