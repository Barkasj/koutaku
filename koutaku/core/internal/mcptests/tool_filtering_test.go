...
func getActualToolsFromGoTestServer(t *testing.T) (tool1, tool2 string) {
	...
	clientConfig := GetGoTestServerConfig(koutakuRoot)
...
func executeToolViaManager(t *testing.T, manager interface{ ExecuteToolCall(*schemas.KoutakuContext, *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error) }, toolName string) error {
	t.Helper()
...
func TestFilteringMultipleClients_DifferentRules(t *testing.T) {
	...
	client1 := GetGoTestServerConfig(koutakuRoot)
	...
	client2 := GetEdgeCaseServerConfig(koutakuRoot)
