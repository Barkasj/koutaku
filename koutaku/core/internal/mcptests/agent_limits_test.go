...
func TestAgent_Timeout(t *testing.T) {
	...
	_, koutakuErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
		mockLLM.MakeChatRequest,
	...
	require.NotNil(t, koutakuErr, "Expected timeout error but got success - timeout is not being enforced!")
	t.Logf("✅ Timeout correctly enforced: %v", koutakuErr.Error)
}
...
func TestAgent_TimeoutDuringExecution(t *testing.T) {
	...
	require.NotNil(t, koutakuErr, "Expected timeout during tool execution but got success - timeout not enforced mid-execution!")
	t.Logf("✅ Timeout during execution correctly enforced: %v", koutakuErr.Error)
}
...
func TestAgent_Timeout_ChatFormat(t *testing.T) {
	...
	_, koutakuErr := manager.CheckAndExecuteAgentForChatRequest(ctx,
		&schemas.KoutakuChatRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
			Input: []schemas.ChatMessage{{Role: schemas.ChatMessageRoleUser, Content: &schemas.ChatMessageContent{ContentStr: schemas.Ptr("Test")}}}},
	...
	require.NotNil(t, koutakuErr, "Chat format timeout not enforced!")
	t.Logf("✅ Chat format timeout enforced: %v", koutakuErr.Error)
}
...
func TestAgent_Timeout_ResponsesFormat(t *testing.T) {
	...
	mockLLM := &MockLLMCaller{
		responsesResponses: []*schemas.KoutakuResponsesResponse{
			CreateResponsesResponseWithToolCalls([]schemas.ResponsesToolMessage{
				{CallID: schemas.Ptr("call-1"), Name: schemas.Ptr("koutakuInternal-slow_responses_tool"), Arguments: schemas.Ptr(`{}`)},
			}),
		},
	}

	_, koutakuErr := manager.CheckAndExecuteAgentForResponsesRequest(ctx,
		&schemas.KoutakuResponsesRequest{Provider: schemas.OpenAI, Model: "gpt-4o",
	...
	require.NotNil(t, koutakuErr, "Responses format timeout not enforced!")
	t.Logf("✅ Responses format timeout enforced: %v", koutakuErr.Error)
}
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-nonexistent_tool"),
						Arguments: `{}`,
					},
				},
			}),
			...
			CreateChatResponseWithToolCalls([]schemas.ChatAssistantMessageToolCall{
				{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name: schemas.Ptr("koutakuInternal-nonexistent"),
						Arguments: `{}`,
					},
				},
			}),
