...
func TestReadToolFile_Basic(t *testing.T) {
	...
	readResult, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &readCall)
	require.Nil(t, koutakuErr)
...
func TestReadToolFile_NotFound(t *testing.T) {
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &readCall)

	// The tool returns an error or informational message about the non-existent file
	if koutakuErr != nil {
		if koutakuErr.Error != nil {
			assert.Contains(t, koutakuErr.Error.Message, "not found")
		}
...
func TestCodeModeFiles_ResponsesFormat(t *testing.T) {
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &listCall)
	require.Nil(t, koutakuErr, "listToolFiles should not error")
...
func TestCodeModeFiles_FullWorkflow(t *testing.T) {
	...
	listResult, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &listCall)
	require.Nil(t, koutakuErr)
...
result = {"completed": True, "servers": ["TestCodeModeServer"], "methodCount": len(server_methods)}`
	...
	execResult, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &execCall)
	require.Nil(t, koutakuErr)
