...
func TestAgent_ContextToolFilter_BlockedToolError(t *testing.T) {
	...
	require.NotNil(t, koutakuErr, "Should return error when tool is filtered")
	...
	t.Logf("Error message: %s", koutakuErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_Whitelist(t *testing.T) {
	...
	// Context only allows InProcess client (koutakuInternal)
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		...
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"koutakuInternal"}, // Only allow InProcess client
		MaxDepth:         5,
...
func TestAgent_ContextToolFilter_EmptyList(t *testing.T) {
	...
	require.NotNil(t, koutakuErr, "Should return error when tool is filtered by empty context")
	require.Nil(t, result)

	// Verify error indicates tool is not permitted
	require.NotNil(t, koutakuErr.Error)
	t.Logf("Error (expected): %s", koutakuErr.Error.Message)
}
...
func TestAgent_ContextClientFilter_MultipleClients(t *testing.T) {
	...
	manager, mocker, ctx := SetupAgentTestWithClients(t, AgentTestConfig{
		InProcessTools:   []string{"echo"},
		AutoExecuteTools: []string{"*"},
		ClientFiltering:  []string{"koutakuInternal", "temperature-mcp-client"}, // Allow 2 clients
		MaxDepth:         10,
...
func TestAgent_ContextToolFilter_ParallelMixed(t *testing.T) {
	...
	if koutakuErr != nil {
		...
		require.NotNil(t, koutakuErr.Error)
		t.Logf("Error returned: %s", koutakuErr.Error.Message)
		require.Nil(t, result)
	} else {
