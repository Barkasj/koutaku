...
func TestErrorHandling_InProcess_ToolReturnsError(t *testing.T) {
	...
	if koutakuErr != nil {
		...
		assert.Contains(t, koutakuErr.Error.Message, errorMessage, "error message should contain original error")
		t.Logf("✅ Tool error returned as execution error: %s", koutakuErr.Error.Message)
	} else {
...
func TestErrorHandling_InProcess_ToolReturnsError_ResponsesFormat(t *testing.T) {
	...
	result, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, &responsesToolMsg)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldError {
				...
				if koutakuErr != nil {
					...
					if tc.errorMatch != "" {
						assert.Contains(t, koutakuErr.Error.Message, tc.errorMatch,
							"error message should contain expected text for test case '%s'", tc.name)
					}
					t.Logf("✅ %s: Error handled as execution error: %s", tc.name, koutakuErr.Error.Message)
					errorHandled = true
				} else if result != nil {
				...
				assert.Nil(t, koutakuErr, "test case '%s' should not return error", tc.name)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr {
				...
				errorHandled := (koutakuErr != nil) || (result != nil)
				...
				assert.Nil(t, koutakuErr, "test '%s' should not return error", tc.name)
				assert.NotNil(t, result, "result should not be nil")
...
func TestErrorHandling_STDIO_MCPErrorResponse(t *testing.T) {
	...
	errorServerConfig := GetErrorTestServerConfig(koutakuRoot)
...
func TestErrorHandling_STDIO_TimeoutScenario(t *testing.T) {
	...
	assert.NotNil(t, koutakuErr, "should return timeout error")
	assert.Nil(t, result, "result should be nil on timeout")
	assert.Contains(t, koutakuErr.Error.Message, "timed out", "error message should indicate timeout")
	...
	t.Logf("✅ Timeout handled correctly after %v: %s", elapsed, koutakuErr.Error.Message)
}
...
func TestErrorHandling_STDIO_MalformedJSON(t *testing.T) {
	...
	if koutakuErr != nil {
		...
		t.Logf("✅ Malformed JSON handled as error: %s", koutakuErr.Error.Message)
	} else {
...
func TestErrorHandling_ErrorMessageFormat(t *testing.T) {
	...
	if koutakuErr != nil {
		...
		assert.NotEmpty(t, koutakuErr.Error.Message, "error message should not be empty")
		assert.Contains(t, koutakuErr.Error.Message, errorMessage, "error should contain original message")
		t.Logf("✅ Error message format (as error): %s", koutakuErr.Error.Message)
	} else {
...
func TestErrorHandling_MultipleConsecutiveErrors(t *testing.T) {
	...
	for i := 0; i < numErrors; i++ {
		...
		if koutakuErr != nil {
			assert.Contains(t, koutakuErr.Error.Message, errorMessage, "error %d should contain correct message", i+1)
		} else {
...
func TestErrorHandling_ErrorWithSpecialCharacters(t *testing.T) {
	...
	for i, msg := range specialMessages {
		...
		if koutakuErr != nil {
			...
			assert.True(t,
				strings.Contains(koutakuErr.Error.Message, msg) ||
					strings.Contains(koutakuErr.Error.Message, strings.ReplaceAll(msg, "\n", "\\n")) ||
					strings.Contains(koutakuErr.Error.Message, strings.ReplaceAll(msg, "\t", "\\t")),
...
func createThrowErrorToolCall(id, errorMessage string) schemas.ChatAssistantMessageToolCall {
	...
	return schemas.ChatAssistantMessageToolCall{
		...
		Type: schemas.Ptr("function"),
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-throw_error"),
			Arguments: string(argsJSON),
		},
	}
}
