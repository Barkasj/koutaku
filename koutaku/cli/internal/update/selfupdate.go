...
func RunSelfUpdate(currentVersion string) error {
	fmt.Println("Checking latest koutaku version...")
	...
	fmt.Printf("Updating koutaku from %s to %s...\n", currentVersion, latest)

	binaryName := "koutaku"
	if runtime.GOOS == "windows" {
		binaryName = "koutaku.exe"
	}
	downloadURL := fmt.Sprintf("%s/koutaku-cli/%s/%s/%s/%s", baseURL, latest, runtime.GOOS, runtime.GOARCH, binaryName)
	...
	tmpFile, err := os.CreateTemp("", ".koutaku-update-*")
	if err != nil {
	...
	fmt.Printf("Updated koutaku from %s to %s. Please restart.\n", currentVersion, latest)
	return nil
}
...
func resolveManagedBinaryTarget(binaryName string) (string, error) {
	...
	return koutakuCLIManagedBinaryPath(binaryName)
}

func koutakuCLIManagedBinaryPath(binaryName string) (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, ".koutaku", "bin", binaryName), nil
}
...
func isWrapperManagedBinaryPath(path, binaryName string) bool {
	...
	homePath, err := koutakuCLIManagedBinaryPath(binaryName)
	...
	cachePrefix := filepath.Clean(filepath.Join(cacheRoot, "koutaku")) + string(os.PathSeparator)
...
func stageUpdateBinary(downloadPath, targetPath string, mode os.FileMode) (string, error) {
	stageFile, err := os.CreateTemp(filepath.Dir(targetPath), ".koutaku-stage-*")
	if err != nil {
		return "", err
	}
