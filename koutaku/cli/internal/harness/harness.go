...
// Harness defines a coding assistant CLI that Koutaku can launch and manage.
type Harness struct {
	ID               string
	Label            string
	Binary           string
	...
	// WriteNativeConfig persists the koutaku connection settings into the
	// harness CLI's own config file so the same configuration is available
	// when users launch the CLI directly outside koutaku.
...
// opencodePreLaunch writes temporary OpenCode config files when Koutaku needs
...
func opencodePreLaunch(baseURL, apiKey, model string) ([]string, func(), error) {
  ...
  "provider": {
	...
	f, err := os.CreateTemp("", "koutaku-opencode-*.json")
	if err != nil {
...
func opencodeModelRef(model string) string {
	return "koutaku/" + strings.TrimSpace(model)
}
...
func opencodeTUIPreLaunch() ([]string, func(), error) {
	...
	f, err := os.CreateTemp("", "koutaku-opencode-tui-*.json")
	if err != nil {
