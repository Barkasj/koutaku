/**
 * Koutaku docs — Base URL persistence
 *
...
(function () {
  ...
  if (window.__koutakuBaseUrlSwitcherLoaded) return;
  window.__koutakuBaseUrlSwitcherLoaded = true;

  var DEFAULT_URL = "http://localhost:8080";
  var STORAGE_KEY = "koutaku_base_url";
  ...
  function findPlaygroundUrlInputs() {
    ...
    if (!el || el.__koutakuPlaygroundBound) return [];
    return [el];
  }
  ...
  function syncPlaygroundInputs(state) {
    ...
    for (var i = 0; i < inputs.length; i++) {
      var el = inputs[i];
      if (el.__koutakuPlaygroundBound) continue;
      el.__koutakuPlaygroundBound = true;
