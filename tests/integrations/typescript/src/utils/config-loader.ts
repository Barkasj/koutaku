...
export interface KoutakuConfig {
  base_url: string
...
class ConfigLoader {
  ...
  getIntegrationUrl(integration: string): string {
    ...
    const koutakuConfig = this.config.koutaku
    const baseUrl = koutakuConfig.base_url
    const endpoint = koutakuConfig.endpoints[integration]
  ...
  getKoutakuConfig(): KoutakuConfig {
    if (!this.config) throw new Error('Config not loaded')
    return this.config.koutaku
  }
  ...
  validateConfig(): boolean {
    ...
    const requiredSections = ['koutaku', 'providers', 'api', 'test_settings']
    ...
    const koutaku = this.config.koutaku
    if (!koutaku.base_url || !koutaku.endpoints) {
      throw new Error('Koutaku configuration missing base_url or endpoints')
    }
  ...
  printConfigSummary(): void {
    ...
    for (const [integration, endpoint] of Object.entries(koutaku.endpoints)) {
      const fullUrl = `${koutaku.base_url.replace(/\/$/, '')}/${endpoint}`
