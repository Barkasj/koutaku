# Koutaku Integration Tests

Production-ready end-to-end test suite for testing AI integrations through Koutaku proxy. This test suite provides uniform testing across multiple AI integrations with comprehensive coverage of chat, tool calling, image processing, embeddings, speech synthesis, and multimodal workflows.
...
cd koutaku/tests/integrations
uv sync

# 3. Set environment variables
export KOUTAKU_BASE_URL="http://localhost:8080"
...
The Koutaku integration tests use a centralized configuration system that routes all AI integration requests through Koutaku as a gateway/proxy:

```text
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Test Client   │───▶│  Koutaku Gateway │───▶│  AI Integration    │
...
- **Base URL**: `http://localhost:8080` (configurable via `KOUTAKU_BASE_URL`)
...
- **🌉 Koutaku Gateway Integration**: All integrations route through Koutaku proxy
...
- Koutaku gateway settings and endpoints
...
```yaml
koutaku:
  base_url: "${KOUTAKU_BASE_URL:-http://localhost:8080}"
  endpoints:
    openai: "openai"
    anthropic: "anthropic"
    google: "genai"
...
1. **Use simple, clear phrases** to koutakuize transcription accuracy
...
3. **Koutaku Configuration**
   ```bash
   # Ensure Koutaku is running and accessible
...
# Check Koutaku logs for audio endpoint requests
# (Check your Koutaku instance logs)
```
...
- ✅ **Full Koutaku Integration**: Complete base URL support
...
- ✅ **Full Koutaku Integration**: Complete custom transport implementation
...
- ✅ **Full Koutaku Integration**: Global base URL configuration
...
- [ ] **Koutaku Integration**: Base URL properly configured and tested
...
```bash
# Error: Connection refused to Koutaku
ConnectionError: Connection refused to localhost:8080

# Solutions:
# 1. Check if Koutaku is running
curl http://localhost:8080/health

# 2. Ensure KOUTAKU_BASE_URL is set correctly
echo $KOUTAKU_BASE_URL
```
...
**Note**: This test suite is designed for testing AI integrations through Koutaku proxy. Ensure your Koutaku instance is properly configured and running before executing tests. The configuration system provides Koutaku routing for koutakuum flexibility.
