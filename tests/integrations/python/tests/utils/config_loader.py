"""
Configuration loader for Koutaku integration tests.

This module loads configuration from config.yml and provides utilities
for constructing integration URLs through the Koutaku gateway.
"""
...
class ConfigLoader:
    ...
    def get_integration_url(self, integration: str) -> str:
        ...
        """
        koutaku_config = self._config["koutaku"]
        base_url = koutaku_config["base_url"]
        endpoint = koutaku_config["endpoints"].get(integration, "")
    ...
    def get_koutaku_config(self) -> KoutakuConfig:
        """Get Koutaku configuration"""
        koutaku_data = self._config["koutaku"]
        return KoutakuConfig(
            base_url=koutaku_data["base_url"], endpoints=koutaku_data["endpoints"]
        )
    ...
    def validate_config(self) -> bool:
        ...
        required_sections = ["koutaku", "providers", "api", "test_settings"]

        for section in required_sections:
            if section not in self._config:
                raise ValueError(f"Missing required configuration section: {section}")

        # Validate Koutaku configuration
        koutaku = self._config["koutaku"]
        if "base_url" not in koutaku or "endpoints" not in koutaku:
            raise ValueError("Koutaku configuration missing base_url or endpoints")
    ...
    def print_config_summary(self):
        ...
        for integration, endpoint in koutaku.endpoints.items():
            full_url = f"{koutaku.base_url.rstrip('/')}/{endpoint}"
