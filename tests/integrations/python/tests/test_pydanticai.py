...
This test suite uses Pydantic AI to test against multiple AI providers through Koutaku.
...
Tests Pydantic AI standard interface compliance and Koutaku integration:
...
def setup_pydanticai():
    """Setup Pydantic AI with Koutaku configuration and dummy credentials"""
    ...
    os.environ["GEMINI_API_KEY"] = "dummy-gemini-api-key-koutaku-handles-auth"
    os.environ["CO_API_KEY"] = "dummy-cohere-key-koutaku-handles-auth"
...
def get_openai_model(model_name: str | None = None) -> OpenAIChatModel:
    """Create an OpenAI model configured for Koutaku"""
    ...
    provider = OpenAIProvider(
        ...
        api_key="dummy-openai-key-koutaku-handles-auth"
    )
    return OpenAIChatModel(model_name, provider=provider)


def get_anthropic_model(model_name: str = "claude-3-haiku-20240307") -> Optional[Any]:
    """Create an Anthropic model configured for Koutaku"""
    ...
    provider = AnthropicProvider(
        base_url=base_url,
        api_key="dummy-anthropic-key-koutaku-handles-auth"
    )
    return AnthropicModel(model_name, provider=provider)


def get_google_model(model_name: str = "gemini-2.0-flash") -> Optional[Any]:
    """Create a Google model configured for Koutaku"""
    if not GOOGLE_AVAILABLE:
        return None

    base_url = get_integration_url("pydanticai")

    # Configure GoogleProvider with Koutaku endpoint
    provider = GoogleProvider(
        api_key="dummy-google-api-key-koutaku-handles-auth",
        base_url=base_url
    )
...
def get_cohere_model(model_name: str = "command-r7b-12-2024") -> Optional[Any]:
    """Create a Cohere model configured for Koutaku"""
    ...
    # We create a custom client pointing to Koutaku and pass it to CohereProvider
    cohere_client = CohereAsyncClient(
        api_key="dummy-cohere-key-koutaku-handles-auth",
        base_url=base_url
    )
...
class TestPydanticAIIntegration:
    """Comprehensive Pydantic AI integration tests through Koutaku"""
