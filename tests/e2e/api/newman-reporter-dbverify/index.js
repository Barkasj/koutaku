 ...
 *   3. --reporter-dbverify-config   (path to Koutaku config.json; auto-detects type + DSN)
...
// ─── Koutaku config.json reader ───────────────────────────────────────────────

/**
 * Resolve an EnvVar field from Koutaku config.
...
function postgresUrlFromConfig(c) {
  ...
  const user     = resolveEnvVar(c.user)     || 'koutaku';
  const password = resolveEnvVar(c.password) || '';
  const dbName   = resolveEnvVar(c.db_name)  || 'koutaku';
...
/**
 * Read a Koutaku config.json and return a DB connection URL for the main
 * config_store, or null if not enabled / unreadable.
 */
function dbUrlFromKoutakuConfig(configPath) {
  let raw;
...
/**
 * Read a Koutaku config.json and return a DB connection URL for the logs_store,
 * or null if not enabled / unreadable.
 */
function logsDbUrlFromKoutakuConfig(configPath) {
  let raw;
  try { raw = fs.readFileSync(configPath, 'utf8'); }
  catch (_) { return null; }
...
module.exports = function (newman, options) {
  ...
  if (!dbUrl) {
    dbUrl = dbUrlFromKoutakuConfig(configPath);
  ...
  if (!logsDbUrl) {
    logsDbUrl = logsDbUrlFromKoutakuConfig(configPath);
