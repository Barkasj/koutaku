...
// Optional env:
//
//	KOUTAKU_INTEGRATION_PATH  — override the default /openai/v1/responses
//	KOUTAKU_INTEGRATION_MODEL — model string; defaults to ASYNC_RESPONSES_MODEL default
//
...
func integrationPath() string {
	return envOr("KOUTAKU_INTEGRATION_PATH", "/openai/v1/responses")
}

func integrationModel() string {
	if v := os.Getenv("KOUTAKU_INTEGRATION_MODEL"); v != "" {
		return v
	}
