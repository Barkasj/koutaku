...
// Requires KOUTAKU_VK to be set, which proves VK governance is active on the server.
func TestAuth_Submit_InvalidVK_Returns400(t *testing.T) {
	if cfg.VK == "" {
		t.Skip("KOUTAKU_VK not set — governance may not be active")
	}
...
func TestAuth_VKScoped_DifferentKey_Returns404(t *testing.T) {
	if cfg.VK == "" || cfg.AltVK == "" {
		t.Skip("both KOUTAKU_VK and KOUTAKU_ALT_VK must be set")
	}
	ec := chatCompletionCase()
	_, submitted, body := submitCase(t, ec, vkHeaders(cfg.VK))
	if submitted.ID == "" {
...
// x-goog-api-key header paths in ConvertToKoutakuContext.
...
// polls with the same header.  Verifies the Bearer token path in ConvertToKoutakuContext.
func TestAuth_BearerVK_SameKey_Succeeds(t *testing.T) {
	if cfg.VK == "" || !vkPrefixed(cfg.VK) {
		t.Skip("KOUTAKU_VK not set or does not start with sk-bf- prefix")
	}
	ec := chatCompletionCase()
...
// the same header.  Verifies the x-api-key path in ConvertToKoutakuContext.
...
// with the same header.  Verifies the x-goog-api-key path in ConvertToKoutakuContext.
