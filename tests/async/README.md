...
End-to-end tests for Koutaku's async inference feature (`/v1/async/*` endpoints and integration route headers).
...
```bash
KOUTAKU_VK=sk-bf-... KOUTAKU_ALT_VK=sk-bf-... go test ./... -timeout 300s
```
...
|---|---|---|
| `KOUTAKU_BASE_URL` | `http://localhost:8080` | Koutaku gateway URL |
| `KOUTAKU_VK` | — | Primary virtual key; enables VK-mode tests |
| `KOUTAKU_ALT_VK` | — | Second virtual key; enables cross-VK auth tests |
| `KOUTAKU_POLL_TIMEOUT` | `30s` | Max time to wait for a job to reach terminal state |
| `KOUTAKU_POLL_INTERVAL` | `500ms` | Polling cadence |
| `KOUTAKU_INTEGRATION_PATH` | `/openai/v1/responses` | Override integration route path |
| `KOUTAKU_INTEGRATION_MODEL` | `openai/gpt-4o-mini` | Override model for integration route tests |
...
- Most tests run in two modes: **global** (no VK) and **with_vk** (when `KOUTAKU_VK` is set).
...
- `KOUTAKU_ALT_VK` is only required for cross-VK isolation tests (`TestAuth_VKScoped_DifferentKey_Returns404`, `TestIntegration_VKScope_DifferentKey_Returns4xx`).
