# Koutaku behind NGINX (Docker Compose)

This example runs 3 Koutaku containers behind an NGINX reverse proxy.

## Files

- `docker-compose.yml` - Starts NGINX and 3 Koutaku nodes
- `nginx.conf` - Reverse proxy and load balancing config
- `config.json` - Shared Koutaku config for all nodes
...
NGINX exposes Koutaku on `http://localhost:8080`.
