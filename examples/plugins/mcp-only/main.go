...
func PreMCPHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	...
	if pluginConfig.EnableAudit {
		auditMsg := fmt.Sprintf("MCP request processed at %v", ctx.Value(schemas.KoutakuContextKey("request_id")))
		ctx.SetValue(schemas.KoutakuContextKey("mcp-audit-trail"), auditMsg)
		if pluginConfig.EnableLogging {
...
func PostMCPHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError, error) {
	...
	if pluginConfig.EnableAudit {
		auditTrail := ctx.Value(schemas.KoutakuContextKey("mcp-audit-trail"))
		if pluginConfig.EnableLogging {
			fmt.Printf("[MCP-Only Plugin] Audit trail: %v\n", auditTrail)
		}
	}
	...
	if koutakuErr != nil && koutakuErr.Error != nil {
		fmt.Printf("[MCP-Only Plugin] Error occurred: %v\n", koutakuErr.Error.Message)
	}

	// Example: Transform error responses (configurable)
	if pluginConfig.TransformErrors && koutakuErr != nil && koutakuErr.StatusCode != nil && *koutakuErr.StatusCode == 404 {
		// Convert 404 to a more user-friendly error
		if koutakuErr.Error != nil {
			koutakuErr.Error.Message = "The requested MCP resource was not found. Please check your request."
			if pluginConfig.EnableLogging {
