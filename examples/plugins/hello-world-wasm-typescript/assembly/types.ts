/**
 * Type definitions for Koutaku WASM plugins.
 * 
...
/**
 * KoutakuContext holds request-scoped values passed between hooks.
 * Common keys include:
 * - request_id: Unique identifier for the request
 * - Custom plugin values can be added and will be persisted across hooks
 */
@json
export class KoutakuContext {
  request_id: string = ''
...
export class HTTPStreamChunkHookInput {
  context: JSON.Obj = new JSON.Obj()
  request: JSON.Raw = new JSON.Raw('null')
  chunk: JSON.Raw = new JSON.Raw('null') // KoutakuStreamChunk as JSON
}
...
export class HTTPStreamChunkHookOutput {
  context: JSON.Obj = new JSON.Obj()
  chunk: JSON.Raw = new JSON.Raw('null') // KoutakuStreamChunk as JSON, or null to skip
