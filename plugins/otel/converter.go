...
// convertTraceToResourceSpan converts a Koutaku trace to OTEL ResourceSpan
...
// convertSpanToOTELSpan converts a single Koutaku span to OTEL format
...
func (p *OtelPlugin) getResourceAttributes() []*KeyValue {
	attrs := []*KeyValue{
		...
		kvStr("service.version", p.koutakuVersion),
		kvStr("telemetry.sdk.name", "koutaku"),
...
// convertSpanKind maps Koutaku SpanKind to OTEL SpanKind
...
// convertSpanStatus maps Koutaku SpanStatus to OTEL Status
...
// convertSpanEvents converts Koutaku span events to OTEL events
