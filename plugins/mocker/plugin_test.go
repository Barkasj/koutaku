...
func TestMockerPlugin_Disabled(t *testing.T) {
	...
	_, koutakuErr := client.ChatCompletionRequest(ctx, &schemas.KoutakuChatRequest{
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("Hello, test message"),
				},
			},
		},
	})
...
func TestMockerPlugin_DefaultMockRule(t *testing.T) {
	...
	response, koutakuErr := client.ChatCompletionRequest(ctx, &schemas.KoutakuChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4",
		Input: []schemas.ChatMessage{
			{
	...
	if koutakuErr != nil {
		t.Fatalf("Expected no error, got: %v", koutakuErr)
	}
	if response == nil {
...
func TestMockerPlugin_ErrorResponse(t *testing.T) {
				...
				Responses: []Response{
					{
						...
						Error: &ErrorResponse{
							...
							Type:       koutaku.Ptr("rate_limit"),
							Code:       koutaku.Ptr("429"),
	...
	if koutakuErr.Error.Message != "Rate limit exceeded" {
		t.Errorf("Expected 'Rate limit exceeded', got: %s", koutakuErr.Error.Message)
	}
	if koutakuErr.StatusCode == nil || *koutakuErr.StatusCode != 429 {
		t.Errorf("Expected status code 429, got: %v", koutakuErr.StatusCode)
	}
}
				...
				Responses: []Response{
					{
						Type: ResponseTypeSuccess,
						Content: &SuccessResponse{
							MessageTemplate: koutaku.Ptr("Hello from {{provider}} using model {{model}}"),
						},
					},
				},
			},
...
func TestMockerPlugin_Statistics(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, _ = client.ChatCompletionRequest(ctx, &schemas.KoutakuChatRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4",
			Input: []schemas.ChatMessage{
				{
