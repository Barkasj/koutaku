...
func BenchmarkMockerPlugin_PreHook_SimpleRule(b *testing.B) {
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("Hello, benchmark test"),
				},
			},
		},
	}
	...
	// Convert to KoutakuRequest for PreLLMHook compatibility
	...
	for i := 0; i < b.N; i++ {
		_, _, _ = plugin.PreLLMHook(ctx, koutakuReq)
	}
}
		...
		Rules: []MockRule{
			{
				...
				Probability: 1.0,
				Conditions: Conditions{
					MessageRegex: koutaku.Ptr(`(?i).*hello.*`),
				},
				Responses: []Response{
					{
						Type: ResponseTypeSuccess,
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("Hello, this should match the regex pattern"),
				},
			},
		},
	}
