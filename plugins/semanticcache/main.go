// Package semanticcache provides semantic caching integration for Koutaku plugin.
...
import (
	...
	"github.com/koutaku/koutaku/framework"
...
type StreamChunk struct {
	...
	Response     *schemas.KoutakuResponse // The actual response chunk
...
const (
	...
	DefaultVectorStoreNamespace         string        = "KoutakuSemanticCachePlugin"
...
var VectorStoreProperties = map[string]vectorstore.VectorStoreProperties{
	...
	"from_koutaku_semantic_cache_plugin": {
		DataType:    vectorstore.VectorStorePropertyTypeBoolean,
		Description: "Whether the cache entry was created by the KoutakuSemanticCachePlugin",
	},
}
...
const (
	CacheKey          schemas.KoutakuContextKey = "semantic_cache_key"        // To set the cache key for a request - REQUIRED for all requests
	CacheTTLKey       schemas.KoutakuContextKey = "semantic_cache_ttl"        // To explicitly set the TTL for a request
	CacheThresholdKey schemas.KoutakuContextKey = "semantic_cache_threshold"  // To explicitly set the threshold for a request
	CacheTypeKey      schemas.KoutakuContextKey = "semantic_cache_cache_type" // To explicitly set the cache type for a request
	CacheNoStoreKey   schemas.KoutakuContextKey = "semantic_cache_no_store"   // To explicitly disable storing the response in the cache

	// context keys for internal usage
	requestIDKey              schemas.KoutakuContextKey = "semantic_cache_request_id"
	requestStorageIDKey       schemas.KoutakuContextKey = "semantic_cache_request_storage_id"
	requestHashKey            schemas.KoutakuContextKey = "semantic_cache_request_hash"
	requestEmbeddingKey       schemas.KoutakuContextKey = "semantic_cache_embedding"
	requestEmbeddingTokensKey schemas.KoutakuContextKey = "semantic_cache_embedding_tokens"
	requestParamsHashKey      schemas.KoutakuContextKey = "semantic_cache_params_hash"
	requestModelKey           schemas.KoutakuContextKey = "semantic_cache_model"
	requestProviderKey        schemas.KoutakuContextKey = "semantic_cache_provider"
	isCacheHitKey             schemas.KoutakuContextKey = "semantic_cache_is_cache_hit"
	cacheHitTypeKey           schemas.KoutakuContextKey = "semantic_cache_cache_hit_type"
)
...
func Init(ctx context.Context, config *Config, logger schemas.Logger, store vectorstore.VectorStore) (schemas.LLMPlugin, error) {
	...
	if config.CacheByModel == nil {
		config.CacheByModel = koutaku.Ptr(true)
	}
	if config.CacheByProvider == nil {
		config.CacheByProvider = koutaku.Ptr(true)
	}
	...
	if config.Provider == "" && config.Dimension == 1 {
		...
		if koutaku.IsStandardProvider(config.Provider) && !ProvidersWithEmbeddingSupport[config.Provider] {
		...
		koutaku, err := koutaku.Init(ctx, schemas.KoutakuConfig{
			Logger: logger,
		...
		if err != nil {
			return nil, fmt.Errorf("failed to initialize koutaku for semantic cache: %w", err)
		}

		plugin.client = koutaku
	}
...
func (plugin *Plugin) HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	return nil, nil
}

// HTTPTransportPostHook is not used for this plugin
func (plugin *Plugin) HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	return nil
}

// HTTPTransportStreamChunkHook passes through streaming chunks unchanged
func (plugin *Plugin) HTTPTransportStreamChunkHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, chunk *schemas.KoutakuStreamChunk) (*schemas.KoutakuStreamChunk, error) {
	return chunk, nil
}

func (plugin *Plugin) clearRequestScopedContext(ctx *schemas.KoutakuContext) {
...
// PreLLMHook is called before a request is processed by Koutaku.
...
//
// Parameters:
//   - ctx: Pointer to the schemas.KoutakuContext
//   - req: The incoming Koutaku request
//
// Returns:
//   - *schemas.KoutakuRequest: The original request
//   - *schemas.KoutakuResponse: Cached response if found, nil otherwise
//   - error: Any error that occurred during cache lookup
func (plugin *Plugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
...
//
// Parameters:
//   - ctx: Pointer to the schemas.KoutakuContext containing the request hash and ID
//   - res: The response from the provider to be cached
//   - koutakuErr: The error from the provider, if any (used for success determination)
//
// Returns:
//   - *schemas.KoutakuResponse: The original response, unmodified
//   - *schemas.KoutakuError: The original error, unmodified
//   - error: Any error that occurred during caching preparation (always nil as errors are handled gracefully)
func (plugin *Plugin) PostLLMHook(ctx *schemas.KoutakuContext, res *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	if koutakuErr != nil {
		return res, koutakuErr, nil
	}
	...
	if ok {
		isStreamRequest := koutaku.IsStreamRequestType(requestType)

		if !isStreamRequest || (isStreamRequest && isFinalChunk) {
			if extraFields.CacheDebug == nil {
				extraFields.CacheDebug = &schemas.KoutakuCacheDebug{}
			}
			extraFields.CacheDebug.CacheHit = false
			extraFields.CacheDebug.ProviderUsed = koutaku.Ptr(string(plugin.config.Provider))
			extraFields.CacheDebug.ModelUsed = koutaku.Ptr(plugin.config.EmbeddingModel)
	...
	go func() {
		...
		if koutaku.IsStreamRequestType(requestType) {
			if err := plugin.addStreamingResponse(cacheCtx, requestID, storageID, res, koutakuErr, embeddingToStore, unifiedMetadata, cacheTTL, isFinalChunk); err != nil {
...
func (plugin *Plugin) Cleanup() error {
	...
	// Shutdown the internal Koutaku client used for embeddings
	...
	queries := []vectorstore.Query{
		{
			Field:    "from_koutaku_semantic_cache_plugin",
