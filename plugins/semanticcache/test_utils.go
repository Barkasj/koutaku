...
import (
	...
	"github.com/koutaku/koutaku/framework/vectorstore"
	mocker "github.com/koutaku/koutaku/plugins/mocker"
)
...
func getMockRules() []mocker.MockRule {
	return []mocker.MockRule{
		// Core test prompts
		{
			Name:        "koutaku-definition",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)What is Koutaku.*")},
			Probability: 1.0,
			Responses: []mocker.Response{
				{Type: mocker.ResponseTypeSuccess, Content: &mocker.SuccessResponse{Message: "Koutaku is a unified API for interacting with multiple AI providers."}},
			},
		},
		{
			Name:        "machine-learning-explanation",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)what is machine learning\\?|explain machine learning|machine learning concepts|can you explain machine learning|explain the basics of machine learning")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "ai-explanation",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)what is artificial intelligence\\?|can you explain what ai is\\?|define artificial intelligence")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("What is the capital of France\\?")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "newton-laws",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)describe.*newton.*three laws|describe.*three laws.*newton")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Weather-related prompts
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)what.*weather|weather.*like")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Blockchain and deep learning
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)define blockchain|blockchain technology")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "deep-learning",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)what is deep learning")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Quantum computing
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)quantum computing|explain quantum")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Conversation prompts
		{
			Name:        "hello-greeting",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)^hello$|^hi$|hello.*world")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "how-are-you",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)how are you")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)meaning of life")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "short-story",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)tell me.*short story")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Test-specific prompts
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)test configuration")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "test-messages",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)test.*message|test.*no-store|test.*cache|test.*error|ttl test|threshold test|provider.*test|edge case test")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			Name:        "long-prompt",
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)very long prompt")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		},
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)test.*parameters|performance test")},
			Probability: 1.0,
			Responses: []mocker.Response{
		...
		// Dynamic message patterns (for conversation tests)
		{
			...
			Enabled:     true,
			Conditions:  mocker.Conditions{MessageRegex: koutaku.Ptr("(?i)message \\d+")},
			Probability: 1.0,
			Responses: []mocker.Response{
...
// getMockedKoutakuClient creates a Koutaku client with a mocker plugin for testing
func getMockedKoutakuClient(t *testing.T, ctx *schemas.KoutakuContext, logger schemas.Logger, semanticCachePlugin schemas.LLMPlugin) *koutaku.Koutaku {
	...
	account := &BaseAccount{}
	client, err := koutaku.Init(ctx, schemas.KoutakuConfig{
		Account:    account,
		LLMPlugins: []schemas.LLMPlugin{semanticCachePlugin, mockerPlugin},
		Logger:     logger,
	})
	if err != nil {
		t.Fatalf("Error initializing Koutaku with mocker: %v", err)
	}
...
func NewTestSetupWithVectorStore(t *testing.T, config *Config, storeType vectorstore.VectorStoreType) *TestSetup {
	...
	plugin, err := Init(schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline), config, logger, store)
	if err != nil {
	...
	// Get a mocked Koutaku client
	client := getMockedKoutakuClient(t, ctx, logger, plugin)
...
func CreateBasicChatRequest(content string, temperature float64, maxTokens int) *schemas.KoutakuChatRequest {
...
func CreateStreamingChatRequest(content string, temperature float64, maxTokens int) *schemas.KoutakuChatRequest {
...
func CreateSpeechRequest(input string, voice string) *schemas.KoutakuSpeechRequest {
...
func AssertCacheHit(t *testing.T, response *schemas.KoutakuResponse, expectedCacheType string) {
...
func AssertNoCacheHit(t *testing.T, response *schemas.KoutakuResponse) {
	extraFields := response.GetExtraFields()
...
func CreateEmbeddingRequest(texts []string) *schemas.KoutakuEmbeddingRequest {
...
func CreateBasicResponsesRequest(content string, temperature float64, maxTokens int) *schemas.KoutakuResponsesRequest {
...
func CreateResponsesRequestWithTools(content string, temperature float64, maxTokens int, tools []schemas.ResponsesTool) *schemas.KoutakuResponsesRequest {
...
func CreateResponsesRequestWithInstructions(content string, instructions string, temperature float64, maxTokens int) *schemas.KoutakuResponsesRequest {
	req := CreateBasicResponsesRequest(content, temperature, maxTokens)
...
func CreateStreamingResponsesRequest(content string, temperature float64, maxTokens int) *schemas.KoutakuResponsesRequest {
...
func CreateImageGenerationRequest(prompt string, size string, quality string) *schemas.KoutakuImageGenerationRequest {
	return &schemas.KoutakuImageGenerationRequest{
		...
		Params: &schemas.ImageGenerationParameters{
			Size:    koutaku.Ptr(size),
			Quality: koutaku.Ptr(quality),
			N:       koutaku.Ptr(1),
		},
	}
}
...
func CreateContextWithCacheKey(value string) *schemas.KoutakuContext {
	return schemas.NewKoutakuContextWithValue(context.Background(), schemas.NoDeadline, CacheKey, value)
}

// CreateContextWithCacheKeyAndType creates a context with cache key and cache type
func CreateContextWithCacheKeyAndType(value string, cacheType CacheType) *schemas.KoutakuContext {
	return schemas.NewKoutakuContextWithValue(context.Background(), schemas.NoDeadline, CacheKey, value).WithValue(CacheTypeKey, cacheType)
}

// CreateContextWithCacheKeyAndTTL creates a context with cache key and custom TTL
func CreateContextWithCacheKeyAndTTL(value string, ttl time.Duration) *schemas.KoutakuContext {
	return schemas.NewKoutakuContextWithValue(context.Background(), schemas.NoDeadline, CacheKey, value).WithValue(CacheTTLKey, ttl)
}

// CreateContextWithCacheKeyAndThreshold creates a context with cache key and custom threshold
func CreateContextWithCacheKeyAndThreshold(value string, threshold float64) *schemas.KoutakuContext {
	return schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline).WithValue(CacheKey, value).WithValue(CacheThresholdKey, threshold)
}

// CreateContextWithCacheKeyAndNoStore creates a context with cache key and no-store flag
func CreateContextWithCacheKeyAndNoStore(value string, noStore bool) *schemas.KoutakuContext {
	return schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline).WithValue(CacheKey, value).WithValue(CacheNoStoreKey, noStore)
}
...
func CreateConversationRequest(messages []schemas.ChatMessage, temperature float64, maxTokens int) *schemas.KoutakuChatRequest {
	return &schemas.KoutakuChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4o-mini",
		Input:    messages,
