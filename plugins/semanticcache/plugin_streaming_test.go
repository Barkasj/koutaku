...
func TestStreamingCacheBasicFunctionality(t *testing.T) {
	...
	var responses1 []schemas.KoutakuChatResponse
	for streamMsg := range stream1 {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in first stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuChatResponse != nil {
			responses1 = append(responses1, *streamMsg.KoutakuChatResponse)
		}
	}
	...
	var responses2 []schemas.KoutakuChatResponse
	for streamMsg := range stream2 {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuChatResponse != nil {
			responses2 = append(responses2, *streamMsg.KoutakuChatResponse)
		}
	}
...
func TestStreamingVsNonStreaming(t *testing.T) {
	...
	var streamResponses []schemas.KoutakuChatResponse
	for streamMsg := range stream {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuChatResponse != nil {
			streamResponses = append(streamResponses, *streamMsg.KoutakuChatResponse)
		}
	}
	...
	for _, response := range streamResponses {
		if response.ExtraFields.RawResponse != nil {
			if rawMap, ok := response.ExtraFields.RawResponse.(map[string]interface{}); ok {
				if cachedFlag, exists := rawMap["koutaku_cached"]; exists {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: nonStreamResponse})
...
func TestStreamingChunkOrdering(t *testing.T) {
	...
	var originalChunks []schemas.KoutakuChatResponse
	for streamMsg := range stream1 {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in first stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuChatResponse != nil {
			originalChunks = append(originalChunks, *streamMsg.KoutakuChatResponse)
		}
	}
	...
	var cachedChunks []schemas.KoutakuChatResponse
	for streamMsg := range stream2 {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuChatResponse != nil {
			cachedChunks = append(cachedChunks, *streamMsg.KoutakuChatResponse)
		}
	}
	...
	for i := 0; i < len(cachedChunks) && i < len(originalChunks); i++ {
		...
		if i == len(cachedChunks)-1 {
			AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: &cachedChunks[i]}, string(CacheTypeDirect))
		}
	}
...
func TestSpeechSynthesisStreaming(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{SpeechResponse: response2}, string(CacheTypeDirect))
