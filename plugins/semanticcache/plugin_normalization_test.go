...
func testChatCompletionNormalization(t *testing.T, setup *TestSetup) {
	...
	requests := make([]*schemas.KoutakuChatRequest, len(testCases))
	for i, tc := range testCases {
		requests[i] = &schemas.KoutakuChatRequest{
	...
	for i := 1; i < len(testCases); i++ {
		...
		AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response}, "direct")
...
func testSpeechNormalization(t *testing.T, setup *TestSetup) {
	...
	requests := make([]*schemas.KoutakuSpeechRequest, len(testCases))
	for i, tc := range testCases {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{SpeechResponse: response1})
	WaitForCache(setup.Plugin)
	...
	for i := 1; i < len(testCases); i++ {
		...
		// Should be cache hit due to normalization
		AssertCacheHit(t, &schemas.KoutakuResponse{SpeechResponse: response}, "direct")
		t.Logf("✓ Cache hit for '%s' variation", tc.name)
	}
}
