...
func (plugin *Plugin) prepareDirectCacheLookup(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, cacheKey string) (string, error) {
...
func (plugin *Plugin) performLegacyDirectSearch(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
	...
	filters := []vectorstore.Query{
		...
		{Field: "from_koutaku_semantic_cache_plugin", Operator: vectorstore.QueryOperatorEqual, Value: true},
	}
	...
	if koutaku.IsStreamRequestType(req.RequestType) {
...
func (plugin *Plugin) performDirectChunkLookup(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
...
func (plugin *Plugin) performDirectSearch(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
...
func (plugin *Plugin) generateEmbeddingsForStorage(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) error {
...
func (plugin *Plugin) performSemanticSearch(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, cacheKey string) (*schemas.LLMPluginShortCircuit, error) {
	// Extract text and metadata for embedding
	text, paramsHash, err := plugin.extractTextForEmbedding(req)
	if err != nil {
...
func (plugin *Plugin) buildResponseFromResult(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, result vectorstore.SearchResult, cacheType CacheType, threshold float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
...
func (plugin *Plugin) buildSingleResponseFromResult(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, result vectorstore.SearchResult, responseData interface{}, cacheType CacheType, threshold float64, similarity float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
	...
	var cachedResponse schemas.KoutakuResponse
	...
	extraFields.CacheDebug.HitType = koutaku.Ptr(string(cacheType))
	extraFields.CacheDebug.CacheID = koutaku.Ptr(result.ID)
	extraFields.CacheDebug.RequestedProvider = koutaku.Ptr(string(requestedProvider))
	extraFields.CacheDebug.RequestedModel = koutaku.Ptr(requestedModel)
...
func (plugin *Plugin) buildStreamingResponseFromResult(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest, result vectorstore.SearchResult, streamData interface{}, cacheType CacheType, threshold float64, similarity float64, inputTokens int) (*schemas.LLMPluginShortCircuit, error) {
	requestedProvider, requestedModel, _ := req.GetRequestFields()
	...
	streamChan := make(chan *schemas.KoutakuStreamChunk)
	...
	go func() {
		...
		for i, chunkData := range streamArray {
			...
			// Unmarshal the chunk as KoutakuResponse
			var cachedResponse schemas.KoutakuResponse
			...
			if i == len(streamArray)-1 {
				...
				cacheDebug := schemas.KoutakuCacheDebug{
					CacheHit:          true,
					HitType:           koutaku.Ptr(string(cacheType)),
					CacheID:           koutaku.Ptr(result.ID),
					RequestedProvider: koutaku.Ptr(string(requestedProvider)),
					RequestedModel:    koutaku.Ptr(requestedModel),
				}
				if cacheType == CacheTypeSemantic {
					cacheDebug.ProviderUsed = koutaku.Ptr(string(plugin.config.Provider))
					cacheDebug.ModelUsed = koutaku.Ptr(plugin.config.EmbeddingModel)
			...
			streamChan <- &schemas.KoutakuStreamChunk{
				KoutakuTextCompletionResponse:        cachedResponse.TextCompletionResponse,
				KoutakuChatResponse:                  cachedResponse.ChatResponse,
				KoutakuResponsesStreamResponse:       cachedResponse.ResponsesStreamResponse,
				KoutakuSpeechStreamResponse:          cachedResponse.SpeechStreamResponse,
				KoutakuTranscriptionStreamResponse:   cachedResponse.TranscriptionStreamResponse,
				KoutakuImageGenerationStreamResponse: cachedResponse.ImageGenerationStreamResponse,
			}
		}
	}()
