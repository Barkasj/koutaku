...
func TestDefaultCacheKey_CachesWithoutPerRequestKey(t *testing.T) {
	...
	testRequest := CreateBasicChatRequest("What is Koutaku? Answer in one short sentence.", 0.7, 50)
...
func TestDefaultCacheKey_PerRequestKeyOverridesDefault(t *testing.T) {
	...
	ctx1 := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	...
	ctxDefault2 := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: responseDefault2}, string(CacheTypeDirect))
