...
func TestParameterVariations(t *testing.T) {
	...
	tests := []struct {
		name        string
		request1    *schemas.KoutakuChatRequest
		request2    *schemas.KoutakuChatRequest
		shouldCache bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldCache {
				AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}, string(CacheTypeDirect))
			} else {
				AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2})
			}
		})
	}
}
...
func TestToolVariations(t *testing.T) {
	...
	baseRequest := &schemas.KoutakuChatRequest{
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("What's the weather like today?"),
				},
			},
		},
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: koutaku.Ptr(100),
			Temperature:         koutaku.Ptr(0.5),
		},
	}

	// Request with tools
	requestWithTools := &schemas.KoutakuChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4o-mini",
		Input: []schemas.ChatMessage{
			{
			...
			Tools: []schemas.ChatTool{
				{
					...
					Function: &schemas.ChatToolFunction{
						Name:        "get_weather",
						Description: koutaku.Ptr("Get the current weather"),
	...
	requestWithDifferentTools := &schemas.KoutakuChatRequest{
		Provider: schemas.OpenAI,
		Model:    "gpt-4o-mini",
		Input: []schemas.ChatMessage{
			{
			...
			Tools: []schemas.ChatTool{
				{
					...
					Function: &schemas.ChatToolFunction{
						...
						Description: koutaku.Ptr("Get current weather information"),
						Parameters: &schemas.ToolFunctionParameters{
							Type: "object",
							Properties: schemas.NewOrderedMapFromPairs(
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response3}, "")
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response4})
							...
							ContentBlocks: []schemas.ChatContentBlock{
								{
									...
									Text: koutaku.Ptr("Analyze this image"),
								},
								{
				...
				Input: []schemas.ChatMessage{
					{
						...
						Content: &schemas.ChatMessageContent{
							ContentBlocks: []schemas.ChatContentBlock{
								{
									Type: schemas.ChatContentBlockTypeText,
									Text: koutaku.Ptr("Compare these images"),
								},
								{
									Type: schemas.ChatContentBlockTypeImage,
									ImageURLStruct: &schemas.ChatInputImage{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr(strings.Repeat("This is a very long prompt. ", 100)),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(50),
					Temperature:         koutaku.Ptr(0.2),
				},
			},
		},
		{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("What is AI?"),
						},
					},
					{
						Role: schemas.ChatMessageRoleAssistant,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("AI stands for Artificial Intelligence..."),
						},
					},
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("Can you give me examples?"),
						},
					},
				},
				Params: &schemas.ChatParameters{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("Test max parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(4096),
					PresencePenalty:     koutaku.Ptr(2.0),
					FrequencyPenalty:    koutaku.Ptr(2.0),
					Temperature:         koutaku.Ptr(2.0),
					TopP:                koutaku.Ptr(1.0),
				},
			},
		},
		{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("Test min parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(1),
					PresencePenalty:     koutaku.Ptr(-2.0),
					FrequencyPenalty:    koutaku.Ptr(-2.0),
					Temperature:         koutaku.Ptr(0.0),
					TopP:                koutaku.Ptr(0.01),
				},
			},
		},
		{
				...
				Input: []schemas.ChatMessage{
					{
						Role: schemas.ChatMessageRoleUser,
						Content: &schemas.ChatMessageContent{
							ContentStr: koutaku.Ptr("Test edge case parameters"),
						},
					},
				},
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(1),
					User:                koutaku.Ptr("test-user-id-12345"),
					Temperature:         koutaku.Ptr(0.0),
					TopP:                koutaku.Ptr(0.1),
				},
			},
		},
	}
...
func TestSemanticSimilarityEdgeCases(t *testing.T) {
	...
	similarTests := []struct {
		...
		},
		{
			...
			prompt2:     "What do you know about koutaku?",
			shouldMatch: false,
...
func TestErrorHandlingEdgeCases(t *testing.T) {
	...
	t.Run("Request without cache key", func(t *testing.T) {
		ctxNoKey := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
		...
		AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response})
	...
	t.Run("Request with invalid cache key type", func(t *testing.T) {
		...
		ctxInvalidKey := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline).WithValue(CacheKey, 12345)
