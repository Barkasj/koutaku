					...
					Input: []schemas.ChatMessage{
						{
							...
							Content: &schemas.ChatMessageContent{
								ContentStr: koutaku.Ptr("Call the get_weather function"),
							},
						},
						{
							...
							ChatAssistantMessage: &schemas.ChatAssistantMessage{
								ToolCalls: []schemas.ChatAssistantMessageToolCall{
									{
										ID:   koutaku.Ptr("call_123"),
										Type: koutaku.Ptr("function"),
										Function: schemas.ChatAssistantMessageToolCallFunction{
											Name:      koutaku.Ptr("get_weather"),
...
func TestPrepareDirectCacheLookup_UnsupportedRequestTypeFailsClosed(t *testing.T) {
	...
	req := &schemas.KoutakuRequest{
		...
		PassthroughRequest: &schemas.KoutakuPassthroughRequest{
			Provider: schemas.OpenAI,
			Model:    "gpt-4o-mini",
			Method:   "GET",
			Path:     "/v1/models",
...
func TestPreLLMHookSkipsUnsupportedCountTokensRequest(t *testing.T) {
	...
	req := &schemas.KoutakuRequest{
		...
		CountTokensRequest: &schemas.KoutakuResponsesRequest{
			...
			Input: []schemas.ResponsesMessage{
				{
					...
					Content: &schemas.ResponsesMessageContent{
						ContentStr: koutaku.Ptr("How many tokens is this message?"),
					},
				},
			},
		},
...
// createResponsesRequestWithNilContent builds a KoutakuResponsesRequest with a nil Content message for testing.
func createResponsesRequestWithNilContent() *schemas.KoutakuResponsesRequest {
	return &schemas.KoutakuResponsesRequest{
		...
		Params: &schemas.ResponsesParameters{
			Temperature:     koutaku.Ptr(0.7),
			MaxOutputTokens: koutaku.Ptr(100),
		},
	}
}
