...
func TestImageGenerationCacheBasicFunctionality(t *testing.T) {
	...
	// Verify cache hit
	AssertCacheHit(t, &schemas.KoutakuResponse{ImageGenerationResponse: response2}, string(CacheTypeDirect))
...
func TestImageGenerationDifferentParameters(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ImageGenerationResponse: response2})
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ImageGenerationResponse: response3})
...
func TestImageGenerationStreamCaching(t *testing.T) {
	...
	var responses1 []schemas.KoutakuImageGenerationStreamResponse
	for streamMsg := range stream1 {
		...
		if streamMsg.KoutakuImageGenerationStreamResponse != nil {
			responses1 = append(responses1, *streamMsg.KoutakuImageGenerationStreamResponse)
		}
	}
	duration1 := time.Since(start1)
	...
	var responses2 []schemas.KoutakuImageGenerationStreamResponse
	for streamMsg := range stream2 {
		if streamMsg.KoutakuError != nil {
			t.Fatalf("Error in second stream: %v", streamMsg.KoutakuError)
		}
		if streamMsg.KoutakuImageGenerationStreamResponse != nil {
			responses2 = append(responses2, *streamMsg.KoutakuImageGenerationStreamResponse)
		}
	}
	duration2 := time.Since(start2)
