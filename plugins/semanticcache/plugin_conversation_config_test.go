...
func TestConversationHistoryThresholdBasic(t *testing.T) {
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}, "direct") // Should be cached
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response3}) // Should not cache
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response4}) // Should still be fresh (not cached)
...
func TestConversationHistoryThresholdWithSystemPrompt(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response1}) // Should not cache (exceeds threshold)
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}) // Should not be cached
...
func TestConversationHistoryThresholdWithExcludeSystemPrompt(t *testing.T) {
	...
	AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response1}) // Fresh request, should not hit cache
	...
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response2}, "direct") // Should cache since 3 <= 3 after excluding system
...
func TestConversationHistoryThresholdDifferentValues(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < tc.messages; i++ {
				...
				message := schemas.ChatMessage{
					...
					Content: &schemas.ChatMessageContent{
						ContentStr: koutaku.Ptr("Message " + strconv.Itoa(i+1)),
					},
				}
			...
			AssertNoCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response1}) // Always fresh first time
...
func TestExcludeSystemPromptComparison(t *testing.T) {
	...
	// Should hit cache because system prompts are excluded from cache key
	AssertCacheHit(t, &schemas.KoutakuResponse{ChatResponse: response4}, "direct")
...
func TestExcludeSystemPromptWithMultipleSystemMessages(t *testing.T) {
	...
	conversation1 := []schemas.ChatMessage{
		{
			...
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("You are helpful")},
		},
		{
			Role:    schemas.ChatMessageRoleSystem,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Be concise")},
		},
		{
			Role:    schemas.ChatMessageRoleUser,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Hello")},
		},
		{
			Role:    schemas.ChatMessageRoleAssistant,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Hi!")},
		},
	}
	...
	conversation2 := []schemas.ChatMessage{
		{
			Role:    schemas.ChatMessageRoleSystem,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("You are an expert")},
		},
		{
			Role:    schemas.ChatMessageRoleSystem,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Be detailed")},
		},
		{
			Role:    schemas.ChatMessageRoleUser,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Hello")},
...
func TestExcludeSystemPromptWithNoSystemMessages(t *testing.T) {
	...
	conversation := []schemas.ChatMessage{
		...
		{
			Role:    schemas.ChatMessageRoleAssistant,
			Content: &schemas.ChatMessageContent{ContentStr: koutaku.Ptr("Hi there!")},
		},
	}
