...
func TestUnmarshalJSON_BoolPointerFields(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectCacheByModel:  koutaku.Ptr(true),
			expectCacheByProv:   koutaku.Ptr(true),
			expectExcludeSys:    koutaku.Ptr(true),
		},
		{
			name:                "all set to false",
			json:                `{"dimension": 1536, "cache_by_model": false, "cache_by_provider": false, "exclude_system_prompt": false}`,
			expectCacheByModel:  koutaku.Ptr(false),
			expectCacheByProv:   koutaku.Ptr(false),
			expectExcludeSys:    koutaku.Ptr(false),
		},
		{
			name:                "all omitted",
			json:                `{"dimension": 1536}`,
