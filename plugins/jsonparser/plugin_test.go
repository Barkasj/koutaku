...
// TestJsonParserPluginEndToEnd tests the integration of the JSON parser plugin with Koutaku.
// It performs the following steps:
// 1. Initializes the JSON parser plugin with AllRequests usage
// 2. Sets up a test Koutaku instance with the plugin
...
func TestJsonParserPluginEndToEnd(t *testing.T) {
	...
	// Initialize Koutaku with the plugin
	...
	if err != nil {
		t.Fatalf("Error initializing Koutaku: %v", err)
	}
		...
		Input: []schemas.ChatMessage{
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("Return a JSON object with name, age, and city fields. Example: {\"name\": \"John\", \"age\": 30, \"city\": \"New York\"}"),
				},
			},
		},
	...
	responseChan, koutakuErr := client.ChatCompletionStreamRequest(ctx, request)

	if koutakuErr != nil {
		t.Fatalf("Error in Koutaku request: %v", koutakuErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.KoutakuError != nil {
				t.Logf("Streaming response error: %v", streamResponse.KoutakuError)
			}

			if streamResponse.KoutakuChatResponse != nil {
				if streamResponse.KoutakuChatResponse.Choices != nil {
					for _, outputMsg := range streamResponse.KoutakuChatResponse.Choices {
...
func TestJsonParserPluginPerRequest(t *testing.T) {
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: koutaku.Ptr("Return a JSON object with name and age fields."),
				},
			},
		},
		Params: &schemas.ChatParameters{
	...
	newContext := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline).WithValue(EnableStreamingJSONParser, true)

	// Make the streaming request
	responseChan, koutakuErr := client.ChatCompletionStreamRequest(newContext, request)

	if koutakuErr != nil {
		t.Logf("Error in Koutaku request: %v", koutakuErr)
	}
	...
	if responseChan != nil {
		...
		for streamResponse := range responseChan {
			...
			if streamResponse.KoutakuChatResponse != nil {
				for _, choice := range streamResponse.KoutakuChatResponse.Choices {
