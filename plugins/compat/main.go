...
// Koutaku gateway. It drops unsupported model params first, then rewrites
...
func (p *CompatPlugin) HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	return nil, nil
}

// HTTPTransportPostHook is not used for this plugin
func (p *CompatPlugin) HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	return nil
}

// HTTPTransportStreamChunkHook passes through streaming chunks unchanged.
func (p *CompatPlugin) HTTPTransportStreamChunkHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, chunk *schemas.KoutakuStreamChunk) (*schemas.KoutakuStreamChunk, error) {
	return chunk, nil
}

// PreLLMHook intercepts requests and applies LiteLLM-compatible request normalization.
func (p *CompatPlugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	if ctx == nil || req == nil {
		return req, nil, nil
	}

	convertTextToChatOverride, convertTextToChatOverrideEnabled := ctx.Value(schemas.KoutakuContextKeyCompatConvertTextToChat).(bool)
	convertChatToResponsesOverride, convertChatToResponsesOverrideEnabled := ctx.Value(schemas.KoutakuContextKeyCompatConvertChatToResponses).(bool)
	shouldDropParamsOverride, shouldDropParamsOverrideEnabled := ctx.Value(schemas.KoutakuContextKeyCompatShouldDropParams).(bool)
	shouldConvertParamsOverride, shouldConvertParamsOverrideEnabled := ctx.Value(schemas.KoutakuContextKeyCompatShouldConvertParams).(bool)
	...
	if (shouldDropParamsOverrideEnabled && shouldDropParamsOverride) || (shouldConvertParamsOverrideEnabled && shouldDropParamsOverride) || p.config.ShouldConvertParams || p.config.ShouldDropParams {
		modifiedReq = cloneKoutakuReq(req)
	}
	p.droppedParams = nil
...
func (p *CompatPlugin) PostLLMHook(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	if ctx == nil {
		return result, koutakuErr, nil
	}

	if changeType, ok := ctx.Value(schemas.KoutakuContextKeyChangeRequestType).(schemas.RequestType); ok {
		if result != nil {
		...
		if koutakuErr != nil {
			koutakuErr.ExtraFields.ConvertedRequestType = changeType
		}
	}
...
func (p *CompatPlugin) markForConversion(ctx *schemas.KoutakuContext, provider schemas.ModelProvider, model string, currentType schemas.RequestType, targetType schemas.RequestType) {
	shouldConvert := false
	...
	if shouldConvert {
		ctx.SetValue(schemas.KoutakuContextKeyChangeRequestType, targetType)
	}
}
