...
func isPassthroughErrorResponse(result *schemas.KoutakuResponse) bool {
	return result != nil &&
...
// shouldStoreRaw gates whether raw request/response bytes are written to the entry.
func (p *LoggerPlugin) applyNonStreamingOutputToEntry(entry *logstore.Log, result *schemas.KoutakuResponse, shouldStoreRaw bool) {
	if result == nil {
		return
	}
	// Token usage
	...
	case result.ResponsesResponse != nil && result.ResponsesResponse.Usage != nil:
		usage = result.ResponsesResponse.Usage.ToKoutakuLLMUsage()
...
func (p *LoggerPlugin) applyRealtimeOutputToEntry(entry *logstore.Log, result *schemas.KoutakuResponse, shouldStoreRaw bool) {
	if result == nil || result.ResponsesResponse == nil {
		return
	}

	if usage := result.ResponsesResponse.Usage; usage != nil {
		koutakuUsage := usage.ToKoutakuLLMUsage()
		entry.TokenUsageParsed = koutakuUsage
		entry.PromptTokens = koutakuUsage.PromptTokens
		entry.CompletionTokens = koutakuUsage.CompletionTokens
		entry.TotalTokens = koutakuUsage.TotalTokens
	}
...
func extractRealtimeTranscript(event *schemas.KoutakuRealtimeEvent) string {
...
func (p *LoggerPlugin) calculateCostForLog(logEntry *logstore.Log) (float64, error) {
	...
	// Build a minimal KoutakuResponse matching the request type so that
	...
	// Patch modality-specific output fields that are not captured in KoutakuLLMUsage
...
// buildResponseForRequestType wraps KoutakuLLMUsage into the correct response
// field so that CalculateCost's extractCostInput routes it properly.
func buildResponseForRequestType(requestType schemas.RequestType, usage *schemas.KoutakuLLMUsage, extra schemas.KoutakuResponseExtraFields) *schemas.KoutakuResponse {
	switch requestType {
	...
	case schemas.OCRRequest:
		return &schemas.KoutakuResponse{
			OCRResponse: &schemas.KoutakuOCRResponse{
				ExtraFields: extra,
			},
		}
	case schemas.ResponsesRequest, schemas.ResponsesStreamRequest:
		// Convert KoutakuLLMUsage back to ResponsesResponseUsage, preserving token
		...
		return &schemas.KoutakuResponse{
			ResponsesResponse: &schemas.KoutakuResponsesResponse{
				Usage:       respUsage,
				ExtraFields: extra,
			},
		}
	...
	case schemas.ImageGenerationRequest, schemas.ImageGenerationStreamRequest,
		...
		// Log entries only store KoutakuLLMUsage; convert to ImageUsage for proper routing
	...
	case schemas.VideoGenerationRequest, schemas.VideoRemixRequest:
		// Seconds is not stored in KoutakuLLMUsage; the caller must patch it in from
