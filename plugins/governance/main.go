// Package governance provides comprehensive governance plugin for Koutaku
package governance
...
const (
	governanceRejectedContextKey schemas.KoutakuContextKey = "bf-governance-rejected"
...
type BaseGovernancePlugin interface {
	GetName() string
	EvaluateGovernanceRequest(ctx *schemas.KoutakuContext, evaluationRequest *EvaluationRequest, requestType schemas.RequestType) (*EvaluationResult, *schemas.KoutakuError)
	HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error)
	HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error
	PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error)
	PostLLMHook(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error)
	PreMCPHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, error)
	PostMCPHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError, error)
	Cleanup() error
...
func (p *GovernancePlugin) HTTPTransportPreHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	...
	if virtualKey != nil {
		if virtualKey.TeamID != nil {
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceTeamID, *virtualKey.TeamID)
		}
		if virtualKey.Team != nil {
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceTeamName, virtualKey.Team.Name)
		}
		if virtualKey.CustomerID != nil {
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerID, *virtualKey.CustomerID)
		}
		if virtualKey.Customer != nil {
			ctx.SetValue(schemas.KoutakuContextKeyGovernanceCustomerName, virtualKey.Customer.Name)
		}
	}
...
func (p *GovernancePlugin) governLargePayload(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, virtualKeyValue *string, hasRoutingRules bool) (*schemas.HTTPResponse, error) {
	metadata, _ := ctx.Value(schemas.KoutakuContextKeyLargePayloadMetadata).(*schemas.LargePayloadMetadata)
...
func (p *GovernancePlugin) HTTPTransportPostHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, resp *schemas.HTTPResponse) error {
	return nil
}

// HTTPTransportStreamChunkHook passes through streaming chunks unchanged
func (p *GovernancePlugin) HTTPTransportStreamChunkHook(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, chunk *schemas.KoutakuStreamChunk) (*schemas.KoutakuStreamChunk, error) {
	return chunk, nil
}
...
func (p *GovernancePlugin) loadBalanceProvider(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, body map[string]any, virtualKey *configstoreTables.TableVirtualKey) (map[string]any, error) {
	...
	schemas.AppendToContextList(ctx, schemas.KoutakuContextKeyRoutingEnginesUsed, schemas.RoutingEngineGovernance)
...
func (p *GovernancePlugin) applyRoutingRules(ctx *schemas.KoutakuContext, req *schemas.HTTPRequest, body map[string]any, virtualKey *configstoreTables.TableVirtualKey) (map[string]any, *RoutingDecision, error) {
	// Check if the request has a model field
	modelValue, hasModel := body["model"]
	isGeminiPath := strings.Contains(req.Path, "/genai")
	isBedrockPath := strings.Contains(req.Path, "/bedrock")
	...
	requestType := ""
	if val := ctx.Value(schemas.KoutakuContextKeyHTTPRequestType); val != nil {
	...
	if decision != nil {
		...
		schemas.AppendToContextList(ctx, schemas.KoutakuContextKeyRoutingEnginesUsed, schemas.RoutingEngineRoutingRule)
		...
		if decision.KeyID != "" {
			ctx.SetValue(schemas.KoutakuContextKeyAPIKeyID, decision.KeyID)
		}
...
// Returns a KoutakuError with status 400 if any required headers are missing, or nil if all present.
func (p *GovernancePlugin) validateRequiredHeaders(ctx *schemas.KoutakuContext) *schemas.KoutakuError {
	if p.requiredHeaders == nil || len(*p.requiredHeaders) == 0 {
		return nil
	}
	headers, _ := ctx.Value(schemas.KoutakuContextKeyRequestHeaders).(map[string]string)
	if headers == nil {
	...
	if len(missing) > 0 {
		return &schemas.KoutakuError{
			Type:       koutaku.Ptr("missing_required_headers"),
			StatusCode: koutaku.Ptr(400),
...
// and governance evaluation logic. It returns the evaluation result and a KoutakuError
...
//   - *schemas.KoutakuError: The error to return if request is not allowed, nil if allowed
func (p *GovernancePlugin) EvaluateGovernanceRequest(ctx *schemas.KoutakuContext, evaluationRequest *EvaluationRequest, requestType schemas.RequestType) (*EvaluationResult, *schemas.KoutakuError) {
	...
	if evaluationRequest.VirtualKey != "" {
		...
		if exists {
			...
			return nil, &schemas.KoutakuError{
				Type:       koutaku.Ptr("virtual_key_not_found"),
				StatusCode: koutaku.Ptr(401),
				Error: &schemas.ErrorField{
	...
	if !isVirtualKeyValid && evaluationRequest.UserID == "" && p.isVkMandatory != nil && *p.isVkMandatory {
		...
		p.cfgMutex.RUnlock()
		return nil, &schemas.KoutakuError{
			Type:       koutaku.Ptr("virtual_key_required"),
			StatusCode: koutaku.Ptr(401),
			Error: &schemas.ErrorField{
				Message: message,
			},
	...
	// KoutakuContextKeyMCPAddedTools is populated by AddToolsToRequest (which runs before
	...
	if result.Decision == DecisionAllow && result.VirtualKey != nil {
		if addedTools, ok := ctx.Value(schemas.KoutakuContextKeyMCPAddedTools).([]string); ok && len(addedTools) > 0 {
	...
	case DecisionVirtualKeyNotFound, DecisionVirtualKeyBlocked, DecisionModelBlocked, DecisionProviderBlocked:
		return result, &schemas.KoutakuError{
			Type:       koutaku.Ptr(string(result.Decision)),
			StatusCode: koutaku.Ptr(403),
			Error: &schemas.ErrorField{
...
// Parameters:
//   - ctx: The Koutaku context
//   - req: The Koutaku request to be processed
//
// Returns:
...
//   - error: Any error that occurred during processing
func (p *GovernancePlugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	// If its skip key selection - in that case we need to skip virtual key selection too
	if koutaku.GetBoolFromContext(ctx, schemas.KoutakuContextKeySkipKeySelection) {
		return req, nil, nil
	}
	...
	virtualKeyValue := koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyVirtualKey)
	...
	_, koutakuError := p.EvaluateGovernanceRequest(ctx, evaluationRequest, req.RequestType)
	// Convert KoutakuError to LLMPluginShortCircuit if needed
	if koutakuError != nil {
...
//   - err: The Koutaku error to be processed
//
// Returns:
//   - *schemas.KoutakuResponse: The processed response
//   - *schemas.KoutakuError: The processed error
//   - error: Any error that occurred during processing
func (p *GovernancePlugin) PostLLMHook(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	if _, ok := ctx.Value(governanceRejectedContextKey).(bool); ok {
		return result, err, nil
	}

	// Extract request type, provider, and model
	requestType, provider, requestedModel, _ := koutaku.GetResponseFields(result, err)

	// Extract governance information
	virtualKey := koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyVirtualKey)
	requestID := koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyRequestID)
	// Extract user ID for enterprise user-level governance
	userID := koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyUserID)
...
// Parameters:
//   - ctx: The Koutaku context
//   - req: The Koutaku MCP request to be processed
//
// Returns:
//   - *schemas.KoutakuMCPRequest: The processed request
//   - *schemas.MCPPluginShortCircuit: The plugin short circuit if the request is not allowed
//   - error: Any error that occurred during processing
func (p *GovernancePlugin) PreMCPHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	toolName := req.GetToolName()

	// Skip governance for codemode tools
	if koutaku.IsCodemodeTool(toolName) {
		return req, nil, nil
	}
	...
	// Evaluate governance using common function
	_, koutakuError := p.EvaluateGovernanceRequest(ctx, evaluationRequest, schemas.MCPToolExecutionRequest)

	// Convert KoutakuError to MCPPluginShortCircuit if needed
	if koutakuError != nil {
	...
	if virtualKeyValue != "" {
		...
		if !ok || vk == nil || !vk.IsActive {
			...
			ctx.SetValue(governanceRejectedContextKey, true)
			return req, &schemas.MCPPluginShortCircuit{Error: &schemas.KoutakuError{
				Type:       koutaku.Ptr(string(DecisionVirtualKeyNotFound)),
				StatusCode: koutaku.Ptr(403),
				Error: &schemas.ErrorField{
		...
		if !p.isMCPToolAllowedByVK(vk, toolName) {
			ctx.SetValue(governanceRejectedContextKey, true)
			return req, &schemas.MCPPluginShortCircuit{Error: &schemas.KoutakuError{
				Type:       koutaku.Ptr(string(DecisionMCPToolBlocked)),
				StatusCode: koutaku.Ptr(403),
				Error: &schemas.ErrorField{
...
// Parameters:
//   - ctx: The Koutaku context
//   - resp: The Koutaku MCP response to be processed
//   - koutakuErr: The Koutaku error to be processed
//
// Returns:
//   - *schemas.KoutakuMCPResponse: The processed response
//   - *schemas.KoutakuError: The processed error
//   - error: Any error that occurred during processing
func (p *GovernancePlugin) PostMCPHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError, error) {
	if _, ok := ctx.Value(governanceRejectedContextKey).(bool); ok {
	...
	if success && resp != nil && koutaku.IsCodemodeTool(resp.ExtraFields.ToolName) {
		return resp, koutakuErr, nil
	}
...
func (p *GovernancePlugin) postHookWorker(result *schemas.KoutakuResponse, provider schemas.ModelProvider, model string, requestType schemas.RequestType, virtualKey, requestID, userID string, isFinalChunk bool, pricingScopes *modelcatalog.PricingLookupScopes) {
	// Determine if request was successful
	success := (result != nil)

	// Streaming detection
	isStreaming := koutaku.IsStreamRequestType(requestType)
