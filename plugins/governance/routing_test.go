...
import (
	...
	"github.com/koutaku/koutaku/framework/routing"
...
func TestEvaluateRoutingRules_NilContext(t *testing.T) {
	...
	_, err = engine.EvaluateRoutingRules(schemas.NewKoutakuContext(context.Background(), time.Now()), nil)
	assert.Error(t, err)
...
func TestEvaluateRoutingRules_NoRulesMatch(t *testing.T) {
	...
	decision, err := engine.EvaluateRoutingRules(schemas.NewKoutakuContext(context.Background(), time.Now()), ctx)
	assert.NoError(t, err)
	assert.Nil(t, decision)
}
...
func TestEvaluateRoutingRules_GlobalRuleMatches(t *testing.T) {
	store, err := NewLocalGovernanceStore(context.Background(), NewMockLogger(), nil, &configstore.GovernanceConfig{}, nil)
	require.NoError(t, err)
	bgCtx := schemas.NewKoutakuContext(context.Background(), time.Now())
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("azure"), Model: koutaku.Ptr("gpt-4-turbo"), Weight: 1.0},
		},
		Enabled:  true,
		Scope:    "global",
		Priority: 0,
...
// KoutakuContext under KoutakuContextKeyAPIKeyID.
func TestEvaluateRoutingRules_MultiTargetDeterministicWithPinnedKey(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{
				Provider: koutaku.Ptr("azure"),
				Model:    koutaku.Ptr("gpt-4-turbo"),
				KeyID:    koutaku.Ptr(pinnedKeyID),
				Weight:   1.0,
			},
			{
				Provider: koutaku.Ptr("openai"),
				Model:    koutaku.Ptr("gpt-3.5"),
				Weight:   0.0,
			},
		},
		Enabled:  true,
	...
	// assert the pinned key_id is visible in the KoutakuContext.
	if decision.KeyID != "" {
		bgCtx.SetValue(schemas.KoutakuContextKeyAPIKeyID, decision.KeyID)
	}
	ctxKeyID, _ := bgCtx.Value(schemas.KoutakuContextKeyAPIKeyID).(string)
...
func TestEvaluateRoutingRules_ScopePrecedence(t *testing.T) {
	...
	globalRule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("openai"), Model: koutaku.Ptr("gpt-4o"), Weight: 1.0},
		},
		Enabled:  true,
		Scope:    "global",
		Priority: 0,
...
func TestResolveRoutingWithFallback_NoMatch(t *testing.T) {
	...
	decision, err := resolveRoutingWithFallback(schemas.NewKoutakuContext(context.Background(), time.Now()), ctx, engine)
	require.NoError(t, err)
	require.NotNil(t, decision)
...
func TestEvaluateRoutingRules_ChainRuleReEvaluation(t *testing.T) {
	...
	ruleA := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("openai"), Model: koutaku.Ptr("gpt-4-turbo"), Weight: 1.0},
		},
		Enabled:   true,
		Scope:     "global",
		Priority:  0,
	...
	ruleB := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("azure"), Model: koutaku.Ptr("gpt-4"), Weight: 1.0},
		},
		Enabled:   true,
		Scope:     "global",
		Priority:  1,
...
func TestEvaluateRoutingRules_MaxDepthCutoff(t *testing.T) {
	...
	ruleC := &configstoreTables.TableRoutingRule{
		...
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("anthropic"), Model: koutaku.Ptr("claude-3"), Weight: 1.0},
		},
		Enabled:   true,
		Scope:     "global",
		Priority:  2,
...
func TestCompileAndCacheProgram_ValidExpression_Routing(t *testing.T) {
	...
	rule := &configstoreTables.TableRoutingRule{
		...
		CelExpression: "model == 'gpt-4o'",
		Targets: []configstoreTables.TableRoutingTarget{
			{Provider: koutaku.Ptr("openai"), Weight: 1.0},
		},
		Enabled: true,
	}
