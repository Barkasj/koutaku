	...
	case tea.KeyMsg:
		...
		case phaseVirtualKey:
			...
			if s == "f1" {
				...
				if baseURL != "" {
					openBrowser(baseURL)
					m.notify("opened koutaku dashboard", false)
				}
				return m, nil
			}
			var cmd tea.Cmd
...
func (m chooserModel) View() string {
	...
	if updateVersion := strings.TrimSpace(m.cfg.UpdateVersion); updateVersion != "" {
		...
		body.WriteString(hint.Render("koutaku " + updateVersion + "  "))
	...
	switch m.phase {
	case phaseBaseURL:
		title = accent.Render("Base URL (Koutaku Base URL)")
...
// from the Koutaku API and sends the result as a modelsMsg.
