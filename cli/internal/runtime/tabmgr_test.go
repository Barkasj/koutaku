...
func TestBuildTabBarStringUsesCLIBrandAndRightSideVersion(t *testing.T) {
	...
	if !strings.Contains(got, "Koutaku CLI") {
		t.Fatalf("expected tab bar to contain Koutaku CLI branding, got %q", got)
	}
	if strings.Contains(got, " Koutaku ") {
		t.Fatalf("did not expect legacy Koutaku branding, got %q", got)
	}
