...
// prefix is the koutaku prefix key: Ctrl+B (0x02).
// When pressed, PTY output is frozen and koutaku enters tab mode.
const prefix = 0x02
...
func tabBarBrandString(bg string) string {
	...
	b.WriteString(bg)
	b.WriteString(" ")
	b.WriteString("\x1b[1;36mKoutaku CLI ")
	return b.String()
}
...
func tabBarBrandWidth() int {
	return 1 + len("Koutaku CLI ")
}

func (tm *TabManager) initCursorTrace() {
	path := strings.TrimSpace(os.Getenv("KOUTAKU_CLAUDE_CURSOR_TRACE"))
	if path == "" {
		return
	}
	...
	if err != nil {
		if tm.stderr != nil {
			fmt.Fprintf(tm.stderr, "koutaku: failed to open Claude cursor trace %q: %v\n", path, err)
		}
		return
	}
	tm.cursorTrace = f
