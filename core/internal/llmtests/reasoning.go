...
func RunResponsesReasoningTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesReasoning", func(t *testing.T) {
		...
		responsesReq := &schemas.KoutakuResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				...
				MaxOutputTokens: koutaku.Ptr(25000),
				// Configure reasoning-specific parameters
				Reasoning: &schemas.ResponsesParametersReasoning{
					Effort: koutaku.Ptr("high"), // High effort for complex reasoning
					// Summary: koutaku.Ptr("detailed"), // Detailed summary of reasoning process
				},
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "Reasoning", func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
...
func validateResponsesAPIReasoning(t *testing.T, response *schemas.KoutakuResponsesResponse) bool {
...
func RunChatCompletionReasoningTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ChatCompletionReasoning", func(t *testing.T) {
		...
		chatReq := &schemas.KoutakuChatRequest{
			...
			Params: &schemas.ChatParameters{
				...
				Reasoning: &schemas.ChatReasoning{
					Effort:    koutaku.Ptr("high"), // High effort for complex reasoning
					MaxTokens: koutaku.Ptr(1500),   // Maximum tokens for reasoning output
				},
			},
			Fallbacks: testConfig.Fallbacks,
		}
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "Reasoning", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
...
// Returns true if reasoning indicators are found
func validateChatCompletionReasoning(t *testing.T, response *schemas.KoutakuChatResponse) bool {
...
func RunMultiTurnReasoningTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Reasoning {
		t.Logf("⏭️ Reasoning not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("MultiTurnReasoning", func(t *testing.T) {
		...
		firstResponse, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "MultiTurnReasoning_Step1", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			return client.ChatCompletionRequest(bfCtx, chatReq)
		})
		...
		secondResponse, chatError2 := WithChatTestRetry(t, chatRetryConfig, retryContext2, expectations, "MultiTurnReasoning_Step2", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
