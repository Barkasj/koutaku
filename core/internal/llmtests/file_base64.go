			...
			ContentBlocks: []schemas.ChatContentBlock{
				{Type: schemas.ChatContentBlockTypeText, Text: koutaku.Ptr(text)},
				{
					...
					File: &schemas.ChatInputFile{
						FileData: koutaku.Ptr(documentBase64),
						Filename: koutaku.Ptr("test_document.pdf"),
					},
				},
			},
		},
...
func RunFileBase64Test(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func RunFileBase64ChatCompletionsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileBase64-ChatCompletions", func(t *testing.T) {
		...
		response, chatError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
			...
			chatReq := &schemas.KoutakuChatRequest{
				...
				Params: &schemas.ChatParameters{
					MaxCompletionTokens: koutaku.Ptr(500),
				},
...
func RunFileBase64ResponsesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.FileBase64 {
		t.Logf("File base64 not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("FileBase64-Responses", func(t *testing.T) {
		...
		response, responsesError := WithResponsesTestRetry(t, responsesRetryConfig, retryContext, expectations, "FileBase64", func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			responsesReq := &schemas.KoutakuResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: koutaku.Ptr(500),
				},
				Fallbacks: testConfig.Fallbacks,
			}
