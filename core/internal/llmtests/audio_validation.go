...
func SaveAndValidateAudio(t *testing.T, audioData []byte) (string, error) {
	...
	// Create temp file with unique name in koutaku subdirectory
	tempDir := os.TempDir()
	koutakuDir := filepath.Join(tempDir, "koutaku")
	fileName := fmt.Sprintf("koutaku_test_speech_%s.%s", uuid.New().String(), detectedFormat)
	filePath := filepath.Join(koutakuDir, fileName)

	// Ensure koutaku subdirectory exists
	if err := os.MkdirAll(koutakuDir, 0755); err != nil {
