...
func RunTranscriptionTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("Transcription", func(t *testing.T) {
		...
		for _, tc := range roundTripCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				var transcriptionRequest *schemas.KoutakuTranscriptionRequest
				if testConfig.Provider == schemas.HuggingFace && strings.HasPrefix(testConfig.TranscriptionModel, "fal-ai/") {
					...
					transcriptionRequest = &schemas.KoutakuTranscriptionRequest{
					...
					ttsResponse, err := WithSpeechTestRetry(t, speechRetryConfig, ttsRetryContext, ttsExpectations, "Transcription_RoundTrip_TTS_"+tc.name, func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {
				...
				transcriptionResponse, koutakuErr := WithTranscriptionTestRetry(t, transcriptionRetryConfig, retryContext, expectations, "Transcription_RoundTrip_"+tc.name, func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {
					bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
					return client.TranscriptionRequest(bfCtx, transcriptionRequest)
				})

				if koutakuErr != nil {
					t.Fatalf("❌ Transcription_RoundTrip_"+tc.name+" request failed after retries: %v", GetErrorMessage(koutakuErr))
				}
		...
		t.Run("AdditionalAudioTests", func(t *testing.T) {
			...
			for _, tc := range customCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, customTranscriptionRetryConfig, customRetryContext, customExpectations, "Transcription_Custom_"+tc.name, func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {
						bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
...
func RunTranscriptionAdvancedTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.Transcription {
		t.Logf("Transcription not supported for provider %s", testConfig.Provider)
		return
	}
		...
		t.Run("AllResponseFormats", func(t *testing.T) {
			...
			for _, format := range formats {
				t.Run("Format_"+format, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, formatTranscriptionRetryConfig, formatRetryContext, formatExpectations, "Transcription_Format_"+format, func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {
						bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
						return client.TranscriptionRequest(bfCtx, request)
					})
					if err != nil {
		...
		t.Run("WithCustomParameters", func(t *testing.T) {
			...
			request := &schemas.KoutakuTranscriptionRequest{
				...
				Params: &schemas.TranscriptionParameters{
					Language:       koutaku.Ptr("en"),
					Format:         &audioFormat,
					Prompt:         koutaku.Ptr("This audio contains technical terminology and proper nouns."),
					ResponseFormat: koutaku.Ptr("json"), // Use json instead of verbose_json for whisper-1
				},
				Fallbacks: testConfig.TranscriptionFallbacks,
			}
			...
			response, err := WithTranscriptionTestRetry(t, advancedTranscriptionRetryConfig, advancedRetryContext, advancedExpectations, "Transcription_Advanced_CustomParams", func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {
				bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
				return client.TranscriptionRequest(bfCtx, request)
			})
			if err != nil {
		...
		t.Run("MultipleLanguages", func(t *testing.T) {
			...
			for _, lang := range languages {
				t.Run("Language_"+lang, func(t *testing.T) {
					...
					response, err := WithTranscriptionTestRetry(t, langTranscriptionRetryConfig, langRetryContext, langExpectations, "Transcription_Language_"+lang, func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {
						bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
						return client.TranscriptionRequest(bfCtx, request)
					})
					if err != nil {
...
func validateTranscriptionRoundTrip(t *testing.T, response *schemas.KoutakuTranscriptionResponse, originalText string, testName string, testConfig ComprehensiveTestConfig) {
