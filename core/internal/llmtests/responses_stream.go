...
func RunResponsesStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ResponsesStream", func(t *testing.T) {
		...
		request := &schemas.KoutakuResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: koutaku.Ptr(150),
			},
			...
			func(responseChannel chan *schemas.KoutakuStreamChunk) ResponsesStreamValidationResult {
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.KoutakuResponsesStreamResponse != nil {
							if response.KoutakuResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
								t.Logf("⚠️ Warning: Provider mismatch - expected %s, got %s", testConfig.Provider, response.KoutakuResponsesStreamResponse.ExtraFields.Provider)
							}

							// Log latency for each chunk (can be 0 for inter-chunks)
							t.Logf("📊 Chunk %d latency: %d ms", responseCount+1, response.KoutakuResponsesStreamResponse.ExtraFields.Latency)

							// Process the streaming response
							streamResp := response.KoutakuResponsesStreamResponse
	...
	if testConfig.Scenarios.Reasoning && testConfig.ReasoningModel != "" {
		t.Run("ResponsesStreamWithReasoning", func(t *testing.T) {
			...
			request := &schemas.KoutakuResponsesRequest{
				...
				Input:    messages,
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: koutaku.Ptr(1800),
					Reasoning: &schemas.ResponsesParametersReasoning{
						Effort: koutaku.Ptr("high"),
						// Summary: koutaku.Ptr("detailed"),
					},
	...
	t.Run("ResponsesStreamLifecycle", func(t *testing.T) {
		...
		request := &schemas.KoutakuResponsesRequest{
			...
			Input:    messages,
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: koutaku.Ptr(50),
			},
			Fallbacks: testConfig.Fallbacks,
		}
...
func validateResponsesStreamingResponse(t *testing.T, eventTypes map[schemas.ResponsesStreamResponseType]int, sequenceNumbers []int, finalContent string, lastResponse *schemas.KoutakuStreamChunk, testConfig ComprehensiveTestConfig) StreamingValidationResult {
	var errors []string
	...
	if lastResponse == nil {
		...
		if lastResponse.KoutakuResponsesStreamResponse == nil {
			errors = append(errors, "Last streaming response should have KoutakuResponsesStreamResponse")
		} else {
			if lastResponse.KoutakuResponsesStreamResponse.ExtraFields.Provider != testConfig.Provider {
				errors = append(errors, fmt.Sprintf("Provider mismatch: expected %s, got %s", testConfig.Provider, lastResponse.KoutakuResponsesStreamResponse.ExtraFields.Provider))
			}
		}
	}
	...
	if lastResponse != nil && lastResponse.KoutakuResponsesStreamResponse != nil {
		if lastResponse.KoutakuResponsesStreamResponse.ExtraFields.Latency <= 0 {
			errors = append(errors, fmt.Sprintf("Last streaming chunk missing latency information (got %d ms)", lastResponse.KoutakuResponsesStreamResponse.ExtraFields.Latency))
		} else {
			t.Logf("✅ Total streaming latency: %d ms", lastResponse.KoutakuResponsesStreamResponse.ExtraFields.Latency)
		}
	}
