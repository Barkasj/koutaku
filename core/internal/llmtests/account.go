// Package llmtests provides comprehensive test account and configuration management for the Koutaku system.
	...
	return []schemas.Key{
		{
			...
			Models:             []string{"*"},
			Weight:             0,
			UseForBatchAPI:     koutaku.Ptr(false),
		...
		},
		{
			...
			Models:             []string{"*"},
			Weight:             1.0,
			UseForBatchAPI:     koutaku.Ptr(true),
...
// List-models tests set it on the context as schemas.KoutakuContextKeyDirectKey so Koutaku passes only this key.
...
func (account *ComprehensiveTestAccount) GetKeysForProvider(ctx context.Context, providerKey schemas.ModelProvider) ([]schemas.Key, error) {
	...
	case schemas.Vertex:
		//https://aiplatform.googleapis.com/v1/projects/koutaku-development-433105/locations/global/publishers/google/models/veo-3.1-generate-preview:fetchPredictOperation
