...
func RunMultiTurnConversationTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("MultiTurnConversation", func(t *testing.T) {
		...
		firstRequest := &schemas.KoutakuChatRequest{
		...
		response1, koutakuErr := WithChatTestRetry(t, chatRetryConfig1, retryContext1, expectations1, "MultiTurnConversation_Step1", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
			bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
			return client.ChatCompletionRequest(bfCtx, firstRequest)
		})

		if koutakuErr != nil {
			t.Fatalf("❌ MultiTurnConversation_Step1 request failed after retries: %v", GetErrorMessage(koutakuErr))
		}
		...
		secondRequest := &schemas.KoutakuChatRequest{
			Provider: testConfig.Provider,
			Model:    testConfig.ChatModel,
			Input:    messages2,
			Params: &schemas.ChatParameters{
	...
	response2, koutakuErr := WithChatTestRetry(t, chatRetryConfig2, retryContext2, expectations2, "MultiTurnConversation_Step2", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		bfCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
		return client.ChatCompletionRequest(bfCtx, secondRequest)
	})

	if koutakuErr != nil {
		t.Fatalf("❌ MultiTurnConversation_Step2 request failed after retries: %v", GetErrorMessage(koutakuErr))
	}
