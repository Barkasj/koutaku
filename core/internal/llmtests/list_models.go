...
// listModelsKoutakuContext returns a context for ListModels. For Replicate, sets KoutakuContextKeyDirectKey
...
func listModelsKoutakuContext(parent context.Context, provider schemas.ModelProvider) *schemas.KoutakuContext {
	bfCtx := schemas.NewKoutakuContext(parent, schemas.NoDeadline)
	if provider == schemas.Replicate {
		bfCtx.SetValue(schemas.KoutakuContextKeyDirectKey, ReplicateDirectKeyForListModels())
	}
	return bfCtx
}

// RunListModelsTest executes the list models test scenario
func RunListModelsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ListModels", func(t *testing.T) {
		...
		response, koutakuErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModels", func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
			bfCtx := listModelsKoutakuContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})

		if koutakuErr != nil {
			t.Fatalf("❌ List models request failed after retries: %v", GetErrorMessage(koutakuErr))
		}
...
func RunListModelsResponseMarshalTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsResponseMarshal", func(t *testing.T) {
		...
		response, koutakuErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsResponseMarshal", func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
			bfCtx := listModelsKoutakuContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})
...
// RunListModelsErrorMarshalTest verifies that the KeyStatus ↔ KoutakuError circular
...
func RunListModelsErrorMarshalTest(t *testing.T, _ *koutaku.Koutaku, _ context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsErrorMarshal", func(t *testing.T) {
		...
		// Create the cycle: KoutakuError → ExtraFields.KeyStatuses → KeyStatus → Error → KoutakuError
		koutakuErr.ExtraFields.KeyStatuses = []schemas.KeyStatus{keyStatus}

		// Marshal the KoutakuError (top-level, contains the cycle via KeyStatuses)
		errData, err := schemas.Marshal(koutakuErr)
		if err != nil {
			t.Fatalf("❌ Failed to marshal KoutakuError with circular KeyStatuses: %v", err)
		}
		t.Logf("✅ KoutakuError with circular KeyStatuses marshaled successfully (%d bytes)", len(errData))
...
func RunListModelsPaginationTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ListModels {
		t.Logf("List models not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ListModelsPagination", func(t *testing.T) {
		...
		response, koutakuErr := WithListModelsTestRetry(t, listModelsRetryConfig, retryContext, expectations, "ListModelsPagination", func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
			bfCtx := listModelsKoutakuContext(ctx, testConfig.Provider)
			return client.ListModelsRequest(bfCtx, request)
		})

		if koutakuErr != nil {
			t.Fatalf("❌ List models pagination request failed after retries: %v", GetErrorMessage(koutakuErr))
		}
		...
		if response.NextPageToken != "" {
			...
			nextPageRequest := &schemas.KoutakuListModelsRequest{
				Provider:  testConfig.Provider,
				PageSize:  pageSize,
			...
			nextPageResponse, nextPageErr := WithListModelsTestRetry(t, listModelsRetryConfig, nextPageRetryContext, expectations, "ListModelsPagination_NextPage", func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {
				bfCtx := listModelsKoutakuContext(ctx, testConfig.Provider)
