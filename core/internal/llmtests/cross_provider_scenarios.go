...
func NewOpenAIJudge(client *koutaku.Koutaku, judgeModel string, t *testing.T) *OpenAIJudge {
	return &OpenAIJudge{
		client:     client,
...
func (judge *OpenAIJudge) EvaluateResponse(ctx *schemas.KoutakuContext, evaluation EvaluationRequest) (*EvaluationResult, error) {
  ...
  "fatal_issues": ["serious problems"]
	...
	request := &schemas.KoutakuChatRequest{
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: koutaku.Ptr(600),
...
func NewOpenAIConversationDriver(client *koutaku.Koutaku, driverModel string, t *testing.T) *OpenAIConversationDriver {
...
func (driver *OpenAIConversationDriver) GenerateNextMessage(ctx *schemas.KoutakuContext, request NextMessageRequest) (*GeneratedFollowup, error) {
...
JSON response:
{
  ...
  "test_focus": "what capability this tests"
	...
	llmRequest := &schemas.KoutakuChatRequest{
		Provider: schemas.OpenAI,
		...
		Params: &schemas.ChatParameters{
			MaxCompletionTokens: koutaku.Ptr(300),
...
func RunCrossProviderScenarioTest(t *testing.T, client *koutaku.Koutaku, ctx *schemas.KoutakuContext, config CrossProviderTestConfig, scenario CrossProviderScenario, useResponsesAPI bool) {
...
func RunCrossProviderConsistencyTest(t *testing.T, client *koutaku.Koutaku, ctx *schemas.KoutakuContext, config CrossProviderTestConfig, useResponsesAPI bool) {
	apiType := "Chat Completions"
	if useResponsesAPI {
		apiType = "Responses API"
	}
...
func executeStepWithProvider(t *testing.T, client *koutaku.Koutaku, ctx *schemas.KoutakuContext,
	provider ProviderConfig, history []schemas.ChatMessage, step ScenarioStep, useResponsesAPI bool) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
	...
	if useResponsesAPI {
		...
		request := &schemas.KoutakuResponsesRequest{
			...
			Params: &schemas.ResponsesParameters{
				MaxOutputTokens: koutaku.Ptr(300),
				Temperature:     koutaku.Ptr(0.7),
			},
		}
		...
		return &schemas.KoutakuResponse{ResponsesResponse: responsesResponse}, nil
	} else {
		// Use Chat Completions API
		request := &schemas.KoutakuChatRequest{
			Provider: provider.Provider,
		...
		return &schemas.KoutakuResponse{ChatResponse: chatResponse}, nil
	}
}
