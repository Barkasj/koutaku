...
func RunSpeechSynthesisStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				responseChannel, err := WithStreamRetry(t, retryConfig, retryContext, func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError) {
					requestCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
				...
				for response := range responseChannel {
					...
					if response.KoutakuError != nil {
						streamErrors = append(streamErrors, FormatErrorConcise(ParseKoutakuError(response.KoutakuError)))
						continue
					}

					if response.KoutakuSpeechStreamResponse != nil {
						lastTokenLatency = response.KoutakuSpeechStreamResponse.ExtraFields.Latency
					}
					...
					if response.KoutakuSpeechStreamResponse.Audio == nil {
						streamErrors = append(streamErrors, "Stream response missing audio data")
						continue
					}

					// Log latency for each chunk (can be 0 for inter-chunks)
					t.Logf("📊 Speech chunk %d latency: %d ms", chunkCount+1, response.KoutakuSpeechStreamResponse.ExtraFields.Latency)

					// Collect audio chunks
					if response.KoutakuSpeechStreamResponse.Audio != nil {
						chunkSize := len(response.KoutakuSpeechStreamResponse.Audio)
						if chunkSize == 0 {
							t.Logf("⚠️ Skipping zero-length audio chunk")
							continue
						}
						// Accumulate audio data for codec validation
						audioBuffer.Write(response.KoutakuSpeechStreamResponse.Audio)
						totalBytes += chunkSize
						chunkCount++
						t.Logf("✅ Received audio chunk %d: %d bytes", chunkCount, chunkSize)

						// Validate chunk structure
						if response.KoutakuSpeechStreamResponse.Type != "" && (response.KoutakuSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDelta && response.KoutakuSpeechStreamResponse.Type != schemas.SpeechStreamResponseTypeDone) {
							t.Logf("⚠️ Unexpected object type in stream: %s", response.KoutakuSpeechStreamResponse.Type)
						}
						if response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested != "" && response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested != testConfig.SpeechSynthesisModel {
							t.Logf("⚠️ Unexpected model in stream: %s", response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested)
						}
					}
...
func RunSpeechSynthesisStreamAdvancedTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.SpeechSynthesisStream {
		t.Logf("Speech synthesis streaming not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("SpeechSynthesisStreamAdvanced", func(t *testing.T) {
		t.Run("LongText_HDModel_Streaming", func(t *testing.T) {
			...
			for response := range responseChannel {
				...
				if response.KoutakuSpeechStreamResponse != nil && response.KoutakuSpeechStreamResponse.Audio != nil {
					chunkSize := len(response.KoutakuSpeechStreamResponse.Audio)
					if chunkSize == 0 {
				...
				if response.KoutakuSpeechStreamResponse != nil && response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested != "" && response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested != testConfig.SpeechSynthesisModel {
					t.Logf("⚠️ Unexpected HD model: %s", response.KoutakuSpeechStreamResponse.ExtraFields.OriginalModelRequested)
				}
			}
			...
			for _, voice := range voices {
				...
				t.Run("StreamingVoice_"+voiceCopy, func(t *testing.T) {
					...
					validationResult := WithSpeechStreamValidationRetry(
						...
						retryConfig,
						retryContext,
						func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError) {							
							accumulatedAudio.Reset() // Reset buffer on retry
							requestCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
							return client.SpeechStreamRequest(requestCtx, request)
						},
						func(responseChannel chan *schemas.KoutakuStreamChunk) SpeechStreamValidationResult {
							...
							for response := range responseChannel {
								...
								if response.KoutakuError != nil {
									streamErrors = append(streamErrors, fmt.Sprintf("Error in stream for voice %s: %s", voiceCopy, FormatErrorConcise(ParseKoutakuError(response.KoutakuError))))
									continue
								}
								...
								if response.KoutakuSpeechStreamResponse != nil && response.KoutakuSpeechStreamResponse.Audio != nil && len(response.KoutakuSpeechStreamResponse.Audio) > 0 {
									receivedData = true
									// Accumulate audio data for codec validation
									accumulatedAudio.Write(response.KoutakuSpeechStreamResponse.Audio)
									t.Logf("✅ Received data for voice %s: %d bytes", voiceCopy, len(response.KoutakuSpeechStreamResponse.Audio))
								}
							}
