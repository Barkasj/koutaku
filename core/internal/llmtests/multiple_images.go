...
func RunMultipleImagesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("MultipleImages", func(t *testing.T) {
		...
		response, koutakuError := WithChatTestRetry(t, chatRetryConfig, retryContext, expectations, "MultipleImages", func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		...
		if koutakuError != nil {
			t.Fatalf("❌ Multiple images request failed after retries: %v", GetErrorMessage(koutakuError))
		}
