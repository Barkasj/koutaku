...
func RunStructuredOutputChatTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
...
func testStructuredOutputChatWithValue(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig, expectValue bool) {
	...
	chatOperation := func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
		...
		reqCtx := schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
		if strings.Contains(strings.ToLower(testConfig.ChatModel), "claude") && testConfig.Provider != schemas.Vertex {
			extraHeaders := map[string][]string{
				"anthropic-beta": {"structured-outputs-2025-11-13"},
			}
			reqCtx.SetValue(schemas.KoutakuContextKeyExtraHeaders, extraHeaders)
		}
		...
		chatReq := &schemas.KoutakuChatRequest{
			...
			Params: &schemas.ChatParameters{
				MaxCompletionTokens: koutaku.Ptr(5000),
...
func RunStructuredOutputChatStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
		...
		for {
			...
			case response, ok := <-responseChannel:
				...
				if response.KoutakuChatResponse != nil {
					if len(response.KoutakuChatResponse.Choices) > 0 {
						choice := response.KoutakuChatResponse.Choices[0]
...
func RunStructuredOutputResponsesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.StructuredOutputs {
		t.Logf("Structured outputs not supported for provider %s", testConfig.Provider)
		return
	}
		...
		responsesOperation := func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
			...
			responsesReq := &schemas.KoutakuResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					MaxOutputTokens: koutaku.Ptr(5000),
					Text: &schemas.ResponsesTextConfig{
						Format: &schemas.ResponsesTextConfigFormat{
							Type: "json_schema",
							Name: koutaku.Ptr("decision_schema"),
...
func RunStructuredOutputResponsesStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.StructuredOutputs || !testConfig.Scenarios.CompletionStream {
		t.Logf("Structured outputs streaming not supported for provider %s", testConfig.Provider)
		return
	}
