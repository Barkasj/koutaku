...
func RunImageEditTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEdit", func(t *testing.T) {
		...
		imageEditOperation := func() (*schemas.KoutakuImageGenerationResponse, *schemas.KoutakuError) {
			request := &schemas.KoutakuImageEditRequest{
				...
				Params: &schemas.ImageEditParameters{
					Size: koutaku.Ptr("1024x1024"),
					N:    koutaku.Ptr(1),
					Type: koutaku.Ptr("inpainting"),
					Mask: maskBytes,
				},
				Fallbacks: testConfig.ImageEditFallbacks,
			}

			response, err := client.ImageEditRequest(schemas.NewKoutakuContext(ctx, schemas.NoDeadline), request)
			if err != nil {
				return nil, err
			}
			if response != nil {
...
func RunImageEditStreamTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ImageEditStream", func(t *testing.T) {
		...
		validationResult := WithImageGenerationStreamRetry(
			...
			func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError) {
				return client.ImageEditStreamRequest(schemas.NewKoutakuContext(streamCtx, schemas.NoDeadline), request)
			},
			func(responseChannel chan *schemas.KoutakuStreamChunk) ImageGenerationStreamValidationResult {
				...
				for {
					...
					case response, ok := <-responseChannel:
						...
						if response.KoutakuError != nil {
							streamErrors = append(streamErrors, fmt.Sprintf("Error in stream: %s", GetErrorMessage(response.KoutakuError)))
							continue
						}

						if response.KoutakuImageGenerationStreamResponse != nil {
							receivedData = true
							imgResp := response.KoutakuImageGenerationStreamResponse
