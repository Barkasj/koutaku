...
// basePassthroughChatRequest returns a minimal KoutakuChatRequest suitable for
// conversion into a provider-native passthrough body.
func basePassthroughChatRequest(model string) *schemas.KoutakuChatRequest {
...
// buildPassthroughChatReq converts a minimal KoutakuChatRequest into the
...
func buildPassthroughChatReq(t *testing.T, provider schemas.ModelProvider, model string, stream bool) (passthroughChatReq, bool) {
	...
	case schemas.OpenAI:
		...
		if stream {
			nativeReq.Stream = koutaku.Ptr(true)
		}
...
// RunPassthroughAPITest exercises Koutaku's raw HTTP passthrough API for the
...
func RunPassthroughAPITest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("PassthroughAPI", func(t *testing.T) {
		t.Run("NonStream", func(t *testing.T) {
			...
			resp, koutakuErr := client.Passthrough(bfCtx, testConfig.Provider, &schemas.KoutakuPassthroughRequest{
				Method: "POST",
				Path:   req.path,
				Body:   req.body,
			...
			if koutakuErr != nil {
				t.Fatalf("❌ Passthrough request failed: %s", GetErrorMessage(koutakuErr))
			}
			if resp == nil {
		...
		t.Run("Stream", func(t *testing.T) {
			...
			ch, koutakuErr := client.PassthroughStream(bfCtx, testConfig.Provider, &schemas.KoutakuPassthroughRequest{
				Method:   "POST",
				Path:     req.path,
				Body:     req.body,
				RawQuery: req.query,
			...
			if koutakuErr != nil {
				t.Fatalf("❌ PassthroughStream failed: %s", GetErrorMessage(koutakuErr))
			}
			if ch == nil {
			...
			for chunk := range ch {
				...
				if chunk.KoutakuError != nil {
					t.Fatalf("❌ Stream chunk contained error: %s", GetErrorMessage(chunk.KoutakuError))
				}
				if chunk.KoutakuPassthroughResponse != nil {
					totalBytes += len(chunk.KoutakuPassthroughResponse.Body)
					if len(chunk.KoutakuPassthroughResponse.Body) > 0 {
						chunkCount++
					}
				}
			}
