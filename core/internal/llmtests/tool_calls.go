...
func RunToolCallsTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	...
	t.Run("ToolCalls", func(t *testing.T) {
		...
		validateLocationInChatToolCalls := func(response *schemas.KoutakuChatResponse, apiName string) {
			toolCalls := ExtractChatToolCalls(response)
			validateLocationInToolCalls(t, toolCalls, apiName)
		}

		validateLocationInResponsesToolCalls := func(response *schemas.KoutakuResponsesResponse, apiName string) {
...
func RunToolCallsWithEmptyPropertiesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
	...
	t.Run("ToolCallsWithEmptyProperties", func(t *testing.T) {
			...
			chatReq := &schemas.KoutakuChatRequest{
				...
				Params: &schemas.ChatParameters{
					...
					ToolChoice: &schemas.ChatToolChoice{
						ChatToolChoiceStr: koutaku.Ptr("required"),
					},
				},
				Fallbacks: testConfig.Fallbacks,
			}
			...
			responsesReq := &schemas.KoutakuResponsesRequest{
				...
				Params: &schemas.ResponsesParameters{
					...
					ToolChoice: &schemas.ResponsesToolChoice{
						ResponsesToolChoiceStr: koutaku.Ptr("required"),
					},
				},
			}
			return client.ResponsesRequest(bfCtx, responsesReq)
		...
		validatePingToolCall := func(response *schemas.KoutakuChatResponse, apiName string) {
			toolCalls := ExtractChatToolCalls(response)
		...
		validatePingResponsesToolCall := func(response *schemas.KoutakuResponsesResponse, apiName string) {
			toolCalls := ExtractResponsesToolCalls(response)
			require.True(t, len(toolCalls) > 0, "%s API should have tool calls", apiName)
			pingFound := false
			for _, toolCall := range toolCalls {
...
func RunToolCallsWithNilPropertiesTest(t *testing.T, client *koutaku.Koutaku, ctx context.Context, testConfig ComprehensiveTestConfig) {
	if !testConfig.Scenarios.ToolCalls {
		t.Logf("Tool calls not supported for provider %s", testConfig.Provider)
		return
	}
