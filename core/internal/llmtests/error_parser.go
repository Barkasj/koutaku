...
var (
	...
	CategoryKoutaku    = ErrorCategory{"Koutaku", "Koutaku Internal Error", "🌉"}
	CategoryUnknown    = ErrorCategory{"Unknown", "Unknown Error", "❓"}
)

// ParseKoutakuError converts a KoutakuError into a human-readable ParsedError
func ParseKoutakuError(err *schemas.KoutakuError) ParsedError {
	if err == nil {
		return ParsedError{
	...
	parsed.Technical["is_koutaku_error"] = err.IsKoutakuError
...
func categorizeError(err *schemas.KoutakuError) (category, title string) {
	...
	case err.IsKoutakuError:
		return CategoryKoutaku.Name, "Koutaku Internal Error"
	}
...
func parseHTTPError(err *schemas.KoutakuError, parsed *ParsedError) {
	if err.StatusCode != nil {
...
func parseAuthError(err *schemas.KoutakuError, parsed *ParsedError) {
	message := strings.ToLower(err.Error.Message)
...
func parseRateLimitError(err *schemas.KoutakuError, parsed *ParsedError) {
...
func parseProviderError(err *schemas.KoutakuError, parsed *ParsedError) {
...
func parseValidationError(err *schemas.KoutakuError, parsed *ParsedError) {
...
func parseTimeoutError(err *schemas.KoutakuError, parsed *ParsedError) {
...
func parseQuotaError(err *schemas.KoutakuError, parsed *ParsedError) {
...
func parseModelError(err *schemas.KoutakuError, parsed *ParsedError) {
	message := strings.ToLower(err.Error.Message)
...
func parseGenericError(err *schemas.KoutakuError, parsed *ParsedError) {
...
// LogError logs a KoutakuError in a readable format
func LogError(t *testing.T, err *schemas.KoutakuError, context string) {
	if err == nil {
		return
	}
...
// LogErrorConcise logs a KoutakuError in a concise format
func LogErrorConcise(t *testing.T, err *schemas.KoutakuError, context string) {
	if err == nil {
		return
	}
...
func RequireNoError(t *testing.T, err *schemas.KoutakuError, msgAndArgs ...interface{}) {
	if err != nil {
		parsed := ParseKoutakuError(err)
...
func AssertNoError(t *testing.T, err *schemas.KoutakuError, msgAndArgs ...interface{}) bool {
	if err != nil {
		parsed := ParseKoutakuError(err)
		message := "Expected no error"
		if len(msgAndArgs) > 0 {
...
func getCategory(name string) ErrorCategory {
	...
	case CategoryKoutaku.Name:
		return CategoryKoutaku
	default:
...
func IsRetryableError(err *schemas.KoutakuError) bool {
	if err == nil {
		return false
	}
...
func GetRetryDelay(err *schemas.KoutakuError, attempt int) int {
	if err == nil {
		return 0
	}
