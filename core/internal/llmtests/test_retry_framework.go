...
func parseRetryAfterFromError(err *schemas.KoutakuError) time.Duration {
...
//     are already handled by koutaku core, so these retries focus on content/functionality.
//
...
// DeepCopyKoutakuStreamChunk creates a deep copy of a KoutakuStreamChunk object to avoid pooling issues
func DeepCopyKoutakuStreamChunk(original *schemas.KoutakuStreamChunk) *schemas.KoutakuStreamChunk {
	if original == nil {
		return nil
	}

	// Use reflection to create a deep copy
	return deepCopyReflect(original).(*schemas.KoutakuStreamChunk)
}
...
// This focuses specifically on LLM behavior inconsistencies, not HTTP errors (handled by Koutaku core)
type TestRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
...
type ChatRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuChatResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type TextCompletionRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuTextCompletionResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type ResponsesRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuResponsesResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type SpeechRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuSpeechResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type TranscriptionRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuTranscriptionResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type EmbeddingRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuEmbeddingResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type ImageGenerationRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuImageGenerationResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type CountTokensRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuCountTokensResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type ListModelsRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuListModelsResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type BatchCreateRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuBatchCreateResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type BatchListRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuBatchListResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type BatchRetrieveRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuBatchRetrieveResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type BatchCancelRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuBatchCancelResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type BatchResultsRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuBatchResultsResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type FileUploadRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuFileUploadResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type FileListRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuFileListResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type FileRetrieveRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuFileRetrieveResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type FileDeleteRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuFileDeleteResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
type FileContentRetryCondition interface {
	ShouldRetry(response *schemas.KoutakuFileContentResponse, err *schemas.KoutakuError, context TestRetryContext) (bool, string)
	GetConditionName() string
}
...
func WithChatTestRetry(
	...
	scenarioName string,
	operation func() (*schemas.KoutakuChatResponse, *schemas.KoutakuError),
) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuChatResponse
	...
	for attempt := 1; attempt <= config.MaxAttempts; attempt++ {
		...
		if response != nil {
			...
			// Network errors are handled by koutaku core, so these are content/functionality validation errors
			...
			// All retries failed validation - create a KoutakuError to force test failure
			...
			// Return nil response + KoutakuError so calling test fails
			statusCode := 400
			testFailureError := &schemas.KoutakuError{
				IsKoutakuError: true,
...
func WithResponsesTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError),
) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuResponsesResponse
	var lastError *schemas.KoutakuError
...
func WithStreamRetry(
	...
	context TestRetryContext,
	operation func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError),
) (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError) {
	var lastChannel chan *schemas.KoutakuStreamChunk
	var lastError *schemas.KoutakuError
		...
		if attempt < config.MaxAttempts {
			...
			if isTimeoutError(lastError) {
				...
				if shouldRetryFromConditions {
					...
					// Network errors are handled by koutaku core, so these are likely transient
					shouldRetry = true
					errorMsg := GetErrorMessage(lastError)
					if !strings.Contains(errorMsg, "❌") {
						errorMsg = fmt.Sprintf("❌ %s", errorMsg)
...
func isTimeoutError(err *schemas.KoutakuError) bool {
	if err == nil {
		return false
	}

	// Check error category first (from ParseKoutakuError categorization)
...
type DualAPITestResult struct {
	ChatCompletionsResponse *schemas.KoutakuChatResponse
	ChatCompletionsError    *schemas.KoutakuError
	ResponsesAPIResponse    *schemas.KoutakuResponsesResponse
	ResponsesAPIError       *schemas.KoutakuError
	BothSucceeded           bool
}
...
// Network errors are handled by koutaku core, so retries here focus on content/functionality validation
...
func checkChatRetryConditions(response *schemas.KoutakuChatResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []ChatRetryCondition) (bool, string) {
	for _, condition := range conditions {
...
func checkResponsesRetryConditions(response *schemas.KoutakuResponsesResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []ResponsesRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithTextCompletionTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuTextCompletionResponse, *schemas.KoutakuError),
) (*schemas.KoutakuTextCompletionResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuTextCompletionResponse
	var lastError *schemas.KoutakuError
...
func WithSpeechTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError),
) (*schemas.KoutakuSpeechResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuSpeechResponse
	var lastError *schemas.KoutakuError
...
func checkTextCompletionRetryConditions(response *schemas.KoutakuTextCompletionResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []TextCompletionRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func checkSpeechRetryConditions(response *schemas.KoutakuSpeechResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []SpeechRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithEmbeddingTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuEmbeddingResponse, *schemas.KoutakuError),
) (*schemas.KoutakuEmbeddingResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuEmbeddingResponse
	var lastError *schemas.KoutakuError
...
func WithTranscriptionTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError),
) (*schemas.KoutakuTranscriptionResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuTranscriptionResponse
	var lastError *schemas.KoutakuError
	...
	for attempt := 1; attempt <= config.MaxAttempts; attempt++ {
		...
		if err != nil && attempt < config.MaxAttempts {
			...
			// ALWAYS retry on non-structural errors (network errors are handled by koutaku core)
...
func checkEmbeddingRetryConditions(response *schemas.KoutakuEmbeddingResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []EmbeddingRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithCountTokensTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuCountTokensResponse, *schemas.KoutakuError),
) (*schemas.KoutakuCountTokensResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuCountTokensResponse
	var lastError *schemas.KoutakuError
...
func checkCountTokensRetryConditions(response *schemas.KoutakuCountTokensResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []CountTokensRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func checkTranscriptionRetryConditions(response *schemas.KoutakuTranscriptionResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []TranscriptionRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithImageGenerationRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuImageGenerationResponse, *schemas.KoutakuError),
) (*schemas.KoutakuImageGenerationResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuImageGenerationResponse
	var lastError *schemas.KoutakuError
...
func checkImageGenerationRetryConditions(response *schemas.KoutakuImageGenerationResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []ImageGenerationRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithListModelsTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError),
) (*schemas.KoutakuListModelsResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuListModelsResponse
	var lastError *schemas.KoutakuError
...
func checkListModelsRetryConditions(response *schemas.KoutakuListModelsResponse, err *schemas.KoutakuError, context TestRetryContext, conditions []ListModelsRetryCondition) (bool, string) {
	for _, condition := range conditions {
		if shouldRetry, reason := condition.ShouldRetry(response, err, context); shouldRetry {
			return true, fmt.Sprintf("%s: %s", condition.GetConditionName(), reason)
		}
...
func WithBatchCreateTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuBatchCreateResponse, *schemas.KoutakuError),
) (*schemas.KoutakuBatchCreateResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuBatchCreateResponse
	var lastError *schemas.KoutakuError
...
func WithBatchListTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuBatchListResponse, *schemas.KoutakuError),
) (*schemas.KoutakuBatchListResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuBatchListResponse
	var lastError *schemas.KoutakuError
...
func WithBatchRetrieveTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuBatchRetrieveResponse, *schemas.KoutakuError),
) (*schemas.KoutakuBatchRetrieveResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuBatchRetrieveResponse
	var lastError *schemas.KoutakuError
...
func WithBatchCancelTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuBatchCancelResponse, *schemas.KoutakuError),
) (*schemas.KoutakuBatchCancelResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuBatchCancelResponse
	var lastError *schemas.KoutakuError
...
func WithBatchResultsTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuBatchResultsResponse, *schemas.KoutakuError),
) (*schemas.KoutakuBatchResultsResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuBatchResultsResponse
	var lastError *schemas.KoutakuError
...
func WithFileUploadTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuFileUploadResponse, *schemas.KoutakuError),
) (*schemas.KoutakuFileUploadResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuFileUploadResponse
	var lastError *schemas.KoutakuError
...
func WithFileListTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuFileListResponse, *schemas.KoutakuError),
) (*schemas.KoutakuFileListResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuFileListResponse
	var lastError *schemas.KoutakuError
...
func WithFileRetrieveTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuFileRetrieveResponse, *schemas.KoutakuError),
) (*schemas.KoutakuFileRetrieveResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuFileRetrieveResponse
	var lastError *schemas.KoutakuError
...
func WithFileDeleteTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuFileDeleteResponse, *schemas.KoutakuError),
) (*schemas.KoutakuFileDeleteResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuFileDeleteResponse
	var lastError *schemas.KoutakuError
...
func WithFileContentTestRetry(
	...
	expectations ResponseExpectations,
	scenarioName string,
	operation func() (*schemas.KoutakuFileContentResponse, *schemas.KoutakuError),
) (*schemas.KoutakuFileContentResponse, *schemas.KoutakuError) {

	var lastResponse *schemas.KoutakuFileContentResponse
	var lastError *schemas.KoutakuError
...
func WithSpeechStreamValidationRetry(
	...
	context TestRetryContext,
	operation func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError),
	validateStream func(chan *schemas.KoutakuStreamChunk) SpeechStreamValidationResult,
...
func WithResponsesStreamValidationRetry(
	...
	context TestRetryContext,
	operation func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError),
	validateStream func(chan *schemas.KoutakuStreamChunk) ResponsesStreamValidationResult,
...
func WithChatStreamValidationRetry(
	...
	context TestRetryContext,
	operation func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError),
	validateStream func(chan *schemas.KoutakuStreamChunk) ChatStreamValidationResult,
...
func WithImageGenerationStreamRetry(
	...
	context TestRetryContext,
	operation func() (chan *schemas.KoutakuStreamChunk, *schemas.KoutakuError),
	validateStream func(chan *schemas.KoutakuStreamChunk) ImageGenerationStreamValidationResult) ImageGenerationStreamValidationResult {
