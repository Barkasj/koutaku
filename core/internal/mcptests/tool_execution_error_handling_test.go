...
func TestToolExecution_Timeout(t *testing.T) {
	...
	if koutakuErr != nil {
		...
		assert.Contains(t, strings.ToLower(koutakuErr.Error.Message), "timeout",
			"Error should indicate timeout: %s", koutakuErr.Error.Message)
	} else if result != nil {
...
func TestToolExecution_TimeoutChatAndResponses(t *testing.T) {
	...
	t.Run("chat_format", func(t *testing.T) {
		...
		_, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
		if koutakuErr != nil {
			t.Logf("Chat format timeout error: %v", koutakuErr.Error.Message)
		}
	})
	...
	t.Run("responses_format", func(t *testing.T) {
		...
		_, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, &responsesTool)
		if koutakuErr != nil {
			t.Logf("Responses format timeout error: %v", koutakuErr.Error.Message)
		}
	})
}
...
func TestToolExecution_ToolReturnsError(t *testing.T) {
	...
	for i, errMsg := range errorMessages {
		t.Run(fmt.Sprintf("error_%d", i), func(t *testing.T) {
			...
			if koutakuErr != nil {
				assert.Contains(t, koutakuErr.Error.Message, errMsg)
...
func TestToolExecution_DivisionByZero(t *testing.T) {
	...
	if koutakuErr != nil {
		assert.Contains(t, strings.ToLower(koutakuErr.Error.Message), "zero")
	} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
...
func TestToolExecution_MissingRequiredArguments(t *testing.T) {
	...
	for _, tc := range invalidArgTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Should return error
			if koutakuErr != nil {
				t.Logf("Got expected error: %v", koutakuErr.Error.Message)
			} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
				// Error in result
...
func TestToolExecution_NonExistentTool(t *testing.T) {
	...
	if koutakuErr == nil && (result == nil || result.Content == nil || result.Content.ContentStr == nil) {
		// No error and no result - tool wasn't found
		t.Logf("Tool not found (as expected)")
	} else if koutakuErr != nil {
		// Got error about tool not found or not available
		errorMsg := strings.ToLower(koutakuErr.Error.Message)
		assert.True(t, strings.Contains(errorMsg, "not found") || strings.Contains(errorMsg, "not available"),
			"Error should mention 'not found' or 'not available': %s", koutakuErr.Error.Message)
	} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
...
func TestToolExecution_ToolNotInExecuteList(t *testing.T) {
	...
	for i := range clients {
		if clients[i].ExecutionConfig.ID == "koutakuInternal" {
			clients[i].ExecutionConfig.ToolsToExecute = []string{"koutakuInternal-echo"}
	...
	if koutakuErr == nil && (result == nil || result.Content == nil || result.Content.ContentStr == nil) {
		...
		assert.True(t, strings.Contains(errorMsg, "not found") || strings.Contains(errorMsg, "not available") || strings.Contains(errorMsg, "not permitted"),
			"Error should mention 'not found', 'not available', or 'not permitted': %s", koutakuErr.Error.Message)
	} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
		// Error in result content
		resultStr := strings.ToLower(*result.Content.ContentStr)
...
func TestToolExecution_ErrorInBothFormats(t *testing.T) {
	...
	t.Run("chat_format", func(t *testing.T) {
		...
		// Should handle error
		if koutakuErr != nil {
			assert.Contains(t, koutakuErr.Error.Message, errorMsg)
		} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
...
func TestToolExecution_MultipleErrorsInSequence(t *testing.T) {
	...
	_, err1 := koutaku.ExecuteChatMCPTool(ctx, &toolCall1)
	if err1 != nil {
	...
	_, err2 := koutaku.ExecuteChatMCPTool(ctx, &toolCall2)
	if err2 != nil {
	...
	_, err3 := koutaku.ExecuteChatMCPTool(ctx, &toolCall3)
	if err3 != nil {
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &validToolCall)
	if koutakuErr != nil {
...
func TestToolExecution_LargeErrorMessage(t *testing.T) {
	...
	if koutakuErr != nil {
		assert.NotEmpty(t, koutakuErr.Error.Message)
		t.Logf("Error message length: %d", len(koutakuErr.Error.Message))
	} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
...
func TestExecuteToolCode_RuntimeError(t *testing.T) {
	...
	for i, code := range runtimeErrorCodes {
		t.Run(fmt.Sprintf("runtime_error_%d", i), func(t *testing.T) {
			...
			if koutakuErr != nil {
				t.Logf("Got koutaku error (expected): %v", koutakuErr.Error.Message)
			} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
				returnValue, hasError, errorMsg := ParseCodeModeResponse(t, *result.Content.ContentStr)
				if hasError {
