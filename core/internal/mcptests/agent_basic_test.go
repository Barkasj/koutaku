...
func (m *MockLLMCaller) MakeChatRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuChatRequest) (*schemas.KoutakuChatResponse, *schemas.KoutakuError) {
...
func (m *MockLLMCaller) MakeResponsesRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuResponsesRequest) (*schemas.KoutakuResponsesResponse, *schemas.KoutakuError) {
...
func TestAgent_BasicLoop(t *testing.T) {
	...
	// Execute agent mode
	result, koutakuErr := manager.CheckAndExecuteAgentForChatRequest(
		ctx,
		originalReq,
		initialResponse,
	...
	require.Nil(t, koutakuErr, "agent loop should complete successfully")
...
func TestAgent_BasicLoop_ResponsesFormat(t *testing.T) {
	...
	result, koutakuErr := manager.CheckAndExecuteAgentForResponsesRequest(
		ctx,
		originalReq,
		initialResponse,
...
func TestAgent_MixedAutoAndNonAutoTools(t *testing.T) {
	...
	for _, tc := range finalMessage.ChatAssistantMessage.ToolCalls {
		if *tc.Function.Name == "koutakuInternal-calculator" {
			found = true
			break
		}
	}
...
func TestAgent_OnlyNonAutoTools(t *testing.T) {
	...
	require.NotNil(t, finalMessage.ChatAssistantMessage)
	require.NotEmpty(t, finalMessage.ChatAssistantMessage.ToolCalls)
	assert.Equal(t, "koutakuInternal-echo", *finalMessage.ChatAssistantMessage.ToolCalls[0].Function.Name)
}
...
func TestAgent_WithRealLLM_Simple(t *testing.T) {
	...
	result, koutakuErr := koutaku.ChatCompletionRequest(ctx, req)
	if koutakuErr != nil {
		// Skip if there's an API error (likely missing/invalid API key or config issue)
		t.Skipf("Skipping real LLM test due to API error: %v", koutakuErr.Error)
	}
	require.NotNil(t, result)
