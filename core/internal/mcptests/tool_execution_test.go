...
func TestDirectToolExecution_ChatFormat(t *testing.T) {
	...
	require.Nil(t, koutakuErr, "tool execution should succeed")
...
func TestDirectToolExecution_ResponsesFormat(t *testing.T) {
	...
	result, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, &toolCall)
	require.Nil(t, koutakuErr, "tool execution should succeed")
	require.NotNil(t, result, "should have result")
...
func TestToolExecutionWithArguments(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Nil(t, koutakuErr, "calculator execution should succeed")
			require.NotNil(t, result, "should have result")
...
func TestToolExecutionTimeout(t *testing.T) {
	...
	if koutakuErr != nil && koutakuErr.Error != nil {
		t.Logf("✅ Got expected timeout error: %v", koutakuErr.Error.Message)
	} else if result != nil {
...
func TestToolExecutionReturnsError(t *testing.T) {
	...
	if koutakuErr != nil && koutakuErr.Error != nil {
		assert.Contains(t, koutakuErr.Error.Message, errorMessage)
		t.Logf("✅ Error properly propagated: %v", koutakuErr.Error.Message)
...
func TestToolExecutionLargeResponse(t *testing.T) {
	...
	require.Nil(t, koutakuErr, "large response should not error")
...
func TestToolExecutionEmptyResponse(t *testing.T) {
	...
	require.Nil(t, koutakuErr, "empty response should not crash")
...
func TestToolExecutionStructuredResponse(t *testing.T) {
	...
	require.Nil(t, koutakuErr, "structured response should work")
	assert.NotNil(t, result, "should have result")
...
func TestToolExecutionParallel(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		go func(id int) {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("tool execution error: %v", koutakuErr)
			}
		}(i)
	}
...
func TestToolExecutionExtraFields(t *testing.T) {
	...
	// Note: ExtraFields are in KoutakuMCPResponse, not ChatMessage
}
...
func TestToolExecutionPreservesCallID(t *testing.T) {
	...
	chatResult, chatErr := koutaku.ExecuteChatMCPTool(ctx, &chatToolCall)
	...
	responsesResult, responsesErr := koutaku.ExecuteResponsesMCPTool(ctx, &responsesToolCall)
...
func TestToolExecutionMultipleClients(t *testing.T) {
	...
	echoResult, echoErr := koutaku.ExecuteChatMCPTool(ctx, &echoToolCall)
	...
	inProcessToolCall := schemas.ChatAssistantMessageToolCall{
		...
		Type: schemas.Ptr("function"),
		Function: schemas.ChatAssistantMessageToolCallFunction{
			Name:      schemas.Ptr("koutakuInternal-local_tool"),
			Arguments: string(argsJSON),
		},
	}
	localResult, localErr := koutaku.ExecuteChatMCPTool(ctx, &inProcessToolCall)
...
func TestToolExecutionClientNotFound(t *testing.T) {
	...
	if koutakuErr != nil {
		t.Logf("Got expected error: %v", koutakuErr)
	} else if result != nil {
...
func TestToolExecutionMalformedRequest(t *testing.T) {
	...
	t.Run("nil_tool_call", func(t *testing.T) {
		result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, nil)
...
func TestToolExecutionContextCancellation(t *testing.T) {
	...
	// Should be cancelled
	if koutakuErr != nil {
		t.Logf("Got cancellation error: %v", koutakuErr)
	} else if result != nil {
...
func TestToolExecutionContextDeadline(t *testing.T) {
	...
	// Should hit deadline
	if koutakuErr != nil {
		t.Logf("Got deadline error: %v", koutakuErr)
	} else if result != nil {
