...
func TestCodeMode_NoToolsAvailable(t *testing.T) {
	...
	koutaku := setupKoutaku(t)
	koutaku.SetMCPManager(manager)
...
result = main()`
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
...
func TestCodeMode_CallingMCPTool(t *testing.T) {
	...
	code := `echoResult = koutakuInternal.echo(message="Testing MCP call")
...
func TestCodeMode_CallingMultipleServers(t *testing.T) {
	...
	code := `result1 = koutakuInternal.echo(message="from HTTP")
result2 = koutakuInternal.echo(message="from SSE")
result = {
    "http": result1,
    "sse": result2
}`
...
func TestCodeMode_NestedToolCalls(t *testing.T) {
	...
	code := `# First call
echo1 = koutakuInternal.echo(message="step 1")

# Process result
processed = "Processed: " + str(echo1)

# Second call using processed result
echo2 = koutakuInternal.echo(message=processed)

# Third call
calc = koutakuInternal.calculator(operation="add", x=5, y=3)
...
func TestCodeMode_ToolNotInExecuteList(t *testing.T) {
	...
	code := `def main():
    if hasattr(koutakuInternal, "echo"):
        r = koutakuInternal.echo(message="blocked")
...
func TestCodeMode_NonAllowedToolExecution(t *testing.T) {
	...
	// Execute code that tries to access koutakuInternal
	// When ToolsToExecute = [], the server won't be bound in the environment at all
	// This should cause a runtime error: "undefined: koutakuInternal"
	code := `result = koutakuInternal.echo(message="should fail")`
	...
	// Verify we get an error because koutakuInternal is not defined
	// When ToolsToExecute = [], the server is not bound in the Starlark environment
	require.True(t, hasError, "should have execution error because koutakuInternal is not bound when ToolsToExecute is empty")
	assert.Contains(t, strings.ToLower(errorMsg), "undefined", "error should indicate koutakuInternal is undefined")
	t.Logf("Expected error (koutakuInternal unbound due to empty ToolsToExecute): %s", errorMsg)
}
...
func TestCodeMode_ToolExecutionTimeout(t *testing.T) {
	...
	code := `if hasattr(koutakuInternal, "echo"):
    r = koutakuInternal.echo(message="test")
...
func TestCodeMode_ToolCallWithAwait(t *testing.T) {
	...
	code := `result = koutakuInternal.calculator(operation="multiply", x=6, y=7)`
...
func TestCodeMode_ToolCallWithoutAwait(t *testing.T) {
	...
	code := `result = koutakuInternal.echo(message="promise test")`
...
results.append(koutakuInternal.echo(message="first"))
results.append(koutakuInternal.echo(message="second"))
results.append(koutakuInternal.echo(message="third"))
...
func TestCodeMode_MultipleParallelToolCalls(t *testing.T) {
	...
	code := `r1 = koutakuInternal.echo(message="parallel1")
r2 = koutakuInternal.echo(message="parallel2")
r3 = koutakuInternal.calculator(operation="add", x=10, y=20)
...
func TestCodeMode_ToolReturnsError(t *testing.T) {
	...
	code := `def main():
    r = koutakuInternal.throw_error(error_message="intentional error")
...
func TestCodeMode_ToolNotFound(t *testing.T) {
	...
	// Execute code that tries to use koutakuInternal which won't be bound
	// When ToolsToExecute = [], the server won't be bound in the environment at all
	// This should cause a runtime error: "undefined: koutakuInternal"
	code := `result = koutakuInternal.nonexistent_tool(param="value")`
	...
	// Verify we get an error because koutakuInternal is not defined
	// When ToolsToExecute = [], the server is not bound in the Starlark environment
	require.True(t, hasError, "should have execution error because koutakuInternal is not bound")
...
func TestCodeMode_ChatFormat(t *testing.T) {
	...
	code := `r = koutakuInternal.calculator(operation="divide", x=100, y=4)
...
result = {"result": r, "format": "responses"}`
	...
	result, koutakuErr := koutaku.ExecuteResponsesMCPTool(ctx, &responsesTool)
	require.Nil(t, koutakuErr)
	require.NotNil(t, result)
