...
func SetupAgentTest(t *testing.T, config AgentTestConfig) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.KoutakuContext) {
	t.Helper()
	...
	ctx := schemas.NewKoutakuContext(baseCtx, schemas.NoDeadline)
...
func SetupAgentTestWithClients(t *testing.T, config AgentTestConfig, customClients []schemas.MCPClientConfig) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.KoutakuContext) {
	t.Helper()
...
func SetupMultiClientAgentTest(t *testing.T, inProcessTools []string, stdioClients []string, autoExecute []string, maxDepth int) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.KoutakuContext) {
	t.Helper()
...
func SetupContextFilteredAgentTest(t *testing.T, inProcessTools []string, stdioClients []string, autoExecute []string, toolFilter []string, clientFilter []string) (*mcp.MCPManager, *DynamicLLMMocker, *schemas.KoutakuContext) {
	t.Helper()
...
func AssertAgentFinalResponse(t *testing.T, response *schemas.KoutakuChatResponse, expectedFinishReason string, shouldContainText string) {
	t.Helper()
...
func AssertToolExecutedInTurn(t *testing.T, mocker *DynamicLLMMocker, toolName string, turn int) {
	...
	// Check for exact match or with "koutakuInternal-" prefix
	if assistantMsg.ChatAssistantMessage != nil {
		for _, tc := range assistantMsg.ChatAssistantMessage.ToolCalls {
			if tc.Function.Name != nil {
				...
				if fullName == toolName || fullName == "koutakuInternal-"+toolName ||
...
func AssertNoToolCalls(t *testing.T, response *schemas.KoutakuChatResponse) {
	t.Helper()
...
func AssertAgentError(t *testing.T, koutakuErr *schemas.KoutakuError, shouldContain string) {
	t.Helper()

	require.NotNil(t, koutakuErr, "Should return error")
	require.NotNil(t, koutakuErr.Error, "Error field should not be nil")

	if shouldContain != "" {
		assert.Contains(t, koutakuErr.Error.Message, shouldContain, "Error message should contain expected text")
	}
}

// AssertAgentSuccess verifies the agent completed without errors
func AssertAgentSuccess(t *testing.T, response *schemas.KoutakuChatResponse, koutakuErr *schemas.KoutakuError) {
	t.Helper()

	if koutakuErr != nil && koutakuErr.Error != nil {
		t.Logf("koutakuErr: %s", koutakuErr.Error.Message)
	}
	assert.Nil(t, koutakuErr, "Should not return error")
	require.NotNil(t, response, "Response should not be nil")
	require.NotEmpty(t, response.Choices, "Response should have choices")
}
...
func AssertRequestIDChanged(t *testing.T, ctx1 *schemas.KoutakuContext, ctx2 *schemas.KoutakuContext) {
	t.Helper()
...
func AssertRequestIDPropagated(t *testing.T, ctx *schemas.KoutakuContext) {
	t.Helper()
...
// In-process tools are registered with "koutakuInternal-" prefix
// The tool name format is: koutakuInternal-{tool_name} (e.g., "koutakuInternal-echo")
func CreateInProcessToolCall(id, toolName string, args map[string]interface{}) schemas.ChatAssistantMessageToolCall {
	fullToolName := fmt.Sprintf("koutakuInternal-%s", toolName)
	return CreateToolCall(id, fullToolName, args)
}
...
func RunAgentScenario(t *testing.T, scenario AgentScenario) {
	...
	t.Run(scenario.Name, func(t *testing.T) {
		...
		response, koutakuErr := manager.CheckAndExecuteAgentForChatRequest(
			ctx,
			req,
			initialResponse,
			mocker.MakeChatRequest,
			func(ctx *schemas.KoutakuContext, request *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error) {
		...
		for _, assertion := range scenario.Assertions {
			...
			case "success":
				AssertAgentSuccess(t, response, koutakuErr)
			case "final_reason":
...
func SimpleAgentTest(t *testing.T, name string, config AgentTestConfig, responses []ChatResponseFunc, assertions func(*testing.T, *schemas.KoutakuChatResponse, *schemas.KoutakuError, *DynamicLLMMocker)) {
	t.Helper()
	t.Run(name, func(t *testing.T) {
		...
		assertions(t, response, koutakuErr, mocker)
	})
}
...
func CreateAgentTurnWithToolCallsResponses(toolCalls ...schemas.ChatAssistantMessageToolCall) ResponsesResponseFunc {
	return CreateDynamicResponsesResponse(func(history []schemas.ResponsesMessage) *schemas.KoutakuResponsesResponse {
...
func AssertAgentFinalResponseResponses(t *testing.T, result *schemas.KoutakuResponsesResponse, mustContainInContent string) {
	t.Helper()
