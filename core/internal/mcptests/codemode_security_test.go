	...
	for _, tc := range pathTraversalTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectError {
				...
				if koutakuErr != nil {
					assert.Contains(t, koutakuErr.Error.Message, tc.errorMessage)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.shouldFail {
				...
				if koutakuErr != nil {
					...
					failureReason = fmt.Sprintf("koutakuErr: %v", koutakuErr)
				} else if result != nil && result.Content != nil && result.Content.ContentStr != nil {
				...
				if koutakuErr != nil {
					t.Errorf("%s (%s): unexpected koutakuErr: %v", tc.name, tc.description, koutakuErr)
					return
				}
...
func TestListToolFiles_InputValidation(t *testing.T) {
	...
	result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
	require.Nil(t, koutakuErr, "listToolFiles should succeed")
...
func TestReadToolFile_LineNumberBoundaries(t *testing.T) {
	...
	listResult, _ := koutaku.ExecuteChatMCPTool(ctx, &listCall)
	...
	for _, tc := range boundaryTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if koutakuErr != nil {
				t.Logf("%s: Got koutaku error (acceptable): %v", tc.description, koutakuErr)
			} else {
