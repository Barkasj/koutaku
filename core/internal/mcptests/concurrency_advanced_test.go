...
func TestConcurrent_CodeModeExecution(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			if koutakuErr != nil {
				errors <- fmt.Errorf("execution %d failed: %v", id, koutakuErr.Error.Message)
				return
			}
			if result == nil {
...
func TestConcurrent_CodeModeExecutionWithToolCalls(t *testing.T) {
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			switch id % 3 {
			case 0:
				code = fmt.Sprintf(`await koutakuInternal.echo({message: "test-%d"})`, id)
			case 1:
				code = fmt.Sprintf(`await koutakuInternal.calculator({operation: "add", x: %d, y: 10})`, id)
			case 2:
				code = `await koutakuInternal.get_time({timezone: "UTC"})`
			}
	...
	for i := 0; i < 10; i++ {
		...
		go func(id int) {
			...
			for _, client := range clients {
				if client.ExecutionConfig.ID == "koutakuInternal" {
		...
		go func(id int) {
			...
			case 2:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("koutakuInternal-get_time"),
						Arguments: toJSON(argsMap),
					},
				}
			case 3:
				...
				toolCall = schemas.ChatAssistantMessageToolCall{
					...
					Type: schemas.Ptr("function"),
					Function: schemas.ChatAssistantMessageToolCallFunction{
						Name:      schemas.Ptr("koutakuInternal-search"),
						Arguments: toJSON(argsMap),
					},
				}
			}
...
func TestConcurrent_FilteringChanges(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		...
		go func(id int) {
			...
			var ctx *schemas.KoutakuContext

			if id%2 == 0 {
				// Even: allow all tools
				baseCtx := context.Background()
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeClients, []string{"*"})
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeTools, []string{"koutakuInternal-*"})
				ctx = schemas.NewKoutakuContext(baseCtx, schemas.NoDeadline)
			} else {
				// Odd: allow only echo
				baseCtx := context.Background()
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeClients, []string{"*"})
				baseCtx = context.WithValue(baseCtx, schemas.MCPContextKeyIncludeTools, []string{"koutakuInternal-echo"})
				ctx = schemas.NewKoutakuContext(baseCtx, schemas.NoDeadline)
			}
	...
	for i := 0; i < numConcurrent; i++ {
		...
		go func(id int) {
			...
			for time.Now().Before(stopTime) {
				...
				result, koutakuErr := koutaku.ExecuteChatMCPTool(ctx, &toolCall)
				if koutakuErr != nil {
					errors <- fmt.Errorf("execution %d-%d failed: %v", id, counter, koutakuErr.Error.Message)
					return
				}
				if result != nil {
					successCount.Add(1)
