...
func setupMCPManagerWithRequestIDFunc(t *testing.T, fetchNewRequestIDFunc func(ctx *schemas.KoutakuContext) string, clientConfigs ...schemas.MCPClientConfig) *mcp.MCPManager {
	t.Helper()
...
//
// Key concepts:
// - KoutakuContextKeyRequestID: Current request ID (updated each iteration)
// - KoutakuMCPAgentOriginalRequestID: Original request ID (preserved)
...
func TestAgent_RequestID_Propagation(t *testing.T) {
	...
	// Assertions
	require.Nil(t, koutakuErr, "Should complete without error")
	...
	storedOriginalID, ok := ctx.Value(schemas.KoutakuMCPAgentOriginalRequestID).(string)
	...
	currentRequestID, ok := ctx.Value(schemas.KoutakuContextKeyRequestID).(string)
...
func TestAgent_RequestID_MixedAutoAndNonAuto(t *testing.T) {
	...
	// Setup context and mocker
	ctx := createTestContext()
	ctx.SetValue(schemas.KoutakuContextKeyRequestID, "req-mixed-001")
