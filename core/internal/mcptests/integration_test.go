...
func TestIntegration_FullChatWorkflow(t *testing.T) {
	...
	// End-to-end test: Setup koutaku with MCP, add multiple clients,
	// execute tools, verify workflow, check health, remove client

	// 1. Setup koutaku with MCP manager
	...
	echoResult, echoErr := koutaku.ExecuteChatMCPTool(ctx, &echoCall)
	...
	calcResult, calcErr := koutaku.ExecuteChatMCPTool(ctx, &calcCall)
...
func TestIntegration_FullResponsesWorkflow(t *testing.T) {
	...
	echoToolCall := GetSampleResponsesToolCallMessage("call-1", "koutakuInternal-echo", map[string]interface{}{
	...
	echoResult, echoErr := koutaku.ExecuteResponsesMCPTool(ctx, echoToolCall.ResponsesToolMessage)
	require.Nil(t, echoErr, "echo tool should execute successfully")
	require.NotNil(t, echoResult, "echo tool should return result")
	assert.Equal(t, schemas.ResponsesMessageTypeFunctionCallOutput, *echoResult.Type)

	// Execute calculator tool (Responses format)
	calcToolCall := GetSampleResponsesToolCallMessage("call-2", "koutakuInternal-calculator", map[string]interface{}{
		"operation": "multiply",
		"x":         float64(5),
		"y":         float64(7),
	})
	calcResult, calcErr := koutaku.ExecuteResponsesMCPTool(ctx, calcToolCall.ResponsesToolMessage)
	require.Nil(t, calcErr, "calculator tool should execute successfully")
	require.NotNil(t, calcResult, "calculator tool should return result")
...
func TestIntegration_MultiClientMultiTool(t *testing.T) {
	...
	weatherResult, weatherErr := koutaku.ExecuteChatMCPTool(ctx, &weatherCall)
	...
	// Tools are registered without prefix but accessed with koutakuInternal- prefix
	// Check that we have at least the registered tools (they might be named either way depending on how they're exposed)
	hasEcho := allTools["echo"] || allTools["koutakuInternal-echo"]
	hasCalculator := allTools["calculator"] || allTools["koutakuInternal-calculator"]
	hasWeather := allTools["get_weather"] || allTools["koutakuInternal-get_weather"]
...
func TestIntegration_ReconnectDuringExecution(t *testing.T) {
	...
	if koutakuErr != nil {
		t.Logf("Initial execution: %v", koutakuErr)
	}
...
func TestIntegration_ErrorRecovery(t *testing.T) {
	...
	result1, err1 := koutaku.ExecuteChatMCPTool(ctx, &echoCall)
	...
	_, err2 := koutaku.ExecuteChatMCPTool(ctx, &errorCall)
	...
	result3, err3 := koutaku.ExecuteChatMCPTool(ctx, &calcCall)
...
func TestIntegration_PartialFailure(t *testing.T) {
	...
	errors := make([]*schemas.KoutakuError, 3)

	// Tool 1: Echo (should succeed)
	echoCall := GetSampleEchoToolCall("call-1", "first tool")
	results[0], errors[0] = koutaku.ExecuteChatMCPTool(ctx, &echoCall)
	...
	results[1], errors[1] = koutaku.ExecuteChatMCPTool(ctx, &errorCall)

	// Tool 3: Calculator (should succeed)
	calcCall := GetSampleCalculatorToolCall("call-3", "multiply", 6.0, 7.0)
	results[2], errors[2] = koutaku.ExecuteChatMCPTool(ctx, &calcCall)
