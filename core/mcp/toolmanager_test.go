...
func toolNamesFromChatRequest(req *schemas.KoutakuRequest) []string {
...
func toolNamesFromResponsesRequest(req *schemas.KoutakuRequest) []string {
...
// contextWithUserAgent creates a KoutakuContext with KoutakuContextKeyUserAgent set.
func contextWithUserAgent(ua string) *schemas.KoutakuContext {
	ctx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	ctx.SetValue(schemas.KoutakuContextKeyUserAgent, ua)
	return ctx
}
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_StripsPrefix(t *testing.T) {
	...
	// Koutaku MCP server. The prefix is mcp__{server}__{tool_name}.
	tools := []schemas.ChatTool{
		makeTool("mcp__koutaku__echo"),
		makeTool("mcp__koutaku__calculator"),
	}

	m := buildIntegrationDuplicateCheckMap(tools, schemas.ClaudeCLI.String(), defaultLogger)

	// Bare tool names must be recognised so Koutaku does not inject them again.
	for _, want := range []string{"echo", "calculator"} {
		if !m[want] {
	...
	for _, want := range []string{"mcp__koutaku__echo", "mcp__koutaku__calculator"} {
		if !m[want] {
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_MultipleServers(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp__koutaku__executeToolCode"),
		makeTool("mcp__koutaku__listToolFiles"),
...
func TestBuildIntegrationDuplicateCheckMap_ClaudeCLI_MixedPrefixedAndBare(t *testing.T) {
	...
	for _, want := range []string{"echo", "mcp__koutaku__echo", "search"} {
		if !m[want] {
...
func TestBuildIntegrationDuplicateCheckMap_GeminiCLI_PrefixedTools_StripsPrefix(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp_koutaku_echo"),
		makeTool("mcp_koutaku_calculator"),
	}
...
// dual-gateway scenario: Koutaku stores tools as "{client}-{tool}" and Gemini CLI
...
func TestBuildIntegrationDuplicateCheckMap_GeminiCLI_ClientPrefixedTools(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp_koutaku_testing_exa-web_fetch_exa"),
		makeTool("mcp_koutaku_testing_exa-web_search_exa"),
		makeTool("mcp_koutaku_testing_websets-cancel_enrichment"),
		makeTool("mcp_koutaku_ctx7-resolve-library-id"),
	}
...
// hyphens in the original Koutaku tool name are converted to underscores. The dedup
...
func TestBuildIntegrationDuplicateCheckMap_CodexCLI_ClientPrefixedTools(t *testing.T) {
	...
	tools := []schemas.ChatTool{
		makeTool("mcp__koutaku__testing_exa_web_fetch_exa"),
		makeTool("mcp__koutaku__testing_exa_web_search_exa"),
		makeTool("mcp__koutaku__testing_websets_cancel_enrichment"),
		makeTool("mcp__koutaku__ctx7_query_docs"),
	}

	m := buildIntegrationDuplicateCheckMap(tools, schemas.CodexCLI.String(), defaultLogger)

	// All-underscore forms must be in the map (callers normalize Koutaku names before lookup).
	for _, want := range []string{
...
// underscores). Strip "mcp__" then skip past first "__" to get the Koutaku tool name.
func TestBuildIntegrationDuplicateCheckMap_QwenCLI_ClientPrefixedTools(t *testing.T) {
	t.Parallel()

	tools := []schemas.ChatTool{
		makeTool("mcp__koutaku__testing_exa-web_fetch_exa"),
		makeTool("mcp__koutaku__testing_exa-web_search_exa"),
		makeTool("mcp__koutaku__testing_websets-cancel_enrichment"),
		makeTool("mcp__koutaku__ctx7-resolve-library-id"),
	}
...
// buildChatRequest builds a minimal KoutakuChatRequest with the given pre-existing
// tool names already populated in Params.Tools.
func buildChatRequest(existingToolNames ...string) *schemas.KoutakuRequest {
...
func TestParseAndAddToolsToRequest_NoUserAgent_NoDuplicate(t *testing.T) {
	...
	// Request already has "echo" — Koutaku should not add it again.
...
func TestParseAndAddToolsToRequest_ClaudeCLI_NoDuplicate(t *testing.T) {
	...
	// Available MCP tools registered on the Koutaku server.
	cm := &mockToolClientManager{
		tools: []schemas.ChatTool{
			makeTool("echo"),
			makeTool("calculator"),
	...
	req := buildChatRequest("mcp__koutaku__echo", "mcp__koutaku__calculator")
...
func TestParseAndAddToolsToRequest_ClaudeCLI_NewToolsInjected(t *testing.T) {
	...
	// Koutaku should inject search (not already present in any form).
	cm := &mockToolClientManager{
		tools: []schemas.ChatTool{
			makeTool("echo"),
			makeTool("search"),
	...
	req := buildChatRequest("mcp__koutaku__echo")
	ctx := contextWithUserAgent(schemas.ClaudeCLI.String())
...
// buildResponsesRequest creates a minimal KoutakuResponsesRequest with the given
// pre-existing tool names already populated in Params.Tools.
func buildResponsesRequest(existingToolNames ...string) *schemas.KoutakuRequest {
...
func TestParseAndAddToolsToRequest_ResponsesAPI_ClaudeCLI_NoDuplicate(t *testing.T) {
	...
	req := buildResponsesRequest("mcp__koutaku__echo")
	ctx := contextWithUserAgent(schemas.ClaudeCLI.String())
	...
	if countOccurrences(names, "mcp__koutaku__echo") != 1 {
		t.Errorf("Responses/ClaudeCLI: prefixed name should remain exactly once, got %d",
			countOccurrences(names, "mcp__koutaku__echo"))
	}
}
...
// single underscore separator, hyphens in the Koutaku tool name are preserved).
// Strip up to and including the first "_" to recover the Koutaku tool name.
func TestBuildIntegrationDuplicateCheckMap_OpenCode_ClientPrefixedTools(t *testing.T) {
	t.Parallel()

	tools := []schemas.ChatTool{
		makeTool("koutaku_testing_exa-web_fetch_exa"),
		makeTool("koutaku_testing_exa-web_search_exa"),
		makeTool("koutaku_testing_websets-cancel_enrichment"),
		makeTool("koutaku_ctx7-query-docs"),
		makeTool("koutaku_filesystem-create_directory"),
	}
	...
	for _, want := range []string{
		"koutaku_testing_exa-web_fetch_exa",
		"koutaku_ctx7-query-docs",
	} {
		if !m[want] {
...
func TestParseAndAddToolsToRequest_OpenCode_NoDuplicate(t *testing.T) {
	...
	req := buildChatRequest(
		"koutaku_testing_exa-web_fetch_exa",
		"koutaku_ctx7-query-docs",
		"koutaku_filesystem-create_directory",
	)
	...
	// Bare Koutaku names must NOT be injected again — they're covered by prefixed entries.
