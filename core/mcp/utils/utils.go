...
func ResolvePerUserOAuthToken(ctx *schemas.KoutakuContext, client *schemas.MCPClientState, oauth2Provider schemas.OAuth2Provider) (string, error) {
	if oauth2Provider == nil {
		return "", fmt.Errorf("per-user OAuth requires an OAuth2Provider but none is configured")
	}

	virtualKeyID, _ := ctx.Value(schemas.KoutakuContextKeyGovernanceVirtualKeyID).(string)
	userID, _ := ctx.Value(schemas.KoutakuContextKeyUserID).(string)
	sessionToken, _ := ctx.Value(schemas.KoutakuContextKeyMCPUserSession).(string)

	// Optional X-Bf-User-Id header overrides user identity; if absent, falls back to virtual key
	if mcpUserID, _ := ctx.Value(schemas.KoutakuContextKeyMCPUserID).(string); mcpUserID != "" {
		userID = mcpUserID
	}
	...
	if err != nil {
		...
		isMCPGateway, _ := ctx.Value(schemas.KoutakuContextKeyIsMCPGateway).(bool)
...
func BuildRedirectURIFromContext(ctx *schemas.KoutakuContext) string {
	if uri, ok := ctx.Value(schemas.KoutakuContextKeyOAuthRedirectURI).(string); ok && uri != "" {
		return uri
	}
	return ""
}
...
func GetHeadersForToolExecution(ctx *schemas.KoutakuContext, client *schemas.MCPClientState) http.Header {
	...
	if extraHeaders, ok := ctx.Value(schemas.KoutakuContextKeyMCPExtraHeaders).(map[string][]string); ok {
