...
const (
	...
	KoutakuMCPVersion                   = "1.0.0"           // Version identifier for Koutaku
	KoutakuMCPClientName                = "KoutakuClient"   // Name for internal Koutaku MCP client
	KoutakuMCPClientKey                 = "koutakuInternal" // Key for internal Koutaku client in clientMap
	MCPLogPrefix                        = "[Koutaku MCP]"   // Consistent logging prefix
...
// MCPManager manages MCP integration for Koutaku core.
// It provides a bridge between Koutaku and various MCP servers, supporting
...
// ToolsManager and CodeMode. Call this after attaching an externally-created MCPManager to a Koutaku
// instance so that nested tool calls in code mode can run through Koutaku's plugin hooks.
...
//   - req: The Koutaku request to add tools to
//
// Returns:
//   - *schemas.KoutakuRequest: The request with tools added
func (m *MCPManager) AddToolsToRequest(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) *schemas.KoutakuRequest {
	return m.toolsManager.ParseAndAddToolsToRequest(ctx, req)
}

func (m *MCPManager) GetAvailableTools(ctx *schemas.KoutakuContext) []schemas.ChatTool {
...
//
// Returns:
//   - *schemas.KoutakuMCPResponse: The result response containing tool execution output (ChatMessage or ResponsesMessage)
//   - error: Any error that occurred during tool execution
func (m *MCPManager) ExecuteToolCall(ctx *schemas.KoutakuContext, request *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPResponse, error) {
...
//
// Returns:
//   - *schemas.KoutakuChatResponse: The final response after agent execution (or original if no tool calls)
...
//
// Returns:
//   - *schemas.KoutakuResponsesResponse: The final response after agent execution (or original if no tool calls)
//   - *schemas.KoutakuError: Any error that occurred during agent execution
