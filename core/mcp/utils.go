...
func retrieveExternalTools(ctx context.Context, client *client.Client, clientName string, logger schemas.Logger) (map[string]schemas.ChatTool, map[string]string, error) {
	...
	for _, mcpTool := range toolsResponse.Tools {
		...
		// Convert MCP tool schema to Koutaku format
		koutakuTool := convertMCPToolToKoutakuSchema(&mcpTool, logger)
		...
		if koutakuTool.Function != nil {
			koutakuTool.Function.Name = prefixedToolName
		}
		// Store the tool with the prefixed name
		tools[prefixedToolName] = koutakuTool
...
// convertMCPToolToKoutakuSchema converts an MCP tool definition to Koutaku format.
func convertMCPToolToKoutakuSchema(mcpTool *mcp.Tool, logger schemas.Logger) schemas.ChatTool {
	...
	// Clone bool pointers so Koutaku's copy is independent of the upstream mcp.Tool lifetime.
...
func hasToolCallsForChatResponse(response *schemas.KoutakuChatResponse) bool {
...
func hasToolCallsForResponsesResponse(response *schemas.KoutakuResponsesResponse) bool {
