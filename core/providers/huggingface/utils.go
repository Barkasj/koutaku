...
func (provider *HuggingFaceProvider) buildModelHubURL(request *schemas.KoutakuListModelsRequest, inferenceProvider inferenceProvider) string {
	values := url.Values{}
...
func splitIntoModelProvider(koutakuModelName string) (inferenceProvider, string, error) {
	// Extract provider and model name
	t := strings.Count(koutakuModelName, "/")
	if t == 0 {
		return "", "", fmt.Errorf("invalid model name format: %s", koutakuModelName)
	}
	var prov inferenceProvider
	var model string
	if t > 1 {
		before, after, _ := strings.Cut(koutakuModelName, "/")
		prov = inferenceProvider(before)
		model = after
	} else if t == 1 {
		prov = ""
		model = koutakuModelName
	}
...
func (provider *HuggingFaceProvider) getInferenceProviderRouteURL(ctx *schemas.KoutakuContext, inferenceProvider inferenceProvider, modelName string, requestType schemas.RequestType) (string, error) {
	defaultPath := ""
...
func (provider *HuggingFaceProvider) getModelInferenceProviderMapping(ctx context.Context, huggingfaceModelName string) (map[inferenceProvider]HuggingFaceInferenceProviderMapping, *schemas.KoutakuError) {
	// Check cache first
...
// KoutakuError indicating the operation isn't supported for the requested
// request type or provider.
func (provider *HuggingFaceProvider) getValidatedProviderModelID(ctx context.Context, inferenceProvider inferenceProvider, huggingfaceModelName string, requiredTask string, requestType schemas.RequestType) (string, *schemas.KoutakuError) {
	providerName := provider.GetProviderKey()

	providerMapping, koutakuErr := provider.getModelInferenceProviderMapping(ctx, huggingfaceModelName)
	if koutakuErr != nil {
		return "", koutakuErr
	}
...
func (provider *HuggingFaceProvider) downloadAudioFromURL(ctx context.Context, audioURL string) ([]byte, error) {
	...
	defer wait()
	if koutakuErr != nil {
		return nil, fmt.Errorf("failed to download audio: %v", koutakuErr)
	}
