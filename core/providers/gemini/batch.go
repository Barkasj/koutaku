...
// ToKoutakuBatchStatus converts Gemini batch job state to Koutaku status.
func ToKoutakuBatchStatus(geminiState string) schemas.BatchStatus {
	switch geminiState {
...
// ToGeminiBatchStatus converts Koutaku batch status to Gemini batch job state.
...
// ToGeminiBatchJobResponse converts Koutaku batch create response to Gemini batch job response format.
func ToGeminiBatchJobResponse(resp *schemas.KoutakuBatchCreateResponse) *GeminiBatchJobResponse {
	if resp == nil {
		return nil
	}
...
// ToGeminiBatchRetrieveResponse converts a Koutaku batch retrieve response to Gemini batch job response format.
func ToGeminiBatchRetrieveResponse(resp *schemas.KoutakuBatchRetrieveResponse) *GeminiBatchJobResponse {
	if resp == nil {
		return nil
	}
...
// ToGeminiBatchListResponse converts a Koutaku batch list response to Gemini format.
func ToGeminiBatchListResponse(resp *schemas.KoutakuBatchListResponse) *GeminiBatchListResponse {
	if resp == nil {
		return nil
	}
...
func (provider *GeminiProvider) downloadBatchResultsFile(ctx context.Context, key schemas.Key, fileName string) ([]schemas.BatchResultItem, []schemas.BatchError, *schemas.KoutakuError) {
	...
	if err != nil {
		return nil, nil, providerUtils.NewKoutakuOperationError(schemas.ErrProviderResponseDecode, err)
	}
