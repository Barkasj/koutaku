...
// ToGeminiChatCompletionRequest converts a KoutakuChatRequest to Gemini's generation request format for chat completion
func ToGeminiChatCompletionRequest(koutakuReq *schemas.KoutakuChatRequest) (*GeminiGenerationRequest, error) {
	if koutakuReq == nil {
		return nil, nil
	}
	...
	if koutakuReq.Params != nil {
		...
		var err error
		geminiReq.GenerationConfig, err = convertParamsToGenerationConfig(koutakuReq.Params, []string{}, koutakuReq.Model)
		if err != nil {
			return nil, err
		}
		// Handle tool-related parameters
		if len(koutakuReq.Params.Tools) > 0 {
			geminiReq.Tools = convertKoutakuToolsToGemini(koutakuReq.Params.Tools)

			// Convert tool choice to tool config
			if koutakuReq.Params.ToolChoice != nil {
				geminiReq.ToolConfig = convertToolChoiceToToolConfig(koutakuReq.Params.ToolChoice)
			}
		}
	...
	contents, systemInstruction := convertKoutakuMessagesToGemini(koutakuReq.Input)
...
// ToKoutakuChatResponse converts a GenerateContentResponse to a KoutakuChatResponse
func (response *GenerateContentResponse) ToKoutakuChatResponse() *schemas.KoutakuChatResponse {
	koutakuResp := &schemas.KoutakuChatResponse{
	...
	if !response.CreateTime.IsZero() {
		koutakuResp.Created = int(response.CreateTime.Unix())
	}

	// Handle empty candidates (filtered/malformed responses)
	if len(response.Candidates) == 0 {
		finishReason := ConvertGeminiFinishReasonToKoutaku(FinishReasonMalformedFunctionCall)
	...
	if isErrorFinishReason(candidate.FinishReason) {
		finishReason := ConvertGeminiFinishReasonToKoutaku(candidate.FinishReason)
		return createErrorResponse(response, finishReason, false)
	}
	...
	if candidate.Content != nil && len(candidate.Content.Parts) > 0 {
		for _, part := range candidate.Content.Parts {
			...
			if part.Text != "" {
				...
				if len(part.ThoughtSignature) > 0 {
					...
					reasoningDetails = append(reasoningDetails, schemas.ChatReasoningDetails{
						Index:     len(reasoningDetails),
						Type:      schemas.KoutakuReasoningDetailsTypeEncrypted,
		...
		// Convert finish reason to Koutaku format.
		...
		koutakuResp.Choices = append(koutakuResp.Choices, schemas.KoutakuResponseChoice{
			Index:        0,
			FinishReason: &finishReason,
			LogProbs:     ConvertGeminiLogprobsResultToKoutaku(candidate.LogprobsResult),
	...
	koutakuResp.Usage = ConvertGeminiUsageMetadataToChatUsage(response.UsageMetadata)
...
// ToKoutakuChatCompletionStream converts a Gemini streaming response to a Koutaku Chat Completion Stream response
// Returns the response, error (if any), and a boolean indicating if this is the last chunk
func (response *GenerateContentResponse) ToKoutakuChatCompletionStream(state *GeminiStreamState) (*schemas.KoutakuChatResponse, *schemas.KoutakuError, bool) {
	if response == nil {
	...
	if isLastChunk && candidate.FinishReason != "" {
		reason := ConvertGeminiFinishReasonToKoutaku(candidate.FinishReason)
	...
	streamResponse.Choices = []schemas.KoutakuResponseChoice{choice}
...
// createErrorResponse creates a complete KoutakuChatResponse for error cases
func createErrorResponse(response *GenerateContentResponse, finishReason string, isStream bool) *schemas.KoutakuChatResponse {
	var choice schemas.KoutakuResponseChoice
	if isStream {
		choice = schemas.KoutakuResponseChoice{
			Index:        0,
			FinishReason: &finishReason,
	...
	errorResp := &schemas.KoutakuChatResponse{
		ID:      response.ResponseID,
		Model:   response.ModelVersion,
		Object:  objectType,
