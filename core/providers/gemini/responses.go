...
func (request *GeminiGenerationRequest) ToKoutakuResponsesRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuResponsesRequest {
	if request == nil {
		return nil
	}
	...
	// Create the KoutakuResponsesRequest
	...
	if len(inputMessages) > 0 {
		koutakuReq.Input = inputMessages
	}
...
func ToGeminiResponsesRequest(koutakuReq *schemas.KoutakuResponsesRequest) (*GeminiGenerationRequest, error) {
	if koutakuReq == nil {
		return nil, nil
	}
	...
	if koutakuReq.Params != nil {
		var err error
		geminiReq.GenerationConfig, err = geminiReq.convertParamsToGenerationConfigResponses(koutakuReq.Params)
		if err != nil {
			return nil, err
		}
		...
		if len(koutakuReq.Params.Tools) > 0 {
			geminiReq.Tools = convertResponsesToolsToGemini(koutakuReq.Params.Tools)

			// Convert tool choice if present
			if koutakuReq.Params.ToolChoice != nil {
				geminiReq.ToolConfig = convertResponsesToolChoiceToGemini(koutakuReq.Params.ToolChoice)
			}
		}
	}
	...
	if koutakuReq.Input != nil {
		contents, systemInstruction, err := convertResponsesMessagesToGeminiContents(koutakuReq.Input)
		if err != nil {
			return nil, err
		}
	...
	if koutakuReq.Params != nil {
		if koutakuReq.Params.Instructions != nil {
...
// ToResponsesKoutakuResponsesResponse converts a Gemini GenerateContentResponse to a KoutakuResponsesResponse
func (response *GenerateContentResponse) ToResponsesKoutakuResponsesResponse() *schemas.KoutakuResponsesResponse {
	if response == nil {
		return nil
	}
	...
	koutakuResp.Usage = ConvertGeminiUsageMetadataToResponsesUsage(response.UsageMetadata)

	// Convert candidates to Responses output messages
	if len(response.Candidates) > 0 {
		candidate := response.Candidates[0]

		// Persist finish reason as Koutaku canonical stop_reason
		if candidate.FinishReason != "" && candidate.FinishReason != FinishReasonUnspecified {
			stopReason := ConvertGeminiFinishReasonToKoutaku(candidate.FinishReason)
			koutakuResp.StopReason = &stopReason

			if isErrorFinishReason(candidate.FinishReason) {
				failedStatus := "failed"
				koutakuResp.Status = &failedStatus

				errMsg := candidate.FinishMessage
				if errMsg == "" {
					errMsg = string(candidate.FinishReason)
				}
				koutakuResp.Error = &schemas.ResponsesResponseError{
					Code:    stopReason,
					Message: errMsg,
				}
...
func ToGeminiResponsesResponse(koutakuResp *schemas.KoutakuResponsesResponse) *GenerateContentResponse {
	if koutakuResp == nil {
		return nil
	}
	...
	if koutakuResp.ID != nil {
		geminiResp.ResponseID = *koutakuResp.ID
	}

	// Set creation time
	if koutakuResp.CreatedAt > 0 {
		geminiResp.CreateTime = time.Unix(int64(koutakuResp.CreatedAt), 0)
	}
	...
	if len(koutakuResp.Output) > 0 {
		...
		for i := range koutakuResp.Output {
			msg := &koutakuResp.Output[i]
		...
		for i, msg := range koutakuResp.Output {
			...
			if msg.Type != nil && *msg.Type == schemas.ResponsesMessageTypeFunctionCall && msg.ResponsesToolMessage != nil {
				...
				if len(thoughtSignature) > 0 {
					...
					if i+1 < len(koutakuResp.Output) {
						nextMsg := koutakuResp.Output[i+1]
		...
		if len(currentParts) > 0 {
			...
			// Determine finish reason: prefer StopReason (Koutaku canonical), fall back to IncompleteDetails
			if koutakuResp.StopReason != nil {
				candidate.FinishReason = ConvertKoutakuFinishReasonToGemini(*koutakuResp.StopReason)
			} else if koutakuResp.IncompleteDetails != nil {
				switch koutakuResp.IncompleteDetails.Reason {
				case "max_tokens":
	...
	if koutakuResp.Usage != nil {
		geminiResp.UsageMetadata = ConvertKoutakuResponsesUsageToGeminiUsageMetadata(koutakuResp.Usage)
	}

	return geminiResp
}

// KoutakuToGeminiStreamState tracks state when converting Koutaku streams to Gemini format
type KoutakuToGeminiStreamState struct {
...
// NewKoutakuToGeminiStreamState creates a new state for Koutaku→Gemini streaming
func NewKoutakuToGeminiStreamState() *KoutakuToGeminiStreamState {
	return &KoutakuToGeminiStreamState{
...
func ToGeminiResponsesStreamResponse(koutakuResp *schemas.KoutakuResponsesStreamResponse, state *KoutakuToGeminiStreamState) *GenerateContentResponse {
	if koutakuResp == nil {
		return nil
	}
	...
	if state == nil {
		state = NewKoutakuToGeminiStreamState()
	}

	// Buffer web search call
	if koutakuResp.Type == schemas.ResponsesStreamResponseTypeOutputItemDone &&
		koutakuResp.Item != nil &&
		koutakuResp.Item.Type != nil &&
		*koutakuResp.Item.Type == schemas.ResponsesMessageTypeWebSearchCall {
		state.WebSearchCall = koutakuResp.Item
		state.HasWebSearch = true
		return nil // Don't emit yet, wait for completion
	}

	// Buffer annotations
	if koutakuResp.Type == schemas.ResponsesStreamResponseTypeOutputTextAnnotationAdded &&
		koutakuResp.Annotation != nil {
		state.Annotations = append(state.Annotations, *koutakuResp.Annotation)
		return nil // Don't emit yet, wait for completion
	}
	...
	if koutakuResp.Type == schemas.ResponsesStreamResponseTypeOutputItemDone &&
		koutakuResp.Item != nil &&
		koutakuResp.Item.Content != nil &&
		koutakuResp.Item.Content.ContentBlocks != nil {
		for _, block := range koutakuResp.Item.Content.ContentBlocks {
			if block.Type == schemas.ResponsesOutputMessageContentTypeRenderedContent &&
				block.ResponsesOutputMessageContentRenderedContent != nil &&
				block.ResponsesOutputMessageContentRenderedContent.RenderedContent != "" {
	...
	case schemas.ResponsesStreamResponseTypeFunctionCallArgumentsDone:
		...
		if koutakuResp.Item != nil && koutakuResp.Item.ResponsesToolMessage != nil {
			argsStr = koutakuResp.Item.ResponsesToolMessage.Arguments
			name = koutakuResp.Item.ResponsesToolMessage.Name
			callID = koutakuResp.Item.ResponsesToolMessage.CallID
		}
		if argsStr == nil && koutakuResp.Arguments != nil {
			// Some providers (e.g., Anthropic) send Arguments directly on the response
			argsStr = koutakuResp.Arguments
			// Try to get name and callID from state if available
			if state != nil {
				outputIndex := 0
				if koutakuResp.OutputIndex != nil {
					outputIndex = *koutakuResp.OutputIndex
				}
				if name == nil {
	...
	case schemas.ResponsesStreamResponseTypeOutputItemAdded:
		if koutakuResp.Item != nil && koutakuResp.Item.ResponsesReasoning != nil && koutakuResp.Item.EncryptedContent != nil {
			candidate.Content.Parts = append(candidate.Content.Parts, &Part{
				ThoughtSignature: []byte(*koutakuResp.Item.ResponsesReasoning.EncryptedContent),
			})
		}
		// Track function call metadata for later use in FunctionCallArgumentsDone
		if koutakuResp.Item != nil && koutakuResp.Item.Type != nil &&
			*koutakuResp.Item.Type == schemas.ResponsesMessageTypeFunctionCall &&
			...
			if koutakuResp.Item.ResponsesToolMessage.Name != nil {
				state.ToolCallNames[outputIndex] = *koutakuResp.Item.ResponsesToolMessage.Name
			}
			if koutakuResp.Item.ResponsesToolMessage.CallID != nil {
				state.ToolCallIDs[outputIndex] = *koutakuResp.Item.ResponsesToolMessage.CallID
			}
		}
		return nil
	...
	case schemas.ResponsesStreamResponseTypeContentPartAdded:
		...
		if koutakuResp.Part != nil {
			part, err := convertContentBlockToGeminiPart(*koutakuResp.Part)
			if err == nil && part != nil {
				candidate.Content.Parts = append(candidate.Content.Parts, part)
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeCompleted:
		if koutakuResp.Response != nil {
			...
			if koutakuResp.Response.Model != "" {
				streamResp.ModelVersion = koutakuResp.Response.Model
			}

			// Convert usage metadata if available
			if koutakuResp.Response.Usage != nil {
				streamResp.UsageMetadata = ConvertKoutakuResponsesUsageToGeminiUsageMetadata(koutakuResp.Response.Usage)
			}

			// Derive finish reason from StopReason when present
			if koutakuResp.Response.StopReason != nil {
				candidate.FinishReason = ConvertKoutakuFinishReasonToGemini(*koutakuResp.Response.StopReason)
			} else {
				candidate.FinishReason = FinishReasonStop
			}
	...
	case schemas.ResponsesStreamResponseTypeFailed:
		candidate.FinishReason = FinishReasonOther
		if koutakuResp.Response != nil && koutakuResp.Response.Error != nil {
			streamResp.PromptFeedback = &GenerateContentResponsePromptFeedback{
				BlockReason:        "ERROR",
				BlockReasonMessage: koutakuResp.Response.Error.Message,
			}
		}
	...
	case schemas.ResponsesStreamResponseTypeRefusalDone:
		if koutakuResp.Refusal != nil && *koutakuResp.Refusal != "" {
			candidate.FinishReason = FinishReasonSafety
		}
...
func (state *GeminiResponsesStreamState) closeTextItemIfOpen(sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
...
// ToKoutakuResponsesStream converts a Gemini stream event to Koutaku Responses Stream responses
func (response *GenerateContentResponse) ToKoutakuResponsesStream(sequenceNumber int, state *GeminiResponsesStreamState) ([]*schemas.KoutakuResponsesStreamResponse, *schemas.KoutakuError) {
	...
	if !state.HasEmittedCreated {
		...
		createdResp := &schemas.KoutakuResponsesResponse{
			ID:        state.MessageID,
		...
		inProgressResp := &schemas.KoutakuResponsesResponse{
			ID:        state.MessageID,
			CreatedAt: state.CreatedAt,
		}
		if state.Model != nil {
...
func processGeminiPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func processGeminiTextPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
	...
	if part.Text != "" {
		...
		streamResponse := &schemas.KoutakuResponsesStreamResponse{
...
func processGeminiThoughtPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func processGeminiThoughtSignaturePart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func processGeminiFunctionCallPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
	...
	status := "in_progress"
	addedEvent := &schemas.KoutakuResponsesStreamResponse{
		Type:           schemas.ResponsesStreamResponseTypeOutputItemAdded,
		SequenceNumber: sequenceNumber + len(responses),
		OutputIndex:    &outputIndex,
		ItemID:         &toolUseID,
	...
	doneEvent := &schemas.KoutakuResponsesStreamResponse{
	...
	outputItemDone := &schemas.KoutakuResponsesStreamResponse{
		Type:           schemas.ResponsesStreamResponseTypeOutputItemDone,
		SequenceNumber: sequenceNumber + len(responses),
		OutputIndex:    &outputIndex,
		ItemID:         &toolUseID,
...
func processGeminiFunctionResponsePart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func processGeminiInlineDataPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func processGeminiFileDataPart(part *Part, state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func closeGeminiTextItem(state *GeminiResponsesStreamState, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func closeGeminiOpenItems(state *GeminiResponsesStreamState, groundingMetadata *GroundingMetadata, usage *GenerateContentResponseUsageMetadata, sequenceNumber int, finishReason FinishReason, finishMessage string) []*schemas.KoutakuResponsesStreamResponse {
	...
	koutakuUsage := ConvertGeminiUsageMetadataToResponsesUsage(usage)

	completedResp := &schemas.KoutakuResponsesResponse{
		ID:        state.MessageID,
		CreatedAt: state.CreatedAt,
		Usage:     koutakuUsage,
	}
	...
	if finishReason != "" {
		stopReason := ConvertGeminiFinishReasonToKoutaku(finishReason)
...
func FinalizeGeminiResponsesStream(state *GeminiResponsesStreamState, usage *GenerateContentResponseUsageMetadata, sequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
...
// buildGroundingMetadataFromWebSearch converts a Koutaku web_search_call message to Gemini GroundingMetadata
...
func emitWebSearchFromGroundingMetadata(
	...
	sequenceNumber int,
) []*schemas.KoutakuResponsesStreamResponse {
	var responses []*schemas.KoutakuResponsesStreamResponse
...
func generateSyntheticFunctionCallArgumentDeltas(argumentsJSON string, outputIndex *int, itemID *string, baseSequenceNumber int) []*schemas.KoutakuResponsesStreamResponse {
	var events []*schemas.KoutakuResponsesStreamResponse
	...
	for i := 0; i < len(runes); i += chunkSize {
		...
		deltaEvent := &schemas.KoutakuResponsesStreamResponse{
