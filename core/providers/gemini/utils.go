...
var (
	// Maps Gemini finish reasons to Koutaku format
	geminiFinishReasonToKoutaku = map[FinishReason]string{
	...
	// Maps Koutaku canonical finish reasons back to the most representative Gemini finish reason
	koutakuToGeminiFinishReason = map[string]FinishReason{
...
// ConvertGeminiFinishReasonToKoutaku converts Gemini finish reasons to Koutaku format
func ConvertGeminiFinishReasonToKoutaku(providerReason FinishReason) string {
	if koutakuReason, ok := geminiFinishReasonToKoutaku[providerReason]; ok {
		return koutakuReason
	}
	return string(providerReason)
}

// ConvertKoutakuFinishReasonToGemini converts Koutaku canonical finish reasons back to Gemini format.
func ConvertKoutakuFinishReasonToGemini(koutakuReason string) FinishReason {
	if geminiReason, ok := koutakuToGeminiFinishReason[koutakuReason]; ok {
		return geminiReason
	}
	return FinishReasonStop
}

// ConvertGeminiUsageMetadataToChatUsage converts Gemini usage metadata to Koutaku chat LLM usage
func ConvertGeminiUsageMetadataToChatUsage(metadata *GenerateContentResponseUsageMetadata) *schemas.KoutakuLLMUsage {
	if metadata == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToSpeechUsage converts Gemini usage metadata to Koutaku speech usage
...
// convertKoutakuSpeechUsageToGeminiUsageMetadata converts Koutaku speech usage to Gemini usage metadata
func convertKoutakuSpeechUsageToGeminiUsageMetadata(usage *schemas.SpeechUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToTranscriptionUsage converts Gemini usage metadata to Koutaku transcription usage
...
// convertKoutakuTranscriptionUsageToGeminiUsageMetadata converts Koutaku transcription usage to Gemini usage metadata
func convertKoutakuTranscriptionUsageToGeminiUsageMetadata(usage *schemas.TranscriptionUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// convertGeminiUsageMetadataToImageUsage converts Gemini usage metadata to Koutaku image usage
...
// convertKoutakuImageUsageToGeminiUsageMetadata converts Koutaku image usage to Gemini usage metadata
func convertKoutakuImageUsageToGeminiUsageMetadata(usage *schemas.ImageUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
...
// ConvertGeminiUsageMetadataToResponsesUsage converts Gemini usage metadata to Koutaku responses usage
...
func ConvertKoutakuResponsesUsageToGeminiUsageMetadata(usage *schemas.ResponsesResponseUsage) *GenerateContentResponseUsageMetadata {
	if usage == nil {
		return nil
	}
	metadata := &GenerateContentResponseUsageMetadata{
...
// convertParamsToGenerationConfig converts Koutaku parameters to Gemini GenerationConfig
...
// convertKoutakuToolsToGemini converts Koutaku tools to Gemini format
func convertKoutakuToolsToGemini(koutakuTools []schemas.ChatTool) []Tool {
	geminiTool := Tool{}

	for _, tool := range koutakuTools {
		if tool.Type == "" {
			continue
		}
...
// convertFunctionParametersToSchema converts Koutaku function parameters to Gemini Schema
...
// convertToolChoiceToToolConfig converts Koutaku tool choice to Gemini tool config
...
// convertKoutakuMessagesToGemini converts Koutaku messages to Gemini format
func convertKoutakuMessagesToGemini(messages []schemas.ChatMessage) ([]Content, *Content) {
					...
					if part.ThoughtSignature == nil && len(message.ChatAssistantMessage.ReasoningDetails) > 0 {
						...
						for _, reasoningDetail := range message.ChatAssistantMessage.ReasoningDetails {
							if reasoningDetail.ID != nil && *reasoningDetail.ID == lookupID &&
								reasoningDetail.Type == schemas.KoutakuReasoningDetailsTypeEncrypted &&
...
func downloadImageFromURL(ctx context.Context, imageURL string) (string, error) {
	...
	defer wait()
	if koutakuErr != nil {
		return "", fmt.Errorf("failed to download image: %v", koutakuErr)
	}
...
// ConvertGeminiLogprobsResultToKoutaku converts a Gemini LogprobsResult to Koutaku KoutakuLogProbs
func ConvertGeminiLogprobsResultToKoutaku(result *LogprobsResult) *schemas.KoutakuLogProbs {
	...
	return &schemas.KoutakuLogProbs{Content: content}
}
