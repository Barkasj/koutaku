...
import (
	...
	"github.com/koutaku/koutaku/core/internal/llmtests"
...
func TestEmptyCandidatesRegression(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var koutakuResp *schemas.KoutakuChatResponse

			if tt.isStream {
				koutakuResp, _, _ = tt.response.ToKoutakuChatCompletionStream(gemini.NewGeminiStreamState())
			} else {
				koutakuResp = tt.response.ToKoutakuChatResponse()
			}

			// Critical: Choices must NEVER be empty (this was the PR #1018 bug)
			require.NotNil(t, koutakuResp, "Response should not be nil")
			require.NotEmpty(t, koutakuResp.Choices, "Empty choices array")
			require.Len(t, koutakuResp.Choices, 1, "Should have exactly one error choice")
...
func TestToKoutakuEmbeddingResponsePreservesPrecision(t *testing.T) {
	const want = 0.12345678901234568

	resp := gemini.ToKoutakuEmbeddingResponse(&gemini.GeminiEmbeddingResponse{
...
func TestThoughtSignatureInToolCalls(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NotNil(t, koutakuResp, "Response should not be nil")
			require.NotEmpty(t, koutakuResp.Choices, "Should have choices")
			...
			for _, detail := range reasoningDetails {
				if detail.Type == schemas.KoutakuReasoningDetailsTypeEncrypted && detail.Signature != nil {
					foundEncrypted = true
					break
				}
			}
...
func TestMissingThoughtSignatureUsesBypassSentinel(t *testing.T) {
	result, err := gemini.ToGeminiChatCompletionRequest(&schemas.KoutakuChatRequest{
...
// TestKoutakuToGeminiToolConversion tests the conversion of tools from Koutaku to Gemini format
func TestKoutakuToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.KoutakuChatRequest
...
func TestKoutakuToGeminiToolConversion_PropertyOrdering(t *testing.T) {
	input := &schemas.KoutakuChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestKoutakuToGeminiToolConversion_NestedPropertyOrdering(t *testing.T) {
	input := &schemas.KoutakuChatRequest{
		Model: "gemini-2.0-flash",
		Input: []schemas.ChatMessage{{
			Role:    schemas.ChatMessageRoleUser,
...
func TestResponsesStructuredOutputConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.KoutakuResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
	}{
		{
...
// TestKoutakuResponsesToGeminiToolConversion tests the conversion of tools from Koutaku Responses API to Gemini format
func TestKoutakuResponsesToGeminiToolConversion(t *testing.T) {
	tests := []struct {
		name     string
		input    *schemas.KoutakuResponsesRequest
		validate func(t *testing.T, result *gemini.GeminiGenerationRequest)
...
func TestConvertGeminiUsageMetadataToChatUsage(t *testing.T) {
	tests := []struct {
		...
		expected *schemas.KoutakuLLMUsage
	}{
		{
...
// TestConvertGeminiUsageMetadataToResponsesUsage tests the conversion of Gemini usage metadata to Koutaku responses usage
...
func TestGeminiToolInputKeyOrderPreservation(t *testing.T) {
	...
	result, err := gemini.ToGeminiChatCompletionRequest(koutakuReq)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestGenAIThinkingLevel_RoundTripPreservesLevelNotBudget(t *testing.T) {
	...
	koutakuReq := geminiReq.ToKoutakuResponsesRequest(koutakuCtx)
	require.NotNil(t, koutakuReq.Params)
	require.NotNil(t, koutakuReq.Params.Reasoning)
	require.NotNil(t, koutakuReq.Params.Reasoning.Effort)
	assert.Equal(t, "minimal", *koutakuReq.Params.Reasoning.Effort)
	assert.Nil(t, koutakuReq.Params.Reasoning.MaxTokens, "thinkingLevel must not populate reasoning max_tokens")

	roundTrip, err := gemini.ToGeminiResponsesRequest(koutakuReq)
	require.NoError(t, err)
...
// Regression: MAX_TOKENS from Gemini must survive Gemini → Koutaku → Gemini on the GenAI path
// (StopReason used to be dropped, so clients saw STOP instead of MAX_TOKENS).
func TestGenAIFinishReasonMaxTokens_PersistsThroughKoutakuRoundTrip(t *testing.T) {
	...
	koutakuResp := geminiResp.ToResponsesKoutakuResponsesResponse()
	require.NotNil(t, koutakuResp)
	require.NotNil(t, koutakuResp.StopReason)
	assert.Equal(t, "length", *koutakuResp.StopReason)

	out := gemini.ToGeminiResponsesResponse(koutakuResp)
