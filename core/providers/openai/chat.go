...
// ToKoutakuChatRequest converts an OpenAI chat request to Koutaku format
func (req *OpenAIChatRequest) ToKoutakuChatRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuChatRequest {
	...
	return &schemas.KoutakuChatRequest{
		Provider:  provider,
		Model:     model,
		Input:     ConvertOpenAIMessagesToKoutakuMessages(req.Messages),
		Params:    &req.ChatParameters,
		Fallbacks: schemas.ParseFallbacks(req.Fallbacks),
	}
}

// ToOpenAIChatRequest converts a Koutaku chat completion request to OpenAI format
func ToOpenAIChatRequest(ctx *schemas.KoutakuContext, koutakuReq *schemas.KoutakuChatRequest) *OpenAIChatRequest {
	...
	openaiReq := &OpenAIChatRequest{
		...
		Messages: ConvertKoutakuMessagesToOpenAIMessages(koutakuReq.Input),
	}

	if koutakuReq.Params != nil {
		openaiReq.ChatParameters = *koutakuReq.Params
	...
	case schemas.XAI:
		...
		openaiReq.applyXAICompatibility(koutakuReq.Model)
		return openaiReq
	case schemas.Gemini:
		openaiReq.filterOpenAISpecificParameters()
	...
	case schemas.Vertex:
		...
		if schemas.IsMistralModel(koutakuReq.Model) {
			openaiReq.applyMistralCompatibility()
		}
		return openaiReq
	...
	default:
		...
		if isCustomProvider, ok := ctx.Value(schemas.KoutakuContextKeyIsCustomProvider).(bool); ok && isCustomProvider {
			return openaiReq
		}
		openaiReq.filterOpenAISpecificParameters()
		return openaiReq
