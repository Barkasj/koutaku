...
// CustomResponseHandler is a function that produces a Koutaku response from a Koutaku request.
// T is the concrete Koutaku response type (e.g. KoutakuEmbeddingResponse, KoutakuTextCompletionResponse, KoutakuChatResponse, KoutakuResponsesResponse, KoutakuImageGenerationResponse, KoutakuTranscriptionResponse).
type responseHandler[T any] func(responseBody []byte, response *T, requestBody []byte, sendBackRawRequest bool, sendBackRawResponse bool) (rawRequest interface{}, rawResponse interface{}, koutakuErr *schemas.KoutakuError)

func ConvertOpenAIMessagesToKoutakuMessages(messages []OpenAIMessage) []schemas.ChatMessage {
	koutakuMessages := make([]schemas.ChatMessage, len(messages))
	for i, message := range messages {
		koutakuMessages[i] = schemas.ChatMessage{
			Name:            message.Name,
			Role:            message.Role,
		...
		if message.OpenAIChatAssistantMessage != nil {
			koutakuMessages[i].ChatAssistantMessage = &schemas.ChatAssistantMessage{
...
func ConvertKoutakuMessagesToOpenAIMessages(messages []schemas.ChatMessage) []OpenAIMessage {
