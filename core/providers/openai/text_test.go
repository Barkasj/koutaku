...
func TestToOpenAITextCompletionRequest_FireworksUsesCacheIsolation(t *testing.T) {
	...
	result := ToOpenAITextCompletionRequest(koutakuReq)
	if result == nil {
	...
	if _, ok := koutakuReq.Params.ExtraParams["prompt_cache_key"]; !ok {
		t.Fatalf("expected original extra params to remain unchanged, got %#v", koutakuReq.Params.ExtraParams)
	}
	...
	wireBody, koutakuErr := providerUtils.CheckContextAndGetRequestBody(
		...
		func() (providerUtils.RequestBodyWithExtraParams, error) {
			return ToOpenAITextCompletionRequest(koutakuReq), nil
		},
	)
	if koutakuErr != nil {
