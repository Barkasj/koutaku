...
// ToMistralTranscriptionRequest converts a Koutaku transcription request to Mistral format.
func ToMistralTranscriptionRequest(koutakuReq *schemas.KoutakuTranscriptionRequest) *MistralTranscriptionRequest {
	if koutakuReq == nil || koutakuReq.Input == nil || len(koutakuReq.Input.File) == 0 {
		return nil
	}

	req := &MistralTranscriptionRequest{
		Model:    koutakuReq.Model,
		File:     koutakuReq.Input.File,
		Filename: koutakuReq.Input.Filename,
	}

	if koutakuReq.Params != nil {
		req.Language = koutakuReq.Params.Language
		req.Prompt = koutakuReq.Params.Prompt
		...
		if koutakuReq.Params.ExtraParams != nil {
			if temp, ok := schemas.SafeExtractFloat64Pointer(koutakuReq.Params.ExtraParams["temperature"]); ok {
				req.Temperature = temp
			}
			if granularities, ok := koutakuReq.Params.ExtraParams["timestamp_granularities"].([]string); ok {
...
// ToKoutakuTranscriptionResponse converts a Mistral transcription response to Koutaku format.
func (r *MistralTranscriptionResponse) ToKoutakuTranscriptionResponse() *schemas.KoutakuTranscriptionResponse {
	if r == nil {
		return nil
	}
...
func createMistralTranscriptionMultipartBody(req *MistralTranscriptionRequest, providerName schemas.ModelProvider) (*bytes.Buffer, string, *schemas.KoutakuError) {
...
func parseTranscriptionFormDataBodyFromRequest(writer *multipart.Writer, req *MistralTranscriptionRequest, providerName schemas.ModelProvider) *schemas.KoutakuError {
	...
	if req.Language != nil {
		if err := writer.WriteField("language", *req.Language); err != nil {
			return providerUtils.NewKoutakuOperationError("failed to write language field",  err)
		}
	}
	...
	for _, granularity := range req.TimestampGranularities {
		if err := writer.WriteField("timestamp_granularities[]", granularity); err != nil {
			return providerUtils.NewKoutakuOperationError("failed to write timestamp_granularities field",  err)
		}
	}
	...
	if err != nil {
		return providerUtils.NewKoutakuOperationError("failed to create form file",  err)
	}
...
// ToKoutakuTranscriptionStreamResponse converts a Mistral streaming event to Koutaku format.
func (e *MistralTranscriptionStreamEvent) ToKoutakuTranscriptionStreamResponse() *schemas.KoutakuTranscriptionStreamResponse {
	if e == nil {
		return nil
	}
