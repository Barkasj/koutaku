...
// TestToMistralOCRRequest tests conversion from Koutaku OCR request to Mistral OCR request.
func TestToMistralOCRRequest(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name     string
		input    *schemas.KoutakuOCRRequest
...
// TestToKoutakuOCRResponse tests conversion from Mistral OCR response to Koutaku OCR response.
func TestToKoutakuOCRResponse(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name     string
		input    *MistralOCRResponse
		validate func(t *testing.T, result *schemas.KoutakuOCRResponse)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			result := tt.input.ToKoutakuOCRResponse()
			tt.validate(t, result)
		})
	}
}
...
func TestOCRWithMockServer(t *testing.T) {
	...
	tests := []struct {
		name           string
		request        *schemas.KoutakuOCRRequest
		statusCode     int
		responseBody   interface{}
		expectError    bool
		errorContains  string
		validateError  func(t *testing.T, err *schemas.KoutakuError)
		validateResult func(t *testing.T, resp *schemas.KoutakuOCRResponse)
	}{
		{
...
func TestMistralOCRIntegration(t *testing.T) {
	...
	resp, koutakuErr := provider.OCR(ctx, schemas.Key{Value: *schemas.NewEnvVar(apiKey)}, request)

	require.Nil(t, koutakuErr, "OCR request failed: %v", koutakuErr)
	require.NotNil(t, resp)
	assert.Equal(t, "mistral-ocr-latest", resp.Model)
