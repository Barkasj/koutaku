...
func TestParseMistralError_UsesExportedConverterMetadata(t *testing.T) {
	...
	assert.Equal(t, "invalid request", koutakuErr.Error.Message)
	assert.Equal(t, schemas.Ptr("invalid_request_error"), koutakuErr.Error.Type)
	assert.Equal(t, schemas.Ptr("bad_request"), koutakuErr.Error.Code)
...
func TestMistralProvider_CustomAliasChatStreamUsesBaseCompatibilityAndAliasMetadata(t *testing.T) {
	...
	ctx.SetValue(schemas.KoutakuContextKeyIsCustomProvider, true)
	...
	postHookRunner := func(_ *schemas.KoutakuContext, response *schemas.KoutakuResponse, err *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError) {
		return response, err
	}

	stream, koutakuErr := provider.ChatCompletionStream(ctx, postHookRunner, nil, schemas.Key{}, request)
	require.Nil(t, koutakuErr)

	var firstResponse *schemas.KoutakuChatResponse
	for chunk := range stream {
		if chunk.KoutakuError != nil {
			t.Fatalf("unexpected stream error: %s", chunk.KoutakuError.Error.Message)
		}
		if chunk.KoutakuChatResponse != nil {
			firstResponse = chunk.KoutakuChatResponse
			break
		}
	}

	require.NotNil(t, firstResponse)
	// Note: ExtraFields.Provider on stream chunks is populated by koutaku.go's
...
func TestMistralProvider_CustomAliasEmbeddingReportsAliasMetadata(t *testing.T) {
	...
	response, koutakuErr := provider.Embedding(ctx, schemas.Key{}, request)
	require.Nil(t, koutakuErr)
	require.NotNil(t, response)

	// Note: ExtraFields.Provider and ResolvedModelUsed are populated by
	// koutaku.go's dispatcher via PopulateExtraFields, not by provider
