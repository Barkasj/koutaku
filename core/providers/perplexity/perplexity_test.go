...
func TestToKoutakuChatResponse_Citations(t *testing.T) {
	t.Parallel()

	t.Run("citations are mapped to koutaku response", func(t *testing.T) {
		...
		koutakuResp := response.ToKoutakuChatResponse("sonar-pro")

		if koutakuResp == nil {
			t.Fatal("expected non-nil koutaku response")
		}

		// Verify citations are preserved
		if len(koutakuResp.Citations) != 3 {
			t.Fatalf("expected 3 citations, got %d", len(koutakuResp.Citations))
		}
		if koutakuResp.Citations[0] != "https://example.com/article1" {
			t.Errorf("expected first citation to be 'https://example.com/article1', got '%s'", koutakuResp.Citations[0])
		}
		if koutakuResp.Citations[1] != "https://example.com/article2" {
			t.Errorf("expected second citation to be 'https://example.com/article2', got '%s'", koutakuResp.Citations[1])
		}
		if koutakuResp.Citations[2] != "https://example.com/article3" {
			t.Errorf("expected third citation to be 'https://example.com/article3', got '%s'", koutakuResp.Citations[2])
		}

		// Verify search results are preserved
		if len(koutakuResp.SearchResults) != 1 {
			t.Fatalf("expected 1 search result, got %d", len(koutakuResp.SearchResults))
		}

		// Verify videos are preserved
		if len(koutakuResp.Videos) != 1 {
			t.Fatalf("expected 1 video, got %d", len(koutakuResp.Videos))
		}

		// Verify usage citation tokens are mapped
		if koutakuResp.Usage == nil || koutakuResp.Usage.CompletionTokensDetails == nil {
			t.Fatal("expected usage with completion token details")
		}
		if koutakuResp.Usage.CompletionTokensDetails.CitationTokens == nil || *koutakuResp.Usage.CompletionTokensDetails.CitationTokens != 5 {
			t.Error("expected citation_tokens to be 5")
		}
		if koutakuResp.Usage.CompletionTokensDetails.NumSearchQueries == nil || *koutakuResp.Usage.CompletionTokensDetails.NumSearchQueries != 2 {
	...
	t.Run("nil citations remain nil", func(t *testing.T) {
		...
		if koutakuResp.Citations != nil {
			t.Errorf("expected nil citations, got %v", koutakuResp.Citations)
		}
		if koutakuResp.SearchResults != nil {
			t.Errorf("expected nil search results, got %v", koutakuResp.SearchResults)
		}
	})
	...
	t.Run("empty citations slice is preserved", func(t *testing.T) {
		...
		if len(koutakuResp.Citations) != 0 {
			t.Errorf("expected 0 citations, got %d", len(koutakuResp.Citations))
		}
	})
}
...
func TestWebSearchOption_JSONSerialization(t *testing.T) {
	...
	t.Run("search_type extraction from ExtraParams", func(t *testing.T) {
		...
		result := perplexity.ToPerplexityChatCompletionRequest(koutakuReq)
...
func TestKoutakuCost_DeserializationWithPerplexityFields(t *testing.T) {
	t.Parallel()
	...
	t.Run("deserializes new cost breakdown fields", func(t *testing.T) {
		...
		var cost schemas.KoutakuCost
		if err := cost.UnmarshalJSON([]byte(jsonStr)); err != nil {
			t.Fatalf("failed to unmarshal KoutakuCost: %v", err)
		}
