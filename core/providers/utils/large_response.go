...
func PrepareResponseStreaming(ctx *schemas.KoutakuContext, client *fasthttp.Client, resp *fasthttp.Response) *fasthttp.Client {
...
func MaterializeStreamErrorBody(ctx *schemas.KoutakuContext, resp *fasthttp.Response) {
	responseThreshold, _ := ctx.Value(schemas.KoutakuContextKeyLargeResponseThreshold).(int64)
	if responseThreshold <= 0 {
		return
	}
...
func FinalizeResponseWithLargeDetection(
...
) ([]byte, bool, *schemas.KoutakuError) {
	responseThreshold, _ := ctx.Value(schemas.KoutakuContextKeyLargeResponseThreshold).(int64)
	...
	if responseThreshold <= 0 {
		...
		if err != nil {
			return nil, false, NewKoutakuOperationError(schemas.ErrProviderResponseDecode, err)
		}
	...
	if contentLength > 0 && int64(contentLength) <= responseThreshold {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			if readErr != nil {
				return nil, false, NewKoutakuOperationError(schemas.ErrProviderResponseDecode, readErr)
			}
	...
	if contentLength <= 0 {
		if bodyStream := resp.BodyStream(); bodyStream != nil {
			...
			ctx.SetValue(schemas.KoutakuContextKeyLargeResponseMode, true)
			ctx.SetValue(schemas.KoutakuContextKeyLargeResponseReader, closableReader)
			ctx.SetValue(schemas.KoutakuContextKeyLargeResponseContentLength, contentLength)
			if ct := string(resp.Header.ContentType()); ct != "" {
				ctx.SetValue(schemas.KoutakuContextKeyLargeResponseContentType, ct)
			}
			previewLen := min(len(bodyBytes), 1048576)
			ctx.SetValue(schemas.KoutakuContextKeyLargePayloadResponsePreview, string(bodyBytes[:previewLen]))
			return nil, true, nil
		}
		// No stream — buffered fallback
		body, err := CheckAndDecodeBody(resp)
	...
	if ps, ok := ctx.Value(schemas.KoutakuContextKeyLargePayloadPrefetchSize).(int); ok && ps > 0 {
		prefetchSize = ps
	}
	...
	ctx.SetValue(schemas.KoutakuContextKeyLargePayloadResponsePreview, string(prefetchBuf[:previewLen]))

	return nil, true, nil
}

// ParseOpenAIUsageFromBytes parses OpenAI-format usage from raw JSON bytes into KoutakuLLMUsage.
...
func ParseOpenAIUsageFromBytes(data []byte) *schemas.KoutakuLLMUsage {
	var usage struct {
	...
	result := &schemas.KoutakuLLMUsage{}
...
func SetupStreamingPassthrough(ctx *schemas.KoutakuContext, resp *fasthttp.Response) bool {
