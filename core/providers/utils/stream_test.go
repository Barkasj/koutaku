...
func TestCheckFirstStreamChunk_ErrorInFirstChunk(t *testing.T) {
	stream := make(chan *schemas.KoutakuStreamChunk, 2)
	stream <- &schemas.KoutakuStreamChunk{
...
func TestCheckFirstStreamChunk_ValidFirstChunk(t *testing.T) {
	stream := make(chan *schemas.KoutakuStreamChunk, 3)
	chunk1 := &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{
			ID: "chatcmpl-123",
		},
	}
	chunk2 := &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{
			ID: "chatcmpl-123",
		},
	}
	...
	got1 := <-wrapped
	if got1.KoutakuChatResponse == nil || got1.KoutakuChatResponse.ID != "chatcmpl-123" {
		t.Error("first chunk not re-injected correctly")
	}

	// Second chunk should follow
	got2 := <-wrapped
	if got2.KoutakuChatResponse == nil || got2.KoutakuChatResponse.ID != "chatcmpl-123" {
...
func TestCheckFirstStreamChunk_EmptyStream(t *testing.T) {
	stream := make(chan *schemas.KoutakuStreamChunk)
	close(stream)
...
func TestCheckFirstStreamChunk_ErrorInSecondChunk(t *testing.T) {
	...
	// Read all chunks
	got1 := <-wrapped
	if got1.KoutakuChatResponse == nil {
		t.Error("first chunk should be valid data")
	}
	got2 := <-wrapped
	if got2.KoutakuError == nil {
...
func TestCheckFirstStreamChunk_ErrorDrainsSource(t *testing.T) {
	stream := make(chan *schemas.KoutakuStreamChunk, 5)
	stream <- &schemas.KoutakuStreamChunk{
		KoutakuError: &schemas.KoutakuError{
			Error: &schemas.ErrorField{
	...
	stream <- &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{ID: "1"},
	}
	stream <- &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{ID: "2"},
	}
	close(stream)
...
func TestCheckFirstStreamChunk_CtxCancelUnblocksWrapper(t *testing.T) {
	...
	src := make(chan *schemas.KoutakuStreamChunk, 1)
	src <- &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{ID: "1"},
	}
	...
	src <- &schemas.KoutakuStreamChunk{
		KoutakuChatResponse: &schemas.KoutakuChatResponse{ID: "3"},
	}
	close(src)
