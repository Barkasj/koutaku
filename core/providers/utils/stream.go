...
func CheckFirstStreamChunkForError(
	ctx context.Context,
	stream chan *schemas.KoutakuStreamChunk,
) (chan *schemas.KoutakuStreamChunk, <-chan struct{}, *schemas.KoutakuError) {
	...
	if firstChunk.KoutakuError != nil && firstChunk.KoutakuError.Error != nil &&
		(firstChunk.KoutakuError.Error.Message != "" || firstChunk.KoutakuError.Error.Code != nil || firstChunk.KoutakuError.Error.Type != nil) {
		...
		return nil, done, firstChunk.KoutakuError
	}

	// First chunk is valid data — wrap channel to re-inject it
	done := make(chan struct{})
	wrapped := make(chan *schemas.KoutakuStreamChunk, max(cap(stream), 1))
	wrapped <- firstChunk
	go func() {
		defer close(done)
		defer close(wrapped)
