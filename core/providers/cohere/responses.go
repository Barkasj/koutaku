...
// convertCohereContentBlockToKoutaku converts CohereContentBlock to schemas.ContentBlock for Responses
func convertCohereContentBlockToKoutaku(cohereBlock CohereContentBlock) schemas.ResponsesMessageContentBlock {
...
func (chunk *CohereStreamEvent) ToKoutakuResponsesStream(sequenceNumber int, state *CohereResponsesStreamState) ([]*schemas.KoutakuResponsesStreamResponse, *schemas.KoutakuError, bool) {
	switch chunk.Type {
...
// ConvertResponsesTextFormatToCohere converts Koutaku Responses Text.Format to Cohere's typed format
...
// ToCohereResponsesRequest converts a KoutakuRequest (Responses structure) to CohereChatRequest
func ToCohereResponsesRequest(koutakuReq *schemas.KoutakuResponsesRequest) (*CohereChatRequest, error) {
	if koutakuReq == nil {
		return nil, nil
	}
	...
	if koutakuReq.Params != nil {
		if koutakuReq.Params.MaxOutputTokens != nil {
			cohereReq.MaxTokens = koutakuReq.Params.MaxOutputTokens
		}
		...
		if koutakuReq.Params.Reasoning != nil {
			if koutakuReq.Params.Reasoning.MaxTokens != nil {
				...
				if koutakuReq.Params.Reasoning.Effort != nil && *koutakuReq.Params.Reasoning.Effort != "none" {
					maxOutputTokens := providerUtils.GetMaxOutputTokensOrDefault(koutakuReq.Model, DefaultCompletionMaxTokens)
					if koutakuReq.Params.MaxOutputTokens != nil {
						maxOutputTokens = *koutakuReq.Params.MaxOutputTokens
					}
					budgetTokens, err := providerUtils.GetBudgetTokensFromReasoningEffort(*koutakuReq.Params.Reasoning.Effort, MinimumReasoningMaxTokens, maxOutputTokens)
					if err != nil {
						return nil, err
					}
		...
		if koutakuReq.Params.Text != nil && koutakuReq.Params.Text.Format != nil {
			cohereReq.ResponseFormat = convertResponsesTextFormatToCohere(koutakuReq.Params.Text.Format)
		}
		if koutakuReq.Params.ExtraParams != nil {
			...
			if thinkingParam, ok := schemas.SafeExtractFromMap(koutakuReq.Params.ExtraParams, "thinking"); ok {
				if thinkingMap, ok := thinkingParam.(map[string]interface{}); ok {
					...
					cohereReq.Thinking = thinking
					koutakuReq.Params.ExtraParams["thinking"] = thinkingMap
				}
			}
		}
	}
	...
	if koutakuReq.Params != nil && koutakuReq.Params.ToolChoice != nil {
		cohereReq.ToolChoice = convertKoutakuToolChoiceToCohereToolChoice(*koutakuReq.Params.ToolChoice)
	}

	// Process ResponsesInput (which contains the Responses items)
	if koutakuReq.Input != nil {
		cohereReq.Messages = ConvertKoutakuMessagesToCohereMessages(koutakuReq.Input, koutakuReq.Params)
	}

	return cohereReq, nil
}

// ToKoutakuResponsesResponse converts CohereChatResponse to KoutakuResponse (Responses structure)
func (response *CohereChatResponse) ToKoutakuResponsesResponse() *schemas.KoutakuResponsesResponse {
	if response == nil {
		return nil
	}
	...
	if response.Usage != nil {
		...
		koutakuResp.Usage = usage
	}

	// Convert output message to Responses format
	if response.Message != nil {
		outputMessages := ConvertCohereMessagesToKoutakuMessages([]CohereMessage{*response.Message}, true)
...
// ConvertKoutakuMessagesToCohereMessages converts an array of Koutaku ResponsesMessage to Cohere message format
// This is the main conversion method from Koutaku to Cohere - handles all message types and returns messages
func ConvertKoutakuMessagesToCohereMessages(koutakuMessages []schemas.ResponsesMessage, params *schemas.ResponsesParameters) []CohereMessage {
	...
	for _, msg := range koutakuMessages {
		...
		case schemas.ResponsesMessageTypeMessage:
			...
			if role == "system" {
				...
				systemMsgs := convertKoutakuMessageToCohereSystemContent(&msg)
				systemContent = append(systemContent, systemMsgs...)
			} else {
				// Convert regular message
				cohereMsg := convertKoutakuMessageToCohereMessage(&msg)
				if cohereMsg != nil {
					if role == "assistant" {
		...
		case schemas.ResponsesMessageTypeReasoning:
			...
			reasoningBlocks := convertKoutakuReasoningToCohereThinking(&msg)
		...
		case schemas.ResponsesMessageTypeFunctionCall:
			...
			assistantMsg := convertKoutakuFunctionCallToCohereMessage(&msg)
			if assistantMsg != nil {
		...
		case schemas.ResponsesMessageTypeFunctionCallOutput:
			...
			toolMsg := convertKoutakuFunctionCallOutputToCohereMessage(&msg)
			if toolMsg != nil {
...
// ConvertCohereMessagesToKoutakuMessages converts an array of Cohere messages to Koutaku ResponsesMessage format
// This is the main conversion method from Cohere to Koutaku - handles all message types and content blocks
func ConvertCohereMessagesToKoutakuMessages(cohereMessages []CohereMessage, isOutputMessage bool) []schemas.ResponsesMessage {
	var koutakuMessages []schemas.ResponsesMessage

	for _, msg := range cohereMessages {
		convertedMessages := convertSingleCohereMessageToKoutakuMessages(&msg, isOutputMessage)
...
// convertKoutakuToolChoiceToCohere converts schemas.ToolChoice to CohereToolChoice
func convertKoutakuToolChoiceToCohereToolChoice(toolChoice schemas.ResponsesToolChoice) *CohereToolChoice {
...
// convertKoutakuMessageToCohereSystemContent converts a Koutaku system message to Cohere system content
func convertKoutakuMessageToCohereSystemContent(msg *schemas.ResponsesMessage) []string {
	var systemContent []string
...
// convertKoutakuMessageToCohereMessage converts a regular Koutaku message to Cohere message
func convertKoutakuMessageToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
	role := "user"
	if msg.Role != nil {
		role = string(*msg.Role)
	}
...
// convertKoutakuReasoningToCohereThinking converts a Koutaku reasoning message to Cohere thinking blocks
func convertKoutakuReasoningToCohereThinking(msg *schemas.ResponsesMessage) []CohereContentBlock {
...
// convertKoutakuFunctionCallToCohereMessage converts a Koutaku function call to Cohere message
func convertKoutakuFunctionCallToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
...
// convertKoutakuFunctionCallOutputToCohereMessage converts a Koutaku function call output to Cohere message
func convertKoutakuFunctionCallOutputToCohereMessage(msg *schemas.ResponsesMessage) *CohereMessage {
	if msg.ResponsesToolMessage != nil && msg.ResponsesToolMessage.CallID != nil {
...
// convertSingleCohereMessageToKoutakuMessages converts a single Cohere message to Koutaku messages
func convertSingleCohereMessageToKoutakuMessages(cohereMsg *CohereMessage, isOutputMessage bool) []schemas.ResponsesMessage {
		...
		if cohereMsg.Content.StringContent != nil {
			...
			for _, block := range cohereMsg.Content.BlocksContent {
				if block.Type == CohereContentBlockTypeThinking {
					...
					converted := convertCohereContentBlockToKoutaku(block)
					if converted.Type != "" {
...
// convertKoutakuContentBlocksToCohere converts Koutaku content blocks to Cohere format
