...
func TestToAnthropicChatRequest_PreservesPropertyOrder(t *testing.T) {
	...
	defer cancel()
	result, err := ToAnthropicChatRequest(ctx, koutakuReq)
	if err != nil {
...
func TestToAnthropicChatRequest_CachingDeterminism(t *testing.T) {
	makeReq := func(props *schemas.OrderedMap) *schemas.KoutakuChatRequest {
...
func TestToKoutakuChatResponse_MultipleTextBlocksWithThinking(t *testing.T) {
	...
	ctx, cancel := schemas.NewKoutakuContextWithCancel(nil)
	defer cancel()
	result := response.ToKoutakuChatResponse(ctx)

	if result == nil {
		t.Fatal("expected non-nil result")
	}

	// With multiple text blocks, ToKoutakuChatResponse preserves them as ContentBlocks
	...
	if rd[0].Type != schemas.KoutakuReasoningDetailsTypeText {
		t.Errorf("expected reasoning detail type %s, got %s", schemas.KoutakuReasoningDetailsTypeText, rd[0].Type)
	}
...
func TestToKoutakuChatResponse_SingleTextBlockNoThinking(t *testing.T) {
			...
			},
			{
				...
				ChatAssistantMessage: &schemas.ChatAssistantMessage{
					ReasoningDetails: []schemas.ChatReasoningDetails{
						{Index: 0, Type: schemas.KoutakuReasoningDetailsTypeText, Text: &modifiedContent, Signature: &signature},
					},
				},
			},
			{
...
func TestToAnthropicChatRequest_NormalFlowUnchanged(t *testing.T) {
	...
	// no koutaku.content_blocks) produces the same output as before.
