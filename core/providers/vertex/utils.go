...
func getRequestBodyForAnthropicResponses(ctx *schemas.KoutakuContext, request *schemas.KoutakuResponsesRequest, deployment string, isStreaming bool, isCountTokens bool, betaHeaderOverrides map[string]bool, providerExtraHeaders map[string]string) ([]byte, *schemas.KoutakuError) {
	...
	if useRawBody, ok := ctx.Value(schemas.KoutakuContextKeyUseRawRequestBody).(bool); ok && useRawBody {
		...
		if request.Params != nil && request.Params.Tools != nil {
			if toolErr := anthropic.ValidateToolsForProvider(request.Params.Tools, schemas.Vertex); toolErr != nil {
				return nil, providerUtils.NewKoutakuOperationError(toolErr.Error(), nil)
			}
		}
...
func buildResponseFromConfig(deployments map[string]string, allowedModels schemas.WhiteList, blacklistedModels schemas.BlackList) *schemas.KoutakuListModelsResponse {
