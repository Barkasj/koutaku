...
// ToVertexRankRequest converts a Koutaku rerank request to Discovery Engine rank API format.
func ToVertexRankRequest(koutakuReq *schemas.KoutakuRerankRequest, options *vertexRerankOptions) (*VertexRankRequest, error) {
	if koutakuReq == nil {
	...
	if len(koutakuReq.Documents) > vertexMaxRerankRecordsPerQuery {
	...
	rankRequest := &VertexRankRequest{
		...
		Records: make([]VertexRankRecord, len(koutakuReq.Documents)),
	}
	...
	if koutakuReq.Params != nil && koutakuReq.Params.TopN != nil {
	...
	trimmedModel := strings.TrimSpace(koutakuReq.Model)
	if trimmedModel == "" {
...
// ToKoutakuRerankRequest converts a Discovery Engine rank request to Koutaku format.
func (req *VertexRankRequest) ToKoutakuRerankRequest(ctx *schemas.KoutakuContext) *schemas.KoutakuRerankRequest {
	if req == nil {
		return nil
	}
	...
	for _, record := range req.Records {
		...
		koutakuReq.Documents = append(koutakuReq.Documents, doc)
	}
...
// ToKoutakuRerankResponse converts a Discovery Engine rank response to Koutaku format.
func (response *VertexRankResponse) ToKoutakuRerankResponse(documents []schemas.RerankDocument, returnDocuments bool) (*schemas.KoutakuRerankResponse, error) {
	if response == nil {
