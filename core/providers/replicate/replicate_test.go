...
func TestFileUpload_OrdersMetadataBeforeFile(t *testing.T) {
	...
	_, koutakuErr := provider.FileUpload(ctx, schemas.Key{}, &schemas.KoutakuFileUploadRequest{
...
// TestKoutakuToReplicateChatRequestConversion tests the conversion from Koutaku chat request to Replicate format
// with special handling based on model names
func TestKoutakuToReplicateChatRequestConversion(t *testing.T) {
	maxTokens := 100
	temp := 0.7
	topP := 0.9
	seed := 42
...
// TestKoutakuToReplicateImageGenerationConversion tests the conversion from Koutaku image generation request
// to Replicate format with special handling for different model families
func TestKoutakuToReplicateImageGenerationConversion(t *testing.T) {
	...
	tests := []struct {
		name     string
		input    *schemas.KoutakuImageGenerationRequest
		validate func(t *testing.T, result *replicate.ReplicatePredictionRequest)
		wantErr  bool
	}{
		{
...
// TestKoutakuToReplicateResponsesRequestConversion tests the conversion from Koutaku responses request to Replicate format
func TestKoutakuToReplicateResponsesRequestConversion(t *testing.T) {
	maxOutputTokens := 100
	temp := 0.7
	topP := 0.9
...
// TestReplicateToKoutakuResponsesResponse tests the conversion from Replicate prediction response to Koutaku responses format
func TestReplicateToKoutakuResponsesResponse(t *testing.T) {
	...
	tests := []struct {
		...
		validate func(t *testing.T, result *schemas.KoutakuResponsesResponse)
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := tt.input.ToKoutakuResponsesResponse()
			if tt.validate != nil {
				tt.validate(t, actual)
			}
		})
