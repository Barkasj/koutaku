...
func ToReplicateResponsesRequest(koutakuReq *schemas.KoutakuResponsesRequest) (*ReplicatePredictionRequest, error) {
	if koutakuReq == nil {
	...
	if strings.HasPrefix(koutakuReq.Model, "openai/") && strings.Contains(koutakuReq.Model, "gpt-5-structured") {
		// handle responses style request
		if len(koutakuReq.Input) > 0 {
			input.InputItemList = koutakuReq.Input
		}
		if koutakuReq.Params != nil {
			if koutakuReq.Params.Instructions != nil {
				input.Instructions = koutakuReq.Params.Instructions
			}
			if koutakuReq.Params.Tools != nil {
				input.Tools = koutakuReq.Params.Tools
			}
			if koutakuReq.Params.MaxOutputTokens != nil {
				input.MaxOutputTokens = koutakuReq.Params.MaxOutputTokens
			}
			if koutakuReq.Params.Text != nil {
				input.JsonSchema = koutakuReq.Params.Text
			}
			if koutakuReq.Params.ExtraParams != nil {
				input.ExtraParams = koutakuReq.Params.ExtraParams
			}
		}
	} else {
		...
		if len(koutakuReq.Input) > 0 {
			...
			if strings.HasPrefix(koutakuReq.Model, string(schemas.OpenAI)) {
				input.Messages = schemas.ToChatMessages(koutakuReq.Input)
			} else {
				...
				for _, msg := range koutakuReq.Input {
					if msg.Content == nil {
						continue
					}
				...
				modelSupportsSystemPrompt := supportsSystemPrompt(koutakuReq.Model)
		...
		if koutakuReq.Params != nil {
			...
			if params.Instructions != nil && *params.Instructions != "" {
				if supportsSystemPrompt(koutakuReq.Model) {
...
func (response *ReplicatePredictionResponse) ToKoutakuResponsesResponse() *schemas.KoutakuResponsesResponse {
	if response == nil {
		return nil
	}
	...
	koutakuResponse.Output = outputMessages
	...
	koutakuResponse.Status = &status

	// Set error if present
	if response.Error != nil && *response.Error != "" {
		koutakuResponse.Error = &schemas.ResponsesResponseError{
			Code:    "provider_error",
	...
	if response.Logs != nil {
		...
		if found {
			koutakuResponse.Usage = &schemas.ResponsesResponseUsage{
