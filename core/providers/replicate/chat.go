...
func ToReplicateChatRequest(koutakuReq *schemas.KoutakuChatRequest) (*ReplicatePredictionRequest, error) {
	...
	isGPT5Structured := strings.HasPrefix(koutakuReq.Model, string(schemas.OpenAI)) && strings.Contains(koutakuReq.Model, "gpt-5-structured")

	// openai models support messages
	if len(koutakuReq.Input) > 0 && strings.HasPrefix(koutakuReq.Model, string(schemas.OpenAI)) {
		if isGPT5Structured {
			...
			input.Messages = koutakuReq.Input
		}
	} else {
	...
	if koutakuReq.Params != nil {
		...
		if params.MaxCompletionTokens != nil {
			...
			} else if strings.HasPrefix(koutakuReq.Model, string(schemas.OpenAI)) {
...
// ToKoutakuChatResponse converts a Replicate prediction response to Koutaku format
func (response *ReplicatePredictionResponse) ToKoutakuChatResponse() *schemas.KoutakuChatResponse {
	if response == nil {
		return nil
	}
