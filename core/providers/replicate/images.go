...
// ToReplicateImageGenerationInput converts a Koutaku image generation request to Replicate prediction input
func ToReplicateImageGenerationInput(koutakuReq *schemas.KoutakuImageGenerationRequest) *ReplicatePredictionRequest {
	...
	if koutakuReq.Params != nil {
		...
		if koutakuReq.Params.N != nil {
			input.NumberOfImages = koutakuReq.Params.N
		}
...
// ToKoutakuImageGenerationResponse converts a Replicate prediction response to Koutaku format
func ToKoutakuImageGenerationResponse(
...
// ToReplicateImageEditInput converts a Koutaku image edit request to Replicate prediction input
func ToReplicateImageEditInput(koutakuReq *schemas.KoutakuImageEditRequest) *ReplicatePredictionRequest {
	if koutakuReq == nil || koutakuReq.Input == nil {
		return nil
	}
	...
	if len(koutakuReq.Input.Images) > 0 {
		images := make([]string, 0, len(koutakuReq.Input.Images))
		...
		if len(images) > 0 {
			...
			fieldName := getInputImageFieldName(koutakuReq.Model)
