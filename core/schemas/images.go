...
// KoutakuImageGenerationRequest represents an image generation request in koutaku format
type KoutakuImageGenerationRequest struct {
...
func (b *KoutakuImageGenerationRequest) GetRawRequestBody() []byte {
...
// KoutakuImageGenerationResponse represents the image generation response in koutaku format
type KoutakuImageGenerationResponse struct {
	...
	*ImageGenerationResponseParameters
	...
	ExtraFields KoutakuResponseExtraFields `json:"extra_fields,omitempty"`
}
...
func (r *KoutakuImageGenerationResponse) BackfillParams(req *KoutakuRequest) {
	if r == nil || req == nil {
		return
	}
...
func getModelFromRequest(req *KoutakuRequest) string {
	if req == nil {
		return ""
	}
	switch {
...
func getNumInputImagesSizeAndQualityFromRequest(req *KoutakuRequest) (numInputImages int, size string, quality string) {
	if req == nil {
		return 0, "", ""
	}
...
type ImageUsage struct {
	...
	NumInputImages      int                `json:"num_input_images,omitempty"` // Number of input images from the request (populated by Koutaku)
}

type ImageTokenDetails struct {
	NImages     int `json:"-"` // Number of images generated (used internally for koutaku)
	ImageTokens int `json:"image_tokens,omitempty"`
	TextTokens  int `json:"text_tokens,omitempty"`
}

// Streaming Response
type KoutakuImageGenerationStreamResponse struct {
	ID                string                     `json:"id,omitempty"`
	...
	RevisedPrompt     string                     `json:"revised_prompt,omitempty"`
	Usage             *ImageUsage                `json:"usage,omitempty"`
	Error             *KoutakuError              `json:"error,omitempty"`
...
// - Size on ImageGenerationResponseParameters (from request params if not in response)
// - Quality (low, medium, high, auto) only
func (r *KoutakuImageGenerationStreamResponse) BackfillParams(req *KoutakuRequest) {
	numInputImages, size, quality := getNumInputImagesSizeAndQualityFromRequest(req)
...
// KoutakuImageEditRequest represents an image edit request in koutaku format
type KoutakuImageEditRequest struct {
	Provider       ModelProvider        `json:"provider"`
	Model          string               `json:"model"`
...
// GetRawRequestBody implements [utils.RequestBodyGetter].
func (b *KoutakuImageEditRequest) GetRawRequestBody() []byte {
	return b.RawRequestBody
}
...
// KoutakuImageVariationRequest represents an image variation request in koutaku format
type KoutakuImageVariationRequest struct {
	Provider       ModelProvider             `json:"provider"`
	Model          string                    `json:"model"`
...
// GetRawRequestBody implements [utils.RequestBodyGetter].
func (b *KoutakuImageVariationRequest) GetRawRequestBody() []byte {
	return b.RawRequestBody
}
...
// KoutakuImageVariationResponse represents the image variation response in koutaku format
// It uses the same structure as image generation response
type KoutakuImageVariationResponse = KoutakuImageGenerationResponse
