...
// Plugin defines the interface for Koutaku plugins.
// Plugins can intercept and modify requests and responses at different stages
// of the processing pipeline.
// User can provide multiple plugins in the KoutakuConfig.
...
// - No Plugin errors are returned to the caller; they are logged as warnings by the Koutaku instance.
...
//
// IMPORTANT: When returning KoutakuError from PreLLMHook or PostLLMHook:
// - You can set the AllowFallbacks field to control fallback behavior
// - AllowFallbacks = &true: Allow Koutaku to try fallback providers
...
type BasePlugin interface {
	...
	// Cleanup is called on koutaku shutdown.
	// It allows plugins to clean up any resources they have allocated.
	// Returns any error that occurred during cleanup, which will be logged as a warning by the Koutaku instance.
	Cleanup() error
}
...
type HTTPTransportPlugin interface {
	...
	// HTTPTransportPreHook is called at the HTTP transport layer before requests enter Koutaku core.
	// It receives a serializable HTTPRequest and allows plugins to modify it in-place.
	// Only invoked when using HTTP transport (koutaku-http), not when using Koutaku as a Go SDK directly.
	...
	HTTPTransportPreHook(ctx *KoutakuContext, req *HTTPRequest) (*HTTPResponse, error)

	// HTTPTransportPostHook is called at the HTTP transport layer after requests exit Koutaku core.
	...
	HTTPTransportPostHook(ctx *KoutakuContext, req *HTTPRequest, resp *HTTPResponse) error

	// HTTPTransportStreamChunkHook is called for each chunk during streaming responses.
	// It receives the KoutakuStreamChunk BEFORE they are written to the client.
	// Only invoked for streaming responses when using HTTP transport (koutaku-http).
	// Works with both native .so plugins and WASM plugins due to serializable types.
	//
	// Plugins can modify the chunk by returning a different KoutakuStreamChunk.
	// Return the original chunk unchanged if no modification is needed.
	//
	// Return values:
	// - (*KoutakuStreamChunk, nil): Continue with the (potentially modified) KoutakuStreamChunk
	// - (nil, nil): Skip this KoutakuStreamChunk entirely (don't send to client)
	// - (*KoutakuStreamChunk, error): Log warning and continue with the KoutakuStreamChunk
	// - (nil, error): Send back error to the client and stop the streaming
	//
	// Return (*KoutakuStreamChunk, nil) unchanged if the plugin doesn't need streaming chunk interception.
	HTTPTransportStreamChunkHook(ctx *KoutakuContext, req *HTTPRequest, chunk *KoutakuStreamChunk) (*KoutakuStreamChunk, error)
}

type LLMPlugin interface {
	BasePlugin

	PreLLMHook(ctx *KoutakuContext, req *KoutakuRequest) (*KoutakuRequest, *LLMPluginShortCircuit, error)
	PostLLMHook(ctx *KoutakuContext, resp *KoutakuResponse, koutakuErr *KoutakuError) (*KoutakuResponse, *KoutakuError, error)
}

type MCPPlugin interface {
	BasePlugin

	PreMCPHook(ctx *KoutakuContext, req *KoutakuMCPRequest) (*KoutakuMCPRequest, *MCPPluginShortCircuit, error)
	PostMCPHook(ctx *KoutakuContext, resp *KoutakuMCPResponse, koutakuErr *KoutakuError) (*KoutakuMCPResponse, *KoutakuError, error)
}
...
//
// Example backends: OpenTelemetry collectors, Datadog, Jaeger, Koutaku, etc.
//
