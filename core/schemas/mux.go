...
func (cu *KoutakuLLMUsage) ToResponsesResponseUsage() *ResponsesResponseUsage {
	if cu == nil {
		return nil
	}
...
func (ru *ResponsesResponseUsage) ToKoutakuLLMUsage() *KoutakuLLMUsage {
	if ru == nil {
		return nil
	}

	usage := &KoutakuLLMUsage{
...
// ToResponsesRequest converts a KoutakuChatRequest to KoutakuResponsesRequest format
func (cr *KoutakuChatRequest) ToResponsesRequest() *KoutakuResponsesRequest {
	if cr == nil {
		return &KoutakuResponsesRequest{}
	}

	brr := &KoutakuResponsesRequest{
...
// ToChatRequest converts a KoutakuResponsesRequest to KoutakuChatRequest format
func (brr *KoutakuResponsesRequest) ToChatRequest() *KoutakuChatRequest {
	if brr == nil {
		return &KoutakuChatRequest{}
	}

	bcr := &KoutakuChatRequest{
...
func responsesStatusFromChatFinishReason(finishReason string) (status string, incompleteDetails *ResponsesResponseIncompleteDetails, mapped bool) {
	switch finishReason {
	case string(KoutakuFinishReasonLength):
		return "incomplete", &ResponsesResponseIncompleteDetails{Reason: "max_output_tokens"}, true
	case string(KoutakuFinishReasonStop), string(KoutakuFinishReasonToolCalls):
...
// ToKoutakuResponsesResponse converts the KoutakuChatResponse to KoutakuResponsesResponse format
// This converts Chat-style fields (Choices) to Responses API format
func (cr *KoutakuChatResponse) ToKoutakuResponsesResponse() *KoutakuResponsesResponse {
	if cr == nil {
		return nil
	}

	// Create new KoutakuResponsesResponse from Chat fields
	responsesResp := &KoutakuResponsesResponse{
		ID:            Ptr(cr.ID),
		Object:        "response",
...
// ToKoutakuChatResponse converts a KoutakuResponsesResponse to KoutakuChatResponse format
// This converts Responses API format to Chat-style fields (Choices)
func (responsesResp *KoutakuResponsesResponse) ToKoutakuChatResponse() *KoutakuChatResponse {
	if responsesResp == nil {
		return nil
	}

	// Create new KoutakuChatResponse from Responses fields
	chatResp := &KoutakuChatResponse{
	...
	if len(responsesResp.Output) > 0 {
		...
		choices := make([]KoutakuResponseChoice, 0, len(chatMessages))
		for i, chatMsg := range chatMessages {
			choice := KoutakuResponseChoice{
				Index: i,
	...
	if responsesResp.Usage != nil {
		...
		chatResp.Usage = responsesResp.Usage.ToKoutakuLLMUsage()
	}
...
// ToKoutakuResponsesStreamResponse converts the KoutakuChatResponse from Chat streaming format to Responses streaming format
// This converts Chat stream chunks (Choices with Deltas) to KoutakuResponsesStreamResponse format
// Returns a slice of responses to support cases where a single event produces multiple responses
func (cr *KoutakuChatResponse) ToKoutakuResponsesStreamResponse(state *ChatToResponsesStreamState) []*KoutakuResponsesStreamResponse {
	if cr == nil || state == nil {
		return nil
	}
	...
	// Convert first streaming choice to KoutakuResponsesStreamResponse
	...
	var responses []*KoutakuResponsesStreamResponse
	...
	if delta.Role != nil && !state.HasEmittedCreated {
		...
		response := &KoutakuResponsesResponse{
			ID:        state.MessageID,
			CreatedAt: state.CreatedAt,
		}
		responses = append(responses, &KoutakuResponsesStreamResponse{
	...
	if hasContent || (hasReasoning && !state.TextItemAdded) {
		...
		if hasContent || (!state.TextItemHasContent && (hasReasoning || hasContent)) {
			...
			response := &KoutakuResponsesStreamResponse{
...
// ToKoutakuChatResponse converts a KoutakuResponsesStreamResponse chunk to a KoutakuChatResponse (chat.completion.chunk).
func (rsr *KoutakuResponsesStreamResponse) ToKoutakuChatResponse() *KoutakuChatResponse {
	if rsr == nil {
		return nil
	}
	...
	case ResponsesStreamResponseTypeOutputTextDelta:
		resp.Choices = []KoutakuResponseChoice{
			{
				Index: 0,
	...
	case ResponsesStreamResponseTypeCompleted, ResponsesStreamResponseTypeIncomplete:
		finishReason := string(KoutakuFinishReasonStop)
		if rsr.Type == ResponsesStreamResponseTypeIncomplete {
			finishReason = string(KoutakuFinishReasonLength)
		}
		resp.Choices = []KoutakuResponseChoice{
			{
				Index:        0,
		...
		if rsr.Response != nil {
			if rsr.Response.Usage != nil {
				resp.Usage = rsr.Response.Usage.ToKoutakuLLMUsage()
			}
			...
			if rsr.Type == ResponsesStreamResponseTypeCompleted {
				for _, output := range rsr.Response.Output {
					if output.Type != nil && *output.Type == ResponsesMessageTypeFunctionCall {
						finishReason = string(KoutakuFinishReasonToolCalls)
...
// ToKoutakuTextCompletionResponse converts a KoutakuChatResponse to a KoutakuTextCompletionResponse
func (cr *KoutakuChatResponse) ToKoutakuTextCompletionResponse() *KoutakuTextCompletionResponse {
	if cr == nil {
		return nil
	}

	if len(cr.Choices) == 0 {
		return &KoutakuTextCompletionResponse{
			ID:                cr.ID,
			Model:             cr.Model,
			Object:            "text_completion",
			SystemFingerprint: cr.SystemFingerprint,
			Usage:             cr.Usage,
			ExtraFields: KoutakuResponseExtraFields{
