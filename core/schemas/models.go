...
type ListModelsByKeyResult struct {
	Response *KoutakuListModelsResponse
	Err      *KoutakuError
	KeyID    string
}
...
// circular reference: KeyStatus.Error → KoutakuError.ExtraFields.KeyStatuses → KeyStatus.
...
type KoutakuListModelsRequest struct {
	...
	// Unfiltered: If true, the response will include all models for the provider, regardless of the allowed models (internal koutaku use only, not sent to the provider)
...
type KoutakuListModelsResponse struct {
...
// ApplyPagination applies offset-based pagination to a KoutakuListModelsResponse.
...
func (response *KoutakuListModelsResponse) ApplyPagination(pageSize int, pageToken string) *KoutakuListModelsResponse {
	if response == nil {
		return nil
	}
	...
	if offset >= totalItems {
		...
		return &KoutakuListModelsResponse{
			Data:          []Model{},
	...
	paginatedResponse := &KoutakuListModelsResponse{
		Data:        paginatedData,
		ExtraFields: response.ExtraFields,
		KeyStatuses: response.KeyStatuses,
	}
...
type Model struct {
	...
	// through the Koutaku pipeline for integration reverse-conversion. Never serialized.
