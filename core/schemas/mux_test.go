...
func TestToKoutakuResponsesStreamResponse_PopulatesFinalDoneTextAndCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	makeChunk := func(role *string, content *string, finishReason *string) *KoutakuChatResponse {
		return &KoutakuChatResponse{
			ID:    "chatcmpl-test",
			Model: "test-model",
	...
	part1 := "Hello"
	part2 := " world"
	stop := string(KoutakuFinishReasonStop)

	var all []*KoutakuResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToKoutakuResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part1, nil).ToKoutakuResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part2, nil).ToKoutakuResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &stop).ToKoutakuResponsesStreamResponse(state)...)

	var outputTextDone *KoutakuResponsesStreamResponse
	var completed *KoutakuResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
...
func TestToKoutakuResponsesResponse_MapsLengthToIncomplete(t *testing.T) {
	length := string(KoutakuFinishReasonLength)
	resp := (&KoutakuChatResponse{
		Choices: []KoutakuResponseChoice{
			{FinishReason: &length},
		},
	}).ToKoutakuResponsesResponse()
...
func TestToKoutakuResponsesResponse_MapsToolCallsToCompleted(t *testing.T) {
	toolCalls := string(KoutakuFinishReasonToolCalls)
	resp := (&KoutakuChatResponse{
		Choices: []KoutakuResponseChoice{
...
func TestToKoutakuResponsesResponse_PrioritizesLengthAcrossChoices(t *testing.T) {
	stop := string(KoutakuFinishReasonStop)
	length := string(KoutakuFinishReasonLength)
	resp := (&KoutakuChatResponse{
		Choices: []KoutakuResponseChoice{
...
func TestToKoutakuResponsesResponse_UnknownFinishReasonLeavesStatusUnset(t *testing.T) {
...
func TestToKoutakuResponsesStreamResponse_IncludesFunctionCallsInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	role := string(ChatMessageRoleAssistant)
	part1 := "Let me help"
	toolCallsFinish := string(KoutakuFinishReasonToolCalls)
	...
	// Role chunk
	all = append(all, (&KoutakuChatResponse{
		ID:    "chatcmpl-test",
		Model: "test-model",
		Choices: []KoutakuResponseChoice{
			{
	...
	}).ToKoutakuResponsesStreamResponse(state)...)
...
func TestToKoutakuResponsesStreamResponse_ToolCallsOnlyInCompletedOutput(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
...
func TestToKoutakuResponsesStreamResponse_MapsLengthToIncompleteEvent(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
	...
	var all []*KoutakuResponsesStreamResponse
	all = append(all, makeChunk(&role, nil, nil).ToKoutakuResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, &part, nil).ToKoutakuResponsesStreamResponse(state)...)
	all = append(all, makeChunk(nil, nil, &length).ToKoutakuResponsesStreamResponse(state)...)

	var completed *KoutakuResponsesStreamResponse
	var incomplete *KoutakuResponsesStreamResponse
	for _, evt := range all {
		if evt == nil {
			continue
		}
