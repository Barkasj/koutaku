...
// RealtimeEventType represents the type of a Koutaku unified Realtime event.
type RealtimeEventType string

// Client-to-server event types (sent by the client through Koutaku)
const (
...
// user input item in the canonical Koutaku realtime schema.
func IsRealtimeUserInputEvent(event *KoutakuRealtimeEvent) bool {
	return event != nil &&
		event.Item != nil &&
...
// tool output item in the canonical Koutaku realtime schema.
func IsRealtimeToolOutputEvent(event *KoutakuRealtimeEvent) bool {
	return event != nil &&
		event.Item != nil &&
...
// input-audio transcript in the canonical Koutaku realtime schema.
func IsRealtimeInputTranscriptEvent(event *KoutakuRealtimeEvent) bool {
	return event != nil && event.Type == RTEventInputAudioTransCompleted
}

// KoutakuRealtimeEvent is the unified Koutaku envelope for all Realtime events.
// Provider converters translate between this format and the provider-native protocol.
type KoutakuRealtimeEvent struct {
	...
	// promoted into the common Koutaku schema.
...
type RealtimeProvider interface {
	...
	ExchangeRealtimeWebRTCSDP(ctx *KoutakuContext, key Key, model string, sdp string, session json.RawMessage) (string, *KoutakuError)
	ToKoutakuRealtimeEvent(providerEvent json.RawMessage) (*KoutakuRealtimeEvent, error)
	ToProviderRealtimeEvent(koutakuEvent *KoutakuRealtimeEvent) (json.RawMessage, error)
	...
	ShouldStartRealtimeTurn(event *KoutakuRealtimeEvent) bool
	...
	ShouldForwardRealtimeEvent(event *KoutakuRealtimeEvent) bool
...
type RealtimeLegacyWebRTCProvider interface {
	ExchangeLegacyRealtimeWebRTCSDP(ctx *KoutakuContext, key Key, sdp string, session json.RawMessage, model string) (string, *KoutakuError)
}
...
type RealtimeUsageExtractor interface {
	ExtractRealtimeTurnUsage(terminalEventRaw []byte) *KoutakuLLMUsage
...
type RealtimeSessionProvider interface {
	CreateRealtimeClientSecret(ctx *KoutakuContext, key Key, endpointType RealtimeSessionEndpointType, rawRequest json.RawMessage) (*KoutakuPassthroughResponse, *KoutakuError)
}
...
func ParseRealtimeEvent(raw []byte) (*KoutakuRealtimeEvent, error) {
