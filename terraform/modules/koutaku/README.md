# Koutaku Terraform Module

Single entry point for deploying [Koutaku](https://github.com/koutaku/koutaku) on AWS, GCP, Azure, or any Kubernetes cluster. This module handles configuration merging, image resolution, and routes to the appropriate cloud-provider sub-module based on your `cloud_provider` and `service` selections.

## Usage

```hcl
module "koutaku" {
  source = "github.com/koutaku/koutaku//terraform/modules/koutaku"
...
provider "kubernetes" {
  host                   = module.koutaku.cluster_endpoint
  cluster_ca_certificate = base64decode(module.koutaku.cluster_ca)
...
The module supports three ways to provide Koutaku configuration, which are merged in order of precedence:
...
All 18 top-level properties from the [Koutaku config schema](../../../transports/config.schema.json) are exposed as Terraform variables:
...
| Output             | Description                                     |
|--------------------|--------------------------------------------------|
| `service_url`      | URL to access the deployed Koutaku service       |
