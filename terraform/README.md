# Koutaku Terraform Modules

Deploy Koutaku on AWS, GCP, Azure, or any Kubernetes cluster using a single Terraform module.
...
```hcl
module "koutaku" {
  source         = "github.com/koutaku/koutaku//terraform/modules/koutaku?ref=terraform/v0.1.0"
  ...
  config_store = {
    enabled = true
    type    = "postgres"
    config  = { host = var.db_host, port = "5432", user = "koutaku", password = var.db_password, db_name = "koutaku" }
  }
}
```
...
Koutaku config can come from two sources simultaneously. Terraform variables override matching keys from the base file.
...
See the [module README](modules/koutaku/README.md#provider-configuration) for provider configuration examples per cloud.
...
Tests cover all 7 deployment targets across 10 test files. See the [module README](modules/koutaku/README.md#testing) for details.

## Directory Structure

```text
terraform/
  modules/koutaku/              # Top-level module (the only thing you call)
...
| `image_tag` | `"latest"` | Koutaku Docker image tag |
...
| Output | Description |
|--------|-------------|
| `service_url` | URL to access Koutaku |
