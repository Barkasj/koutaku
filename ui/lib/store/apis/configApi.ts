...
import { KoutakuConfig, GlobalProxyConfig, LatestReleaseResponse } from "@/lib/types/config";
...
export const configApi = baseApi.injectEndpoints({
	endpoints: (builder) => ({
		...
		getCoreConfig: builder.query<KoutakuConfig, { fromDB?: boolean }>({
			query: ({ fromDB = false } = {}) => ({
				url: "/config",
		...
		updateCoreConfig: builder.mutation<null, KoutakuConfig>({
			query: (data) => ({
				url: "/config",
				method: "PUT",
