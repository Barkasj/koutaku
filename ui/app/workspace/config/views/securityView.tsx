...
export default function SecurityView() {
	...
	useEffect(() => {
		...
		if (koutakuConfig?.auth_config) {
			setAuthConfig(koutakuConfig.auth_config);
		}
	...
	const hasChanges = useMemo(() => {
		...
		const usernameChanged =
			authConfig.admin_username?.value !== koutakuConfig?.auth_config?.admin_username?.value ||
			authConfig.admin_username?.env_var !== koutakuConfig?.auth_config?.admin_username?.env_var ||
			authConfig.admin_username?.from_env !== koutakuConfig?.auth_config?.admin_username?.from_env;
		const passwordChanged =
			authConfig.admin_password?.value !== koutakuConfig?.auth_config?.admin_password?.value ||
			authConfig.admin_password?.env_var !== koutakuConfig?.auth_config?.admin_password?.env_var ||
			authConfig.admin_password?.from_env !== koutakuConfig?.auth_config?.admin_password?.from_env;
		const authChanged =
			authConfig.is_enabled !== koutakuConfig?.auth_config?.is_enabled ||
			usernameChanged ||
			passwordChanged ||
			authConfig.disable_auth_on_inference !== koutakuConfig?.auth_config?.disable_auth_on_inference;
	...
	}, [config, localConfig, authConfig, koutakuConfig]);
	...
	}, [koutakuConfig, localConfig, authConfig, updateCoreConfig]);
							...
							<div className="flex items-center justify-between">
								<div className="space-y-0.5">
									...
									<p className="text-muted-foreground text-sm">
										Set up authentication credentials to protect your Koutaku dashboard. Once configured, use the generated token for all
										admin API calls.
									</p>
								</div>
				...
				<div className="flex items-center justify-between space-x-2 rounded-lg border p-4">
					<div className="space-y-0.5">
						...
						<p className="text-muted-foreground text-sm">
							...
							Koutaku will directly use the key.
						</p>
					</div>
					<Switch
...
const RestartWarning = () => {
	return (
		<Alert variant="destructive" className="mt-2">
			...
			<AlertDescription>Need to restart Koutaku to apply changes.</AlertDescription>
		</Alert>
	);
};