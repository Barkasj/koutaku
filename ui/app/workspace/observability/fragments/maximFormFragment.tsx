...
interface KoutakuFormFragmentProps {
	initialConfig?: {
		enabled?: boolean;
		api_key?: string;
		log_repo_id?: string;
...
export function KoutakuFormFragment({ initialConfig, onSave, onDelete, isDeleting = false, isLoading = false }: KoutakuFormFragmentProps) {
	const hasKoutakuAccess = useRbac(RbacResource.Observability, RbacOperation.Update);
									...
									<FormControl>
										<div className="relative">
											<Input
												...
												placeholder="Enter your Koutaku API key"
												disabled={!hasKoutakuAccess}
												{...field}
												className="pr-10"
											/>
											<Button
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<Input placeholder="Enter log repository ID" disabled={!hasKoutakuAccess} {...field} value={field.value ?? ""} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
				...
				<div className="flex w-full flex-row items-center">
							...
							<FormItem className="flex items-center gap-2 py-2">
								...
								<FormControl>
									<Switch
										...
										disabled={!hasKoutakuAccess}
										data-testid="koutaku-connector-enable-toggle"
									/>
								</FormControl>
							</FormItem>
						)}
					...
					<div className="ml-auto flex justify-end space-x-2 py-2">
						...
						<Button
							...
							disabled={!hasKoutakuAccess || isLoading || !form.formState.isDirty}
						>
							Reset
						</Button>
						<TooltipProvider>
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										type="submit"
										disabled={!hasKoutakuAccess || !form.formState.isDirty || !form.formState.isValid}
										isLoading={isSaving}
									>
										Save Koutaku Configuration
									</Button>
								</TooltipTrigger>
