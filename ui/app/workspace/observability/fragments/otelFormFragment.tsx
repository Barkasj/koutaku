...
export function OtelFormFragment({
	...
	const form = useForm<OtelFormSchema, any, OtelFormSchema>({
		...
		defaultValues: {
			...
			otel_config: {
				service_name: initialConfig?.service_name ?? "koutaku",
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem className="w-full">
									...
									<FormDescription>If kept empty, the service name will be set to "koutaku"</FormDescription>
									<FormControl>
										<Input placeholder="koutaku" disabled={!hasOtelAccess} {...field} />
									</FormControl>
									<FormMessage />
								</FormItem>
							)}
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem className="w-full">
											...
											<FormDescription>
												File path to the CA certificate on the Koutaku server. Leave empty to use system root CAs.
											</FormDescription>
											<FormControl>
