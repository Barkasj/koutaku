...
export default function APIKeysView() {
	const { data: koutakuConfig, isLoading } = useGetCoreConfigQuery({ fromDB: true });
	const isAuthConfigure = useMemo(() => {
		return koutakuConfig?.auth_config?.is_enabled;
	}, [koutakuConfig]);
...
--data '{ 
  ...
  "messages": [ 
			...
			<Alert variant="default">
				...
				<AlertDescription>
					<p className="text-md text-muted-foreground">
						...
						.<br />
						<br />
						Once generated you will need to use this API key for all API calls to the Koutaku admin APIs and UI.
					</p>
				</AlertDescription>
			</Alert>
		);
	...
	const isInferenceAuthDisabled = koutakuConfig?.auth_config?.disable_auth_on_inference ?? false;
				...
				<AlertDescription>
					<p className="text-md text-muted-foreground">
						{isInferenceAuthDisabled ? (
							...
							<>
								Use Basic auth with your admin credentials when making API calls to Koutaku. Encode your credentials in the standard{" "}
								<code className="bg-muted rounded px-1 py-0.5 text-sm">username:password</code> format with base64 encoding.
							</>
						)}
					</p>
