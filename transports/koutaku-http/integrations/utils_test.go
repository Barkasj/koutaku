...
func newTestKoutakuContext() *schemas.KoutakuContext {
...
// sets the HTTP status code from the provider's KoutakuError.StatusCode field.
// All three providers (OpenAI, Anthropic, Bedrock) return actual HTTP error codes
// for pre-stream errors, so Koutaku must propagate them faithfully.
func TestSendStreamError_PropagatesProviderStatusCode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			koutakuCtx := newTestKoutakuContext()
			...
			router.sendStreamError(ctx, koutakuCtx, config, koutakuErr)
...
// OpenAI error format. OpenAI's ErrorConverter returns *schemas.KoutakuError directly,
// which serializes to {"is_koutaku_error":false,"status_code":400,"error":{...}}.
func TestSendStreamError_OpenAIErrorFormat(t *testing.T) {
	...
	assert.Contains(t, result, "is_koutaku_error")
	assert.Contains(t, result, "status_code")
	assert.Contains(t, result, "error")
	assert.Equal(t, false, result["is_koutaku_error"])
...
// stored in the KoutakuContext are forwarded to the HTTP response. This ensures
...
func TestSendStreamError_ForwardsProviderHeaders(t *testing.T) {
	...
	koutakuCtx.SetValue(schemas.KoutakuContextKeyProviderResponseHeaders, map[string]string{
