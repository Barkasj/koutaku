...
func NewMCPHandler(mcpManager MCPManager, governanceManager GovernanceManager, client *koutaku.Koutaku, store *lib.Config, oauthHandler *OAuthHandler) *MCPHandler {
	return &MCPHandler{
		client:            client,
		store:             store,
		mcpManager:        mcpManager,
...
func (h *MCPHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *MCPHandler) getMCPClients(ctx *fasthttp.RequestCtx) {
	...
	// Get actual connected clients from Koutaku
	clientsInKoutaku, err := h.client.GetMCPClients()
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to get MCP clients from Koutaku: %v", err))
		return
	}
	// Create a map of connected clients for quick lookup
	connectedClientsMap := make(map[string]schemas.MCPClient)
	for _, client := range clientsInKoutaku {
...
func (h *MCPHandler) getMCPClientsPaginated(ctx *fasthttp.RequestCtx, limitStr, offsetStr, searchStr string) {
	...
	// Get connected clients from Koutaku engine for state/tools merge
	clientsInKoutaku, err := h.client.GetMCPClients()
	if err != nil {
		SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to get MCP clients from Koutaku: %v", err))
		return
...
func (h *MCPHandler) addMCPClient(ctx *fasthttp.RequestCtx) {
	...
	if req.AuthType == "oauth" {
		...
		// Build redirect URI - use Koutaku's own callback endpoint
	...
	// Convert to schemas.MCPClientConfig for runtime koutaku client (without tool_pricing)
	...
	if err := h.mcpManager.AddMCPClient(ctx, schemasConfig); err != nil {
		...
		if h.store.ConfigStore != nil {
			if err := h.store.ConfigStore.DeleteMCPClientConfig(ctx, schemasConfig.ID); err != nil {
				logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart koutaku to keep core and database in sync", err))
				SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart koutaku to keep core and database in sync", err))
				return
			}
		}
	...
	if err := h.mcpManager.UpdateMCPClient(ctx, id, schemasConfig); err != nil {
		...
		if h.store.ConfigStore != nil && oldDBConfig != nil {
			if rollbackErr := h.store.ConfigStore.UpdateMCPClientConfig(ctx, id, oldDBConfig); rollbackErr != nil {
				logger.Error(fmt.Sprintf("Failed to rollback MCP client DB update: %v. please restart koutaku to keep core and database in sync", rollbackErr))
			}
		}
...
func (h *MCPHandler) completeMCPClientOAuth(ctx *fasthttp.RequestCtx) {
		...
		if err := h.mcpManager.AddMCPClient(ctx, mcpClientConfig); err != nil {
			...
			if h.store.ConfigStore != nil {
				if delErr := h.store.ConfigStore.DeleteMCPClientConfig(ctx, mcpClientConfig.ID); delErr != nil {
					logger.Error(fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart koutaku to keep core and database in sync", delErr))
					SendError(ctx, fasthttp.StatusInternalServerError, fmt.Sprintf("Failed to delete MCP client config from database: %v. please restart koutaku to keep core and database in sync", delErr))
					return
				}
			}
	...
	// Add MCP client to Koutaku (this will save to database and connect)
	if err := h.mcpManager.AddMCPClient(ctx, mcpClientConfig); err != nil {
