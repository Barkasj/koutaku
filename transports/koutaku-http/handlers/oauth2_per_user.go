...
// This file implements Koutaku's OAuth 2.1 Authorization Server for per-user MCP
...
// these endpoints to authenticate users before accessing Koutaku's /mcp endpoint.
package handlers
...
// PerUserOAuthHandler implements Koutaku's OAuth 2.1 Authorization Server.
...
func (h *PerUserOAuthHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *PerUserOAuthHandler) handleAuthorize(ctx *fasthttp.RequestCtx) {
	...
	cookie.SetKey("__koutaku_flow_secret")
...
// validateFlowBrowserSecret checks that the request carries the __koutaku_flow_secret
...
func validateFlowBrowserSecret(ctx *fasthttp.RequestCtx, flow *tables.TablePerUserOAuthPendingFlow) bool {
	...
	secret := ctx.Request.Header.Cookie("__koutaku_flow_secret")
	if len(secret) == 0 {
		return false
	}
...
// to either their Koutaku session (runtime flow) or a PendingFlow (consent flow).
//
...
func (h *PerUserOAuthHandler) handleUpstreamAuthorize(ctx *fasthttp.RequestCtx) {
	...
	if flowID != "" {
		...
		// Runtime flow: validate the existing Koutaku session.
		koutakuSession, err := h.store.ConfigStore.GetPerUserOAuthSessionByID(ctx, sessionID)
		if err != nil || koutakuSession == nil {
			SendError(ctx, fasthttp.StatusUnauthorized, "Invalid or expired session")
			return
		}
		if !koutakuSession.ExpiresAt.After(time.Now()) {
			SendError(ctx, fasthttp.StatusUnauthorized, "Invalid or expired session")
			return
		}
		virtualKeyID = strVal(koutakuSession.VirtualKeyID)
		userID = strVal(koutakuSession.UserID)
	...
	// Build redirect URI (Koutaku's callback endpoint).
	scheme := "http"
