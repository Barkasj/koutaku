...
func (h *GovernanceHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *GovernanceHandler) createRoutingRule(ctx *fasthttp.RequestCtx) {
	...
	if err := h.governanceManager.ReloadRoutingRule(ctx, rule.ID); err != nil {
		SendError(ctx, 500, fmt.Sprintf("Failed to reload routing rule in memory: %v, please restart koutaku to sync with the database", err))
		return
	}
