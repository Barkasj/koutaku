...
func (m *mockRealtimeMintingGovernancePlugin) EvaluateGovernanceRequest(ctx *schemas.KoutakuContext, evaluationRequest *governance.EvaluationRequest, _ schemas.RequestType) (*governance.EvaluationResult, *schemas.KoutakuError) {
	m.evaluateCalls++
	m.seenUserID = ""
	m.seenVirtualKey = ""
	m.seenProvider = ""
	...
	if ctx != nil && m.seenVirtualKey == "" {
		m.seenVirtualKey = koutaku.GetStringFromContext(ctx, schemas.KoutakuContextKeyVirtualKey)
	}
	if m.err != nil {
		return nil, m.err
	}
...
func (m *mockRealtimeMintingGovernancePlugin) HTTPTransportPreHook(_ *schemas.KoutakuContext, _ *schemas.HTTPRequest) (*schemas.HTTPResponse, error) {
	return nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) HTTPTransportPostHook(_ *schemas.KoutakuContext, _ *schemas.HTTPRequest, _ *schemas.HTTPResponse) error {
	return nil
}

func (m *mockRealtimeMintingGovernancePlugin) PreLLMHook(_ *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PostLLMHook(_ *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
	return result, koutakuErr, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PreMCPHook(_ *schemas.KoutakuContext, req *schemas.KoutakuMCPRequest) (*schemas.KoutakuMCPRequest, *schemas.MCPPluginShortCircuit, error) {
	return req, nil, nil
}

func (m *mockRealtimeMintingGovernancePlugin) PostMCPHook(_ *schemas.KoutakuContext, resp *schemas.KoutakuMCPResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuMCPResponse, *schemas.KoutakuError, error) {
...
func TestRealtimeClientSecretsEvaluateMintingGovernance_RequiresAccess(t *testing.T) {
	...
	defer koutakuCtx.Done()

	err := handler.evaluateMintingGovernance(koutakuCtx, schemas.OpenAI, "gpt-realtime")
	if err == nil {
...
func TestRealtimeClientSecretsEvaluateMintingGovernance_PassesContext(t *testing.T) {
	...
	koutakuCtx := schemas.NewKoutakuContext(context.Background(), schemas.NoDeadline)
	defer koutakuCtx.Done()
	koutakuCtx.SetValue(schemas.KoutakuContextKeyUserID, "user_123")
	koutakuCtx.SetValue(schemas.KoutakuContextKeyVirtualKey, "sk-bf-123")

	if err := handler.evaluateMintingGovernance(koutakuCtx, schemas.OpenAI, "gpt-realtime"); err != nil {
