...
import (
	...
	"github.com/koutaku/koutaku/framework/encrypt"
...
func SecurityHeadersMiddleware() schemas.KoutakuHTTPMiddleware {
...
func CorsMiddleware(config *lib.Config) schemas.KoutakuHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			startTime := time.Now()
			...
			defer func() {
				...
				if traceID, ok := ctx.UserValue(schemas.KoutakuContextKeyTraceID).(string); ok && traceID != "" {
...
func RequestDecompressionMiddleware(config *lib.Config) schemas.KoutakuHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
...
func TransportInterceptorMiddleware(config *lib.Config) schemas.KoutakuHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			// Get or create KoutakuContext from fasthttp context
			koutakuCtx := getKoutakuContextFromFastHTTP(ctx)
			...
			for _, plugin := range plugins {
				...
				pluginCtx := koutakuCtx.WithPluginScope(&pluginName)
				...
				if err != nil {
					...
					if logs := koutakuCtx.DrainPluginLogs(); len(logs) > 0 {
						ctx.SetUserValue(schemas.KoutakuContextKeyTransportPluginLogs, logs)
					}
			...
			if preHookLogs := koutakuCtx.DrainPluginLogs(); len(preHookLogs) > 0 {
				ctx.SetUserValue(schemas.KoutakuContextKeyTransportPluginLogs, preHookLogs)
			}
			// Apply modifications back to fasthttp context
			applyHTTPRequestToCtx(ctx, req)
			// Adding user values
			for key, value := range koutakuCtx.GetUserValues() {
			...
			if deferred, ok := ctx.UserValue(schemas.KoutakuContextKeyDeferTraceCompletion).(bool); ok && deferred {
				...
				slot, ok := ctx.UserValue(schemas.KoutakuContextKeyTransportPostHookCompleter).(*atomic.Value)
				if !ok {
					return
				}
				...
				if logs, ok := ctx.UserValue(schemas.KoutakuContextKeyTransportPluginLogs).([]schemas.PluginLogEntry); ok {
					preHookLogs = logs
				}
				...
				completer := func() ([]schemas.PluginLogEntry, error) {
					...
					postHookLogs, err := runTransportPostHooksCaptured(capturedReq, capturedResp, plugins, koutakuCtx)
					allLogs := preHookLogs
			...
			_ = runTransportPostHooks(ctx, plugins, koutakuCtx, true)
		}
	}
}
...
// KoutakuContextKeyTransportPluginLogs) rather than directly on KoutakuContext,
// because transport hooks operate at the fasthttp layer before/after the core
// KoutakuContext lifecycle. These logs are merged into the trace by the
// TracingMiddleware at trace completion, alongside core-level plugin logs
// which travel through KoutakuContext → Trace → AttachPluginLogs.
func runTransportPostHooks(ctx *fasthttp.RequestCtx, plugins []schemas.HTTPTransportPlugin, koutakuCtx *schemas.KoutakuContext, applyResponse bool) error {
	...
	for i := len(plugins) - 1; i >= 0; i-- {
		...
		if err != nil {
			...
			if postHookLogs := koutakuCtx.DrainPluginLogs(); len(postHookLogs) > 0 {
				if existing, ok := ctx.UserValue(schemas.KoutakuContextKeyTransportPluginLogs).([]schemas.PluginLogEntry); ok {
					ctx.SetUserValue(schemas.KoutakuContextKeyTransportPluginLogs, append(existing, postHookLogs...))
				} else {
					ctx.SetUserValue(schemas.KoutakuContextKeyTransportPluginLogs, postHookLogs)
				}
			}
...
func runTransportPostHooksCaptured(capturedReq *schemas.HTTPRequest, capturedResp *schemas.HTTPResponse, plugins []schemas.HTTPTransportPlugin, koutakuCtx *schemas.KoutakuContext) ([]schemas.PluginLogEntry, error) {
...
// getKoutakuContextFromFastHTTP gets or creates a KoutakuContext from fasthttp context.
func getKoutakuContextFromFastHTTP(ctx *fasthttp.RequestCtx) *schemas.KoutakuContext {
	return schemas.NewKoutakuContext(ctx, schemas.NoDeadline)
}
...
func fasthttpToHTTPRequest(ctx *fasthttp.RequestCtx, req *schemas.HTTPRequest) {
	...
	ctx.VisitUserValuesAll(func(key, value any) {
		...
		// Skip internal Koutaku system keys and tracing keys
	...
	if threshold, ok := ctx.UserValue(schemas.KoutakuContextKeyLargePayloadRequestThreshold).(int64); ok && threshold > 0 {
	...
	if isLargePayload, ok := ctx.UserValue(schemas.KoutakuContextKeyLargePayloadMode).(bool); ok && isLargePayload {
		return
	}
...
func fasthttpResponseToHTTPResponse(ctx *fasthttp.RequestCtx, resp *schemas.HTTPResponse) {
	...
	if threshold, ok := ctx.UserValue(schemas.KoutakuContextKeyLargeResponseThreshold).(int64); ok && threshold > 0 {
...
func (m *AuthMiddleware) InferenceMiddleware() schemas.KoutakuHTTPMiddleware {
...
func (m *AuthMiddleware) APIMiddleware() schemas.KoutakuHTTPMiddleware {
...
func (m *AuthMiddleware) middleware(shouldSkip func(*configstore.AuthConfig, string) bool) schemas.KoutakuHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			if authConfig == nil || !authConfig.IsEnabled {
				...
				ctx.SetUserValue(schemas.KoutakuContextKeySessionToken, "")
				next(ctx)
				return
			}
					...
					if isInferenceWSEndpoint(path) {
						...
						if ticket != "" && m.wsTicketStore != nil {
							...
							if sessionToken != "" && validateSession(ctx, m.store, sessionToken) {
								ctx.SetUserValue(schemas.KoutakuContextKeySessionToken, sessionToken)
								next(ctx)
								return
							}
						...
						if token != "" {
							if validateSession(ctx, m.store, token) {
								ctx.SetUserValue(schemas.KoutakuContextKeySessionToken, token)
								next(ctx)
								return
							}
							SendError(ctx, fasthttp.StatusUnauthorized, "Unauthorized")
						...
						if cookieToken != "" && validateSession(ctx, m.store, cookieToken) {
							ctx.SetUserValue(schemas.KoutakuContextKeySessionToken, cookieToken)
							next(ctx)
							return
						}
						SendError(ctx, fasthttp.StatusUnauthorized, "Unauthorized")
...
func (m *TracingMiddleware) Middleware() schemas.KoutakuHTTPMiddleware {
	return func(next fasthttp.RequestHandler) fasthttp.RequestHandler {
		return func(ctx *fasthttp.RequestCtx) {
			...
			ctx.SetUserValue(schemas.KoutakuContextKeyTraceID, traceID)
			...
			if parentSpanID != "" {
				ctx.SetUserValue(schemas.KoutakuContextKeyParentSpanID, parentSpanID)
			}
			...
			ctx.SetUserValue(schemas.KoutakuContextKeyTraceCompleter, func(transportLogs []schemas.PluginLogEntry) {
			...
			if rootSpan != nil {
				...
				if spanID, ok := spanCtx.Value(schemas.KoutakuContextKeySpanID).(string); ok {
					ctx.SetUserValue(schemas.KoutakuContextKeySpanID, spanID)
				}
			}
			defer func() {
				...
				if transportLogs, ok := ctx.UserValue(schemas.KoutakuContextKeyTransportPluginLogs).([]schemas.PluginLogEntry); ok && len(transportLogs) > 0 {
					tracer.AttachPluginLogs(traceID, transportLogs)
				}
