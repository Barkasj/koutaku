...
func NewWSRealtimeHandler(client *koutaku.Koutaku, config *lib.Config, pool *bfws.Pool) *WSRealtimeHandler {
...
func (h *WSRealtimeHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *WSRealtimeHandler) handleUpgrade(ctx *fasthttp.RequestCtx) {
	...
	if err != nil {
		...
		upgradeErr := upgrader.Upgrade(ctx, func(conn *ws.Conn) {
			...
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", err.Error()))
		})
		if upgradeErr != nil {
	...
	if provider == nil || !ok || !rtProvider.SupportsRealtimeAPI() {
		...
		upgradeErr := upgrader.Upgrade(ctx, func(conn *ws.Conn) {
			defer conn.Close()
			clientConn := newRealtimeClientConn(conn)
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", "provider does not support realtime: "+string(providerKey)))
		})
		if upgradeErr != nil {
			logger.Warn("websocket upgrade failed for %s: %v", path, upgradeErr)
		}
	...
	err = upgrader.Upgrade(ctx, func(conn *ws.Conn) {
		...
		if sessionErr != nil {
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(429, "rate_limit_exceeded", sessionErr.Error()))
			return
		}
...
func (h *WSRealtimeHandler) runRealtimeSession(
	...
	if koutakuCtx == nil {
		clientConn.writeRealtimeError(newRealtimeWireKoutakuError(500, "server_error", "failed to create request context"))
		return
	}
	defer cancel()
	...
	provider := h.client.GetProviderByKey(providerKey)
	if provider == nil {
		clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", "provider not found: "+string(providerKey)))
		return
	}
	...
	key, err := h.client.SelectKeyForProviderRequestType(koutakuCtx, schemas.RealtimeRequest, providerKey, model)
	if err != nil {
		clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", err.Error()))
		return
	}
	...
	if err != nil {
		clientConn.writeRealtimeError(newRealtimeWireKoutakuError(502, "server_error", err.Error()))
		return
	}
	...
	go func() {
		errCh <- h.relayClientToRealtimeProvider(clientConn, session, upstream, rtProvider, koutakuCtx, providerKey, model, key)
	}()
	go func() {
		errCh <- h.relayRealtimeProviderToClient(clientConn, session, upstream, rtProvider, koutakuCtx, providerKey, model, key)
	}()
...
func (h *WSRealtimeHandler) relayClientToRealtimeProvider(
	...
	for {
		...
		if messageType != ws.TextMessage {
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", "realtime websocket only accepts text messages"))
			return nil
		}

		event, err := schemas.ParseRealtimeEvent(message)
		if err != nil {
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", "failed to parse realtime event JSON"))
			continue
		}
		...
		if startsTurn {
			if session.PeekRealtimeTurnHooks() != nil {
				clientConn.writeRealtimeError(newRealtimeWireKoutakuError(400, "invalid_request_error", "Conversation already has an active response in progress."))
				continue
			}
			if toolSummary != "" {
			...
			if koutakuErr := startRealtimeTurnHooks(h.client, koutakuCtx, session, provider, providerKey, model, &key, event.Type); koutakuErr != nil {
				clientConn.writeRealtimeError(koutakuErr)
				return nil
			}
		}
		...
		if err != nil {
			if startsTurn {
				if finalizeErr := finalizeRealtimeTurnHooksWithError(
					...
					nil,
					newRealtimeWireKoutakuError(400, "invalid_request_error", err.Error()),
				); finalizeErr != nil {
		...
		if err := upstream.WriteMessage(ws.TextMessage, providerEvent); err != nil {
			finalizeRealtimeTurnHooksWithError(
				...
				schemas.RTEventError,
				nil,
				newRealtimeWireKoutakuError(502, "server_error", "failed to write realtime event upstream"),
			)
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(502, "server_error", "failed to write realtime event upstream"))
			return err
		}
	}
}
	...
	for {
		...
		if err != nil {
			...
			finalizeRealtimeTurnHooksWithError(
				...
				schemas.RTEventError,
				nil,
				newRealtimeWireKoutakuError(502, "server_error", "upstream realtime websocket stream interrupted"),
			)
			clientConn.writeRealtimeError(newRealtimeWireKoutakuError(502, "server_error", "upstream realtime websocket stream interrupted"))
			return err
		}

		if messageType == ws.TextMessage {
			event, err := provider.ToKoutakuRealtimeEvent(message)
			if err != nil {
				finalizeRealtimeTurnHooksWithError(
					h.client,
					koutakuCtx,
					...
					schemas.RTEventError,
					message,
					newRealtimeWireKoutakuError(502, "server_error", "failed to translate upstream realtime event"),
				)
				clientConn.writeRealtimeError(newRealtimeWireKoutakuError(502, "server_error", "failed to translate upstream realtime event"))
				return err
			}
			if event != nil {
			...
			if event != nil {
				...
				if event.Type == provider.RealtimeTurnFinalEvent() {
					...
					if koutakuErr := finalizeRealtimeTurnHooks(h.client, koutakuCtx, session, provider, providerKey, model, &key, message, contentOverride); koutakuErr != nil {
						clientConn.writeRealtimeError(koutakuErr)
						return nil
					}
				} else if event.Error != nil {
					turnErr := newKoutakuErrorFromRealtimeError(providerKey, model, message, event.Error)
				...
				if len(event.RawData) == 0 {
					...
					if err != nil {
						clientConn.writeRealtimeError(newRealtimeWireKoutakuError(502, "server_error", "failed to encode translated realtime event"))
						return err
					}
				}
			}
...
func (c *realtimeClientConn) writeRealtimeError(koutakuErr *schemas.KoutakuError) {
	payload := newRealtimeTurnErrorEventPayload(koutakuErr)
...
func newRealtimeWireKoutakuError(status int, code, message string) *schemas.KoutakuError {
	errType := code
