...
func NewProviderHandler(modelsManager ModelsManager, inMemoryStore *lib.Config, client *koutaku.Koutaku) *ProviderHandler {
...
type ProviderResponse struct {
	...
	SendBackRawRequest       bool                             `json:"send_back_raw_request"`            // Include raw request in KoutakuResponse
	SendBackRawResponse      bool                             `json:"send_back_raw_response"`           // Include raw response in KoutakuResponse
...
func (h *ProviderHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
func (h *ProviderHandler) addProvider(ctx *fasthttp.RequestCtx) {
	...
	if payload.CustomProviderConfig != nil {
		...
		if koutaku.IsStandardProvider(payload.Provider) {
		...
		if !koutaku.IsSupportedBaseProvider(payload.CustomProviderConfig.BaseProviderType) {
