...
type MCPToolManager interface {
	...
	ExecuteChatMCPTool(ctx context.Context, toolCall *schemas.ChatAssistantMessageToolCall) (*schemas.ChatMessage, *schemas.KoutakuError)
	ExecuteResponsesMCPTool(ctx context.Context, toolCall *schemas.ResponsesToolMessage) (*schemas.ResponsesMessage, *schemas.KoutakuError)
}
...
func (h *MCPServerHandler) RegisterRoutes(r *router.Router, middlewares ...schemas.KoutakuHTTPMiddleware) {
...
// into the KoutakuContext. This avoids header-based identity propagation which
...
func injectMCPSessionIdentity(koutakuCtx *schemas.KoutakuContext, session *tables.TablePerUserOAuthSession) {
	koutakuCtx.SetValue(schemas.KoutakuContextKeyIsMCPGateway, true)
	if session != nil {
		if session.AccessToken != "" {
			koutakuCtx.SetValue(schemas.KoutakuContextKeyMCPUserSession, session.AccessToken)
		}
		if session.VirtualKeyID != nil && *session.VirtualKeyID != "" && session.VirtualKey != nil && session.VirtualKey.Value != "" {
			koutakuCtx.SetValue(schemas.KoutakuContextKeyVirtualKey, session.VirtualKey.Value)
		}
		if session.UserID != nil && *session.UserID != "" {
			koutakuCtx.SetValue(schemas.KoutakuContextKeyUserID, *session.UserID)
		}
	}
}
...
func (h *MCPServerHandler) syncServer(server *server.MCPServer, availableTools []schemas.ChatTool, toolFilter []string) {
	...
	for _, tool := range availableTools {
		...
		handler := func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			...
			// Convert to Koutaku tool call format
			...
			if err != nil {
				...
				return mcp.NewToolResultError(fmt.Sprintf("Tool execution failed: %v", koutaku.GetErrorMessage(err))), nil
			}
		...
		// Map Koutaku annotations back to MCP tool annotations
...
func (h *MCPServerHandler) getMCPServerForRequest(ctx *fasthttp.RequestCtx) (*server.MCPServer, *tables.TablePerUserOAuthSession, error) {
	...
	// Check for Koutaku per-user OAuth Bearer token (not a VK)
...
// getPerUserOAuthSession extracts and validates a Koutaku-issued per-user OAuth
...
func getVKFromRequest(ctx *fasthttp.RequestCtx) string {
	if value := strings.TrimSpace(string(ctx.Request.Header.Peek(string(schemas.KoutakuContextKeyVirtualKey)))); value != "" {
		return value
	}
