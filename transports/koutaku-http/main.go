...
//
//	To bind to all interfaces for container usage, set KOUTAKU_HOST=0.0.0.0 or use -host 0.0.0.0
//
// Integration Support:
// Koutaku supports multiple AI provider integrations through dedicated HTTP endpoints.
// Each integration exposes API-compatible endpoints that accept the provider's native request format,
// automatically convert it to Koutaku's unified format, process it, and return the expected response format.
//
...
// from Koutaku's unified model routing, fallbacks, monitoring capabilities, and high-performance configuration management.
//
...
import (
	...
	koutakuServer "github.com/koutaku/koutaku/transports/koutaku-http/server"
)
...
var logger = koutaku.NewDefaultLogger(schemas.LogLevelInfo)
var server *koutakuServer.KoutakuHTTPServer
...
//   - host: Host to bind the server to (default: localhost, can be overridden with KOUTAKU_HOST env var)
...
func init() {
	...
	defaultHost := os.Getenv("KOUTAKU_HOST")
	if defaultHost == "" {
		defaultHost = koutakuServer.DefaultHost
	}
	defaultLogLevel := strings.ToLower(os.Getenv("LOG_LEVEL"))
	if defaultLogLevel == "" {
		defaultLogLevel = koutakuServer.DefaultLogLevel
	}
	// Initializing server
	server = koutakuServer.NewKoutakuHTTPServer(Version, uiContent)
	// Updating server properties from flags
	flag.StringVar(&server.Port, "port", koutakuServer.DefaultPort, "Port to run the server on")
	flag.StringVar(&server.Host, "host", defaultHost, "Host to bind the server to (default: localhost, override with KOUTAKU_HOST env var)")
	flag.StringVar(&server.AppDir, "app-dir", koutakuServer.DefaultAppDir, "Application data directory (contains config.json and logs)")
	flag.StringVar(&server.LogLevel, "log-level", defaultLogLevel, "Logger level (debug, info, warn, error). Default is info.")
	flag.StringVar(&server.LogOutputStyle, "log-style", koutakuServer.DefaultLogOutputStyle, "Logger output type (json or pretty). Default is JSON.")
}
...
║═══════════════════════════════════════════════════════════║
║             https://github.com/koutaku/koutaku            ║
╚═══════════════════════════════════════════════════════════╝
...
`, versionLine)
	...
	koutakuServer.SetLogger(logger)
	...
	logger.Info("Time spent in Koutaku server bootstrap %d ms", time.Since(t).Milliseconds())
	err = server.Start()
	if err != nil {
