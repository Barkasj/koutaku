...
This file contains comprehensive tests for the Koutaku configuration system,
...
func createTempDir(t *testing.T) string {
	t.Helper()
	dir, err := os.MkdirTemp("", "koutaku-test-*")
	if err != nil {
...
func (p *mockLLMPlugin) PreLLMHook(ctx *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}

func (p *mockLLMPlugin) PostLLMHook(ctx *schemas.KoutakuContext, resp *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
...
func TestResolveFrameworkPricingConfig(t *testing.T) {
	...
	t.Run("pricing_url with missing env var falls back to literal string", func(t *testing.T) {
		...
		rawURL := "env.KOUTAKU_TEST_PRICING_URL_NONEXISTENT_XYZ"
		prev, existed := os.LookupEnv("KOUTAKU_TEST_PRICING_URL_NONEXISTENT_XYZ")
		os.Unsetenv("KOUTAKU_TEST_PRICING_URL_NONEXISTENT_XYZ")
		t.Cleanup(func() {
			if existed {
				os.Setenv("KOUTAKU_TEST_PRICING_URL_NONEXISTENT_XYZ", prev)
			}
		})
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
	...
	t.Run("pricing_url with valid env var is resolved", func(t *testing.T) {
		t.Setenv("KOUTAKU_TEST_PRICING_URL_VALID", "https://resolved.example.com/pricing.json")
		rawURL := "env.KOUTAKU_TEST_PRICING_URL_VALID"
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
				PricingURL: &rawURL,
			},
	...
	t.Run("partial/embedded env string is treated as literal (no substitution)", func(t *testing.T) {
		...
		t.Setenv("KOUTAKU_TEST_PRICING_HOST", "host.example.com")
		embeddedURL := "https://env.KOUTAKU_TEST_PRICING_HOST/pricing.json"
		fileConfig := &framework.FrameworkConfig{
			Pricing: &modelcatalog.Config{
	...
	t.Run("db corrupted (zero) with valid file interval uses file value and requests db backfill", func(t *testing.T) {
		// Real-world recovery scenario: a pre-fix Koutaku wrote 0 nanoseconds (interpreted
...
func TestRemoveProvider_SkipDBUpdate(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), schemas.KoutakuContextKeySkipDBUpdate, true)
