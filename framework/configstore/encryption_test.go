...
func init() {
	encrypt.Init(testEncryptionKey, koutaku.NewDefaultLogger(schemas.LogLevelInfo))
}
...
func setupEncryptionTestStore(t *testing.T) (*RDBConfigStore, *gorm.DB) {
	...
	store := &RDBConfigStore{logger: koutaku.NewDefaultLogger(schemas.LogLevelInfo)}
	store.db.Store(db)
...
func TestEncryptPlaintextRows_EncryptionDisabled_Noop(t *testing.T) {
	...
	encrypt.Init("", koutaku.NewDefaultLogger(schemas.LogLevelInfo))
	t.Cleanup(func() {
		encrypt.Init(testEncryptionKey, koutaku.NewDefaultLogger(schemas.LogLevelInfo))
	})
