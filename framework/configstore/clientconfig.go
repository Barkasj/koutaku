...
// ClientConfig represents the core configuration for Koutaku HTTP transport and the Koutaku Client.
...
type ClientConfig struct {
	...
	InitialPoolSize                 int                              `json:"initial_pool_size"`       // The initial pool size for the koutaku client
...
func (p *ProviderConfig) Redacted() *ProviderConfig {
	...
	for i, key := range p.Keys {
		...
		if key.UseForBatchAPI != nil {
			...
			redactedConfig.Keys[i].UseForBatchAPI = koutaku.Ptr(false)
		}
...
// AuthConfig represents configured auth config for Koutaku dashboard
