...
type TableOauthUserSession struct {
	...
	SessionToken     string    `gorm:"type:varchar(255)" json:"-"`                              // Koutaku session ID (links to oauth_per_user_sessions)
	SessionTokenHash string    `gorm:"type:varchar(64);uniqueIndex" json:"-"`                   // SHA-256 hash of SessionToken for secure lookups
	GatewaySessionID string    `gorm:"type:varchar(255);index" json:"-"`                        // Koutaku MCP gateway session ID (separate from SessionToken)
...
type TableOauthUserToken struct {
	...
	SessionToken     string     `gorm:"type:varchar(255)" json:"-"`                                                          // Maps to Koutaku session (fallback for anonymous users)
...
// MCP clients (like Claude Code) register themselves with Koutaku's OAuth
...
// TablePerUserOAuthSession stores Koutaku-issued access tokens for authenticated
// MCP connections. When a user authenticates via Koutaku's OAuth flow, a session
...
type TablePerUserOAuthSession struct {
	ID               string           `gorm:"type:varchar(255);primaryKey" json:"id"`
	AccessToken      string           `gorm:"type:text;not null" json:"-"`                       // Koutaku-issued access token (encrypted)
	AccessTokenHash  string           `gorm:"type:varchar(64);uniqueIndex" json:"-"`             // SHA-256 hash for secure lookups
	RefreshToken     string           `gorm:"type:text" json:"-"`                                // Koutaku-issued refresh token (encrypted, optional)
