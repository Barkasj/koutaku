...
func (a *Accumulator) buildCompleteMessageFromChatStreamChunks(chunks []*ChatStreamChunk) *schemas.ChatMessage {
	...
	type rdAccum struct {
		...
		typ                          schemas.KoutakuReasoningDetailsType
		id, signature                *string
	}
...
func (a *Accumulator) processAccumulatedChatStreamingChunks(requestID string, respErr *schemas.KoutakuError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processChatStreamingResponse(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*ProcessedStreamResponse, error) {
	...
	requestType, provider, model, resolvedModel := koutaku.GetResponseFields(result, koutakuErr)
	...
	if koutakuErr != nil {
		...
		if result.TextCompletionResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = koutaku.Ptr(fmt.Sprintf("%v", result.TextCompletionResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
		...
		if result.ChatResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = koutaku.Ptr(fmt.Sprintf("%v", result.ChatResponse.ExtraFields.RawResponse))
		}
		if isFinalChunk {
			if a.pricingManager != nil {
				cost := a.pricingManager.CalculateCost(result, modelcatalog.PricingLookupScopesFromContext(ctx, string(result.GetExtraFields().Provider)))
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedChatStreamingChunks(requestID, koutakuErr, isFinalChunk)
		if processErr != nil {
