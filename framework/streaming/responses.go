...
// deepCopyResponsesStreamResponse creates a deep copy of KoutakuResponsesStreamResponse
// to prevent shared data mutation between different plugin accumulators
func deepCopyResponsesStreamResponse(original *schemas.KoutakuResponsesStreamResponse) *schemas.KoutakuResponsesStreamResponse {
	if original == nil {
		return nil
	}

	copy := &schemas.KoutakuResponsesStreamResponse{
		Type:           original.Type,
	...
	if original.Response != nil {
		copy.Response = &schemas.KoutakuResponsesResponse{}
...
func (a *Accumulator) processAccumulatedResponsesStreamingChunks(requestID string, respErr *schemas.KoutakuError, isFinalChunk bool) (*AccumulatedData, error) {
...
func (a *Accumulator) processResponsesStreamingResponse(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*ProcessedStreamResponse, error) {
	...
	_, provider, requestedModel, resolvedModel := koutaku.GetResponseFields(result, koutakuErr)

	isFinalChunk := koutaku.IsFinalChunk(ctx)
	chunk := a.getResponsesStreamChunk()
	chunk.Timestamp = time.Now()
	chunk.ErrorDetails = koutakuErr

	if koutakuErr != nil {
		chunk.FinishReason = koutaku.Ptr("error")
		if koutakuErr.ExtraFields.RawResponse != nil {
			if rawBytes, marshalErr := sonic.Marshal(koutakuErr.ExtraFields.RawResponse); marshalErr == nil {
				chunk.RawResponse = koutaku.Ptr(string(rawBytes))
			}
		}
		...
		if result.ResponsesStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = koutaku.Ptr(fmt.Sprintf("%v", result.ResponsesStreamResponse.ExtraFields.RawResponse))
		}
		...
		if result.ResponsesStreamResponse.Response != nil &&
			...
			chunk.TokenUsage = result.ResponsesStreamResponse.Response.Usage.ToKoutakuLLMUsage()
		}
	...
	if isFinalChunk {
		...
		data, processErr := a.processAccumulatedResponsesStreamingChunks(requestID, koutakuErr, isFinalChunk)
		if processErr != nil {
