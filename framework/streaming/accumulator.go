...
// Returns the value of KoutakuContextKeyAccumulatorID.
func getAccumulatorID(ctx *schemas.KoutakuContext) (string, bool) {
	if id, ok := ctx.Value(schemas.KoutakuContextKeyAccumulatorID).(string); ok && id != "" {
		return id, true
	}
	return "", false
}
...
func (a *Accumulator) ProcessStreamingResponse(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*ProcessedStreamResponse, error) {
	...
	if result != nil {
		...
		requestType = koutakuErr.ExtraFields.RequestType
	}
	...
	if isChatStreaming {
		...
		return a.processChatStreamingResponse(ctx, result, koutakuErr)
		...
		if requestType == schemas.TranscriptionStreamRequest {
			return a.processTranscriptionStreamingResponse(ctx, result, koutakuErr)
		}
		if requestType == schemas.SpeechStreamRequest {
			return a.processAudioStreamingResponse(ctx, result, koutakuErr)
		}
	} else if isResponsesStreaming {
		// Handle responses streaming with responses accumulation
		return a.processResponsesStreamingResponse(ctx, result, koutakuErr)
	} else if isImageStreaming {
		// Handle image streaming
		return a.processImageStreamingResponse(ctx, result, koutakuErr)
	}
