...
func (a *Accumulator) buildCompleteImageFromImageStreamChunks(chunks []*ImageStreamChunk) *schemas.KoutakuImageGenerationResponse {
	...
	for i := range len(chunks) {
		if chunks[i].Delta != nil && (chunks[i].FinishReason != nil || chunks[i].Delta.Type == schemas.ImageGenerationEventTypeCompleted || chunks[i].Delta.Type == schemas.ImageEditEventTypeCompleted) {
			finalResponse := &schemas.KoutakuImageGenerationResponse{
...
func (a *Accumulator) processAccumulatedImageStreamingChunks(requestID string, koutakuErr *schemas.KoutakuError, isFinalChunk bool) (*AccumulatedData, error) {
	...
	if len(acc.ImageStreamChunks) > 0 {
		...
		if lastChunk.Delta != nil && lastChunk.Delta.Usage != nil {
			...
			data.TokenUsage = &schemas.KoutakuLLMUsage{
...
func (a *Accumulator) processImageStreamingResponse(ctx *schemas.KoutakuContext, result *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*ProcessedStreamResponse, error) {
	...
	if koutakuErr != nil {
		...
		newDelta := &schemas.KoutakuImageGenerationStreamResponse{
		...
		if result.ImageGenerationStreamResponse.ExtraFields.RawResponse != nil {
			chunk.RawResponse = koutaku.Ptr(fmt.Sprintf("%v", result.ImageGenerationStreamResponse.ExtraFields.RawResponse))
		}
		...
		if isFinalChunk {
			...
			chunk.FinishReason = koutaku.Ptr("completed")
		}
	}
	...
	if isFinalChunk {
		...
		if shouldProcess {
			data, processErr := a.processAccumulatedImageStreamingChunks(requestID, koutakuErr, isFinalChunk)
			if processErr != nil {
	...
	// Both logging and koutaku plugins return early when !isFinalChunk.
	return &ProcessedStreamResponse{
		RequestID:      requestID,
		StreamType:     StreamTypeImage,
		Provider:       provider,
