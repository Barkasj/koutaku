...
// It returns the response and an optional KoutakuError.
type AsyncOperation func(ctx *schemas.KoutakuContext) (any, *schemas.KoutakuError)
...
func (e *AsyncJobExecutor) SubmitJob(koutakuCtx *schemas.KoutakuContext, resultTTL int, operation AsyncOperation, operationType schemas.RequestType) (*AsyncJob, error) {
	if resultTTL <= 0 {
		resultTTL = DefaultAsyncJobResultTTL
	}

	virtualKeyValue := getVirtualKeyFromContext(koutakuCtx)

	var virtualKeyID *string
	if virtualKeyValue != nil {
		vk, ok := e.governanceStore.GetVirtualKey(koutakuCtx, *virtualKeyValue)
		if !ok {
			return nil, fmt.Errorf("virtual key not found")
		}
		virtualKeyID = &vk.ID
	...
	if koutakuCtx != nil {
		contextValues = koutakuCtx.GetUserValues()
	}
...
func (e *AsyncJobExecutor) executeJob(jobID string, resultTTL int, operation AsyncOperation, contextValues map[any]any) {
	...
	ctx.ClearValue(schemas.KoutakuContextKeyTraceID)
	ctx.ClearValue(schemas.KoutakuContextKeyParentSpanID)
	ctx.ClearValue(schemas.KoutakuContextKeySpanID)

	markFailed := func(msg string) {
		now := time.Now().UTC()
		expiresAt := now.Add(time.Duration(resultTTL) * time.Second)
		errJSON, _ := sonic.Marshal(&schemas.KoutakuError{Error: &schemas.ErrorField{Message: msg}})
	...
	// The koutaku execution flow is very stable and panics are not expected.
	...
	ctx.SetValue(schemas.KoutakuIsAsyncRequest, true)

	// Execute the operation
	resp, koutakuErr := operation(ctx)

	now := time.Now().UTC()
	expiresAt := now.Add(time.Duration(resultTTL) * time.Second)

	if koutakuErr != nil {
		errJSON, err := sonic.Marshal(koutakuErr)
		if err != nil {
			e.logger.Warn("failed to marshal koutaku error: %v", err)
