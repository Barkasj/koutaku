...
func TestDynamicPluginLifecycle(t *testing.T) {
	...
	t.Run("HTTPTransportPreHook", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewKoutakuContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
	...
	t.Run("PostLLMHook", func(t *testing.T) {
		...
		koutakuErr := (*schemas.KoutakuError)(nil)
		pluginCtx, cancel := schemas.NewKoutakuContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, resp, koutakuErr)
		require.NoError(t, err, "PostLLMHook should not return error")
		assert.Equal(t, resp, modifiedResp, "Response should be unchanged")
		assert.Equal(t, koutakuErr, modifiedErr, "Error should be unchanged")
	})
	...
	t.Run("PostHook_WithError", func(t *testing.T) {
		...
		pluginCtx, cancel := schemas.NewKoutakuContextWithTimeout(ctx, 10*time.Second)
		defer cancel()
		modifiedResp, modifiedErr, err := plugin.PostLLMHook(pluginCtx, nil, koutakuErr)
		require.NoError(t, err, "PostLLMHook should not return error")
...
func TestLoadPluginWithOptionalHooks(t *testing.T) {
	...
	t.Run("UnimplementedMCPHooks", func(t *testing.T) {
		...
		mcpReq := &schemas.KoutakuMCPRequest{}
		...
		koutakuErr := &schemas.KoutakuError{}
		returnedResp, returnedErr, hookErr := dynamicPlugin.PostMCPHook(pluginCtx, mcpResp, koutakuErr)
		...
		assert.Equal(t, koutakuErr, returnedErr, "PostMCPHook should return original error")
	})
