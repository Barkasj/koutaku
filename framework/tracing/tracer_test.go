...
func (p *testRealtimeObservabilityPlugin) PreLLMHook(_ *schemas.KoutakuContext, req *schemas.KoutakuRequest) (*schemas.KoutakuRequest, *schemas.LLMPluginShortCircuit, error) {
	return req, nil, nil
}
func (p *testRealtimeObservabilityPlugin) PostLLMHook(_ *schemas.KoutakuContext, resp *schemas.KoutakuResponse, koutakuErr *schemas.KoutakuError) (*schemas.KoutakuResponse, *schemas.KoutakuError, error) {
...
func TestTracer_StartSpan_RootSpanWithW3CParent(t *testing.T) {
	...
	// the root span in Koutaku correctly links to the upstream service's span.
	store := NewTraceStore(5*time.Minute, nil)
	defer store.Stop()
	...
	ctx := context.WithValue(context.Background(), schemas.KoutakuContextKeyTraceID, traceID)
	ctx = context.WithValue(ctx, schemas.KoutakuContextKeyParentSpanID, externalParentSpanID)

	// Create root span - this should link to the external parent
	newCtx, handle := tracer.StartSpan(ctx, "koutaku-http-request", schemas.SpanKindHTTPRequest)
	if handle == nil {
	...
	spanID, ok := newCtx.Value(schemas.KoutakuContextKeySpanID).(string)
	if !ok || spanID == "" {
...
func TestIntegration_FullDistributedTraceFlow(t *testing.T) {
	...
	// Step 4: Koutaku creates LLM call span
