...
type DeferredSpanInfo struct {
	...
	AccumulatedResponse *schemas.KoutakuResponse // Full accumulated response from streaming chunks
...
// accumulation for plugins (logging, koutaku). This eliminates storing 1M+ KoutakuResponse
// objects in the old accumulator at high concurrency.
func (s *TraceStore) AppendStreamingChunk(traceID string, chunk *schemas.KoutakuResponse) {
	if chunk == nil {
		return
	}
...
// SetAccumulatedResponse stores the accumulated KoutakuResponse on the deferred span info.
...
func (s *TraceStore) SetAccumulatedResponse(traceID string, resp *schemas.KoutakuResponse) {
	info := s.GetDeferredSpan(traceID)
	if info == nil {
		return
	}
...
// GetAccumulatedResponse returns the accumulated KoutakuResponse for a deferred span.
// Returns nil if no accumulated response has been stored.
func (s *TraceStore) GetAccumulatedResponse(traceID string) *schemas.KoutakuResponse {
	info := s.GetDeferredSpan(traceID)
	if info == nil {
		return nil
	}
