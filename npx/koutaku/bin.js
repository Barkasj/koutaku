...
async function getLatestVersion() {
	const releaseUrl = "https://getkoutaku.ai/latest-release";
...
async function checkVersionExists(version, platformDir, archDir, binaryName) {
	const url = `${BASE_URL}/koutaku/${version}/${platformDir}/${archDir}/${binaryName}`;
...
(async () => {
	...
	if (VERSION === "latest") {
		...
		if (!versionExists) {
			...
			console.error(`Please verify the version exists at: ${BASE_URL}/koutaku/`);
			process.exit(1);
		}
	...
	downloadUrls.push(`${BASE_URL}/koutaku/${VERSION}/${platformDir}/${archDir}/${binaryName}`);

	let lastError = null;
	let binaryWorking = false;

	const koutakuBinDir = namedVersionFound ? join(cacheDir(), "koutaku", namedVersion, "bin") : tmpdir();

	// if the binary directory doesn't exist, create it
	try {
		if (namedVersionFound && !existsSync(koutakuBinDir)) {
			mkdirSync(koutakuBinDir, { recursive: true });
		}
	} catch (mkdirError) {
		console.error(`❌ Failed to create directory ${koutakuBinDir}:`, mkdirError.message);
		process.exit(1);
	}

	for (let i = 0; i < downloadUrls.length; i++) {
		const binaryPath = join(koutakuBinDir, `${binaryName}-${i}`);
	...
	if (!binaryWorking) {
		console.error(`❌ Failed to start Koutaku. Error:`, lastError.message);
