...
help: ## Show this help message
	@$(ECHO) "$(BLUE)Koutaku Development - Available Commands:$(NC)"
	@$(ECHO) ""
...
dev: install-ui install-air setup-workspace $(if $(DEBUG),install-delve) ## Start complete development environment (UI + API with proxy)
	@$(ECHO) "$(GREEN)Starting Koutaku complete development environment...$(NC)"
	...
	@if [ ! -d "transports/koutaku-http/ui" ]; then \
		$(ECHO) "$(YELLOW)Creating transports/koutaku-http/ui directory...$(NC)"; \
		mkdir -p transports/koutaku-http/ui; \
		touch transports/koutaku-http/ui/.tmp; \
	fi
	@$(ECHO) ""
...
dev-pulse: install-ui install-pulse setup-workspace $(if $(DEBUG),install-delve) ## Start complete development environment using pulse for hot reloading
	@$(ECHO) "$(GREEN)Starting Koutaku complete development environment (pulse)...$(NC)"
	@$(ECHO) "$(YELLOW)This will start:$(NC)"
	@$(ECHO) "  1. UI development server (localhost:3000)"
	@$(ECHO) "  2. API server with UI proxy (localhost:$(PORT))"
	@$(ECHO) "$(CYAN)Access everything at: http://localhost:$(PORT)$(NC)"
...
build: build-ui ## Build koutaku-http binary
	@if [ -n "$(LOCAL)" ]; then \
		$(ECHO) "$(GREEN)╔═══════════════════════════════════════════════╗$(NC)"; \
		$(ECHO) "$(GREEN)║  Building koutaku-http with local go.work...  ║$(NC)"; \
		$(ECHO) "$(GREEN)╚═══════════════════════════════════════════════╝$(NC)"; \
	else \
		$(ECHO) "$(GREEN)╔═══════════════════════════════════════╗$(NC)"; \
		$(ECHO) "$(GREEN)║  Building koutaku-http...             ║$(NC)"; \
		$(ECHO) "$(GREEN)╚═══════════════════════════════════════╝$(NC)"; \
	fi
	@if [ -n "$(DYNAMIC)" ]; then \
	...
	if [ "$$TARGET_OS" = "linux" ] && [ "$$HOST_OS" = "linux" ]; then \
		if [ -n "$(DYNAMIC)" ]; then \
			...
			cd transports/koutaku-http && CGO_ENABLED=1 GOOS=$$TARGET_OS GOARCH=$$TARGET_ARCH $(if $(LOCAL),,GOWORK=off) go build \
		...
		$(ECHO) "$(GREEN)Built: tmp/koutaku-http (version: v$(VERSION))$(NC)"; \
...
build-cli: ## Build koutaku CLI binary
	@$(ECHO) "$(GREEN)Building koutaku CLI...$(NC)"
	@mkdir -p ./tmp
	@cd cli && $(if $(LOCAL),,GOWORK=off) go build -ldflags "-X main.version=v0.1.1-dev" -o ../tmp/koutaku .
	@$(ECHO) "$(GREEN)Built: tmp/koutaku$(NC)"
	...
	if [ "$(TARGET_OS)" = "linux" ]; then \
		if [ -n "$(DYNAMIC)" ]; then \
			...
			docker run --rm \
				...
				-w /workspace/transports/koutaku-http \
				-e CGO_ENABLED=1 \
				-e GOOS=$(TARGET_OS) \
		...
		$(ECHO) "$(GREEN)Built: tmp/koutaku-http ($(TARGET_OS)/$(TARGET_ARCH), version: v$(VERSION))$(NC)"; \
	else \
...
docker-image: build-ui ## Build Docker image (LOCAL=1 to use Dockerfile.local)
	...
	@docker build -f $(DOCKERFILE) -t koutaku -t koutaku:$(GIT_SHA) -t koutaku:latest .
	@$(ECHO) "$(GREEN)Docker image built: koutaku, koutaku:$(GIT_SHA), koutaku:latest (using $(DOCKERFILE))$(NC)"
...
docker-run: ## Run Docker container (Usage: make docker-run [CONFIG=path/to/config.json or path/to/dir/])
	...
	docker run -e APP_PORT=$(PORT) -e APP_HOST=0.0.0.0 -p $(PORT):$(PORT) -e LOG_LEVEL=$(LOG_LEVEL) -e LOG_STYLE=$(LOG_STYLE) -v $(shell pwd):/app/data $$CONFIG_MOUNT koutaku

docs: ## Prepare local docs
	@$(ECHO) "$(GREEN)Preparing local docs...$(NC)"
	@cd docs && npx --yes mintlify@latest dev

run: build ## Build and run koutaku-http (no hot reload)
	@$(ECHO) "$(GREEN)Running koutaku-http...$(NC)"
	@./tmp/koutaku-http \
		-host "$(HOST)" \
		-port "$(PORT)" \
		-log-style "$(LOG_STYLE)" \
...
run-cli: build-cli ## Run koutaku CLI (Usage: make run-cli [ARGS="--config ~/.koutaku/config.json"])
	@$(ECHO) "$(GREEN)Running koutaku CLI...$(NC)"
	@./tmp/koutaku $(ARGS)

clean: ## Clean build artifacts and temporary files
	@$(ECHO) "$(YELLOW)Cleaning build artifacts...$(NC)"
	@rm -rf tmp/
	@rm -f transports/koutaku-http/build-errors.log
	@rm -rf transports/koutaku-http/tmp/
...
test: install-gotestsum ## Run tests for koutaku-http
	@$(ECHO) "$(GREEN)Running koutaku-http tests...$(NC)"
	@mkdir -p $(TEST_REPORTS_DIR)
	@cd transports/koutaku-http && GOWORK=off gotestsum \
		--format=$(GOTESTSUM_FORMAT) \
		--junitfile=../../$(TEST_REPORTS_DIR)/koutaku-http.xml \
		-- -v ./...
	@if [ -z "$$CI" ] && [ -z "$$GITHUB_ACTIONS" ] && [ -z "$$GITLAB_CI" ] && [ -z "$$CIRCLECI" ] && [ -z "$$JENKINS_HOME" ]; then \
		if which junit-viewer > /dev/null 2>&1; then \
			$(ECHO) "$(YELLOW)Generating HTML report...$(NC)"; \
			if junit-viewer --results=$(TEST_REPORTS_DIR)/koutaku-http.xml --save=$(TEST_REPORTS_DIR)/koutaku-http.html 2>/dev/null; then \
				$(ECHO) ""; \
				$(ECHO) "$(CYAN)HTML report: $(TEST_REPORTS_DIR)/koutaku-http.html$(NC)"; \
				$(ECHO) "$(CYAN)Open with: open $(TEST_REPORTS_DIR)/koutaku-http.html$(NC)"; \
			else \
				$(ECHO) "$(YELLOW)HTML generation failed. JUnit XML report available.$(NC)"; \
				$(ECHO) "$(CYAN)JUnit XML report: $(TEST_REPORTS_DIR)/koutaku-http.xml$(NC)"; \
			fi; \
		else \
			$(ECHO) ""; \
...
test-http-transport: install-gotestsum ## Run HTTP transport tests
	...
	cd transports/koutaku-http && find . -name "*.go" -path "*/tests/*" -o -name "*_test.go" | head -1 > /dev/null && \
		for dir in $$(find . -name "*_test.go" -exec dirname {} \; | sort -u); do \
			pkg_name=$$(echo $$dir | sed 's|^\./||' | sed 's|/|-|g'); \
			$(ECHO) "Testing $$dir..."; \
			cd $$dir && gotestsum \
...
test-integrations-py: ## Run Python integration tests (Usage: make test-integrations-py [INTEGRATION=openai] [TESTCASE=test_name] [PATTERN=substring] [VERBOSE=1])
	...
	$(ECHO) "$(CYAN)Checking if Koutaku is running on $$TEST_HOST:$$TEST_PORT...$(NC)"; \
	if curl -s -o /dev/null -w "%{http_code}" http://$$TEST_HOST:$$TEST_PORT/health 2>/dev/null | grep -q "200\|404"; then \
		$(ECHO) "$(GREEN)✓ Koutaku is already running$(NC)"; \
	else \
	...
	if [ $$KOUTAKU_STARTED -eq 1 ] && [ -n "$$KOUTAKU_PID" ]; then \
		...
		if [ $$TEST_FAILED -eq 1 ]; then \
			$(ECHO) ""; \
			$(ECHO) "$(YELLOW)Last 50 lines of Koutaku logs:$(NC)"; \
			tail -50 /tmp/koutaku-test.log 2>/dev/null || $(ECHO) "No log file found"; \
		fi; \
	fi; \
	$(ECHO) ""; \
	if [ $$TEST_FAILED -eq 1 ]; then \
		$(ECHO) "$(RED)✗ Integration tests failed$(NC)"; \
		$(ECHO) "$(CYAN)Full Koutaku logs: /tmp/koutaku-test.log$(NC)"; \
		exit 1; \
	else \
...
build-test-plugin: ## Build test plugin for E2E tests (copies to tmp/koutaku-test-plugin.so)
	@$(ECHO) "$(GREEN)Building test plugin for E2E tests...$(NC)"
	@cd examples/plugins/hello-world && make dev
	@mkdir -p tmp
	@cp examples/plugins/hello-world/build/hello-world.so tmp/koutaku-test-plugin.so
	@$(ECHO) "$(GREEN)✓ Test plugin ready at tmp/koutaku-test-plugin.so$(NC)"
...
quick-start: ## Quick start with example config and koutaku plugin
	@$(ECHO) "$(GREEN)Quick starting Koutaku with example configuration...$(NC)"
	@$(MAKE) dev
...
setup-workspace: ## Set up Go workspace with all local modules for development
	...
	@go list -m all | grep "github.com/koutaku/koutaku" | grep -v " v" | sed 's/^/  ✓ /'
	@$(ECHO) ""
	@$(ECHO) "$(CYAN)Remote modules (no local version):$(NC)"
	@go list -m all | grep "github.com/koutaku/koutaku" | grep " v" | sed 's/^/  → /'
	@$(ECHO) ""
